/*
 * @(#)DirectAudioDevice.java	1.29 04/07/09
 *
...
class DirectAudioDevice extends AbstractMixer {
	...
	protected volatile int nativeRefCount = 0;
	...
	public void drain() {
	    ...
	    while (!drained && (id != 0) && nIsStillDraining(id, isSource) && doIO) {
	...
	public int write(byte[] b, int off, int len) {
	    ...
	    while (!flushing) {
	    	nativeRefCount++;
		int thisWritten = nWrite(id, b, off, len,
		                         softwareConversionSize,
		                         leftGain, rightGain);
	    	nativeRefCount--;
		if (thisWritten < 0) {
