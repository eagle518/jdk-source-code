/*
 * @(#)SimpleInputDevice.java	1.56 04/05/18
 *
...
class SimpleInputDevice extends AbstractMixer {
	...
	// $$fb fix for 4915810: 2 Regression-test sound fails: NPE from Mixer.open()
	if (format == null) {
	...
	if (AudioFormat.Encoding.ULAW.equals(format.getEncoding())) {
	    encoding = ULAW;
	} else if (AudioFormat.Encoding.ALAW.equals(format.getEncoding())) {
	    encoding = ALAW;
	}
    ...
    */
	...
	public int read(byte[] b, int off, int len) {
	    ...
	    while (isOpen() && isStartedRunning() && (totalBytesRead < totalBytesToRead)) {
	...
	void implOpen(AudioFormat format, int bufferSize) throws LineUnavailableException {
	    ...
	    if ( (format.getSampleSizeInBits() <= 8) && ( AudioFormat.Encoding.PCM_SIGNED.equals(format.getEncoding()) && (Platform.isSigned8() == false) ) ) {
		convertSign = true;
	    } else if ( (format.getSampleSizeInBits() <= 8) && ( AudioFormat.Encoding.PCM_UNSIGNED.equals(format.getEncoding()) && (Platform.isSigned8() == true) ) ) {
		convertSign = true;
	    }
	...
	public long getLongFramePosition() {
	    return (isOpen()) ? sid.nGetPosition() : super.getLongFramePosition();
	}
