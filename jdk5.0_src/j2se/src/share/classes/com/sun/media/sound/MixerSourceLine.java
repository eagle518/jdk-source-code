/*
 * @(#)MixerSourceLine.java	1.40 04/05/18
 *
...
public class MixerSourceLine extends AbstractDataLine implements SourceDataLine {
	...
	while (isOpen() && isStartedRunning() &&  (totalBytesWritten < totalBytesToWrite)) {
	...
	if (Printer.trace) Printer.trace(">> MixerSourceLine: implOpen with bufferSize="+bufferSize);
	...
	if ( (getFormat().getSampleSizeInBits() == 8) && (AudioFormat.Encoding.PCM_SIGNED.equals(getFormat().getEncoding())) ) {
	    convertSign = true;
	}
	...
	return (id != 0) ? nGetPosition(id) : finalPosition;
    }
	...
	// save the state info
	finalPosition = getLongFramePosition();
