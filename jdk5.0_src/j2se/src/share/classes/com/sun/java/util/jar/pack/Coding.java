...
.     during encoding, all addition results are reduced to the unsigned range.
	...
	for (int dstep = 0; dstep < del; dstep++) {
	    assert(dstep == 0); // dstate[dstep] must be dstate[0]
	...
	assert(canRepresentUnsigned(value));
	return value;
    }
	...
	if (L == 0)  return len * B;
	if (isDelta()) {
	    int[] deltas = makeDeltas(values, start, end, null);
	    if (isSubrange()) {
		for (int i = 0; i < len; i++) {
		    deltas[i] = reduceToSignedRange(deltas[i]);
		}
