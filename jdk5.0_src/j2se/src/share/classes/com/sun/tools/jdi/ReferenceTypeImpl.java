/*
 * @(#)ReferenceTypeImpl.java	1.70 04/05/05
 *
...
implements ReferenceType {
    ...
    /*
     * Nested class names of interest are currently anything with
     *   with same name plus a separator as a prefix.
     */
    private boolean is_my_nested_class(String classname, char sep) {
    ...
    public List nestedTypes() {
	    ...
	    /* The separator is historically '$' but could also be '#' */
            if ( is_my_nested_class(name, '$') ||
		 is_my_nested_class(name, '#') ) {
                nested.add(refType);
            }
        }
        return nested;
