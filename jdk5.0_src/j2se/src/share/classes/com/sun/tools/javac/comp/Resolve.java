 ...
 *************************************************************************/
	...
	if (typeargtypes == null) typeargtypes = Type.emptyList;
	...
	if (instNeeded)
	    return
            infer.instantiateMethod(tvars,
                                    ...
                                    warn);
        return
            argumentsAcceptable(argtypes, mt.argtypes(),
 ...
 ****************************************************************************/
     ...
     */
    public VarSymbol resolveInternalField(int pos, Env<AttrContext> env,
                                          Type site, Name name) {
        Symbol sym = findField(env, site, name, site.tsym);
	if (sym.kind == VAR) return (VarSymbol)sym;
	else throw new FatalError(
		 new Diagnostic("fatal.err.cant.locate.field",
				name));
    }
	...
	for (Scope.Entry e = env.toplevel.namedImportScope.lookup(name); e.scope != null; e = e.next()) {
	    sym = e.sym;
	    if (sym.kind == VAR)
                return isAccessible(env, sym.owner.type, sym)
                    ? sym : new AccessError(sym);
	}
	for (Scope.Entry e = env.toplevel.starImportScope.lookup(name); e.scope != null; e = e.next()) {
	    sym = e.sym;
	    if (sym.kind != VAR) continue;
	    if (bestSoFar.kind < AMBIGUOUS && sym.owner != bestSoFar.owner)
		bestSoFar = new AmbiguityError(bestSoFar, sym);
	    else if (sym.kind < bestSoFar.kind)
		bestSoFar = isAccessible(env, sym.owner.type, sym)
                    ? sym : new AccessError(sym);
	}
	return bestSoFar;
    }
	...
	return (bestSoFar.kind > AMBIGUOUS)
	    ? sym
	    : mostSpecific(sym, bestSoFar, env, site, allowBoxing, useVarargs);
    }
	...
	case MTH:
		...
		Symbol result;
                ...
                if (types.isSubType(mt1.restype(), result2)) {
                    result = m1;
                } else if (types.isSubType(result2, mt1.restype())) {
                    result = m2;
                } else {
		...
		result.type.setThrown(chk.intersect(mt1.thrown(),
						    mt2.thrown()));
		return result;
	    }
	...
	private boolean methodsInherited(Symbol subclass, Symbol superclass) {
	    // return true if methods can be inherited from superclass to
	    // subclass.  The only case in which a subclass doesn't inherit
	    // the superclass methods is: interfaces don't inherit methods
	    // from Object.
	    return
		subclass.isSubClass(superclass, types) &&
	...
	for (Scope.Entry e = env.toplevel.namedImportScope.lookup(name); e.scope != null; e = e.next()) {
            sym = e.sym;
	    if (sym.kind == MTH) {
                if (!isAccessible(env, sym.owner.type, sym)) sym = new AccessError(sym);
		bestSoFar = selectBest(env, sym.owner.type,
...
//	    printscopes(site.tsym.members());//DEBUG
		...
		sym = new ErrorType(name, qualified?site.tsym:syms.noSymbol)
		    .tsym;
	}
	return sym;
    }
    ...
    Symbol access(Symbol sym,
		  ...
		  boolean qualified) {
	if (sym.kind >= AMBIGUOUS)
	    return access(sym, pos, site, name, qualified, Type.emptyList, null);
	else
	    return sym;
    }
	...
	if ((sym.flags() & ABSTRACT) != 0)
	    log.error(pos, "abstract.cant.be.accessed.directly",
		      kindName(sym.kind), sym, sym.location());
    }
 ...
 ****************************************************************************/
     ...
     */
    Symbol resolveIdent(int pos, Env<AttrContext> env,
			Name name, int kind) {
	return access(
     ...
     *  @param typeargtypes  The types of the invocation's type arguments.
     */
    Symbol resolveQualifiedMethod(int pos, Env<AttrContext> env,
				  Type site, Name name, List<Type> argtypes,
				  List<Type> typeargtypes) {
	Symbol sym = findMethod(env, site, name, argtypes, typeargtypes, false, env.info.varArgs=false);
	if (sym.kind >= WRONG_MTHS && source.allowVarargs()) {
	    sym = findMethod(env, site, name, argtypes, typeargtypes, true, false);
	    if (sym.kind >= WRONG_MTHS)
		sym = findMethod(env, site, name, argtypes, typeargtypes, true, env.info.varArgs=true);
	}
	if (sym.kind >= AMBIGUOUS) {
     ...
     *  @param typeargtypes  The types of the invocation's type arguments.
     */
    public MethodSymbol resolveInternalMethod(int pos, Env<AttrContext> env,
					Type site, Name name,
					List<Type> argtypes,
					List<Type> typeargtypes) {
	...
	if (sym.kind == MTH) return (MethodSymbol)sym;
		 ...
		 new Diagnostic("fatal.err.cant.locate.meth",
				name));
    }
     ...
     *			 arguments.
     */
    Symbol resolveConstructor(int pos,
                              Env<AttrContext> env,
			      Type site,
                              List<Type> argtypes,
			      List<Type> typeargtypes) {
     ...
     *  @param useVarargs Box trailing arguments into an array for varargs.
     */
    Symbol resolveConstructor(int pos, Env<AttrContext> env,
     ...
     *  @param typeargtypes  The types of the invocation's type arguments.
     */
    public MethodSymbol resolveInternalConstructor(int pos, Env<AttrContext> env,
					Type site,
					List<Type> argtypes,
					List<Type> typeargtypes) {
	...
	if (sym.kind == MTH) return (MethodSymbol)sym;
		 ...
		 new Diagnostic("fatal.err.cant.locate.ctor", site));
    }
     ...
     */
    Symbol resolveOperator(int pos, int optag,
			   Env<AttrContext> env, List<Type> argtypes) {
	Name name = treeinfo.operatorName(optag);
	Symbol sym = findMethod(env, syms.predefClass.type, name, argtypes,
				null, false, false);
	if (sym.kind >= WRONG_MTHS && source.allowBoxing())
	    sym = findMethod(env, syms.predefClass.type, name, argtypes,
			     null, true, false);
     ...
     *  @param argtypes  The types of the operands.
     */
    Symbol resolveUnaryOperator(int pos, int optag,
			   Env<AttrContext> env, Type arg) {
     ...
     */
    Symbol resolveBinaryOperator(int pos, int optag,
				 Env<AttrContext> env,
     ...
     */
    Symbol resolveSelfContaining(int pos,
				 Env<AttrContext> env,
				 Symbol member) {
	...
	Type thisType =
	    (((t.tsym.owner.kind & (MTH|VAR)) != 0) // local or anon class
 ...
 ****************************************************************************/
    ...
    static Diagnostic kindName(int kind) {
        switch (kind) {
        case PCK: return new Diagnostic("kindname.package");
        case TYP: return new Diagnostic("kindname.class");
        case VAR: return new Diagnostic("kindname.variable");
        case VAL: return new Diagnostic("kindname.value");
        case MTH: return new Diagnostic("kindname.method");
        default : return new Diagnostic("kindname",
	...
	if (t.tag == TYPEVAR ||
	    ...
	    return new Diagnostic("kindname.type.variable");
	else if (t.tag == PACKAGE)
	    return new Diagnostic("kindname.package");
	else if ((t.tsym.flags_field & ANNOTATION) != 0)
	    return new Diagnostic("kindname.annotation");
	else if ((t.tsym.flags_field & INTERFACE) != 0)
	    return new Diagnostic("kindname.interface");
	else
	    return new Diagnostic("kindname.class");
    }
	...
	switch (kind) {
	case ABSENT_VAR:
	    return new Diagnostic("kindname.variable");
	case WRONG_MTHS: case WRONG_MTH: case ABSENT_MTH:
	    return new Diagnostic("kindname.method");
	case ABSENT_TYP:
	    return new Diagnostic("kindname.class");
	default:
 ...
 ****************************************************************************/
	 ...
	 *                   if we looked for a method.
	 */
	void report(Log log, int pos, Type site, Name name,
		    List<Type> argtypes, List<Type> typeargtypes) {
	    if (name != name.table.error) {
		Diagnostic kindname = absentKindName(kind);
		...
		if (kind >= WRONG_MTHS && kind <= ABSENT_MTH) {
			...
			kindname = new Diagnostic("kindname.constructor");
...
//where
	...
	AccessError(Symbol sym) {
	    ...
	    //	    log.rawError(Position.NOPOS, sym + " is inaccessible.");//DEBUG
	}
