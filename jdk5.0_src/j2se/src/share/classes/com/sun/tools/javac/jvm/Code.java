...
/*---------- classfile fields: --------------- */
     ...
     */
    List<char[]> lineInfo = new List<char[]>(); // handled in stack fashion
 ...
 *************************************************************************/
	...
	frame.locals = new Type[nextLocal];
	for (int i=0; i<nextLocal; i++) {
	    if (state.defined.isMember(i) && lvar[i] != null) {
 ...
 ****************************************************************************/
	...
	State dup() {
		...
		state.stack = (Type[])stack.clone();
		if (locks != null) state.locks = (int[])locks.clone();
		if (debugCode) {
