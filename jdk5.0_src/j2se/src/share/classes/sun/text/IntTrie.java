...
public class IntTrie extends Trie
{
    ...
    */
    ...
    {
        if (!Character.isHighSurrogate(lead) || !Character.isLowSurrogate(trail)) {
            throw new IllegalArgumentException(
    ...
    */
                                                    ...
                                                    throws IOException
    {
        ...
        byte[] bb = new byte[m_dataLength_ * 4];
        input.read(bb);
        int j = 0;
        for (int i = 0; i < m_dataLength_; i ++) {
            m_data_[i] = (bb[j++] << 24 & 0xff000000) | (bb[j++] << 16 & 0xff0000) 
		| (bb[j++] << 8 & 0xff00) | (bb[j++] & 0xff);
        }
