...
final class Order3 extends Curve {
     ...
     * in t, ready for solving with solveCubic.
     * We currently have:
     ...
     *     D = C1 - 3*CP1 + 3*CP0 - C0
     */
    public static void getEqn(double eqn[],
			      double c0, double cp0, double cp1, double c1) {
	eqn[0] = c0;
	eqn[1] = (cp0 - c0) * 3.0;
	eqn[2] = (cp1 - cp0 - cp0 + c0) * 3.0;
	eqn[3] = c1 - (cp1 - cp0) * 3.0 - c0;
	return;
    }
	...
	// Copy variables locally for quick access
	double a = ycoeff2;
	double b = ycoeff1;
	double c = ycoeff0 - y;
	...
	if (t0 <= 0) {
	    return new Order3(eqn[0], ystart,
			      eqn[2], eqn[3],
			      eqn[4], eqn[5],
			      eqn[6], yend,
			      dir);
	    ...
	    split(eqn, 0, t0 / t1);
	    return new Order3(eqn[6], ystart,
			      eqn[8], eqn[9],
			      eqn[10], eqn[11],
			      eqn[12], yend,
			      dir);
