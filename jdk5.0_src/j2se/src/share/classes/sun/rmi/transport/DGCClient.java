...
final class DGCClient {
    ...
    private static final long gcInterval =		// default 1 minute
	((Long) java.security.AccessController.doPrivileged(
	...
	private static class CleanRequest {
	    ...
	    public CleanRequest(ObjID[] objIDs, long sequenceNum,
				boolean strong)
	    {
		this.objIDs = objIDs;
	...
	private void makeCleanCalls() {
		...
		try {
		    ...
		    /*
		     * If we get a ConnectException, the target VM likely has
		     * gone away, in which case we shouldn't bother retrying
		     * this clean request forever.  Then again, the server
		     * could just be heavily loaded, so we will give a finite
		     * number of retry opportunities to clean requests that
		     * fail this way.
		     */
