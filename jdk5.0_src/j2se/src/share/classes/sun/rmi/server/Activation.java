...
public class Activation implements Serializable {
    ...
    /** temporary: for debugging inetd launch */
	...
	synchronized (startupLock = new Object()) {
	    ...
	    registry.rebind("java.rmi.activation.ActivationSystem",
			    systemStub);
    	   ...
    	   throws UnknownGroupException, ActivationException
	{
		...
		ActivationID id = (ActivationID) entry.getKey();
		synchronized (idTable) {
		    idTable.remove(id);
		}
		ObjectEntry objEntry = (ObjectEntry) entry.getValue();
		objEntry.removed = true;
	    }
	    objects.clear();
	    restartSet.clear();
	...
	ActivationDesc desc;
	/** the stub (if active) */
	volatile transient MarshalledObject stub = null;
	...
	try {
	     ...
	     **/
	    Channel inheritedChannel = (Channel)
		...
		if (!(inheritedChannel instanceof ServerSocketChannel)) {
		    bomb(getTextResource("rmid.syntax.exec.invalid"));
		}
