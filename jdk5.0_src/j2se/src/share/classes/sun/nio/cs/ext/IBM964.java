		...
		while (sp < sl) {
			...
			case G0:
			    if (byte1 == SS2) // drop into G2 set
			       state = G2;
			    else if (byte1 == SS3)  // not supported
			...
			case G1:
			    ...
			    outputChar = mappingTableG1.charAt(((firstByte
						- 0xa1) * 94) + byte1 - 0xa1);
			    inputSize = 1;
			    break;
			...
			case G4:
			    state = G0;  // valid second byte for G2
			    if ( byte1 < 0xa1 || byte1 > 0xfe) {
			       return CoderResult.malformedForLength(1);
			    }
			    outputChar = mappingTableG2.charAt(((firstByte
								- 0xa1)
							        * 94)  + byte1
								- 0xa1);
			    inputSize = 1;
