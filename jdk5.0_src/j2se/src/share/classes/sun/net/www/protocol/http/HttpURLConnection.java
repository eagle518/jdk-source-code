/*
 * @(#)HttpURLConnection.java	1.158 04/06/23
 *
...
public class HttpURLConnection extends java.net.HttpURLConnection {
    ...
    */
    ...
    private boolean tryTransparentNTLMServer = true; /* try server auth without calling Authenticator */
    private boolean tryTransparentNTLMProxy = true; /* try Proxy auth without calling Authenticator */
    Object authObj; 
	...
	if (host != null && authhdr.isPresent()) {
	    ...
	    } else if ("ntlm".equalsIgnoreCase(scheme) && NTLMAuthentication.isSupported()) {
	...
	public static InputStream getErrorStream(InputStream is, int cl, HttpClient http) {
		...
		if (expected <= bufSize4ES) {
			...
			return null;
		    }  else if (count == expected || (cl < 0 && len <0)) {
