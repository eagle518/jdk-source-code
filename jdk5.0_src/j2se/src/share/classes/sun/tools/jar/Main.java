...
class Main {
     ...
     */
    void update(InputStream in, OutputStream out,
                InputStream manifest) throws IOException 
    {
        ...
        while((e = zis.getNextEntry()) != null) {
            ...
            if ((name.toUpperCase().equals(INDEX)
                && t.containsKey(INDEX)) 
                || (Mflag && name.toUpperCase().equals(MANIFEST))) {
                continue;
            } else if (manifest != null 
                       && name.toUpperCase().equals(MANIFEST)) {
        ...
        if (!foundManifest && (manifest != null)) {
            updateManifest(new Manifest(manifest), zos);
        }
    ...
    void dumpIndex(String rootjar, JarIndex index) throws IOException {
        ...
        update(new FileInputStream(jarFile), new FileOutputStream(scratchFile), null);
        jarFile.delete();
