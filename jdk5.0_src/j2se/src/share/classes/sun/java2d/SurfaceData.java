/*
 * @(#)SurfaceData.java	1.42 04/01/20
 *
...
public abstract class SurfaceData implements Transparency, DisposerTarget {
    ...
    /**
     * Extracts the SurfaceManager from the given Image.  The SurfaceManager
     * then uses the other supplied parameters to choose an appropriate
     * SurfaceData object to be used as the source surface in an image
     * copy operation.
     *
     * REMIND: comp, bgColor, scale are old hacks... we need to come up
     *         with a better mechanism for this...
     */
    public static SurfaceData getSourceSurfaceData(Image img,
                                                   SurfaceData dstData,
                                                   CompositeType comp,
                                                   Color bgColor, 
                                                   boolean scale)
    {
        ...
        return sMgr.getSourceSurfaceData(dstData, comp, bgColor, scale);
    }
     ...
     */
    public static SurfaceData getDestSurfaceData(Image img) {
        SurfaceManager sMgr = SurfaceManager.getManager(img);
        return sMgr.getDestSurfaceData();
    }
    ...
    /**
     * Marks this surface as dirty.  The dirty flag is set to true
    ...
    /**
     * Sets the value of the native dirty flag.  This flag is set to true
     * whenever the surface is rendered to and false whenever we copy from
     * the surface.
     */
	...
	sg2d.imagepipe = imagepipe;
		...
		if (sg2d.alphafill == null) {
                    SurfaceType srcType =
                        ...
                        SurfaceType.OpaqueColor : SurfaceType.AnyColor;
		    sg2d.alphafill =
			MaskFill.getFromCache(srcType,
					      sg2d.fillComp,
					      getSurfaceType());
		    // assert(sg2d.surfaceData == this);
		}
