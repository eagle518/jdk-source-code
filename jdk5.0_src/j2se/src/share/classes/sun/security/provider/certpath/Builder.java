...
public abstract class Builder {
    ...
    /**
     * Retrieves certificates from the certStores using the buildParams
     * and the currentState as a filter
     *
     * @param currentState the current State
     */     
    abstract Collection<X509Certificate> getMatchingCerts(State currentState) 
     ...
     * @throws IOException on incomparable names
     */
    static int distance(GeneralNameInterface base, 
	GeneralNameInterface test) throws IOException 
    {
	switch (base.constrains(test)) {
	case GeneralNameInterface.NAME_DIFF_TYPE:
	    throw new IOException("Names are different types");
	case GeneralNameInterface.NAME_SAME_TYPE:
	    throw new IOException("Names are same type but in different "
		+ "subtrees");
     ...
     * @throws IOException on incomparable names or names in different 
     * namespaces
     */
	...
	if (base.getType() != GeneralNameInterface.NAME_DIRECTORY) {
	    throw new IOException("hopDistance not implemented for this name "
		+ "type");
	}
	...
	for (int i = 0, n = permitted.size(); i < n; i++) {
	    ...
	    try {
		int distance = distance(perName, target);
		if (distance >= 0) {
		    return (distance + 1);
		}
     ...
     */
    void addMatchingCerts(X509CertSelector selector, 
	...
	for (CertStore store : certStores) {
		...
		Collection<X509Certificate> certs = 
		    (Collection<X509Certificate>)store.getCertificates(selector);
		for (X509Certificate cert : certs) {
		    if (!X509CertImpl.isSelfSigned
			(cert, buildParams.getSigProvider())) {
