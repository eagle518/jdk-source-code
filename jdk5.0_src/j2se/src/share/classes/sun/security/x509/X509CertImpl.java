/*
 * @(#)X509CertImpl.java	1.130 03/12/19
 *
...
public class X509CertImpl extends X509Certificate implements DerEncoder {
    ...
    public synchronized Set<AccessDescription> getAuthorityInformationAccess()
			throws CertificateParsingException {

	if (readOnly && authInfoAccess != null) {
	    return authInfoAccess; // Unmodifiable
	}
	authInfoAccess = getAuthorityInformationAccess(this);
	return authInfoAccess;
    }

    public static Set<AccessDescription> getAuthorityInformationAccess(
			X509Certificate cert)
			throws CertificateParsingException {
	try {
	    byte[] extValue = cert.getExtensionValue(AUTH_INFO_ACCESS_OID);
	    if (extValue == null) {
	       return  Collections.<AccessDescription>emptySet();
	    }
	    ...
	    Set<AccessDescription> accessDesc =
		new HashSet<AccessDescription>(derVal.length);
