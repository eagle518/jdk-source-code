...
{
    (void)outStream_writeObjectRef(out, evinfo->thread);
...
writeFieldAccessEvent(JNIEnv *env, PacketOutputStream *out, EventInfo *evinfo)
{
    ...
    (void)outStream_writeObjectRef(out, evinfo->u.field_access.field_clazz);
    (void)outStream_writeFieldID(out, evinfo->u.field_access.field);
    (void)outStream_writeObjectTag(out, evinfo->object);
    (void)outStream_writeObjectRef(out, evinfo->object);
}
                            ...
                            EventInfo *evinfo)
{
    ...
    writeCodeLocation(out, evinfo->clazz, evinfo->method, evinfo->location);
    (void)outStream_writeByte(out, fieldClassTag);
    (void)outStream_writeObjectRef(out, evinfo->u.field_modification.field_clazz);
...
writeClassEvent(JNIEnv *env, PacketOutputStream *out, EventInfo *evinfo)
{
    ...
    (void)outStream_writeObjectRef(out, evinfo->clazz);
                           ...
                           EventCommandSingle *command)
{
    ...
    switch (evinfo->ei) {
        ...
        case EI_VM_DEATH:
            ...
            break;
        default:
	    EXIT_ERROR(JVMTI_ERROR_INVALID_EVENT_TYPE,"unknown event index");
            break;
    }
                              ...
                              FrameEventCommandSingle *command) 
{
    ...
    (void)outStream_writeInt(out, command->id);
    (void)outStream_writeObjectRef(out, command->thread);
...
handleCommand(JNIEnv *env, HelperCommand *command)
{
    switch (command->commandKind) {
        ...
        case COMMAND_SUSPEND_THREAD:
            ...
            break; 
        default:
	    EXIT_ERROR(JVMTI_ERROR_INVALID_EVENT_TYPE,"Event Helper Command");
	    break;
    }
}
...
saveEventInfoRefs(JNIEnv *env, EventInfo *evinfo)
{
	...
	*pobject = NULL;
	...
	EXIT_ERROR(JVMTI_ERROR_INVALID_EVENT_TYPE,"ExceptionOccurred");
    }
}
                         ...
                         struct bag *eventBag)
{
    ...
    if (command == NULL) {
	EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"badAdd(eventBag)");
    }
...
eventHelper_recordClassUnload(jint id, char *signature, struct bag *eventBag)
{
    CommandSingle *command = bagAdd(eventBag);
    if (command == NULL) {
	EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"bagAdd(eventBag)");
    }
...
eventHelper_reportInvokeDone(jbyte sessionID, jthread thread)
{
    ...
    if (command == NULL) {
	EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"HelperCommand");
    }
...
eventHelper_reportVMInit(JNIEnv *env, jbyte sessionID, jthread thread, jbyte suspendPolicy)
{
    HelperCommand *command = jvmtiAllocate(sizeof(*command));
    if (command == NULL) {
	EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"HelperCommmand");
    }
    (void)memset(command, 0, sizeof(*command));
