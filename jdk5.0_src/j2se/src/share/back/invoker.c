...
invoker_enableInvokeRequests(jthread thread) 
{
    ...
    if (request == NULL) {
        EXIT_ERROR(JVMTI_ERROR_INVALID_THREAD, "getting thread invoke request");
    }
...
invokeStatic(JNIEnv *env, InvokeRequest *request)
{
    switch(returnTypeTag(request->methodSignature)) {
        ...
        default:
            EXIT_ERROR(JVMTI_ERROR_NULL_POINTER,"Invalid method signature");
            break;
    }
}
...
invoker_completeInvokeRequest(jthread thread) 
{
    ...
    if (!detached) {
        ...
        (void)outStream_writeObjectTag(&out, exc);
        (void)outStream_writeObjectRef(&out, exc);
