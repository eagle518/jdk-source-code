...
printLastError(jdwpTransportEnv *t, jdwpTransportError err)
{
    ...
    msg = NULL;
    rv = (*t)->GetLastError(t, &msg);
    if (rv == JDWPTRANSPORT_ERROR_NONE) {
        ERROR_MESSAGE(("transport error %d: %s",err,msg));
	jvmtiDeallocate(msg);
    } else if ( msg!=NULL ) {
        ERROR_MESSAGE(("transport error %d: %s",err,msg));
    } else {
...
findTransportOnLoad(void *handle)
{
    ...
    if (handle == NULL) {
        return (jdwpTransport_OnLoad_t)NULL;
    }
    
    nsyms = (int)(sizeof(onLoadSymbols) / sizeof(char *));
    for (i = 0; i < nsyms; i++) {
        onLoad = (jdwpTransport_OnLoad_t)
		     dbgsysFindLibraryEntry(handle, onLoadSymbols[i]);
        if (onLoad) {
            return onLoad;
        }
    }
...
loadTransportLibrary(char *libdir, char *name)
{
    ...
    dbgsysBuildLibName(libname, sizeof(libname), libdir, name);
    
    /* dlopen (unix) / LoadLibrary (windows) the transport library */
    handle = dbgsysLoadLibrary(libname, errmsg, sizeof(errmsg));
    return handle;
}
...
{
    jint rc;
    char *commandLine = jvmtiAllocate(strlen(command) +
                                 strlen(name) +
                                 strlen(address) + 3);
    if (commandLine == NULL) {
			 ...
			 long timeout)
{
	...
	jvmtiError error;
        ...
        info->name = jvmtiAllocate(strlen(name)+1);
	...
	info->timeout = timeout;
        ...
        if (address != NULL) {
            info->address = jvmtiAllocate(strlen(address)+1);
