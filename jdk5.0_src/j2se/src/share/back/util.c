...
static char * getPropertySystemEncoding(JNIEnv *env, char *propertyName);
...
saveGlobalRef(JNIEnv *env, jobject obj, jobject *pobj)
{
    ...
    if ( pobj == NULL ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"saveGlobalRef pobj");
    }
    if ( *pobj != NULL ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"saveGlobalRef *pobj");
    }
    if ( env == NULL ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"saveGlobalRef env");
    }
    if ( obj == NULL ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"saveGlobalRef obj");
    }
...
tossGlobalRef(JNIEnv *env, jobject *pobj)
{
    ...
    if ( pobj == NULL ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"tossGlobalRef pobj");
    }
    obj = *pobj;
    if ( env == NULL ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"tossGlobalRef env");
    }
    if ( obj == NULL ) {
	EXIT_ERROR(JVMTI_ERROR_NULL_POINTER,"tossGlobalRef obj");
    }
...
findClass(JNIEnv *env, const char * name)
{
    ...
    if ( env == NULL ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"findClass env");
    }
    if ( name == NULL || name[0] == 0 ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"findClass name");
    }
    x = JNI_FUNC_PTR(env,FindClass)(env, name);
    if (x == NULL) {
	ERROR_MESSAGE(("JDWP Can't find class %s", name));
	EXIT_ERROR(JVMTI_ERROR_NULL_POINTER,NULL);
    }
...
getMethod(JNIEnv *env, jclass clazz, const char * name, const char *signature)
{
    ...
    if ( env == NULL ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"getMethod env");
    }
    if ( clazz == NULL ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"getMethod clazz");
    }
    if ( name == NULL || name[0] == 0 ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"getMethod name");
    }
    if ( signature == NULL || signature[0] == 0 ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"getMethod signature");
    }
...
getStaticMethod(JNIEnv *env, jclass clazz, const char * name, const char *signature)
{
    ...
    if ( env == NULL ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"getStaticMethod env");
    }
    if ( clazz == NULL ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"getStaticMethod clazz");
    }
    if ( name == NULL || name[0] == 0 ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"getStaticMethod name");
    }
    if ( signature == NULL || signature[0] == 0 ) {
	EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"getStaticMethod signature");
    }
...
util_initialize(void)
{
	...
	gdata->threadSetDaemon = 
        ...
        gdata->stringGetBytes = 
		getMethod(env, gdata->stringClass, 
		    "getBytes", "()[B");
	...
	if ( groupCount == 0 ) {
	    EXIT_ERROR(JVMTI_ERROR_NULL_POINTER, "Can't get system thread group");
	}
	...
	gdata->property_sun_boot_library_path	
			= getPropertySystemEncoding(env, "sun.boot.library.path");
                ...
                jfieldID field)
{
    ...
    switch (typeKey) {
        ...
        case JDWP_TAG(ARRAY):   {
            ...
            (void)outStream_writeByte(out, specificTypeKey(value));
            (void)outStream_writeObjectRef(out, value);
            break;
        }
...
getEnv(void)
{
    ...
    if (rc != JNI_OK) {
        ERROR_MESSAGE(("JDWP Unable to get JNI 1.2 environment, jvm->GetEnv() return code = %d", 
                rc));
	EXIT_ERROR(JVMTI_ERROR_INTERNAL,NULL);
    }
    return env;
}
...
allThreads(jint *count) 
{
    ...
    *count = 0;
    ...
    if (error == JVMTI_ERROR_OUT_OF_MEMORY) {
...
allNestedClasses(jclass parent_clazz, jclass **ppnested, jint *pcount)
{
    ...
    *pcount 	= 0;
	...
	if ( is_a_nested_class(signature, len, candidate_signature, '$') ||
	     is_a_nested_class(signature, len, candidate_signature, '#') ) {
...
createLocalRefSpace(JNIEnv *env, jint capacity) 
{
	...
	EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"PushLocalFrame: Unable to push JNI frame");
    }
...
getPropertyUTF8(JNIEnv *env, char *propertyName)
{
	...
	utf_copy = jvmtiAllocate(strlen(utf) + 1);
	if (utf_copy != NULL) {
	    (void)strcpy(utf_copy, utf);
	}
...
/**
 * Return property value as JDWP allocated string in Default System Encoding
 */
static char *
getPropertySystemEncoding(JNIEnv *env, char *propertyName)
{
    char *cstring;
    jstring valueString;
	...
	/* Call byte[] bytes = valueString.getBytes() */
	bytes = JNI_FUNC_PTR(env,CallObjectMethod)
	    (env, valueString, gdata->stringGetBytes);
	if (JNI_FUNC_PTR(env,ExceptionOccurred)(env)) {
	    JNI_FUNC_PTR(env,ExceptionClear)(env);
            bytes = NULL;
	}

	/* Now we have an array of bytes that should be default encoded bytes */
	if ( bytes != NULL ) {
	    jint len;

	    /* Get length of array */
	    len = JNI_FUNC_PTR(env,GetArrayLength)(env, bytes);
	    if ( len > 0 ) {
		/* make a copy for returning */
		cstring = jvmtiAllocate(len + 1);
		JNI_FUNC_PTR(env,GetByteArrayRegion)(env, bytes, 0, len,
					    (jbyte *)cstring);
		cstring[len] = 0;
	    }
	}
...
eventIndexInit(void)
{
    (void)memset(index2jvmti, 0, sizeof(index2jvmti));
    (void)memset(index2jdwp,  0, sizeof(index2jdwp));
...
eventIndex2jdwp(EventIndex i)
{
	...
	EXIT_ERROR(JVMTI_ERROR_INTERNAL,"bad EventIndex");
    }
...
jvmti2EventIndex(jvmtiEvent kind)
{
    switch ( kind ) {
        ...
        case JVMTI_EVENT_VM_DEATH:
            return EI_VM_DEATH;
        default: 
	    EXIT_ERROR(JVMTI_ERROR_INVALID_EVENT_TYPE,"JVMTI to EventIndex mapping");
            break;
    }
    return (EventIndex)0;
}
