                 ...
                 jclass arrayClass, jint size, char *componentSignature)
{
	...
	if (serror != JDWP_ERROR(NONE)) {
		...
		(void)outStream_writeByte(out, specificTypeKey(array));
		(void)outStream_writeObjectRef(out, array);
	    }
...
newInstance(PacketInputStream *in, PacketOutputStream *out)
{
    ...
    arrayClass = inStream_readClassRef(in);
