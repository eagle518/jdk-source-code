/*
 * @(#)GlyphIterator.cpp	1.20 03/10/24
 *
...
GlyphIterator::GlyphIterator(LEGlyphID *&theGlyphs, GlyphPositionAdjustment *theGlyphPositionAdjustments, 
  ...
  : direction(1), position(-1), nextLimit(theGlyphCount), prevLimit(-1),
    ...
    charIndicesRef(&theCharIndices), charIndices(theCharIndices), 
    glyphCount(theGlyphCount), insertionList(NULL), ownInsertionList(true), 
    srcIndex(-1), destIndex(-1), lookupFlags(theLookupFlags), featureTag(theFeatureTag), 
    glyphTagsRef(&theGlyphTags), glyphTags(theGlyphTags),
