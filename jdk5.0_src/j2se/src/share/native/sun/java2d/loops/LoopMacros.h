...
#define BodyPreProcessLutLut(SRCPTR, PIXLUT, \
			     DSTPTR, DSTTYPE, DSTPREFIX, \
			     SXVAR, DXVAR) \
...
#define SetupConvertOnTheFlyLut(SRC, DST, PIXLUT, SRCINFO, DSTINFO) \
    PIXLUT = (SRCINFO)->lutBase

#define BodyConvertOnTheFlyLut(SRCPTR, PIXLUT, \
			       DSTPTR, DSTTYPE, DSTPREFIX, \
			       SXVAR, DXVAR) \
    do { \
	jint argb = PIXLUT[SRCPTR[SXVAR]]; \
	Store ## DSTTYPE ## From1IntArgb(DSTPTR, DSTPREFIX, DXVAR, argb); \
    } while (0)
...
#define BodyPreProcessLutXparLut(SRCPTR, PIXLUT, \
				 DSTPTR, DSTTYPE, DSTPREFIX, \
				 SXVAR, DXVAR) \
    do { \
...
#define BodyConvertOnTheFlyXparLut(SRCPTR, PIXLUT, \
			           DSTPTR, DSTTYPE, DSTPREFIX, \
				   SXVAR, DXVAR) \
    do { \
	jint argb = PIXLUT[SRCPTR[SXVAR]]; \
...
#define BodyPreProcessLutBgLut(SRCPTR, PIXLUT, \
			       DSTPTR, DSTTYPE, DSTPREFIX, \
			       SXVAR, DXVAR, BGPIXEL) \
    do { \
	jint pix = PIXLUT[SRCPTR[SXVAR]]; \
...
#define BodyConvertOnTheFlyBgLut(SRCPTR, PIXLUT, \
			         DSTPTR, DSTTYPE, DSTPREFIX, \
			         SXVAR, DXVAR, BGPIXEL) \
    do { \
	jint argb = PIXLUT[SRCPTR[SXVAR]]; \
...
#define REGISTER_SOLID_FILLRECT(TYPE) \
    REGISTER_FILLRECT(OpaqueColor, SrcNoEa, TYPE, NAME_SOLID_FILLRECT(TYPE))

#define REGISTER_SOLID_FILLSPANS(TYPE) \
    REGISTER_FILLSPANS(OpaqueColor, SrcNoEa, TYPE, NAME_SOLID_FILLSPANS(TYPE))

#define REGISTER_SOLID_LINE_PRIMITIVES(TYPE) \
    REGISTER_LINE_PRIMITIVES(OpaqueColor, SrcNoEa, TYPE, \
...
#define REGISTER_SOLID_DRAWGLYPHLIST(TYPE) \
    REGISTER_DRAWGLYPHLIST(OpaqueColor, SrcNoEa, TYPE, \
                           NAME_SOLID_DRAWGLYPHLIST(TYPE))

#define REGISTER_SOLID_DRAWGLYPHLISTAA(TYPE) \
    REGISTER_DRAWGLYPHLISTAA(OpaqueColor, SrcNoEa, TYPE, \
...
{ \
 ...
 \
    ...
    BlitLoopScaleWidthHeight(SRC, pSrc, srcBase, pSrcInfo, \
			     ...
			     x, width, height, \
			     sxloc, syloc, sxinc, syinc, shift, \
			     Body ## LUT_STRATEGY ## Lut(pSrc, pixLut, \
							 pDst, DST, \
							 DstWrite, x, 0));\
}
...
{ \
 ...
 \
    ...
    BlitLoopWidthHeight(SRC, pSrc, srcBase, pSrcInfo, \
			DST, pDst, dstBase, pDstInfo, DstWrite, \
			width, height, \
			Body ## LUT_STRATEGY ## XparLut(pSrc, pixLut, \
							pDst, DST, \
							DstWrite, 0, 0)); \
}
...
{ \
 ...
 \
    ...
    BlitLoopWidthHeight(SRC, pSrc, srcBase, pSrcInfo, \
		        DST, pDst, dstBase, pDstInfo, DstWrite, \
		        width, height, \
		        Body ## LUT_STRATEGY ## BgLut(pSrc, pixLut, \
						      pDst, DST, \
						      DstWrite, 0, 0, \
						      bgpixel)); \
}
 ...
 */
#define ClipDrawGlyphList(DST, PIXELS, ROWBYTES, WIDTH, HEIGHT, \
                          LEFT, TOP, RIGHT, BOTTOM, \
...
\
    ...
    for (glyphCounter = 0; glyphCounter < totalGlyphs; glyphCounter++) { \
        DeclareDrawGlyphListClipVars(pixels, rowBytes, width, height, \
                                     left, top, right, bottom) \
        ClipDrawGlyphList(DST, pixels, rowBytes, width, height, \
                          left, top, right, bottom, \
                          clipLeft, clipTop, clipRight, clipBottom, \
