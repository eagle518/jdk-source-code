/*
 * @(#)awt_ImagingLib.c	1.81 04/04/22
 *
                                         ...
                                         jobject jkernel, jint edgeHint) 
{
    ...
    if ((dkern = (mlib_d64 *) calloc(1, w*h*sizeof(mlib_d64))) == NULL) {
    ...
    if ((kdata = (mlib_s32 *) malloc(w*h*sizeof(mlib_s32))) == NULL) {
                                           ...
                                           jobjectArray jtableArrays)
{
    ...
    if (nbands < ncomponents ||
        (jlen==1 && srcImageP->cmodel.supportsAlpha))
    {
        int j;
...
Java_sun_awt_image_ImagingLib_init(JNIEnv *env, jobject this) {
    char *start;
    ...
    awt_getImagingLib(env, &sMlibFns, &sMlibSysFns );
}
