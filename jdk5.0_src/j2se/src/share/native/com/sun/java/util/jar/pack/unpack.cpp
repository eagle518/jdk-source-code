...
#define MDL1 \
  ...
  for (i = 0; i < ATTR_CONTEXT_LIMIT; i++) {
    if (i == ATTR_CONTEXT_CODE)  continue;
...
#if 0 /* Comment out predefined StackMap bands, for now. */
    count = ad.predefCount(CODE_ATTR_StackMap);
    code_StackMap_N.readData(count);
    count = code_StackMap_N.getIntTotal();
    code_StackMap_P.readData(count);
    code_StackMap_local_N.readData(count);
    code_StackMap_stack_N.readData(count);
    count =  code_StackMap_local_N.getIntTotal();
    count += code_StackMap_stack_N.getIntTotal();
    code_StackMap_T.readData(count);
    count = code_StackMap_T.getIntCount(StackMap_RC_tag);
    code_StackMap_RC.readData(count);
    count = code_StackMap_T.getIntCount(StackMap_uninit_P_tag);
    code_StackMap_uninit_P.readData(count);
#endif
...
void unpacker::attr_definitions::readBandData(int idx) {
  ...
  if (lo != null) {
    printcr(1, "counted %d attributes of type %s.%s",
	    count, ATTR_CONTEXT_NAME[attrc], lo->name);
  }
  bool hasCallables = lo->hasCallables();
  band** bands = lo->bands();
  if (!hasCallables) {
...
#if 0 /* Comment out predefined StackMap bands, for now. */
      case ADH_BYTE(ATTR_CONTEXT_CODE, CODE_ATTR_StackMap):
	aname = cp.sym[cpool::s_StackMap];
	putu2(count = code_StackMap_N.getInt());
	for (j = 0; j < count; j++) {
	  putu2(to_bci(code_StackMap_P.getInt()));
	  int count2, k, tag;
	  for (int isStack = 0; isStack <= 1; isStack++) {
	    band* slot_count
	      = !isStack? &code_StackMap_local_N: &code_StackMap_stack_N;
	    putu2(count2 = (*slot_count).getInt());
	    for (k = 0; k < count2; k++) {
	      putu1(tag = code_StackMap_T.getByte());
	      switch (tag) {
	      case StackMap_RC_tag:
		putref(code_StackMap_RC.getRefN());
		break;
	      case StackMap_uninit_P_tag:
		putu2(to_bci(code_StackMap_uninit_P.getInt()));
		break;
	      }
	    }
	  }
...
void unpacker::write_file_to_jar(unpacker::file* f) {
  ...
  printcr(2,"wrote %d bytes to jar entry %s", (int) fsize, f->name);
}
