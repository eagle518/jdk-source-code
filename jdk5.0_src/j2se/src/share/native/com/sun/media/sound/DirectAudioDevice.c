/*
 * @(#)DirectAudioDevice.c	1.11 04/04/01
 *
...
void handleSignEndianConversion(INT8* data, INT8* output, int byteSize, int conversionSize) {
	...
	UINT16* outputSample = (UINT16*) output;
	byteSize = byteSize / 2;
	while (byteSize > 0) {
	    *outputSample = ((*sample & 0xFF) << 8) | ((*sample & 0xFF00) >> 8);
	    byteSize--;
	    sample++;
	    outputSample++;
	}
	...
	UINT32* outputSample = (UINT32*) output;
	byteSize = byteSize / 4;
	while (byteSize > 0) {
	    *outputSample = ((*sample & 0xFF) << 24)
		| ((*sample & 0xFF00) << 8)
		| ((*sample & 0xFF0000) >> 8)
...
#define LOOP_M(INPUT, OUTPUT, TO_SAMPLE, FROM_SAMPLE)                      \
    for ( ; len > 0; --len) {                                              \
	CONVERT_SAMPLE(INPUT, OUTPUT, TO_SAMPLE, FROM_SAMPLE, leftGain);   \
    }                                                                      \
    break

/* macro for conversion of a stereo block */
#define LOOP_S(INPUT, OUTPUT, TO_SAMPLE, FROM_SAMPLE)                      \
    for ( ; len > 0; --len) {                                              \
	CONVERT_SAMPLE(INPUT, OUTPUT, TO_SAMPLE, FROM_SAMPLE, leftGain);   \
	CONVERT_SAMPLE(INPUT, OUTPUT, TO_SAMPLE, FROM_SAMPLE, rightGain);  \
    }                                                                      \
    break
...
void handleGainAndConversion(DAUDIO_Info* info, UINT8* input, UINT8* output,
    ...
    switch (FORMAT2CODE(info->channels,
                        ...
                        info->isBigEndian)) {
	/* 8-bit mono */
    case FORMAT2CODE8(1, 0, 0): LOOP_M(input8, output8, MAP_UINT82SAMPLE, MAP_SAMPLE2UINT8);
    case FORMAT2CODE8(1, 0, 1): LOOP_M(input8, output8, MAP_UINT82SAMPLE, MAP_SAMPLE2INT8);
    case FORMAT2CODE8(1, 1, 0): LOOP_M(input8, output8, MAP_INT82SAMPLE, MAP_SAMPLE2UINT8);
    case FORMAT2CODE8(1, 1, 1): LOOP_M(input8, output8, MAP_INT82SAMPLE, MAP_SAMPLE2INT8);

    /* 8-bit stereo */
    case FORMAT2CODE8(2, 0, 0): LOOP_S(input8, output8, MAP_UINT82SAMPLE, MAP_SAMPLE2UINT8);
    case FORMAT2CODE8(2, 0, 1): LOOP_S(input8, output8, MAP_UINT82SAMPLE, MAP_SAMPLE2INT8);
    case FORMAT2CODE8(2, 1, 0): LOOP_S(input8, output8, MAP_INT82SAMPLE, MAP_SAMPLE2UINT8);
    case FORMAT2CODE8(2, 1, 1): LOOP_S(input8, output8, MAP_INT82SAMPLE, MAP_SAMPLE2INT8);

    /* 16-bit mono (only signed is accepted) */
    case FORMAT2CODE16(1, 0, 0):  LOOP_M(input16, output16, MAP_LE_SHORT2SAMPLE, MAP_SAMPLE2LE_SHORT);
    case FORMAT2CODE16(1, 0, 1):  LOOP_M(input16, output16, MAP_LE_SHORT2SAMPLE, MAP_SAMPLE2BE_SHORT);
    case FORMAT2CODE16(1, 1, 0):  LOOP_M(input16, output16, MAP_BE_SHORT2SAMPLE, MAP_SAMPLE2LE_SHORT);
    case FORMAT2CODE16(1, 1, 1):  LOOP_M(input16, output16, MAP_BE_SHORT2SAMPLE, MAP_SAMPLE2BE_SHORT);

    /* 16-bit stereo (only signed is accepted) */
    case FORMAT2CODE16(2, 0, 0):  LOOP_S(input16, output16, MAP_LE_SHORT2SAMPLE, MAP_SAMPLE2LE_SHORT);
    case FORMAT2CODE16(2, 0, 1):  LOOP_S(input16, output16, MAP_LE_SHORT2SAMPLE, MAP_SAMPLE2BE_SHORT);
    case FORMAT2CODE16(2, 1, 0):  LOOP_S(input16, output16, MAP_BE_SHORT2SAMPLE, MAP_SAMPLE2LE_SHORT);
    case FORMAT2CODE16(2, 1, 1):  LOOP_S(input16, output16, MAP_BE_SHORT2SAMPLE, MAP_SAMPLE2BE_SHORT);
