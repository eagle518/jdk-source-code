...
void initThreadClass(JNIEnv *env) {
    jclass tc = (*env)->FindClass(env, "java/lang/Thread");
    threadClass = (*env)->NewGlobalRef(env, tc);
}


#define AWT_CHECK_HAVE_LOCK() do {                      \
  if (threadClass == NULL) initThreadClass(env);  \
  if (!JVM_HoldsLock(env, threadClass, awt_lock))           \
  { JNU_ThrowInternalError(env,"Following Thread does not have AWT_LOCK !!!!!\n"); \
  } \
} while (0)
#else
...
(JNIEnv *env, jclass clazz, jlong display_name)
{
    Display *dp;
    AWT_CHECK_HAVE_LOCK();   
    dp  =  XOpenDisplay((char *) display_name);
...
Java_sun_awt_X11_XlibWrapper_XDisplayString(JNIEnv *env, jclass clazz, 
                        jlong display) {
    AWT_CHECK_HAVE_LOCK();   
    return (jlong) XDisplayString((Display*)display);
}
...
JNIEXPORT jlong JNICALL Java_sun_awt_X11_XlibWrapper_DefaultScreen (JNIEnv *env, jclass clazz, jlong display) {
    ...
    AWT_CHECK_HAVE_LOCK();   
    return (jlong) DefaultScreen((Display *)display);
}
...
(JNIEnv *env, jclass clazz, jlong display, jlong screen) {
    ...
    AWT_CHECK_HAVE_LOCK();   
    return (jlong) DisplayWidth((Display *) display,screen);
...
(JNIEnv *env, jclass clazz, jlong display, jlong screen) {
    AWT_CHECK_HAVE_LOCK();   
    return (jlong) DisplayWidthMM((Display *) display,screen);
}
...
(JNIEnv *env, jclass clazz, jlong display, jlong screen) {
    ...
    AWT_CHECK_HAVE_LOCK();   
    return (jlong) DisplayHeight((Display *) display,screen);
}
/*
 * Class:     sun_awt_X11_XlibWrapper
...
(JNIEnv *env, jclass clazz, jlong display, jlong screen) {
    AWT_CHECK_HAVE_LOCK();   
    return (jlong) DisplayHeightMM((Display *) display,screen);
}
...
(JNIEnv *env , jclass clazz, jlong display, jlong screen_number) {
    AWT_CHECK_HAVE_LOCK();   
    return (jlong) RootWindow((Display *) display, screen_number);
}
...
(JNIEnv *env , jclass clazz, jlong display) {
    AWT_CHECK_HAVE_LOCK();   
    return ScreenCount((Display *) display);
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    return  XCreateWindow((Display *) display,(Window) window, x, y, w, h,
              border_width, depth, wclass, (Visual *) visual,
...
(JNIEnv *env, jclass clazz, jlong display, jlong window)
{
    ...
    AWT_CHECK_HAVE_LOCK();   
    XSetInputFocus( (Display *)display,(Window) window, RevertToParent, CurrentTime);
...
(JNIEnv *env, jclass clazz, jlong display, jlong window, jlong time)
{
    ...
    AWT_CHECK_HAVE_LOCK();   
    XSetInputFocus( (Display *)display,(Window) window, RevertToParent, time);
...
(JNIEnv *env, jclass clazz, jlong display)
{
    ...
    int revert_to;
    AWT_CHECK_HAVE_LOCK();   
    XGetInputFocus( (Display *)display, &focusOwner, &revert_to);
    return focusOwner;
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    return XGrabPointer( (Display *)display, (Window) window,
...
{
    AWT_CHECK_HAVE_LOCK();   
    return XGrabKeyboard( (Display *)display, (Window) window,
...
{
    AWT_CHECK_HAVE_LOCK();   
    XNextEvent( (Display *) display,(void *)ptr);
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    XMaskEvent( (Display *) display, event_mask, (XEvent *) event_return);
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    XWindowEvent( (Display *) display, (Window)window, event_mask, (XEvent *) event_return);
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    return (jboolean)XFilterEvent((XEvent *)ptr, (Window)window);
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    XPeekEvent((Display *) display,(void *)ptr);
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    XResizeWindow( (Display *) display,(Window) window,width,height);
}
 ...
 * Signature: (JJI)J
 */
JNIEXPORT jlong JNICALL Java_sun_awt_X11_XlibWrapper_XInternAtom
(JNIEnv *env, jclass clazz, jlong display, jlong atom_name, jint ife) {
 ...
 */
JNIEXPORT void JNICALL Java_sun_awt_X11_XlibWrapper_XChangeProperty(
    ...
    jlong type, jint format, jint mode, jlong data, jint nelements) 
{
    ...
    XChangeProperty((Display*)display, window, (Atom)property,
            (Atom)type, format, mode, (unsigned char*)data,
            nelements);
}
/*
 * Class:     sun_awt_X11_XlibWrapper
...
{
    AWT_CHECK_HAVE_LOCK();   
    return XGetWindowProperty((Display*)display, window, property, long_offset, long_length,
                  delete, (Atom)req_type, (Atom*)actual_type, 
                  (int *)actual_format, (unsigned long *)nitems_ptr, 
                  (unsigned long*) bytes_after, (unsigned char**) data_ptr);
}
...
(JNIEnv *env, jclass clazz, jlong display, jlong window, jlong atom) 
{
    ...
    jstring res;
    AWT_CHECK_HAVE_LOCK();   
    status = XGetWindowProperty((Display*)display, window,
...
(JNIEnv *env, jclass clazz, jlong display, jstring jstr, jint ife) {
    ...
    AWT_CHECK_HAVE_LOCK();   
    atom = XInternAtom((Display *) display, cname, ife);
...
(JNIEnv *env, jclass clazz, jlong display, jint shape) {
    AWT_CHECK_HAVE_LOCK();   
    return XCreateFontCursor((Display *) display, (int) shape);
}
...
(JNIEnv *env , jclass clazz, jlong display, jlong source, jlong mask, jlong fore, jlong back, jint x , jint y) {
    ...
    AWT_CHECK_HAVE_LOCK();   
    return (jlong) XCreatePixmapCursor((Display *)display, (Pixmap) source, (Pixmap) mask, (XColor *) fore, (XColor *) back, x, y);
}
...
(JNIEnv *env, jclass clazz, jlong display, jlong drawable, jint width, jint height, jlong width_return, jlong height_return) {
    ...
    AWT_CHECK_HAVE_LOCK();   
    status  =  XQueryBestCursor((Display *) display, (Drawable) drawable, width,height, (unsigned int *) width_return, (unsigned int *) height_return);
...
(JNIEnv *env, jclass clazz, jlong display, jlong w, jlong root_return, jlong child_return, jlong root_x_return , jlong root_y_return, jlong win_x_return, jlong win_y_return, jlong mask_return) {
    ...
    AWT_CHECK_HAVE_LOCK();   
    b = XQueryPointer((Display *) display, (Window) w, (Window *) root_return, (Window *) child_return, (int *) root_x_return, (int *) root_y_return, (int *) win_x_return, (int *) win_y_return, (unsigned int *) mask_return);
...
(JNIEnv *env, jclass clazz, jlong display, jlong window, jlong valuemask, jlong attributes) {
    ...
    AWT_CHECK_HAVE_LOCK();   
    XChangeWindowAttributes((Display *) display, (Window) window, (unsigned long) valuemask, (XSetWindowAttributes *) attributes);
...
{
    AWT_CHECK_HAVE_LOCK();   
    XSetTransientForHint((Display*)display, window, transient_for_window);
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    XSetWMHints((Display*)display, window, (XWMHints*)hints);
}
...
(JNIEnv *env, jclass clazz, jlong display, jlong window, jlong hints) 
{
    XWMHints * get_hints;
    AWT_CHECK_HAVE_LOCK();   
    get_hints = XGetWMHints((Display*)display, window);
    if (get_hints != NULL) {
        memcpy((void*)hints, get_hints, sizeof(XWMHints));
        XFree(get_hints);
    } else {
        memset((void*)hints, 0, sizeof(XWMHints));
    }
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    return XGetPointerMapping((Display*)display, (unsigned char*)map, buttonNumber);
}
...
(JNIEnv *env, jclass clazz, jlong display, jstring program, jstring option) 
{
    ...
    AWT_CHECK_HAVE_LOCK();   
    c_res = XGetDefault((Display*)display, c_program, c_option);
...
{
    AWT_CHECK_HAVE_LOCK();   
    return XIconifyWindow((Display*)display, window, screenNumber);
}
...
{
    unsigned char * str = (unsigned char*)str_ptr;
...
{
    AWT_CHECK_HAVE_LOCK();   
    return JNU_NewStringPlatform(env, ServerVendor((Display*)display));   
}
...
static int ToolkitErrorHandler(Display * dpy, XErrorEvent * event) {
    if (jvm != NULL) {
        ...
        return JNU_CallStaticMethodByName(env, NULL, "sun/awt/X11/XToolkit", "GlobalErrorHandler", "(JJ)I", (jlong)dpy, (jlong)event).i;
    } else {
        return 0;
    }
}
...
(JNIEnv *env, jclass clazz) 
{
    ...
    AWT_CHECK_HAVE_LOCK();   
    return (jlong)XSetErrorHandler(ToolkitErrorHandler);
}
...
{
    return (*(XErrorHandler)handler)((Display*)display, (XErrorEvent*)event_ptr);
}
...
(JNIEnv *env, jclass clazz, jlong display, jobjectArray names_arr, jboolean only_if_exists, jlong atoms) 
{
    ...
    AWT_CHECK_HAVE_LOCK();   
    status = XInternAtoms((Display*)display, names, name_index, only_if_exists, (Atom*)atoms);
    for (index = 0; index < length; index++) {
        free(names[index]);
    }
    free(names);
...
(JNIEnv *env, jclass clazz, jlong display, jlong window, jlong attr_ptr) 
{
    jint status;
    AWT_CHECK_HAVE_LOCK();   
    status =  XGetWindowAttributes((Display*)display, window, (XWindowAttributes*)attr_ptr);
    return status;
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    return XGetWMNormalHints((Display*)display, window, (XSizeHints*)hints, (long*)supplied_return);
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    XDeleteProperty((Display*)display, window, (Atom)atom);
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    return XSendEvent((Display*)display, window, propagate==JNI_TRUE?True:False, (long)event_mask, (XEvent*)event);
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    return XQueryTree((Display*)display, window, (Window *)root_return, (Window*)parent_return, (Window**)children_return, (unsigned int*)nchildren_return);
}
...
{
    memcpy((void*)dest_ptr, (void*)src_ptr, length);
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    return (jlong)XGetVisualInfo((Display*)display, (long)vinfo_mask, (XVisualInfo*)vinfo_template, (int*)nitems_return);
}
...
(JNIEnv *env, jclass clazz, jlong display , jlong colormap, jlong xcolor) {
    ...
    Status status;
    AWT_CHECK_HAVE_LOCK();   
    status = XAllocColor((Display *) display, (Colormap) colormap, (XColor *) xcolor);
...
(JNIEnv *env, jclass clazz, jlong display, jlong drawable, jlong data, jint width, jint height) {
    ...
    return (jlong) XCreateBitmapFromData((Display *) display, (Drawable) drawable, (char *) data, width, height);
}
...
(JNIEnv *env, jclass clazz, jlong display, jlong window, jlong parent, jint x, jint y) {
    AWT_CHECK_HAVE_LOCK();   
    XReparentWindow((Display*)display, window, parent, x, y);
}
...
Java_sun_awt_X11_XlibWrapper_XConvertSelection(JNIEnv *env, jclass clazz, 
    ...
    AWT_CHECK_HAVE_LOCK();   
    XConvertSelection((Display*)display, selection, target, property, requestor,
              time);
}
...
Java_sun_awt_X11_XlibWrapper_XSetSelectionOwner(JNIEnv *env, jclass clazz, 
    ...
    AWT_CHECK_HAVE_LOCK();   
    XSetSelectionOwner((Display*)display, selection, owner, time);
}
...
Java_sun_awt_X11_XlibWrapper_XGetSelectionOwner(JNIEnv *env, jclass clazz, 
                        jlong display, jlong selection) {
    AWT_CHECK_HAVE_LOCK();   
    return (jlong)XGetSelectionOwner((Display*)display, selection);
}
                      ...
                      jlong display, jlong atom)
{
    ...
    char* name;
    AWT_CHECK_HAVE_LOCK();   
    name = (char*) XGetAtomName((Display*)display, atom);
...
Java_sun_awt_X11_XlibWrapper_XMaxRequestSize(JNIEnv *env, jclass clazz, 
                         jlong display) {
    AWT_CHECK_HAVE_LOCK();   
    return XMaxRequestSize((Display*)display);
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    return XCreatePixmap((Display*)display, (Drawable)drawable, width, height, depth);
}
JNIEXPORT jlong JNICALL 
...
{
    AWT_CHECK_HAVE_LOCK();   
    return (jlong) XCreateImage((Display*)display, (Visual*)visual_ptr,
                depth, format, offset, (char*)data,
...
{
    AWT_CHECK_HAVE_LOCK();
    return (jlong) XCreateGC((Display*)display, (Drawable)drawable, valuemask, (XGCValues*)values);
}
...
{
    AWT_CHECK_HAVE_LOCK();   
    XPutImage((Display*)display, (Drawable)drawable, (GC)gc, (XImage*)image, src_x, src_y,
...
{
    AWT_CHECK_HAVE_LOCK();   
    XSetWindowBackgroundPixmap((Display*)display, (Window)window, (Pixmap)pixmap);
}
JNIEXPORT void JNICALL 
...
{
    XIconSize** psize = (XIconSize**)ret_sizes;
    int * pcount = (int *)ret_count;
    Status res;
    AWT_CHECK_HAVE_LOCK();
    res = XGetIconSizes((Display*)display, (Window)window, psize, pcount);
    return res;
}
...
{
    AWT_CHECK_HAVE_LOCK();
    return XdbeQueryExtension((Display*) display, (int *) major_version_return,
...
{
    AWT_CHECK_HAVE_LOCK();
    return XdbeAllocateBackBufferName((Display*) display, (Window) window,
...
{
    AWT_CHECK_HAVE_LOCK();
    return XdbeDeallocateBackBufferName((Display*) display, (XdbeBackBuffer) buffer);
}
...
{
    AWT_CHECK_HAVE_LOCK();
    return XdbeBeginIdiom((Display*) display);
}
...
{
    AWT_CHECK_HAVE_LOCK();
    return XdbeEndIdiom((Display*) display);
}
...
{
    AWT_CHECK_HAVE_LOCK();
    return XdbeSwapBuffers((Display*) display, (XdbeSwapInfo *) swap_info, num_windows);
}
...
Java_sun_awt_X11_XlibWrapper_XKeycodeToKeysym(JNIEnv *env, jclass clazz, 
                                              ...
                                              jint index) {
    AWT_CHECK_HAVE_LOCK();   
    return XKeycodeToKeysym((Display*)display, (unsigned int)keycode, (int)index);
}
...
Java_sun_awt_X11_XlibWrapper_XKeysymToKeycode(JNIEnv *env, jclass clazz, 
    ...
    AWT_CHECK_HAVE_LOCK();   
    return XKeysymToKeycode((Display*)display, (KeySym)keysym);
}
...
Java_sun_awt_X11_XlibWrapper_XGetModifierMapping(JNIEnv *env, jclass clazz, 
                                              jlong display) { 
    AWT_CHECK_HAVE_LOCK();   
    return (jlong) XGetModifierMapping((Display*)display);
}
...
Java_sun_awt_X11_XlibWrapper_XFreeModifiermap(JNIEnv *env, jclass clazz, 
                                              jlong keymap) { 
    AWT_CHECK_HAVE_LOCK();   
    XFreeModifiermap((XModifierKeymap*)keymap);
}
...
Java_sun_awt_X11_XlibWrapper_XChangeActivePointerGrab(JNIEnv *env, jclass clazz, 
    ...
    AWT_CHECK_HAVE_LOCK();   
    XChangeActivePointerGrab((Display*)display, (unsigned int)mask, 
...
Java_sun_awt_X11_XlibWrapper_XNextSecondaryLoopEvent(JNIEnv *env, jclass clazz, 
    ...
    while (!exitSecondaryLoop) {
        if (XCheckIfEvent((Display*)display, (XEvent*)ptr, secondary_loop_event, NULL)) {
            return JNI_TRUE;
        }
