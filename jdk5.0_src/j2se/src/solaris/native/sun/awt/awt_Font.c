  ...
  (JNIEnv *env, jclass cls)
{
    ...
    fontDescriptorIDs.nativeName =
      (*env)->GetFieldID(env, cls, "nativeName",
			 "Ljava/lang/String;");
    fontDescriptorIDs.fontCharset =
      (*env)->GetFieldID(env, cls, "fontCharset",
...
awtJNI_GetFontData(JNIEnv * env, jobject font, char **errmsg)
{
    ...
    if (!JNU_IsNull(env, font) && awtJNI_IsMultiFont(env, font)) {
        ...
        jobject charToByteConverter = NULL;
        jstring charToByteConverterName = NULL;
        ...
        for (i = 0; i < fdata->charset_num; i++) {
	      ...
	      (*env)->GetObjectField(env, fontDescriptor,
            ...
            fdata->flist[i].charset_name = (char *)
                JNU_GetStringPlatformChars(env, charToByteConverterName, NULL);

	    /* We are done with the objects. */
	    (*env)->DeleteLocalRef(env, fontDescriptor);
	    (*env)->DeleteLocalRef(env, fontDescriptorName);
	    (*env)->DeleteLocalRef(env, charToByteConverter);
	    (*env)->DeleteLocalRef(env, charToByteConverterName);
  ...
  (JNIEnv *env, jobject this)
{
    ...
    AWT_LOCK();
    fdata = (struct FontData *)JNU_GetLongFieldAsPtr(env,this,fontIDs.pData);
    ...
    if (awtJNI_IsMultiFont(env, this)) {
        for (i = 0; i < fdata->charset_num; i++) {
    ...
    (*env)->SetLongField(env,this,fontIDs.pData,(jlong) NULL);	
    AWT_UNLOCK();
#endif /* !HEADLESS */
}
