...
void callFocusCallback(jobject focusPeer, int focus_type) {
...
void callFocusHandler(Widget w, int eventType) {
    jobject peer = NULL;
    ...
    cont = FALSE;
    handleFocusEvent(w, &event, (XtPointer)peer, &cont, TRUE);
}
			  ...
			  XEvent* event)
{
    ...
    static jmethodID mid = NULL;
    char *clsName = "java/awt/event/FocusEvent";
    jobject hEvent;
                          ...
                          jint wheelAmt, jint button)
{
    ...
    if (id == java_awt_event_MouseEvent_MOUSE_WHEEL) {
        ...
        hEvent = (*env)->NewObject(env, classMouseEvent, mid,
                                   target, id, jWhen, modifiers,
                                   x, y, clickcount, popuptrigger, button);
    }
