...
void clearFocusPathOnWindow(Window win) {
    if (focusProxyWindow != None && IsRootOf(win, trueFocusWindow)) {
        ...
        XSetInputFocus(awt_display, findShellByProxy(focusProxyWindow), RevertToParent, CurrentTime);
                    ...
                    if (xev.type == ButtonPress) {
                        ...
                        if (window != None) {
                            ...
                            if (winAttr.override_redirect == TRUE && isFocusableWindow(window)) {
                                XSetInputFocus(awt_display, window, RevertToParent, CurrentTime);
                            }
                        }
                    }
...
awt_xsettings_update(int scr, Window owner, void *cookie)
{
	...
	fprintf(stderr, "awt_xsettings_update: NewByteArray failed\n");
	XFree(xsettings);
	return;
    }
    ...
    jstring mainClassName)
{
    ...
    /* fprintf(stderr, "**** awt_multiclick_time = %d\n", awt_multiclick_time); */
