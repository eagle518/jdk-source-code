...
/*
 * @(#)WindowsSelectorImpl.java	1.8 04/04/20
 */
...
final class WindowsSelectorImpl extends SelectorImpl {   
    ...
    private final static class FdMap extends HashMap {
        private MapEntry get(int desc) {
            return (MapEntry)get(new Integer(desc));
        }        
        private Object put(SelectionKeyImpl ski) {
            return put(new Integer(ski.channel.getFDVal()), new MapEntry(ski));
        }        
        private Object remove(SelectionKeyImpl ski) {
            return remove(new Integer(ski.channel.getFDVal()));
        }
    }                    
