...
{
    JNIEnv *env = (JNIEnv *)JNU_GetEnv(jvm, JNI_VERSION_1_2);
    jobject localObj = env->NewLocalRef(m_peerObject);
    if (localObj != NULL) {
        JNI_SET_PDATA(localObj, NULL);
	env->DeleteLocalRef(localObj);
    }
    env->DeleteWeakGlobalRef(m_peerObject);
}
...
void AwtRobot::GetRGBPixels(jint x, jint y, jint width, jint height, jintArray pixelArray)
{
    ...
    VERIFY(::BitBlt(hdcMem, 0, 0, width, height, hdcScreen, x, y, SRCCOPY) != 0);
    ...
    JNIEnv *env, jobject self)
{
    ...
    AwtRobot::GetRobot(self)->scheduleDelete();

    DASSERT(JNI_GET_PDATA(self) == NULL);
