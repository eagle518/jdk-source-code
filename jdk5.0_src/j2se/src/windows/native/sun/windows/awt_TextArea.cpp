/*
 * @(#)awt_TextArea.cpp	1.62 03/12/19
 *
					       ...
					       jint start, jint end)
{
    ...
    JNI_CHECK_NULL_RETURN(text, "null string");

    AwtTextComponent* c = (AwtTextComponent*)pData;
    jstring targetText =
	Java_sun_awt_windows_WTextComponentPeer_getText(env, self);

    /* Adjust positions with the CRs going to be added. */
    size_t cr_start = AwtTextArea::CountNewLines(env, targetText, start);
    size_t cr_end = AwtTextArea::CountNewLines(env, targetText, end);

    start += static_cast<jint>(cr_start);
    end += static_cast<jint>(cr_end);
