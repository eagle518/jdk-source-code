...
#pragma ident "@(#)libjvm_db.c	1.16 04/04/20 10:38:17 JVM"
#endif
/*
...
typedef struct {
    ...
    uintptr_t new_fp; /* fp for the next frame */
    uintptr_t         new_pc; /* pc for the next frame */
    uintptr_t         new_sp; /* sp for the next frame */
...
static int pc_desc_at(Nmethod_t *N)
{
  ...
  for (offs = N->scopes_pcs_beg; offs < N->scopes_pcs_end; offs += SIZE_PcDesc) {
      ...
      if ((pc_diff == 0) /* && (pc_desc->at_call() == at_call) */) { 
          N->pc_desc = pd;
          if (debug) {
...
#ifdef X86_COMPILER2
  /*
   * This is very special case when previous frame is I2CAdapter.
   * It is a temporary workaround hack bellow.
   */
  if (J->i2c_fp) {
    int32_t fp_value;

    if (debug > 2) {
      printf("Jlookup_by_regs: J->i2c_fp = %#lx\n", J->i2c_fp);
    }
    J->i2c_fp = 0;
    err = ps_pread(J->P, J->prev_fr.fp + 2 * SZ32, &jframe->new_sp, SZ32);
    CHECK_FAIL(err);
    J->curr_fr.fp = jframe->new_sp - 2 * SZ32;
    err = ps_pread(J->P, jframe->new_sp - SZ32, &jframe->new_pc, SZ32);
    CHECK_FAIL(err);

    jframe->new_fp = jframe->new_sp - 2 * SZ32;

    /*
     * This is also a workaround for fp's pointing to 0.
     */
    err = ps_pread(J->P, jframe->new_fp, &fp_value, SZ32);
    CHECK_FAIL(err);
    if (!fp_value) {
      /* We return new_fp only if (fp_value != 0) */
      jframe->new_fp = 0;
    }    
  }
#endif /* X86_COMPILER2 */
                   ...
                   size_t size, Jframe_t *jframe)
{
  ...
  if (vtbl == J->nmethod_vtbl) {
    ...
    if (debug) {
        fprintf(stderr, "name_for_codecache: start: %#8llx, methodOop: %#8llx \n",
                        start, methodOop);
    }
    ...
    /* return PS_OK; */
  } else if (vtbl == J->I2CAdapter_vtbl) {
    const char * name;
    ...
    if (err == PS_OK) {
      ...
      strncpy(result, "<unknown I2CAdapter>", size);
    }
...
#ifdef X86_COMPILER2
   {
      ...
      err = ps_pread(J->P, start + OFFSET_CodeBlob_frame_size,
                         &frame_size, SZ32);
      CHECK_FAIL(err);
      frame_size <<= 2;	/* in bytes now */

      err = ps_pread(J->P, start + OFFSET_CodeBlob_link_offset,
                           &link_offset, SZ32);
      CHECK_FAIL(err);

      err = ps_pread(J->P, J->prev_fr.fp + 2 * SZ32, &J->i2c_fp, SZ32);
      CHECK_FAIL(err);

      /*
       * Some very TMP and HACK workaround to get new_sp and new_pc.
       * It has to be re-implemented basing on libjvm sources.
       * This hack is based on memory layout for particular testcase.
       * It is to prototype all other logic.
       */

      err = ps_pread(J->P, J->prev_fr.fp + SZ32 + frame_size,
                           &marker, SZ32);
      CHECK_FAIL(err);

      jframe->new_sp = marker + 4 * SZ32;

      err = ps_pread(J->P, jframe->new_sp - SZ32, &jframe->new_pc, SZ32);
      CHECK_FAIL(err);

      if (debug) {
        fprintf(stderr, "name_for_codecache: %s, link_offset=%d\n",
                         result, link_offset);
        fprintf(stderr, "name_for_codecache: %s, frame_size =%#lx\n",
                         result, frame_size);
        fprintf(stderr, "name_for_codecache: %s, marker     =%#lx\n",
                         result, marker);
      }
      result[size-1] = '\0';
      return PS_OK;
  ...
  if (vtbl != J->RuntimeStub_vtbl) {
     ...
     */
    if (frame_size > 0) {
      frame_size <<= 2;	/* word => byte conversion */
      jframe->new_fp = J->prev_fr.fp + frame_size;
      jframe->new_sp = jframe->new_fp + 2 * SZ32;
    } else {
      memset(&J->curr_fr, 0, sizeof(Frame_t));
      err = ps_pread(J->P, fp, &jframe->new_fp, SZ32);
      CHECK_FAIL(err);

      err = ps_pread(J->P, jframe->new_fp + SZ32, &jframe->new_pc, SZ32);
      CHECK_FAIL(err);
    }
    if (debug) {
      fprintf(stderr, "name_for_codecache: %s, frame_size=%#lx\n",
...
int Jlookup_by_regs(jvm_agent_t* J, const prgregset_t regs, char *name,
  ...
  if (debug)
      fprintf(stderr, "Jlookup_by_regs: BEGIN: fp=%#lx, pc=%#lx\n", regs[R_FP], pc);
    ...
    if (debug > 2) {
        fprintf(stderr, "\nregs[R_I1]=%lx, regs[R_I2]=%lx, regs[R_L1]=%lx, regs[R_L2]=%lx\n",
                         regs[R_I1], regs[R_I2], regs[R_L1], regs[R_L2]);
    }
    ...
    if (J->prev_fr.fp == 0) {
#ifdef X86_COMPILER2
        J->prev_fr.fp = (uintptr_t)(regs[R_SP] - 2 * SZ32);
	/* A workaround for top java frames */
        /* J->prev_fr.fp = (uintptr_t)(regs[R_SP] - 4); */
        if (J->i2c_fp) {
            /* Previous frame was I2CAdapter */
            fp = J->i2c_fp; /* Trick for interpreted frame after i2cAdapter frame */
        }
#else
        J->prev_fr.fp = (uintptr_t)(regs[R_SP] - 4);
#endif /* COMPILER2 */
    ...
    if (ps_pread(J->P,  fp + OFFSET_interpreter_frame_method,
        &methodOopPtr, sizeof(methodOopPtr)) != PS_OK) {
      methodOopPtr = 0;
    }
    if (ps_pread(J->P,  fp + OFFSET_interpreter_frame_bcx_offset,
        &bcx, sizeof(bcx)) != PS_OK) {
      bcx = 0;
    }
#endif /* i386 */
  ...
  if (is_methodOop(J, methodOopPtr)) {
    result = name_for_imethod(J, bcx, methodOopPtr, name, size, jframe);
  } else
#endif /* ! X86_COMPILER2 */

  if (codecache_contains(J, pc)) {
    result = name_for_codecache(J, fp, pc, name, size, jframe);
  }
#ifdef X86_COMPILER2
  else if (is_methodOop(J, methodOopPtr)) {
    result = name_for_imethod(J, bcx, methodOopPtr, name, size, jframe);
  }
#endif /* X86_COMPILER2 */
  else {
  ...
  if (!J->i2c_fp) {
    ...
    if (!jframe->new_pc && jframe->new_fp) {
      err = ps_pread(J->P, jframe->new_fp + SZ32,
                          &jframe->new_pc, SZ32);
      CHECK_FAIL(err);
    }
  }
...
int Jframe_iter(jvm_agent_t *J, prgregset_t gregs, java_stack_f *func, void* cld) {
     ...
     */
     if (jframe.new_sp) {
         *((long *) &gregs[R_SP]) = (long) jframe.new_sp;
     } else {
         *((long *) &gregs[R_SP]) = (long) gregs[R_FP] + 2 * sizeof(long);
     }

     if (jframe.new_fp) {
         *((long *) &gregs[R_FP]) = (long) jframe.new_fp;
     }
     if (jframe.new_pc) {
         *((long *) &gregs[R_PC]) = (long) jframe.new_pc;
     }
#endif	/* X86_COMPILER2 */
