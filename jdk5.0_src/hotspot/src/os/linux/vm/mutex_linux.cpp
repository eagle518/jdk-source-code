...
#pragma ident "@(#)mutex_linux.cpp	1.28 04/04/16 10:52:17 JVM"
#endif
/*
...
void Mutex::wait_for_lock_blocking_implementation(JavaThread *thread) {
  ...
  do {
    ...
    if (threadIsSuspended) {
      ...
      //
      debug_only(_lock_count--;)
      ((os::Linux::Event*)_lock_event)->unlock();
...
void Mutex::print() const {
  ...
  tty->print_cr("Mutex: [0x%x/0x%x] %s - owner: 0x%x", this, _Lock_Event, _name, _owner);
}
#endif
...
bool Monitor::wait(bool no_safepoint_check, long timeout) {
  ...
  long c = _counter;
    ...
    sigset_t oldsigs;
    pthread_sigmask(SIG_BLOCK, os::Linux::unblocked_signals(), &oldsigs);
  #endif
...
void Parker::park(bool isAbsolute, jlong time) {
  ...
  if (time < 0) { // don't wait at all
    ...
    if (!isAbsolute) {
      // must convert to absolute time on linux
      struct timeval startTime;
      int tstat = gettimeofday(&startTime, NULL);
      assert(tstat == 0, "gettimeofday");
      absTime.tv_sec = time / NANOSECS_PER_SEC + startTime.tv_sec;
      absTime.tv_nsec = (time % NANOSECS_PER_SEC) + startTime.tv_usec * 1000;
  ...
  JavaThread *jt = (JavaThread *)thread;
  os::Linux::Event* const mon = (os::Linux::Event*)_lock_event;
