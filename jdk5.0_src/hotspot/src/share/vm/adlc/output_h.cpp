...
#pragma ident "@(#)output_h.cpp	1.160 04/05/04 13:26:49 JVM"
#endif
/*
...
void ArchDesc::buildMachRegisterNumbers(FILE *fp_hpp) {
  if (_register) {
    ...
    _register->reset_RegDefs();
    while( (reg_def = _register->iter_RegDefs()) != NULL ) {
      fprintf(fp_hpp,"  %s_num,\n", reg_def->_regname);
    }
    // Finish defining enumeration
    fprintf(fp_hpp, "  _last_Mach_Reg\n");
    fprintf(fp_hpp, "};\n");
  }
...
void ArchDesc::declareRegSizes(FILE *fp) {
  if (_register) {
    fprintf(fp,"\n");
    fprintf(fp,"// An array of integers representing the size of each register.\n");
    fprintf(fp,"const int regSize[] = {\n");

    // Output the register size for each register
    _register->reset_RegDefs();
    // Pull out first iteration to start comma handling
    int sz = 0;
    RegDef *reg_def = _register->iter_RegDefs();
    if(reg_def != NULL) {
      if(!strcmp(reg_def->_idealtype, "Op_RegD") || 
         !strcmp(reg_def->_idealtype, "Op_RegL")) {
        sz = 2;
      }
      else {
        sz = 1;
...
static void kill_RegMask(FILE *fp) {
  fprintf(fp,"  virtual const RegMask &kill_RegMask() const;\n");
}
...
void gen_inst_format(FILE *fp, FormDict &globals, InstructForm &inst, bool for_c_file = false) {
  ...
  else if( inst.is_ideal_mem() ) { // Make sure 'Volatile' gets printed out
...
void ArchDesc::declareClasses(FILE *fp) {
  ...
  for( ; (oper = (OperandForm*)_operands.iter()) != NULL;) {
    ...
    // Operand definitions that depend upon number of input edges
    {
      ...
      if( num_edges > 0 ) {
        in_RegMask(fp);
        fprintf(fp,"  static const RegMask*  reg_masks[%d];\n", 
                num_edges );
      }
    }
    ...
    if ( oper->_matrule && oper->_matrule->is_base_register(_globalNames) ) {
      ...
      fprintf(fp,"    return (int)SharedInfo::reg2stack(ra_->get_reg_lo(node));/* sReg */\n");
      fprintf(fp,"  }\n");
      fprintf(fp,"  virtual int            reg(PhaseRegAlloc *ra_, const Node *node, int idx) const {\n");
      fprintf(fp,"    return (int)SharedInfo::reg2stack(ra_->get_reg_lo(node->in(idx)));/* sReg */\n");
      fprintf(fp,"  }\n");
    }
  ...
  for( ; (instr = (InstructForm*)_instructions.iter()) != NULL; ) {
    ...
    fprintf(fp,"public:\n");
    fprintf(fp,"  MachOper *opnd_array(int operand_index) const { assert(operand_index < %d, \"invalid _opnd_array index\"); return _opnd_array[operand_index]; }\n", instr->num_opnds() );
    fprintf(fp,"  void      set_opnd_array(int operand_index, MachOper *operand) { assert(operand_index < %d, \"invalid _opnd_array index\"); _opnd_array[operand_index] = operand; }\n", instr->num_opnds() );
    fprintf(fp,"private:\n");
    if ( instr->is_ideal_jump() ) {
    ...
    fprintf(fp,"  virtual uint           num_opnds() const { return %d; }\n",
            instr->num_opnds() );
    out_RegMask(fp);                      // output register mask
    // kill_RegMask(fp);                     // kill register mask
    ...
    while (attr != NULL) {
      ...
      // Check value for ins_pc_relative, and if it is true (1), output vcall
      if (!strcmp(attr->_ident,"ins_pc_relative") && attr->int_val(*this) != 0)
        fprintf(fp,"  virtual bool is_pc_relative() const { return true; }\n");
    ...
    // virtual function/flag: if this instruction matches an ideal 'Copy*' node
    int idx = instr->is_ideal_copy();
    if( idx ) {
      fprintf(fp,"  virtual uint           is_Copy() const  { return %d; };\n",
              idx);
    }

    if (instr->is_ideal_unlock() || instr->is_ideal_call_leaf()) {
      fprintf(fp,"  virtual bool           is_safepoint_node() const  { return false; };\n");
    }
    ...
    // virtual function/flag: if this instruction matches an ideal 'Goto' node
    if( instr->is_ideal_goto() ) {
      fprintf(fp,"  virtual uint           is_Goto() const  { return 1; };\n");
    }

    // virtual function/flag: if instruction matches 'If' | 'Goto' | 
    //           'CountedLoopEnd | 'Jump'
    if( instr->is_ideal_branch() ) {
      fprintf(fp,"  virtual uint           is_Branch() const { return 1; };\n");
    }

#if 0
    // virtual function/flag: if this instruction matches an ideal 'Return'
    if( instr->is_ideal_return() ) {
      fprintf(fp,"  virtual uint           is_Return() const{ return 1; };\n");
    }
#endif

    // virtual function/flag: if this instruction must be the last instruction in a block
    if( instr->is_ideal_branch() || instr->is_ideal_return() ) {
      fprintf(fp,"  virtual uint           is_last_instr() const{ return 1; };\n");
    }
    ...
    fprintf(fp,"  uint                   oper_input_base() const { return %d; }\n",
    ...
    // Constructor
    if ( instr->is_ideal_jump() ) {
      fprintf(fp,"  %sNode() : _index2label(MinJumpTableSize) { _opnds = _opnd_array; ", instr->_ident);
    } else {
      fprintf(fp,"  %sNode() { _opnds = _opnd_array; ", instr->_ident);
      if( can_cisc_spill() && (instr->cisc_spill_alternate() != NULL) ) {
    ...
    fprintf(fp,"}\n");
    if ( instr->is_ideal_jump() ) {
      fprintf(fp, "  virtual Label *label_for_case(int switch_val) const { return _index2label.at(switch_val); }\n");
    }
    // Clone
    fprintf(fp,"  virtual Node          *clone() const;\n");
    ...
    if (instr->is_pinned(_globalNames)) {
      fprintf(fp,"  virtual int pinned() const { return ");
    ...
    if( data_type != Form::none ) {
      fprintf(fp,"  virtual uint           is_Con() const { return 1; }\n");
    }
    ...
    if( data_type != Form::none ) {
      ...
      case Form::idealF:
        ...
        break;
      case Form::idealL:
        fprintf(fp,"    return  TypeLong::make(_opnds[1]->constantL());\n");
        break;
      default:
...
/*    else if ( instr->_matrule && instr->_matrule->_rChild && 
    ...
    if ( instr->check_for_anti_dependence(_globalNames) ) {
      fprintf(fp,"  virtual bool check_for_anti_dependence() const {\n");
      fprintf(fp,"    if( req() < 2 ) return false;\n");
      fprintf(fp,"    return  in(1)->bottom_type()->has_memory();\n");
      fprintf(fp,"  }\n");
    }
...
void ArchDesc::defineStateClass(FILE *fp) {
  ...
  fprintf(fp,"  ~State(void);                     // Destructor\n");
  fprintf(fp,"\n");
  fprintf(fp,"  // Methods created by ADLC and invoked by Reduce\n");
  fprintf(fp,"  MachOper *MachOperGenerator( int opcode, MachNode* leaf, Compile* C );\n");
