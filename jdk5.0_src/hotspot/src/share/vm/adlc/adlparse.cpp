...
#pragma ident "@(#)adlparse.cpp	1.183 03/12/23 16:38:44 JVM"
#endif
/*
...
void ADLParser::instr_parse(void) {
  ...
  do {
    ...
    if      (!strcmp(ident, "predicate")) instr->_predicate = pred_parse();
      ...
      instr->_matrule = match_parse(instr->_localNames);
      // !!!!! -- Update when adding machine-independent control node
      // Special case the treatment of Control instructions.
      if (instr->_matrule) {
        if( strcmp(instr->_matrule->_name,"Return"        )==0 ||
            strcmp(instr->_matrule->_name,"Rethrow"       )==0 ||
            strcmp(instr->_matrule->_name,"TailCall"      )==0 ||
            strcmp(instr->_matrule->_name,"TailJump"      )==0 ||
            strcmp(instr->_matrule->_name,"Halt"          )==0 ||
            strcmp(instr->_matrule->_name,"Goto"          )==0 ||
            strcmp(instr->_matrule->_name,"If"            )==0 ||
            strcmp(instr->_matrule->_name,"CountedLoopEnd")==0 ) {

          // Control instructions return a special result, 'Universe'
          instr->_matrule->_result = "Universe";
        }
      }
    }
...
void ADLParser::enc_class_parse(void) {
  ...
  const char *ec_code = "";      // Code for encoding class
...
void ADLParser::frame_parse(void) {
  ...
  if (_curchar == '%' && *(_ptr+1) == '{') {
    ...
    while (_curchar != '%' && *(_ptr+1) != '}') {
      ...
      if (strcmp(token,"interpreter_arg_ptr_reg")==0) {
        interpreter_arg_ptr_parse(frame, false);
        // Add  reg_class interpreter_arg_ptr_reg
        if( _AD._register != NULL ) {
          RegClass *reg_class = _AD._register->addRegClass("interpreter_arg_ptr_reg");
          char *arg_ptr_reg = frame->_interpreter_arg_ptr_reg;
          if( arg_ptr_reg != NULL ) {
            RegDef *regDef = _AD._register->getRegDef(arg_ptr_reg);
            reg_class->addReg(regDef);     // add regDef to regClass
          }
        }
      }
      if (strcmp(token,"compiler_method_oop_reg")==0) {
        compiler_method_oop_parse(frame, false);
        // Add  reg_class compiler_method_oop_reg
        if( _AD._register != NULL ) {
          RegClass *reg_class = _AD._register->addRegClass("compiler_method_oop_reg");
          char *method_oop_reg = frame->_compiler_method_oop_reg;
      ...
      if (strcmp(token,"out_preserve_stack_slots")==0) {
        frame->_out_preserve_slots = parse_one_arg("out preserve stack slots");
      }
...
//------------------------------interpreter_arg_ptr_parse--------------------
void ADLParser::interpreter_arg_ptr_parse(FrameForm *frame, bool native) {
  frame->_interpreter_arg_ptr_reg = parse_one_arg("arg ptr reg entry");
}
...
//------------------------------compiler_method_oop_parse---------------------
void ADLParser::compiler_method_oop_parse(FrameForm *frame, bool native) {
  frame->_compiler_method_oop_reg = parse_one_arg("compiler method oop");
}
...
void ADLParser::reg_def_parse(void) {
  ...
  // Debug Stuff
  if (_AD._adl_debug > 1) {
    fprintf(stderr,"Register Definition: %s ( %s, %s )\n", rname, 
            (callconv ? callconv : ""), (c_conv ? c_conv : ""));
  }
  
  // Record new register definition.
  _AD._register->addRegDef(rname, callconv, c_conv, idealtype, encoding);
  return;
}
...
// // OLD FORM:
// // Encode rules have the form 
// //   "encode(oper size, ...) %{ "0110...$oper1100" %}
// // Operand encode functions take a size parameter to determine the number of
// // bits to output their value in.
InsEncode *ADLParser::ins_encode_parse(InstructForm &inst) {
  ...
  skipws();                        // Skip whitespace
  if (_curchar != '(') {
    parse_err(SYNERR, "Missing '(' at start of ins_encode description.\n");
    return NULL;
  }
  next_char();                     // move past '('
  skipws();
...
//------------------------------format_parse-----------------------------------
// Identify the end of a replacement/substitution variable
static char *ident_end(char *str) {
  for(char c = *str;
      ('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z')
      || c == '_' || ('0' <= c && c <= '9');
      c = *(++str));
...
MatchNode *ADLParser::matchNode_parse(FormDict &operands, int &depth, int &numleaves, bool atroot) {
  ...
  if (strcmp(token, "ModF")==0) { 
     _AD.foundModFMatchRule(true);
  }
  else if (strcmp(token, "ModD")==0) { 
     _AD.foundModDMatchRule(true);
  }
  else if (strcmp(token, "SinD")==0) { 
     _AD.foundSinDMatchRule(true);
  }
  else if (strcmp(token, "CosD")==0) { 
     _AD.foundCosDMatchRule(true);
  }
  else if (strcmp(token, "TanD")==0) { 
     _AD.foundTanDMatchRule(true);
  }
  else if (strcmp(token, "AtanD")==0) { 
     _AD.foundAtanDMatchRule(true);
  }
  else if (strcmp(token, "SqrtD")==0) { 
     _AD.foundSqrtDMatchRule(true);
  }
  else if (strcmp(token, "PowD")==0) { 
     _AD.foundPowDMatchRule(true);
  }
  ...
  // MatchNode* mroot = new MatchNode(operation, opcode, lChild, rChild);
  ...
  if (!atroot) {
    ...
    int len = (int)strlen(operation) + 3;
    lstr = (lChild) ? lChild->_opType : "";
    rstr = (rChild) ? rChild->_opType : "";
    len += (int)strlen(lstr) + (int)strlen(rstr);
    subtree = (char *)malloc(len);
    sprintf(subtree,"%s %s %s", operation, lstr, rstr);
    ...
    if (iop == NULL) {
      // Length of string is 7 + countersize + null
      int ioplength = 9 + _AD._internalOpCounter/10;
      iop = (char *)malloc(ioplength);
      sprintf(iop,"__intop%d", _AD._internalOpCounter++);
      _AD._internalOps.Insert(subtree, iop);
      _AD._internalOpNames.addName(iop);
      _AD._internalMatch.Insert(iop, mroot);
    }
    // Add the internal operand name to the MatchNode
    mroot->_internalop = iop;
    mroot->_result = iop;
  }
...
char *ADLParser::get_ident_common(bool do_preproc) {
  ...
  // Make sure we do not try to use #defined identifiers.
  if (do_preproc) {
    const char* def = _AD.get_preproc_def(start);
    if (def != NULL)
      parse_err(SYNERR, "unimplemented: using %s defined as %s", start, def);
  }
...
void ADLParser::preproc_define(void) {
  ...
  char* def = _ptr;
  char* end = strchr(def, '\n');
  assert(end != NULL, "#define line must terminate in a newline");
