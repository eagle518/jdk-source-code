...
#pragma ident "@(#)formssel.hpp	1.63 04/05/04 13:26:49 JVM"
#endif
/*
...
public:
  ...
  // Public Methods
  InstructForm(char *id, bool ideal_only = false);
  ~InstructForm();
  ...
  virtual bool        check_for_anti_dependence(FormDict &globals) const;
  ...
  void                define_short_branch_methods(FILE *fp_cpp);
...
public:                            
  ...
  void add_parameter(char *parameter_type, char *parameter_name);
  ...
  // --------------------------- Code Block
  // Add code
  void add_code(char *string_preceeding_replacement_var);
...
public:
  ...
  bool  is(int use_def_kill_enum);
  // Return 'true' if this use def info is a superset of parameter
  bool  isa(int use_def_kill_enum);
