...
#pragma ident "@(#)output_c.cpp	1.155 04/03/26 14:51:54 JVM"
#endif
/*
...
static void defineOut_RegMask(FILE *fp, char *node, const char *regMask) {
  ...
  for( pmatch->next_instruction( parent, inst_position, inst_name, input ); 
    ...
    if( ! pmatch->is_placeholder() ) {
      ...
      if( parent != -1 ) {                // root was initialized
        ...
        fprintf(fp, "  inst%ld = (", inst_position);
        print_block_index(fp, inst_position);
        fprintf(fp, " > 0) ? block->_nodes.at(");
        print_block_index(fp, inst_position);
        fprintf(fp, ")->is_Mach() : NULL;\n");
      }
  ...
  if( preplace != NULL ) {
    ...
    for( preplace->next_operand( inst_num, op_name );
      ...
      if( opnds_index == 0 ) {
        ...
        fprintf(fp, "        ra_->set_pair(root->_idx, ra_->get_reg_hi(inst%ld), ra_->get_reg_lo(inst%ld));\n", inst_num, inst_num);
        fprintf(fp, "        root->_opnds[0] = inst%ld->_opnds[0]->clone(); // result\n", inst_num);
        ...
        fprintf(fp, "        root->_opnds[%d] = inst%ld->_opnds[%d]->clone();\n",
...
void ArchDesc::defineExpand(FILE *fp, InstructForm *node) {
  ...
  if( node->expands() ) {
	...
	fprintf(fp, "  ((MachIfNode*)n%d)->_fcnt = _fcnt;\n",cnt);
      ...
      fprintf(fp,"  n%d->set_opnd_array(0, state->MachOperGenerator( %s, n%d, C ));\n", 
              cnt, machOperEnum(resultOper), cnt);
      ...
      for( expand_instr->reset(); (opid = expand_instr->iter()) != NULL; ) {
        ...
        if ((exp_pos = node->_exprule->_newopers.index(opid)) != -1) {
          new_pos = new_inst->_components.operand_position(parameter,Component::USE);
          ...
          exp_pos = node->_components.operand_position_format(opid);
        ...
        int new_def_pos = new_inst->_components.operand_position(parameter,Component::DEF);
  ...
  if( ! node->expands() && node->needs_projections() ) {
    ...
    fprintf(fp,"  uint ctr = 0;\n");          // Counts number of projections
    ...
    Component *comp = NULL;
    if( node->sets_result() || node->_matrule == NULL )  comp = node->_components.iter();
    ...
    while( (comp = node->_components.iter()) != NULL ) {
      if( comp->isa(Component::DEF) || comp->isa(Component::KILL) ) {
        ...
        fprintf(fp,"  kill = ");
        fprintf(fp,"new (1, C) MachProjNode( %s, MachProjNode::fat_proj, (%s), Op_%s );\n",
                machNode, regmask, ideal_type);
  ...
  void update_state(const char *rep_var) {
    ...
    if ( (*rep_var) != '$' ) {
      ...
      if ( Opcode::as_opcode_type(rep_var) != Opcode::NOT_AN_OPCODE ) {
        ...
        if ( param_no == -1 ) {
          _AD.syntax_err( "Replacement variable %s not found in enc_class.\n",
                          rep_var);
        }
        ...
        int idx  = (opc != NULL) ? _inst._components.operand_position_format(inst_rep_var) : -1;
  ...
  void add_rep_var(const char *rep_var) {
    ...
    if ( ( *rep_var == '$' ) && ( *(rep_var+1) == '$' ) ) {
      ...
      if ( strcmp(rep_var,"$$emit32") == 0 ) {
        ...
        _AD.syntax_err( "Unsupported $$operation '%s'\n",rep_var);
        assert( false, "fatal();");
      }
    }
    else {
...
private:
  ...
  void emit_rep_var(const char *rep_var) {
    ...
    if ( Opcode::as_opcode_type(rep_var) != Opcode::NOT_AN_OPCODE ) {
      ...
      int   param_no  = _encoding.rep_var_index(rep_var);
      if ( param_no == -1 ) {
        _AD.syntax_err( "Replacement variable %s not found in enc_class %s.\n",
...
void ArchDesc::defineEmit(FILE *fp, InstructForm &inst) {
  ...
  // Save current instruction's starting address in global variable.
  // !!!!!
  // Can record this in the CodeBuffer object itself.
  fprintf( fp, "cbuf.set_mark();\n");
...
void ArchDesc::define_oper_interface(FILE *fp, OperandForm &oper, FormDict &globals,
  ...
  if( *encoding == '$' ) {
    ...
    if ( (op->_matrule && op->_matrule->is_base_register(globals)) ) {
      ...
      fprintf(fp,"      return ra_->reg2offset(ra_->get_reg_lo(node->in(idx)));/* sReg */\n");
      fprintf(fp,"    }\n");
      fprintf(fp,"    // Access register number from myself\n");
      fprintf(fp,"    return ra_->reg2offset(ra_->get_reg_lo(node));/* sReg */\n");
...
// Utility to define the virtual method -- MachNode::clone()
static void define_inst_clone(FILE *fp, InstructForm &inst) {
  fprintf(fp,"\n");
  fprintf(fp,  "Node *%sNode::clone() const {\n", inst._ident);
  fprintf(fp,  "  %sNode *n = (%sNode*)Node::clone();\n", inst._ident, inst._ident);
  fprintf(fp,  "  n->_opnds = n->_opnd_array;\n");
  uint  num_opnds = inst.num_opnds();
  for ( uint i = 0; i < num_opnds; ++i ) {
    fprintf(fp,"  n->_opnd_array[%d] = _opnd_array[%d]->clone();\n", i,i);
  }
  // Clone the _prob & _fcnt fields of Ideal IfNodes
  if( inst.is_ideal_if() ) {
    fprintf(fp, "  n->_prob = _prob;\n");
    fprintf(fp, "  n->_fcnt = _fcnt;\n");
  }
  fprintf(fp,  "  return n;\n");
  fprintf(fp,  "}\n");
}
...
void ArchDesc::defineClasses(FILE *fp) {
  ...
  for( ; (instr = (InstructForm*)_instructions.iter()) != NULL; ) {
    ...
    //    defineKill_RegMask(fp, instr->_ident, kill_mask(*instr));
    fprintf(fp,"\n");
  }
  ...
  for( ; (instr = (InstructForm*)_instructions.iter()) != NULL; ) {
    ...
    instr->define_short_branch_methods(fp);
  }
  ...
  // Output the definitions for instruction clone method
  // 
  //  MachNode *%sNode::clone() const {
  //    %sNode *n = (%sNode*)Node::clone();
  //    n->_opnd_array[0] = _opnd_array[0]->clone();
  //    ...
  //    return n;
  //  }
  //
  ...
  for( ; (instr = (InstructForm*)_instructions.iter()) != NULL; ) {
    ...
    define_inst_clone(_CPP_CLONE_file._fp, *instr);
  }
...
// Information needed to generate the decision array for instruction chain rule
class OutputRematerialize : public OutputMap {
public:
  OutputRematerialize(FILE *hpp, FILE *cpp, FormDict &globals, ArchDesc &AD) 
    : OutputMap(hpp, cpp, globals, AD) {};

  void declaration(){ fprintf(_hpp, "\n");
                      fprintf(_hpp, "extern const int rematerialize[];\n");
                      fprintf(_hpp, "\n");
  }
  void closing()     { fprintf(_cpp, "  0 // no trailing comma\n");
                       OutputMap::closing();
  }
  void definition() { fprintf(_cpp, "const int rematerialize[] = {\n"); }
  void map(OpClassForm &opc)   { fprintf(_cpp, "  0"); }
  void map(OperandForm &oper)  { fprintf(_cpp, "  0"); }
  void map(char        *name)  { fprintf(_cpp, "  0"); }
  void map(InstructForm &inst) { 
    fprintf(_cpp, "  %s", inst.rematerialize(_globals,_AD.get_registers()) ? "1" : "0");
  }
};
...
//---------------------------build_rematerialize_map--------------------------
// Build  mapping from enumeration for densely packed operands
// TO result and child types.
void ArchDesc::build_rematerialize_map(FILE *fp_hpp, FILE *fp_cpp) {
  // // // Preserve in case we decide to use this table instead of another
  // // Construct the table for rematerialization
  // OutputRematerialize output_remat(fp_hpp, fp_cpp, _globalNames, *this);
  // build_map(output_remat);
}
...
static void path_to_constant(FILE *fp, FormDict &globals,
  ...
  if ( (mnode->_lChild == NULL) && (mnode->_rChild == NULL)
    ...
    if (         strcmp(optype,"ConI") == 0 ) {
      ...
      fprintf(fp, "_leaf->is_Bool()->_test._test");
    } else {
...
void ArchDesc::buildMachOperGenerator(FILE *fp_cpp) {
  ...
  fprintf(fp_cpp, "(int opcode, MachNode* mach, Compile* C)");
  fprintf(fp_cpp, "{\n");
  fprintf(fp_cpp, "\n");
...
void ArchDesc::buildMachNode(FILE *fp_cpp, InstructForm *inst, const char *indent) {
  ...
  if ( (inst->num_post_match_opnds() != 0) ) {
	...
	continue;
      ...
      fprintf(fp_cpp, "MachOperGenerator(%s,%s,C));\n", opcode, parent);
      }
  } 
  ...
  if( inst->is_ideal_if() ) {
    fprintf(fp_cpp, "%s node->_prob = _leaf->is_If()->_prob;\n", indent);
    fprintf(fp_cpp, "%s node->_fcnt = _leaf->is_If()->_fcnt;\n", indent);
  }
...
void InstructForm::declare_cisc_version(ArchDesc &AD, FILE *fp_hpp) {
  if( AD.can_cisc_spill() ) {
    ...
    if (inst_cisc != NULL) {
      ...
      fprintf(fp_hpp, "  virtual const RegMask *in_oper_RegMask(uint idx, uint operand, uint skipped) const;\n");
    }
    if( is_cisc_alternate() ) {
      fprintf(fp_hpp, "  virtual bool           is_cisc_alternate() const { return true; }\n");    
    }
  }
}
...
void InstructForm::define_cisc_version(ArchDesc &AD, FILE *fp_cpp) {
  ...
  if( AD.can_cisc_spill() && (inst_cisc != NULL) ) {
    ...
    uint       num_operands = inst_cisc->num_opnds();
    assert( num_operands == this->num_opnds(), "Must have same number of operands");
    ...
    fprintf(fp_cpp, "}\n");
    //
    // Return the operand's regmask except for cisc-operand
    fprintf(fp_cpp, "const RegMask *%sNode::in_oper_RegMask(uint idx, uint operand, uint skipped) const {\n", this->_ident);
    fprintf(fp_cpp, "  if( _cisc_RegMask && (int)operand == cisc_operand() ) {\n");
    fprintf(fp_cpp, "    return _cisc_RegMask;\n");
    fprintf(fp_cpp, "  } else {\n");
    fprintf(fp_cpp, "    return opnd_array(operand)->in_RegMask(idx-skipped);\n");
    fprintf(fp_cpp, "  }\n");
    fprintf(fp_cpp, "}\n");
    //
    ...
    fprintf(fp_cpp, "\n");
    // Cisc version must use same node index for access through allocator's tables
    fprintf(fp_cpp, "  // Cisc version must use same node index\n");
    ...
    fprintf(fp_cpp, "  for( int i = 0; i < %d; i++ ) {\n", num_operands);
    fprintf(fp_cpp, "    if( i != cisc_operand() ) \n");
    fprintf(fp_cpp, "      node->set_opnd_array(i, opnd_array(i)->clone());\n");
    fprintf(fp_cpp, "  }\n");
...
void InstructForm::declare_short_branch_methods(FILE *fp_hpp) {
  if (has_short_branch_form()) {
    ...
    fprintf(fp_hpp, "  virtual bool           may_be_short_branch() const { return true; }\n");
...
void InstructForm::define_short_branch_methods(FILE *fp_cpp) {
  if (has_short_branch_form()) {
    InstructForm *short_branch = short_branch_form();
    const char   *name         = short_branch->_ident;
    uint          num_operands = short_branch->num_opnds();
    ...
    fprintf(fp_cpp, "  // Short branch version must use same node index\n");
    fprintf(fp_cpp, "  node->set_idx( _idx );\n");
    // Copy machine-independent inputs
    fprintf(fp_cpp, "  // Copy machine-independent inputs\n");
    fprintf(fp_cpp, "  for( uint j = 0; j < req(); j++ ) {\n");
    ...
    fprintf(fp_cpp, "  }\n");
    // Copy machine operands to new MachNode
    fprintf(fp_cpp, "  // Copy my operands\n");
    fprintf(fp_cpp, "  for( int i = 0; i < %d; i++ ) {\n", num_operands);
    fprintf(fp_cpp, "    node->set_opnd_array(i, opnd_array(i)->clone());\n");
    fprintf(fp_cpp, "  }\n");
...
//---------------------------buildInstructMatchChecks--------------------------
// Output the methods to Matcher which check whether or not a specific
// instruction has a matching rule for the host architecture.
void ArchDesc::buildInstructMatchChecks(FILE *fp_cpp) const {
  fprintf(fp_cpp, "\n\n");
  fprintf(fp_cpp,
         "const bool Matcher::modFSupported(void) { return %s; }\n\n",
         _foundModFMatchRule ? "true" : "false");
  fprintf(fp_cpp,
         "const bool Matcher::modDSupported(void) { return %s; }\n\n",
         _foundModDMatchRule ? "true" : "false");
  fprintf(fp_cpp,
         "const bool Matcher::sinDSupported(void) { return %s; }\n\n",
         _foundSinDMatchRule ? "true" : "false");
  fprintf(fp_cpp,
         "const bool Matcher::cosDSupported(void) { return %s; }\n\n",
         _foundCosDMatchRule ? "true" : "false");
  fprintf(fp_cpp,
         "const bool Matcher::tanDSupported(void) { return %s; }\n\n",
         _foundTanDMatchRule ? "true" : "false");
  fprintf(fp_cpp,
         "const bool Matcher::atanDSupported(void) { return %s; }\n\n",
         _foundAtanDMatchRule ? "true" : "false");
  fprintf(fp_cpp,
         "const bool Matcher::sqrtDSupported(void) { return %s; }\n\n",
         _foundSqrtDMatchRule ? "true" : "false");
  fprintf(fp_cpp,
         "const bool Matcher::powDSupported(void) { return %s; }\n\n",
         _foundPowDMatchRule ? "true" : "false");
}
...
void ArchDesc::buildFrameMethods(FILE *fp_cpp) {
  ...
  fprintf(fp_cpp,"uint Matcher::stack_alignment() { return %s; }\n\n",
          _frame->_alignment);
  ...
  if (_frame->_return_addr_loc) {
    ...
    fprintf(fp_cpp," return SharedInfo::stack2reg(%s); }\n\n",
            _frame->_return_addr);
  }
  ...
  fprintf(fp_cpp,"{ return %s; }\n\n", _frame->_out_preserve_slots);
  // varargs C out slots killed
  fprintf(fp_cpp,"uint Compile::varargs_C_out_slots_killed() const ");
  fprintf(fp_cpp,"{ return %s; }\n\n", _frame->_varargs_C_out_slots_killed);
  // Java Argument Position
  fprintf(fp_cpp,"void Matcher::calling_convention(OptoRegPair *sig, uint length, bool is_outgoing) {\n");
  fprintf(fp_cpp,"%s\n", _frame->_calling_convention);
  fprintf(fp_cpp,"}\n\n");
  // Native Argument Position
  fprintf(fp_cpp,"void Matcher::c_calling_convention(OptoRegPair *sig, uint length) {\n");
  ...
  // Interpreter's Argument Pointer Register, mask definition, and encoding
  fprintf(fp_cpp,"OptoReg::Name Matcher::interpreter_arg_ptr_reg() {");
  fprintf(fp_cpp," return OptoReg::Name(%s_num); }\n\n",
          _frame->_interpreter_arg_ptr_reg);
  fprintf(fp_cpp,"const RegMask &Matcher::interpreter_arg_ptr_reg_mask() {");
  fprintf(fp_cpp," return INTERPRETER_ARG_PTR_REG_mask; }\n\n");
  fprintf(fp_cpp,"int Matcher::interpreter_arg_ptr_reg_encode() {");
  fprintf(fp_cpp," return _regEncode[interpreter_arg_ptr_reg()]; }\n\n");

  // Compiler's Method Oop Register, mask definition, and encoding
  fprintf(fp_cpp,"OptoReg::Name Matcher::compiler_method_oop_reg() {");
  fprintf(fp_cpp," return OptoReg::Name(%s_num); }\n\n",
          _frame->_compiler_method_oop_reg);
  fprintf(fp_cpp,"const RegMask &Matcher::compiler_method_oop_reg_mask() {");
  fprintf(fp_cpp," return COMPILER_METHOD_OOP_REG_mask; }\n\n");
  fprintf(fp_cpp,"int Matcher::compiler_method_oop_reg_encode() {");
  fprintf(fp_cpp," return _regEncode[compiler_method_oop_reg()]; }\n\n");
    ...
    for( ; (instr = (InstructForm*)_instructions.iter()) != NULL; ) {
      ...
      for( ; !found_cisc_alternate && (instr2 = (InstructForm*)_instructions.iter2()) != NULL; ) {
        ...
        if ( instr2->_matrule != NULL
          ...
          found_cisc_alternate = found_cisc_alternate;
        }
      }
    }
  }
