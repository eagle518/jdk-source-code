...
#pragma ident "@(#)klass.cpp	1.102 04/01/27 14:36:48 JVM"
#endif
/*
...
bool Klass::can_be_statically_bound(methodOop m) {
  if (m->is_final_method()) return true;
  // if it doesn't have a vtable entry, it's statically bound
  ResourceMark rm;
  klassVtable* vt = Klass::cast(m->method_holder())->vtable();
  return vt->index_of(m) == -1;
}
...
methodOop Klass::lookup_method(symbolOop name, symbolOop signature) const {
...
klassOop Klass::base_create_klass_oop(KlassHandle& klass, int size, 
  ...
  Klass*   kl = (Klass*) vtbl.allocate_permanent(klass, size, CHECK_0);
...
void* Klass_vtbl::operator new(size_t ignored, KlassHandle& klass,
                               int size, TRAPS) {
  klassOop k = (klassOop) CollectedHeap::permanent_obj_allocate(klass, size, CHECK_0);
  return k->klass_part();
}
...
void Klass::append_to_sibling_list() {
  ...
  assert(SharedSkipVerify || super->is_interface()
         || super->superklass() == NULL || !is_interface(),
         "an interface can only be a subklass of "
         "another interface or of Object (not of a klass)");
...
bool Klass::oop_being_unloaded(BoolObjectClosure* is_alive, oop obj) {
