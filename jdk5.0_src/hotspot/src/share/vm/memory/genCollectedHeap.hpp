...
#pragma ident "@(#)genCollectedHeap.hpp	1.71 04/06/15 12:17:34 JVM"
#endif
/*
...
public:
  GenCollectedHeap(CollectorPolicy *policy);
  ...
  virtual CollectedHeap::Name kind() { return CollectedHeap::GenCollectedHeap; }
  ...
  // Iterate through each generation, giving them a chance
  // to examine a modified oop, "p", and take appropriate
  // actions, such as updating internal generational data.
  void examine_modified_oop(oop* p) {
    for (int i = 0; i < _n_gens; i++) {
      _gens[i]->examine_modified_oop(p);
    }
    perm_gen()->examine_modified_oop(p);
  }

  // Ensure parseability: override
  ...
  // Override.
  void prepare_for_verify() PRODUCT_RETURN;

  // Override.
  void verify(bool allow_dirty, bool silent) PRODUCT_RETURN;
  ...
  int n_gens() const {
    assert(_n_gens == collector_policy()->number_of_generations(), "Sanity");
    return _n_gens;
  }
  ...
  // scanned.  The "cso" argument determines which entries of the
  // SystemDictionary the closure is applied to.  "CSO_none" does none;
  // "CSO_AllClasses" applies the closure to all entries for "system"
  // classes and loaders.
  void process_strong_roots(int level, bool younger_gens_as_roots,
