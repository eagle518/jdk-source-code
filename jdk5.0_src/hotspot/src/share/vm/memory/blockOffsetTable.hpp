...
#pragma ident "@(#)blockOffsetTable.hpp	1.36 03/12/23 16:40:53 JVM"
#endif
/*
...
// for example, the train generation.)
...
class BlockOffsetArray: public BlockOffsetTable {
 ...
 protected:
  ...
  static int power_to_cards_back(uint i) {
    return 1 << (LogBase * i);
  }
  static size_t power_to_words_back(uint i) {
    return power_to_cards_back(i) * N_words;
  }
  static int entry_to_cards_back(u_char entry) {
  ...
  // "start" and "end" must be card boundaries.  Sets the entries
  // corresponding to the cards starting at "start" and ending at "end"
  // to point back to the card before "start".
  void set_aligned_region_to_point_to_start(HeapWord* start, HeapWord* end); 

  // The cards from start to end, inclusive, are assumed to point backwards
  // before start_card.  Fix them up to point to the card before start_card.
  void fix_up_alloced_region(size_t start_card, size_t end_card);
 ...
 public:
  ...
  virtual void resize(size_t new_word_size) {
    ...
    if (_end < new_end && !init_to_zero()) {
      ...
      // set all the newly added cards to the initial value
      set_aligned_region_to_point_to_start(_end, new_end);
    }
  ...
  inline void single_block(HeapWord* blk, size_t size) {
  ...
  inline void alloc_block(HeapWord* blk, size_t size) {
...
class BlockOffsetArrayNonContigSpace: public BlockOffsetArray {
 ...
 public:
  ...
  inline void mark_block(HeapWord* blk, size_t size) {
...
class BlockOffsetArrayContigSpace: public BlockOffsetArray {
 ...
 public:
  ...
  inline void alloc_block(HeapWord* blk_start, HeapWord* blk_end) {
