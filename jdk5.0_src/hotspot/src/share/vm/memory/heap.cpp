...
#pragma ident "@(#)heap.cpp	1.41 03/12/23 16:41:16 JVM"
#endif
/*
...
bool CodeHeap::reserve(size_t reserved_size, size_t committed_size, size_t segment_size) {
  ...
  if (!_memory.initialize(align_to_allocation_size(reserved_size), align_to_allocation_size(committed_size))) {
    return false;
  }
...
void CodeHeap::verify() {
  ...
  assert(len == _free_segments, "wrong freelist");  
