...
#pragma ident "@(#)compactibleFreeListSpace.hpp	1.62 04/01/11 09:11:14 JVM"
#endif
/*
...
class PromotedObject VALUE_OBJ_CLASS_SPEC {
 ...
 public:
  ...
  inline bool hasDisplacedMark() const {
    return _next & displaced_mark;
  }
  inline void clearNext()        { _next = NULL; }
...
class CompactibleFreeListSpace: public CompactibleSpace {
  ...
  // "Size" of rescan tasks (executed during parallel remark phases
  ...
  // par_seq_tasks which lives in Space. XXX
 ...
 private:
  ...
  // indexed free lists.
  HeapWord* allocate_adaptive_freelists(size_t size, bool large_noref);
  ...
  HeapWord* allocate_non_adaptive_freelists(size_t size, bool large_noref);
  ...
  void verify_objects_initialized() const PRODUCT_RETURN;
 ...
 public:
  ...
  virtual HeapWord*
       object_iterate_mem_careful(MemRegion mr,
  ...
  HeapWord* allocate(size_t size, bool large_noref);
  ...
  bool obj_allocated_since_save_marks(const oop obj) const {
    assert(contains(obj), "Wrong space?");
  ...
  void verifyIndexedFreeLists()           const PRODUCT_RETURN;
  void verifyIndexedFreeList(size_t size) const PRODUCT_RETURN;
