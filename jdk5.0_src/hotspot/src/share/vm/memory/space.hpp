...
#pragma ident "@(#)space.hpp	1.127 03/12/23 16:41:29 JVM"
#endif
/*
...
//         - CarSpace    -- used for TrainGeneration

// Forward decls.
class Space;
class Train;
class CarRememberedSet;
class CarTableDesc;
...
class Space: public CHeapObj {
 ...
 public:
  ...
  // IF "this" is a CarSpace, return it, else return NULL.
  virtual CarSpace* toCarSpace() {
    return NULL;
  }
...
#define SCAN_AND_ADJUST_POINTERS(adjust_obj_size) {				\
  ...
  if (q < t && _first_dead > q &&						\
      oop(q)->mark() == markOopDesc::prototype()) {				\
...
#define SCAN_AND_COMPACT(obj_size) {						\
  ...
  if (q < t && _first_dead > q &&						\
    ...
    while (q < end) {								\
      ...
      assert(oop(q)->mark() == markOopDesc::prototype(), "should have prototype mark");		\
...
class EdenSpace : public ContiguousSpace {
 ...
 public:
  ...
  #define EdenSpace_PAR_OOP_ITERATE_DECL(OopClosureType, nv_suffix)  \
    void par_oop_iterate(MemRegion mr, OopClosureType* blk);

    ALL_PAR_OOP_ITERATE_CLOSURES(EdenSpace_PAR_OOP_ITERATE_DECL)
  #undef EdenSpace_PAR_OOP_ITERATE_DECL
};
...
// Class CarSpace describes an individual car in a train

class CarSpace: public OffsetTableContigSpace {
  friend class TrainGeneration;
  friend class VMStructs;
 private:
  CarSpace*         _next_car;
  CarRememberedSet* _remembered_set;
  size_t            _blocks;
  CarTableDesc*     _desc;
  bool              _marked;
  // Oversized car, relinked rather than scavenged
  bool              _oversized;
  static size_t     _car_size;
  static size_t     _car_size_in_words;
  // Cache value to save multiply in critical loops
  static size_t     _oversized_car_size_in_words;

  void set_blocks(size_t n)                     { _blocks = n; }
  void set_remembered_set(CarRememberedSet* rs) { _remembered_set = rs; }
  void set_desc(CarTableDesc* desc)             { _desc = desc; }
 ...
 public:
  // Sizes
  static size_t car_size()                  { return _car_size; }
  static size_t car_size_in_words()         { return _car_size_in_words; }
  // Next car in train
  CarSpace* next_car() const                { return _next_car; }
  void set_next_car(CarSpace* next)         { _next_car = next; }
  // Local remembered set
  CarRememberedSet* remembered_set() const  { return _remembered_set; }
  // Car table description corresponding to this car
  CarTableDesc* desc() const                { return _desc; }
  // Number of 2^LogOfCarSpaceSize blocks allocated for this car
  size_t blocks() const                     { return _blocks; }
  // Special marking bit (support for special large object handling)
  bool marked() const                       { return _marked; }

  // Oversized car, relinked rather than scavenged
  bool oversized() const                    { return _oversized; }
  // Is this size "oversized"? Note that we currently restrictex
  // an oversized object to a car size or more, which simplifies
  // allocation (large objects need to be in a car by themselves).
  static bool is_oversized_size(size_t size) {
    assert(UseOversizedCarHandling || OversizedCarThreshold > 0, 
           "OversizedCarThreshold should not be zero!"); 
    return UseOversizedCarHandling && 
           (size >= _oversized_car_size_in_words); 
  }

  // Given a size, calculate the number of default size car blocks it'll
  // take to accomodate that size
  static size_t car_block_size(size_t size) {
    return ((size - 1) / _car_size_in_words) + 1; 
  }

  // Train containing this car
  inline Train* train() const;
  // Number assigned to this car
  inline juint car_number() const;
  // Number assigned to containing train
  inline julong train_number() const;

  // Allocation override, is aware of oversized objects
  virtual HeapWord* allocate(size_t word_size);

  // Collection support
  void set_target(bool value);
  void set_marked(bool value)               { _marked = value; }
  void set_oversized(bool value)            { _oversized = value; }

  // Constructor
  CarSpace(TrainGeneration* tg,
           CardTableRS* ct,
