...
#pragma ident "@(#)ciTypeFlow.cpp	1.33 04/05/25 15:29:03 JVM"
#endif
/*
...
// ------------------------------------------------------------------
// ciTypeFlow::Range::get_block_for
//
// Find a block with this Range which has a compatible JsrSet.
...
ciTypeFlow::Block* ciTypeFlow::Range::get_block_for(ciTypeFlow::JsrSet* jsrs, CreateOption option) {
  Arena* arena = _outer->arena();
  if (_blocks == NULL) {
    // Query only?
    if (option == no_create)  return NULL;

    // Allocate the growable array.
    _blocks = new (arena) GrowableArray<Block*>(arena, 4, 0, NULL);
  }
  ...
  Block* new_block = new (arena) Block(_outer, this, jsrs);
...
// ------------------------------------------------------------------
// ciTypeFlow::Range::private_copy_count
//
int ciTypeFlow::Range::private_copy_count(ciTypeFlow::JsrSet* jsrs) const {
  if (_blocks == NULL) {
    return 0;
  }
...
#ifndef PRODUCT
static int in_range_print_count = 0;

// ------------------------------------------------------------------
// ciTypeFlow::Range::print_on
void ciTypeFlow::Range::print_on(outputStream* st) const {
  in_range_print_count++;
  st->print_cr("--------------------------------------------------------");
  st->print   ("Range [%d - %d) control : ", start(), limit());
  if (control() == fall_through_bci) {
    st->print_cr("%d:fall through", limit());
  } else {
    st->print_cr("%d:%s", control(),
	      Bytecodes::name(outer()->method()->java_code_at_bci(control())));
  }
...
ciTypeFlow::Block::successors(ciByteCodeStream* str,
  ...
  if (_successors == NULL) {
    ...
    ciTypeFlow* analyzer = range()->outer();
	...
	new (arena) GrowableArray<Block*>(arena, 1, 0, NULL);
      // No successors
    } else if (control() == ciTypeFlow::Range::fall_through_bci) {
...
void ciTypeFlow::Block::print_on(outputStream* st) const {
  if ((Verbose || WizardMode) && in_range_print_count == 0) {
    outer()->method()->print_codes(start(), limit());
  }
  st->print_cr("  ====================================================  ");
  st->print ("  ");
...
ciTypeFlow::ciTypeFlow(ciEnv* env, ciMethod* method, int osr_bci) {
  ...
  // Allocate the bci to Range mapping.
  _range_map = NEW_ARENA_ARRAY(arena(), Range*, code_size());
...
ciTypeFlow::Range* ciTypeFlow::split_range_at(int bci) {
  Range* new_range = new (arena()) Range(this);
  Range* former_range = range_at(bci);
  assert(former_range != NULL, "must not be NULL");

  new_range->set_limit(bci);
  former_range->set_start(bci);

  for (int pos=bci-1; pos >= 0; pos--) {
    Range* current_range = range_at(pos);
    if (current_range == former_range) {
      // Replace it.
      set_range_at(pos, new_range);
      new_range->set_start(pos);
    } else if (current_range == NULL) {
...
// ------------------------------------------------------------------
// ciTypeFlow::make_range_at
//
// Make a new range start at the specified bci.
ciTypeFlow::Range* ciTypeFlow::make_range_at(int bci) {
  if (bci == code_size()) {
    // Allow an attempt to create a Range beginning at
    // the end of the code to silently fail.  This is fine.
    return NULL;
  }

  Range* range = range_at(bci);
  if (range == NULL) {
    // This is our first time visiting this bytecode.  Create
    // a fresh range and assign it this starting point.
    range = new (arena()) Range(this);
    range->set_start(bci);
    set_range_at(bci, range);
    return range;
  } else if (range->start() == bci) {
    // The range begins at bci.  Simply return it.
    return range;
  } else {
    // We have already created a range containing bci but
    // not starting at bci.  This existing range needs to
    // be split into two.
    return split_range_at(bci);
  }
}

// ------------------------------------------------------------------
// ciTypeFlow::mark_known_range_starts
//
...
void ciTypeFlow::mark_known_range_starts() {
  make_range_at(start_bci());

  for (ciExceptionHandlerStream str(method());
       !str.is_done();
       str.next()) {
    ciExceptionHandler* handler = str.handler();

    // By artificially marked range boundaries at the start
    // and limit of all of the exception ranges, we provide
    // the invariant that each range has a fixed set of active
    // exception handlers.
    make_range_at(handler->start());
    make_range_at(handler->limit());

    // Clearly, a new range begins at the handler bci of an
    // exception.
    make_range_at(handler->handler_bci());
  }
}


// ------------------------------------------------------------------
// ciTypeFlow::find_ranges
//
// Define the ranges of basic blocks in the method.  Build the
// bci to Range mapping.
void ciTypeFlow::find_ranges() {
  // Mark some known range starting points.
  mark_known_range_starts();

  // Start a range at the entry point of the method.
  Range* current_range = make_range_at(0);
  int limit = code_size();
  ...
  while ((code = str.next()) != ciByteCodeStream::EOBC) {
    ...
    // Determine if a new range has been made at the current bci.  If
    // this range differs from our current range, switch to the new
    // one and end the old one.
    assert(current_range != NULL, "must always have a current range");
    Range* new_range = range_at(current_bci);
    if (new_range == NULL) {
      // We have not marked this bci as the start of a new range.
      // Keep interpreting the current_range.
      set_range_at(current_bci, current_range);
    } else {
      current_range->set_limit(current_bci);
    ...
    case Bytecodes::_ifnull:       case Bytecodes::_ifnonnull:
    case Bytecodes::_goto:
      // A branch.  A new range starts at the jump target.
      // We also assume a new range starts at the next bci.
      current_range->set_control(current_bci);
      if (next_bci < limit) {
	make_range_at(next_bci);
      }
      make_range_at(str.get_dest());
      break;
	...
	make_range_at(next_bci);
      ...
      make_range_at(str.get_far_dest());
      break;
	...
	Bytecode_tableswitch_at(str.cur_bcp());
      ...
      make_range_at(current_bci + tableswitch->default_offset());
      while (--len >= 0) {
	make_range_at(current_bci + tableswitch->dest_offset_at(len));
      }
      // Also assume the next bci starts a new range.
      if (next_bci < limit) {
	make_range_at(next_bci);
      }
      break; 
	...
	Bytecode_lookupswitch_at(str.cur_bcp());
      ...
      int npairs = lookupswitch->number_of_pairs(); 
      make_range_at(current_bci + lookupswitch->default_offset());
      while(--npairs >= 0) {
	LookupswitchPair *pair = lookupswitch->pair_at(npairs);
	make_range_at(current_bci + pair->offset());
      }
      // Also assume the next bci starts a new range.
      if (next_bci < limit) {
	make_range_at(next_bci);
    ...
    case Bytecodes::_lreturn:    case Bytecodes::_freturn:
    case Bytecodes::_dreturn:    case Bytecodes::_areturn:
    case Bytecodes::_return:     case Bytecodes::_ret:
      // Assume the next bci begins a new range.
      current_range->set_control(current_bci);
      if (next_bci < limit) {
	make_range_at(next_bci);
      }
  ...
  // End the last Range.
  current_range->set_limit(limit);

#ifdef ASSERT
  {
    // Ranges must partition the space of bcis.  Verify this.
    Range* current = NULL;
    int prev_limit = 0;
    for (int bci = 0; bci < code_size(); bci++) {
      Range* range = range_at(bci);
      if (range != NULL && range != current) {
	assert(range->start() == bci, "range must map its 1st bci");
	assert(prev_limit == range->start(), "this range must touch previous");
	prev_limit = range->limit();
	current = range;
      }
      assert(current->contains(bci), "current range must include this bci");
    }
    assert(prev_limit == code_size(), "last range must end the code");
  }
#endif
}
...
ciTypeFlow::Block* ciTypeFlow::block_at(int bci, ciTypeFlow::JsrSet* jsrs, CreateOption option) {
  ...
  Range* range = range_at(bci);
  assert(range->start() == bci, "bad range boundaries");
  Block* block = range->get_block_for(jsrs, option);
...
void ciTypeFlow::flow_block(ciTypeFlow::Block* block,
  ...
  if (control != ciTypeFlow::Range::fall_through_bci) {
    limit = control;
  }
...
#else
    // InstructionCountCutoff is the number of instructions at which
    // the graph builder will bail out.  Probably if we already have lots of BBs,
    // the compiler will generate at least twice that many instructions and bail out.
    int block_limit = InstructionCountCutoff / 2;
#endif
...
void ciTypeFlow::map_blocks() {
  ...
  for (int bci = 0; bci < limit_bci; bci++) {
    ...
    if (range != NULL && range != current) {
      current = range;
      int block_count = range->block_count();
      for (int i = 0; i < block_count; i++) {
	Block* block = range->block_at(i);
