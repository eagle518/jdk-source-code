...
#pragma ident "@(#)thread.cpp	1.737 04/07/14 14:46:06 JVM"
#endif
/*
...
//
bool JavaThread::is_ext_suspend_completed(bool called_by_wait, uint32_t *bits) {
  ...
  *bits |= 0x00000001;
  ...
  do {
    ...
    if (thread_state() == _thread_blocked && is_suspend_equivalent()) {
      ...
      *bits |= 0x00001000;
      return true;
    } else if (thread_state() == _thread_in_native && frame_anchor()->walkable()) {
      ...
      *bits |= 0x00002000;
      return true;
               ...
               thread_state() == _thread_in_native_trans &&
      ...
      // were called by wait_for_ext_suspend_completion(), then it
      ...
      *bits |= 0x00004000;
      ...
      // We have to temporarily drop the SR_lock in order for the
      // thread to make progress towards self-suspension and to
      // prevent deadlock in the case of a safepoint.
      MutexUnlockerEx ul(SR_lock(), Mutex::_no_safepoint_check_flag);

      // We wait for the thread to transition to a more usable state.
      for (int i = 0; i < SUSPENDRETRYCOUNT; i++) {
      ...
      // regain SR_lock as we leave this block
    }
...
//
bool JavaThread::wait_for_ext_suspend_completion(int retries, uint32_t *bits) {
  ...
  *bits |= 0x00010000;
  ...
  {
    MutexLockerEx ml(SR_lock(), Mutex::_no_safepoint_check_flag);
    is_suspended = is_ext_suspend_completed(true /* called_by_wait */, bits);
...
void Thread::print_on_error(outputStream* st, char* buf, int buflen) const {
  ...
  else if (is_ConcurrentMarkSweep_thread()) st->print("ConcurrentMarkSweepThread");
...
bool Thread::set_as_starting_thread() {
 // NOTE: this must be called inside the main thread.
  return os::create_main_thread(this);
}
...
NamedThread::~NamedThread() {
  assert(_name == NULL, "memory leak, call free_name()");
}
...
void WatcherThread::print() const {
  tty->print("\"%s\" ", name());
  Thread::print();
  tty->cr();
}
...
void JavaThread::initialize() {
  ...
  set_jni_functions((struct JNINativeInterface_*)jni_functions());
  set_vm_result(NULL);
  ...
  if (UseTLAB) {
    tlab().initialize();  // initialize thread-local alloc buffer related fields
  }
...
void JavaThread::exit(bool destroy_vm) {
  ...
  if (!destroy_vm || Universe::is_jdk12x_version()) {
    ...
    if (uncaught_exception.not_null()) {
      ...
      Events::log("uncaught exception INTPTR_FORMAT " " INTPTR_FORMAT " " INTPTR_FORMAT", uncaught_exception(), threadObj(), group());
      { 
        EXCEPTION_MARK;
...
void JavaThread::check_and_handle_async_exceptions(bool check_unsafe_error) {
  ...
  if (has_last_Java_frame() && has_async_condition()) {
	...
	(!SafepointPolling && last_frame().is_safepoint_blob_frame())) {
      ...
      CodeBlob *cb = CodeCache::find_blob(caller_fr.pc());
      if (cb != NULL && cb->is_nmethod() && ((nmethod*)cb)->is_patched_for_deopt()) { 
	if (TraceExceptions) {
	  ResourceMark rm;
  ...
  if (_pending_async_exception != NULL) {
	...
	tty->print_cr(" of type: %s", instanceKlass::cast(_pending_async_exception->klass())->external_name());
      ...
      _pending_async_exception = NULL;
      clear_has_async_exception_with_lock();
    }        
  }
...
void JavaThread::send_thread_stop(oop java_throwable)  {
  ...
  if (thread_state() == _thread_in_native) {
    ...
    if (!has_pending_exception() || !pending_exception()->is_a(SystemDictionary::threaddeath_klass())) {
      ...
      NOT_PRODUCT(Exceptions::debug_check_abort(instanceKlass::cast(pending_exception()->klass())->external_name()));
    }
  } else {
...
void JavaThread::java_suspend() {
  ...
  { MutexLockerEx ml(SR_lock(), Mutex::_no_safepoint_check_flag);
    ...
    if (is_ext_suspend_completed(false /* !called_by_wait */, &debug_bits) ) {
      return;
    }
  }
...
void JavaThread::print_thread_state() const {
  tty->print_cr("Thread state: %s", _get_thread_state_name(_thread_state));
};
#endif // PRODUCT
...
void JavaThread::print() const {
  ...
  tty->print_cr("[" INTPTR_FORMAT ".." INTPTR_FORMAT "]", (intptr_t)last_Java_sp() & ~right_n_bits(12), highest_lock());
#ifndef PRODUCT
  print_thread_state();
...
void JavaThread::print_on_error(outputStream* st, char *buf, int buflen) const {
  ...
  if (thread_obj) {
     ...
     UNICODE::as_utf8((jchar*) name->base(T_CHAR), name->length(), buf, buflen);
     st->print("%s\"", buf);
...
// Threads::print() is called at safepoint by VM_PrintThreads operation.
void Threads::print(bool print_stacks, bool internal_format) {
  ...
  tty->cr();
  Universe::heap()->print_gc_threads();
...
static void dump_deadlock_cycle(JavaThread* thread, 
                                GrowableArray<JavaThread*>* deadlock_threads,
                                bool print_to_tty) {
  if (print_to_tty) {
    tty->cr();
  ...
  do {
    if (deadlock_threads != NULL) {
      // Put the blocked thread in the list
      deadlock_threads->append(currentThread);
    }
    ...
    if (print_to_tty) {
      if (obj != NULL) {
        ...
        tty->print(" (JVMTI/JVMPI raw monitor),\n  which is held by");
      }
    }
...
void Threads::find_deadlocks(GrowableArray<JavaThread*>* deadlock_threads, bool print_to_tty) {
  ...
  ALL_JAVA_THREADS(q) {
    ...
    while (waitingToLock != NULL) {
      ...
      if (currentThread->depth_first_number() < 0) {
        ...
        // We have a (new) cycle
        number_of_deadlocks ++;
        dump_deadlock_cycle(currentThread, deadlock_threads, print_to_tty);
        break;
      }
