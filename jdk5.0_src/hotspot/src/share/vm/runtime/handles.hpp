...
#pragma ident "@(#)handles.hpp	1.107 03/12/23 16:43:47 JVM"
#endif
/*
...
class Handle VALUE_OBJ_CLASS_SPEC {
 ...
 // Used indirectly only by GrowableArrays
 // dummy parameter internal_use_only to make it very clear you should
 // not be calling this!
 Handle(const intptr_t handle_as_intptr_t, bool internal_use_only)       { _handle = (oop*) handle_as_intptr_t; }
 ...
 public:
  ...
  // Used by GrowableArray references
  operator GrET*() const                         { return (GrET*)_handle; }
...
class KlassHandle: public Handle {
 protected:
  ...
  /* this constructor/conversion method exists solely to make the casts
   * in GrowableArray work */
  friend class GrowableArray<KlassHandle>;
  KlassHandle (const intptr_t handle_as_intptr_t) : Handle(handle_as_intptr_t, true) {
    assert(SharedSkipVerify || is_null() || obj()->is_klass(),
           "not a klassOop");
  }
 ...
 public:
  ...
  KlassHandle (Klass* kl) : Handle(kl ? kl->as_klassOop() : NULL) {
    assert(SharedSkipVerify || is_null() || obj()->is_klass(), "not a klassOop");
  }
  ...
  KlassHandle (Thread *thread, Klass* kl)
    : Handle(thread, kl ? kl->as_klassOop() : NULL) {
...
#define DEF_HANDLE(type, is_a)                   \
  ...
  class type##Handle: public Handle {            \
   protected:                                    \
    ...
    /* this constructor/conversion method exists solely to make the */       \
    /* casts in GrowableArray work */                                        \
    friend class GrowableArray< type##Handle >;                              \
    type##Handle (const intptr_t handle_as_intptr_t) : Handle(handle_as_intptr_t, true) {     \
...
#define DEF_KLASS_HANDLE(type, is_a)             \
  class type##Handle : public KlassHandle {      \
   ...
   protected:                                    \
    ...
    /* casts in GrowableArray work. */                                        \
    friend class GrowableArray< type##Handle >;                               \
    type##Handle (const intptr_t obj_as_int)                                  \
      : KlassHandle(klassOop(obj_as_int)) {                                   \
      assert(SharedSkipVerify || is_null() ||                                 \
             klassOop(obj_as_int)->klass_part()->is_a(), "illegal type");     \
   }                                                                          \
