...
#pragma ident "@(#)interfaceSupport.hpp	1.155 04/06/09 09:33:01 JVM"
#endif
/*
...
class ThreadStateTransition : public StackObj {
 ...
 public:
  ...
  static inline void transition(JavaThread *thread, JavaThreadState from, JavaThreadState to) {
    ...
    OrderAccess::fence(); // Force a fence between the write above & read below
...
// VM-internal runtime interface support
  ...
  #define TRACE_CALL(result_type, header)                            \
    ...
    if (TraceRuntimeCalls) InterfaceSupport::trace(#result_type, #header);
#else
  #define TRACE_CALL(result_type, header)                            \
    /* do nothing */
#endif
...
extern "C" {                                                         \
  static result_type JNICALL header {                                \
