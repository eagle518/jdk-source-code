...
#pragma ident "@(#)utf8.cpp	1.22 03/12/23 16:44:52 JVM"
#endif
/*
...
char* UTF8::next_character(const char* str, jint* value) {
  ...
  /* See if it's legal supplementary character: 
     11101101 1010xxxx 10xxxxxx 11101101 1011xxxx 10xxxxx */
...
int UTF8::unicode_length(const char* str, int len) {
  int result = 0;
  jchar value;
  for (const char* p = str; p - str < len; p = next(p, &value)) result++;
  return result;
}
...
int UTF8::unicode_length(const char* str) {
  int result = 0;
  jchar value;
  for (const char* p = str; *p; p = next(p, &value)) result++;
  return result;
}
...
void UTF8::convert_to_unicode(const char* utf8_str, jchar* unicode_str, int unicode_length) {
  ...
  for (int index = 0; index < unicode_length; index++) {
    p = UTF8::next(p, &unicode_str[index]);
  }
}
...
jbyte* UTF8::strrchr(jbyte* base, int length, jbyte c) {
  ...
  assert(0 <= c && c < 128, "does not work for non-ASCII characters");
