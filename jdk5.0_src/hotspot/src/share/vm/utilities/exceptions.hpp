...
#pragma ident "@(#)exceptions.hpp	1.38 03/12/23 16:44:44 JVM"
#endif
/*
...
class ThreadShadow: public CHeapObj {
 protected:
  oop  _pending_exception;                       // overlays Thread::_pending_exception
...
class Exceptions {
 ...
 public:
  ...
  static void _throw_msg(Thread* thread, const char* file, int line, symbolHandle name, const char* message, Handle loader, Handle protection_domain);
  static void _throw_msg(Thread* thread, const char* file, int line,
                         symbolOopDesc *name, const char* message);
  static void _throw_msg(Thread* thread, const char* file, int line,
                         symbolHandle name, const char* message);
  static void _throw_args(Thread* thread, const char* file, int line, symbolHandle name, symbolHandle signature, JavaCallArguments* args);
  static void fthrow(Thread* thread, const char* file, int line, symbolHandle name, const char* format, ...);

  // Create an initialize a new exception
  static Handle new_exception(Thread* thread, symbolHandle name, symbolHandle signature, JavaCallArguments* args, Handle loader, Handle protection_domain);
 static Handle new_exception(Thread* thread, symbolHandle name, const char* message, Handle loader, Handle protection_domain);

 static Handle new_exception(Thread* thread, symbolOopDesc *name,
                             const char* message);

  static void throw_stack_overflow_exception(Thread* thread, const char* file, int line);

  // debugging
  static void debug_trace(Thread* thread, Handle exception);
