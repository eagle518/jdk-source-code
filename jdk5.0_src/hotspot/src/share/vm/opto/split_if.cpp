...
#pragma ident "@(#)split_if.cpp	1.57 03/12/23 16:43:00 JVM"
#endif
//
...
Node *PhaseIdealLoop::split_thru_region( Node *n, Node *region ) {
  ...
  for( uint i = 1; i < region->req(); i++ ) {
    Node *x = n->clone();
    Node *in = n->in(0);
    if( in->in(0) == region ) x->set_req( 0, in->in(i) );
...
bool PhaseIdealLoop::split_up( Node *n, Node *blk1, Node *blk2 ) {
  ...
  if( n->is_Load() ) { // see bug 4655758
    // Special handling because of potential anti-dependences which 
    // are not taken into account in set_late_ctrl(), so the Load's block, as
    // returned by get_ctrl(), may actually be below an anti-dependent store.
    // Check if this load has a block local input and update its control so 
    // that split_up() will proces it.  Because memory true-dependence is 
    // explicit, we do not have to worry about early_ctrl being above a store
    // that must precede the load (early_ctrl is after all inputs are available).
    Node *early_ctrl = get_early_ctrl(n);
    if( early_ctrl == blk1 || early_ctrl == blk2 ) { // Load has block-local input
      set_ctrl(n, early_ctrl);
    }
  }
  ...
  if( n->is_Cmp() ) {
    ...
    Node *bol, *cmov;
    if( !(n->outcnt() == 1 && 
          (bol = n->unique_out()->is_Bool()) &&
          (get_ctrl(bol) == blk1 ||
           get_ctrl(bol) == blk2) &&
          bol->outcnt() == 1 &&
          (cmov = bol->unique_out()->is_CMove()) &&
          (get_ctrl(cmov) == blk1 ||
           get_ctrl(cmov) == blk2) ) ) {
  ...
  if( n->is_Store() && n->in(MemNode::Memory)->in(0) == n->in(0) ) {
    ...
    for (DUIterator_Fast imax,i = memphi->fast_outs(imax); i < imax; i++) {
      LoadNode *load = memphi->fast_out(i)->is_Load();
      if( load && alias_idx == C->get_alias_index(_igvn.type(load->in(MemNode::Address))->is_ptr()) )
        set_ctrl(load,blk1);
    }
  }
  ...
  if( prior_n == new_false || prior_n == new_true ) {
    ...
    if( def->is_CFG() ) {
      ...
      for( uint i = 1; i < phi_post->req(); i++ ) // For all paths
        phi_post->set_req( i, spinup( iff_dom, new_false, new_true, prior_n->in(i), def, cache ) );
...
void PhaseIdealLoop::do_split_if( Node *iff ) {
  ...
  for (DUIterator_Last j2min, j2 = iff->last_outs(j2min); j2 >= j2min; --j2) {
    ...
    set_idom(ifpx, region_dom, dom_depth(region_dom));
