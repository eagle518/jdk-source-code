...
#pragma ident "@(#)addnode.cpp	1.123 04/06/04 09:07:30 JVM"
#endif
/*
...
static bool commute( Node *add, int con_left, int con_right ) {
  ...
  if( (phi = in1->is_Phi()) && !phi->is_copy() && phi->region()->is_Loop() && phi->in(2)==add)
    return false;
  if( (phi = in2->is_Phi()) && !phi->is_copy() && phi->region()->is_Loop() && phi->in(2)==add){
    add->swap_edges(1, 2);
    return true;
  }

  // Check for making long spline calculations
  uint opa = add->Opcode();
  uint op1 = in1->Opcode();
  uint op2 = in2->Opcode();
  if( 0 && opa == op1 && opa != op2 ) {
    add->swap_edges(1, 2);
    return true;
  } else if( 0 && opa != op1 && opa == op2 )
    return false;
...
Node *AddNode::Ideal(PhaseGVN *phase, bool can_reshape) {
  ...
  if( con_right && t2 != Type::TOP && // Right input is a constant?
    ...
    if( t12->singleton() && t12 != Type::TOP ) { // Left input is an add of a constant?
      ...
      Node *x1 = add1->in(1);
      Node *x2 = phase->makecon( add1->is_Add()->add_ring( t2, t12 ));
...
Node *AddINode::Ideal(PhaseGVN *phase, bool can_reshape) {
  ...
  if( op1 == Op_SubI ) {
    ...
    if( t_sub1->singleton() && t_2->singleton() && t_sub1 != Type::TOP && t_2 != Type::TOP )
      return new (3) SubINode(phase->makecon( add_ring( t_sub1, t_2 ) ),
                              in(1)->in(2) );
    // Convert "(a-b)+(c-d)" into "(a+c)-(b+d)"
    if( op2 == Op_SubI ) {
      // Check for dead cycle: d = (a-b)+(c-d)
      if( in(1)->in(2) == this || in(2)->in(2) == this ) 
        return phase->C->top();
      Node *sub1 = phase->transform(new (3) AddINode(in(1)->in(1), in(2)->in(1) ) );
      Node *sub2 = phase->transform(new (3) AddINode(in(1)->in(2), in(2)->in(2) ) );
      return new (3) SubINode(sub1, sub2 );
    }
  }

  // Convert "x+(0-y)" into "(x-y)"
  if( op2 == Op_SubI && phase->type(in(2)->in(1)) == TypeInt::ZERO ) 
    return new (3) SubINode(in(1), in(2)->in(2) );
  ...
  if( op1 == Op_URShiftI && op2 == Op_ConI &&
    ...
    if( z < 5 && -5 < y && y < 0 ) {
      ...
      if( t_in11 != Type::TOP && (t_in11->is_int()->_lo >= -(y << z)) ) {
        Node *a = phase->transform( new (3) AddINode( in(1)->in(1), phase->intcon(y<<z) ) );
        return new (3) URShiftINode( a, in(1)->in(2) );
      }
    }
  }
  ...
  if( op2 == Op_AddI &&
      ...
      op1 != Op_ConI && 
      0 ) {
    Node *shift = phase->transform(new (3) LShiftINode(in(1),phase->intcon(1)));
    return new (3) AddINode(shift,in(2)->in(2));
  }
...
Node *AddLNode::Ideal(PhaseGVN *phase, bool can_reshape) {
  ...
  if( op1 == Op_SubL ) {
    ...
    const Type *t_2    = phase->type( in(2)        );
    if( t_sub1->singleton() && t_2->singleton() && t_sub1 != Type::TOP && t_2 != Type::TOP )
      return new (3) SubLNode(phase->makecon( add_ring( t_sub1, t_2 ) ),
                              in(1)->in(2) );
    // Convert "(a-b)+(c-d)" into "(a+c)-(b+d)"
    if( op2 == Op_SubL ) {
      // Check for dead cycle: d = (a-b)+(c-d)
      if( in(1)->in(2) == this || in(2)->in(2) == this ) 
        return phase->C->top();
      Node *sub1 = phase->transform(new (3) AddLNode(in(1)->in(1), in(2)->in(1) ) );
      Node *sub2 = phase->transform(new (3) AddLNode(in(1)->in(2), in(2)->in(2) ) );
      return new (3) SubLNode(sub1, sub2 );
    }
  }

  // Convert "x+(0-y)" into "(x-y)"
  if( op2 == Op_SubL && phase->type(in(2)->in(1)) == TypeLong::ZERO ) 
    return new (3) SubLNode(in(1), in(2)->in(2) );
  ...
  if( op2 == Op_AddL &&
      ...
      op1 != Op_ConL && 
      0 ) {
    Node *shift = phase->transform(new (3) LShiftLNode(in(1),phase->intcon(1)));
    return new (3) AddLNode(shift,in(2)->in(2));
  }
...
Node *AddPNode::Ideal(PhaseGVN *phase, bool can_reshape) {
  ...
  const AddPNode *addp = in(Address)->is_AddP(); // Left input is an AddP?
  if( addp && addp->in(Base) != phase->C->top() ) {
    if( addp == addp->in(Address)->is_AddP() ) { // Check for dead loop.
      return phase->C->top();
    }
    ...
    if( t12->is_con() ) {       // Left input is an add of a constant?
      ...
      set_req(Address, phase->transform(new (4) AddPNode(in(Base),addp->in(Address),in(Offset))));
  ...
  /* 
   4895131. This condition is not enough to identify an unsafe access.
   To do this convertion to rawptr here we need to fix the type system first.
   Move it to parser (to LibraryCallKit::inline_unsafe_*()) for now.
  ...
  if (phase->type(in(Address)) == TypePtr::NULL_PTR) {
    ...
    NOT_LP64( offset = phase->transform(new (2) ConvI2LNode(offset)) );
    return new (2) CastL2PNode(offset);
  }
  */
  ...
  if( add->Opcode() == Op_AddX && add->in(1) != add ) {
    ...
    if( t22->singleton() && (t22 != Type::TOP) ) {  // Right input is an add of a constant?
      set_req(Address, phase->transform(new (4) AddPNode(in(Base),in(Address),add->in(1))));
...
const Type *MaxINode::add_ring( const Type *t0, const Type *t1 ) const {
  ...
  return TypeInt::make( MAX2(r0->_lo,r1->_lo), MAX2(r0->_hi,r1->_hi) );
}
...
Node *MinINode::Ideal(PhaseGVN *phase, bool can_reshape) {
  ...
  if( l->Opcode() == Op_MinI ) {
    if( l == l->in(1) ) { // Check for dead loop.
      return phase->C->top();
    }
    r = phase->transform(new (3) MinINode(l->in(2),r));
    l = l->in(1);
    set_req(1, l);
    set_req(2, r);
    return this;
  ...
  if( r->Opcode() == Op_MinI ) {
    if( r == r->in(2) ) { // Check for dead loop.
      return phase->C->top();
    }
    y = r->in(1);
    // Check final part of MIN tree
    ...
    if( x->_idx > y->_idx ) 
      return new (3) MinINode(r->in(1),phase->transform(new (3) MinINode(l,r->in(2))));
    ...
    return new (3) MinINode(phase->transform(new (3) AddINode(x,phase->intcon(MIN2(x_off,y_off)))),r->in(2));
  } else {
    // See if covers: MIN2(x+c0,y+c1)
    if( !phase->eqv(x,y) ) return NULL;
    // If (y == x) transform MIN2(x+c0,x+c1) into x+c0 or x+c1 which less.
    return new (3) AddINode(x,phase->intcon(MIN2(x_off,y_off)));
  }
