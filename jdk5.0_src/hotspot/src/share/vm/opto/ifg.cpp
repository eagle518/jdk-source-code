...
#pragma ident "@(#)ifg.cpp	1.53 04/07/14 07:30:18 JVM"
#endif
/*
...
uint PhaseChaitin::build_ifg_physical( ResourceArea *a ) {
  Compile::TracePhase t3("buildIFG", &_t_buildIFGphysical, TimeCompiler);
  ...
  for( uint i = 0; i < _cfg._num_blocks; i++ ) {
    ...
    for( j = last_inst + 1; j > 1; j-- ) {
      ...
      if( r ) {
        ...
        if( lrgs(r).is_bound() && !(n->rematerialize()) ) {
    ...
    if( j < b->_nodes.size() && j < b->end_idx()+1 ) {
      Node *cur = b->_nodes[j];
      MachNode *m = cur->is_Mach();
      while( cur->is_Proj() || (m && m->is_MachNullCheck()) || cur->is_Catch() ) {
        j--;
        cur = b->_nodes[j];
        m = cur->is_Mach();
      }
