...
#pragma ident "@(#)reg_split.cpp	1.62 04/07/14 07:30:17 JVM"
#endif
/*
    ...
    for( insidx = 1; insidx <= b->end_idx(); insidx++ ) {
      ...
      if( !nullcheck ) {
        ...
        for( inpidx = 1; inpidx < cnt; inpidx++ ) {
          ...
          if( inpidx > old_last && ((inpidx - oopoff) & 1) == 0 ) {
            continue;
          }
          // Get lidx of input
            ...
            if( inpidx >= oopoff ) {
              ...
              if( ((inpidx - oopoff) & 1) == 0 &&// derived vs base?
                ...
                if( k == cnt ) {// No instance of debug_defs[slidx]
                  ...
                  // subsequent iterations. We have to process the added 
                  // base later since it is not handled yet at this point.
                  cnt += 2;
                }
              }
              continue;
