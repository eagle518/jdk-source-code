...
#pragma ident "@(#)jvmtiEnvThreadState.cpp	1.10 03/12/23 16:43:17 JVM"
#endif
/*
...
static void get_current_location(JavaThread* thread, jmethodID *method_id_p, int* bci) {
...
void JvmtiEnvThreadState::reset_current_location(jvmtiEvent event_type, bool enabled) {
  ...
  if (enabled) {
    ...
    if (event_type == JVMTI_EVENT_SINGLE_STEP && _thread->has_last_Java_frame()) {
      jmethodID method_id;
      int bci;
      get_current_location(_thread, &method_id, &bci);
