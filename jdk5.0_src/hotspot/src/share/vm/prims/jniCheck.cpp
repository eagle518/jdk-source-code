...
#pragma ident "@(#)jniCheck.cpp	1.30 04/07/12 12:56:37 JVM"
#endif
/*
...
static const struct JNINativeInterface_ * unchecked_jni_NativeInterface;
...
functionEnterCritical(JavaThread* thr)
{
  if (thr->has_pending_exception()) {
    NativeReportJNIFatalError(thr, "JNI call made with exception pending");
  }
}
...
checkArray(JavaThread* thr, jarray jArray, int elementType)
{
  ...
  aOop = (arrayOopDesc *)jniCheck::validate_object(thr, jArray);
  if (!aOop || !aOop->is_array())
    ReportJNIFatalError(thr, fatal_non_array);

  if (elementType != -1) {
    if (aOop->is_typeArray()) {
      BasicType array_type = typeArrayKlass::cast(aOop->klass())->type();
...
static inline bool is_frame_handle(JavaThread* thr, jobject obj) {
...
klassOop jniCheck::validate_class(JavaThread* thr, jclass clazz, bool allow_primitive) {
  ...
  if (!(k || allow_primitive && java_lang_Class::is_primitive(mirror))) {
    ReportJNIFatalError(thr, fatal_class_not_a_class);
  }
  return k;
}
...
void jniCheck::validate_call_object(JavaThread* thr, jobject obj, jmethodID method_id) {
  ...
  if (jniIdSupport::check_method(k_oop, method_id) != jniIdSupport::valid_id) {
...
void jniCheck::validate_call_class(JavaThread* thr, jclass clazz, jmethodID method_id) {
  /* validate the class being passed */
  ASSERT_OOPS_ALLOWED;
  klassOop k_oop = jniCheck::validate_class(thr, clazz, false);
...
JNI_ENTRY_CHECKED(jweak, 
  checked_jni_NewWeakGlobalRef(JNIEnv *env,
    ...
    functionEnter(thr);
    if (obj == NULL)
      NativeReportJNIFatalError(thr, "Null object passed to NewWeakGlobalRef");
 ...
 */
const struct JNINativeInterface_  checked_jni_NativeInterface = {
    NULL,
    NULL,
    NULL,
...
const struct JNINativeInterface_* jni_functions_check() {
