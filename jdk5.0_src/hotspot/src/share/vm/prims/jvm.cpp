...
#pragma ident "@(#)jvm.cpp	1.521 04/06/23 11:58:29 JVM"
#endif
/*
...
JVM_ENTRY(jobject, JVM_InitProperties(JNIEnv *env, jobject properties))
  ...
  // Add the sun.management.compiler property for the compiler's name
  {
    ...
    CORE_ONLY(     const char* compiler_name = "";)
    COMPILER1_ONLY(const char* compiler_name = "HotSpot 64-Bit Client Compiler";)
    COMPILER2_ONLY(const char* compiler_name = "HotSpot 64-Bit Server Compiler";)
#else
    CORE_ONLY(     const char* compiler_name = "";)
    COMPILER1_ONLY(const char* compiler_name = "HotSpot Client Compiler";)
    COMPILER2_ONLY(const char* compiler_name = "HotSpot Server Compiler";)
#endif
...
JVM_ENTRY(jclass, JVM_FindLoadedClass(JNIEnv *env, jobject loader, jstring name))
  ...
  symbolHandle klass_name = oopFactory::new_symbol_handle(str, str_len,CHECK_0);
...
JVM_ENTRY(jstring, JVM_GetClassSignature(JNIEnv *env, jclass cls))
  ...
  if (java_lang_Class::is_primitive(JNIHandles::resolve(cls))) {    
    ...
    Handle str = java_lang_String::create_from_symbol(sym, CHECK_0);
...
JVM_ENTRY(jbyteArray, JVM_GetClassAnnotations(JNIEnv *env, jclass cls))
  ...
  if (java_lang_Class::is_primitive(JNIHandles::resolve(cls))) {    
    ...
    klassOop k = java_lang_Class::as_klassOop(JNIHandles::resolve(cls));
    return (jbyteArray) JNIHandles::make_local(env, instanceKlass::cast(k)->class_annotations());
  }
JVM_END
...
JVM_ENTRY(jobjectArray, JVM_GetClassDeclaredFields(JNIEnv *env, jclass ofClass, jboolean publicOnly))
{
  ...
  if (java_lang_Class::is_primitive(JNIHandles::resolve_non_null(ofClass)) ||
      Klass::cast(java_lang_Class::as_klassOop(JNIHandles::resolve_non_null(ofClass)))->oop_is_array()) {
    // Return empty array
...
JVM_ENTRY(jobjectArray, JVM_GetClassDeclaredMethods(JNIEnv *env, jclass ofClass, jboolean publicOnly))
{
  ...
  for (i = 0; i < methods_length; i++) {
    ...
    if (!method->is_initializer()) {
      if (!publicOnly || method->is_public()) {
        oop m = Reflection::new_method(method, UseNewReflection, false, CHECK_0);
...
JVM_ENTRY(jobject, JVM_GetClassConstantPool(JNIEnv *env, jclass cls))
{
  ...
  instanceKlassHandle k(THREAD, java_lang_Class::as_klassOop(JNIHandles::resolve_non_null(cls)));
  Handle jcp = sun_reflect_ConstantPool::create(CHECK_0);
  sun_reflect_ConstantPool::set_cp_oop(jcp(), k->constants());
...
JVM_ENTRY(jclass, JVM_ConstantPoolGetClassAt(JNIEnv *env, jobject unused, jobject jcpool, jint index))
{
  ...
  klassOop k = cp->klass_at(index, CHECK_0);
...
static jobject get_method_at_helper(constantPoolHandle cp, jint index, bool force_resolution, TRAPS) {
  ...
  klassOop k_o;
  if (force_resolution) {
    k_o = cp->klass_at(klass_ref, CHECK_0);
  } else {
  ...
  methodOop m = k->find_method(name, sig);
  if (m == NULL) {
    THROW_MSG_0(vmSymbols::java_lang_RuntimeException(), "Unable to look up method in target class");
  }
  oop method;
  if (!m->is_initializer() || m->is_static()) {
    method = Reflection::new_method(m, true, true, CHECK_0);
  } else {
    method = Reflection::new_constructor(m, CHECK_0);
  }
...
JVM_ENTRY(jobject, JVM_ConstantPoolGetMethodAt(JNIEnv *env, jobject unused, jobject jcpool, jint index))
{
  ...
  constantPoolHandle cp = constantPoolHandle(THREAD, constantPoolOop(JNIHandles::resolve_non_null(jcpool)));
  bounds_check(cp, index, CHECK_0);
  jobject res = get_method_at_helper(cp, index, true, CHECK_0);
  return res;
}
JVM_END
...
JVM_ENTRY(jobject, JVM_ConstantPoolGetMethodAtIfLoaded(JNIEnv *env, jobject unused, jobject jcpool, jint index))
{
  ...
  constantPoolHandle cp = constantPoolHandle(THREAD, constantPoolOop(JNIHandles::resolve_non_null(jcpool)));
  bounds_check(cp, index, CHECK_0);
  jobject res = get_method_at_helper(cp, index, false, CHECK_0);
  return res;
}
JVM_END
...
static jobject get_field_at_helper(constantPoolHandle cp, jint index, bool force_resolution, TRAPS) {
  ...
  oop field = Reflection::new_field(&fd, true, CHECK_0);
...
JVM_ENTRY(jobject, JVM_ConstantPoolGetFieldAt(JNIEnv *env, jobject unused, jobject jcpool, jint index))
{
  ...
  constantPoolHandle cp = constantPoolHandle(THREAD, constantPoolOop(JNIHandles::resolve_non_null(jcpool)));
  bounds_check(cp, index, CHECK_0);
  jobject res = get_field_at_helper(cp, index, true, CHECK_0);
  return res;
}
JVM_END
...
JVM_ENTRY(jobject, JVM_ConstantPoolGetFieldAtIfLoaded(JNIEnv *env, jobject unused, jobject jcpool, jint index))
{
  ...
  constantPoolHandle cp = constantPoolHandle(THREAD, constantPoolOop(JNIHandles::resolve_non_null(jcpool)));
  bounds_check(cp, index, CHECK_0);
  jobject res = get_field_at_helper(cp, index, false, CHECK_0);
  return res;
}
JVM_END
...
JVM_ENTRY(jobjectArray, JVM_ConstantPoolGetMemberRefInfoAt(JNIEnv *env, jobject unused, jobject jcpool, jint index))
{
  ...
  objArrayOop  dest_o = oopFactory::new_objArray(SystemDictionary::string_klass(), 3, CHECK_0);
  objArrayHandle dest(THREAD, dest_o);
  Handle str = java_lang_String::create_from_symbol(klass_name, CHECK_0);
  dest->obj_at_put(0, str());
  str = java_lang_String::create_from_symbol(member_name, CHECK_0);
  dest->obj_at_put(1, str());
  str = java_lang_String::create_from_symbol(member_sig, CHECK_0);
...
JVM_ENTRY(jstring, JVM_ConstantPoolGetStringAt(JNIEnv *env, jobject unused, jobject jcpool, jint index))
{
  ...
  oop str = cp->string_at(index, CHECK_0);
...
JVM_ENTRY(void, JVM_StopThread(JNIEnv* env, jobject jthread, jobject throwable))
  ...
  Events::log("JVM_StopThread thread JavaThread " INTPTR_FORMAT " as oop " INTPTR_FORMAT " [exception " INTPTR_FORMAT "]", receiver, java_thread, throwable);
...
JVM_ENTRY(jclass, JVM_LoadClass0(JNIEnv *env, jobject receiver, 
  ...
  symbolHandle name = oopFactory::new_symbol_handle(str, CHECK_0);
...
JVM_LEAF(jboolean, JVM_AccessVMBooleanFlag(const char* name, jboolean* value, jboolean is_get))
  ...
  return is_get ? CommandLineFlags::boolAt((char*) name, (bool*) value) : CommandLineFlags::boolAtPut((char*) name, (bool*) value);
JVM_END

JVM_LEAF(jboolean, JVM_AccessVMIntFlag(const char* name, jint* value, jboolean is_get))
  JVMWrapper("JVM_AccessVMIntFlag");
  intx v;
  jboolean result = is_get ? CommandLineFlags::intxAt((char*) name, &v) : CommandLineFlags::intxAtPut((char*) name, &v);
  *value = (jint)v;
  return result;
JVM_END
...
// Returns an array of all live Thread objects (VM internal JavaThreads are skipped)
JVM_ENTRY(jobjectArray, JVM_GetAllThreads(JNIEnv *env, jclass dummy))
  ResourceMark rm(THREAD);
  ThreadsListEnumerator tle(THREAD);
  JvmtiVMObjectAllocEventCollector oam;
...
JVM_ENTRY(jobjectArray, JVM_DumpThreads(JNIEnv *env, jclass threadClass, jobjectArray threads))
  ...
  Handle stacktraces = ThreadService::dump_stack_traces(thread_handle_array, num_threads, CHECK_0);
...
JVM_ENTRY(jobjectArray, JVM_GetEnclosingMethodInfo(JNIEnv *env, jclass ofClass))
{
  ...
  objArrayOop dest_o = oopFactory::new_objArray(SystemDictionary::object_klass(), 3, CHECK_0);
  objArrayHandle dest(THREAD, dest_o);
  klassOop enc_k = ik_h->constants()->klass_at(encl_method_class_idx, CHECK_0);
  ...
  if (encl_method_method_idx != 0) {
    ...
    str = java_lang_String::create_from_symbol(sym, CHECK_0);
    dest->obj_at_put(2, str());
  }
  return (jobjectArray) JNIHandles::make_local(dest());
}
                                              ...
                                              jint javaThreadState))
{
  ...
  switch (javaThreadState) {
    case JAVA_THREAD_STATE_NEW : {
      typeArrayOop r = oopFactory::new_typeArray(T_INT, 1, CHECK_0);
    ...
    case JAVA_THREAD_STATE_WAITING : {
      typeArrayOop r = oopFactory::new_typeArray(T_INT, 2, CHECK_0);
      values_h = typeArrayHandle(THREAD, r); 
    ...
    case JAVA_THREAD_STATE_TIMED_WAITING : {
      typeArrayOop r = oopFactory::new_typeArray(T_INT, 3, CHECK_0);
      values_h = typeArrayHandle(THREAD, r); 
                                                ...
                                                jintArray values))
{
  ...
  switch (javaThreadState) {
    case JAVA_THREAD_STATE_NEW : {
      ...
      Handle name = java_lang_String::create_from_str("NEW", CHECK_0);
    ...
    case JAVA_THREAD_STATE_RUNNABLE : {
      ...
      objArrayOop r = oopFactory::new_objArray(SystemDictionary::string_klass(),
                                               ...
                                               CHECK_0);
      names_h = objArrayHandle(THREAD, r); 
      Handle name = java_lang_String::create_from_str("RUNNABLE", CHECK_0);
      names_h->obj_at_put(0, name()); 
      break; 
    }
    case JAVA_THREAD_STATE_BLOCKED : {
      ...
      objArrayOop r = oopFactory::new_objArray(SystemDictionary::string_klass(),
                                               ...
                                               CHECK_0);
      names_h = objArrayHandle(THREAD, r); 
      Handle name = java_lang_String::create_from_str("BLOCKED", CHECK_0);
      names_h->obj_at_put(0, name()); 
      break; 
    }
    case JAVA_THREAD_STATE_WAITING : {
    ...
    case JAVA_THREAD_STATE_TERMINATED : {
      ...
      objArrayOop r = oopFactory::new_objArray(SystemDictionary::string_klass(),
                                               ...
                                               CHECK_0);
      names_h = objArrayHandle(THREAD, r); 
      Handle name = java_lang_String::create_from_str("TERMINATED", CHECK_0);
      names_h->obj_at_put(0, name()); 
      break; 
    }
    default:
