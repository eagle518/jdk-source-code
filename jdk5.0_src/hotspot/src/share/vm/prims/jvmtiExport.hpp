...
#pragma ident "@(#)jvmtiExport.hpp	1.75 04/05/18 14:38:44 JVM"
#endif
/*
...
class JvmtiExport : public AllStatic {
 private:
  ...
  static bool        _must_purge_jvmdi_frames_on_native_exit;
  ...
  inline static void set_must_purge_jvmdi_frames_on_native_exit(bool on)
                                                                       { _must_purge_jvmdi_frames_on_native_exit = on; }
 ...
 private:
  ...
  static GrowableArray<const address>* _pending_compiled_method_unload_code_begins;	
  ...
  static void post_dynamic_code_generated_internal(const char *name, const address code_begin, const address code_end);
  ...
  static void post_dynamic_code_generated(JvmtiEnv* env, const char *name, const address code_begin, 
					  const address code_end);
 ...
 public:  
  ...
  inline static bool must_purge_jvmdi_frames_on_native_exit()     { return _must_purge_jvmdi_frames_on_native_exit; }
  ...
  static void create_jvmdi_interface();
  ...
  static jint get_jvmdi_interface(JavaVM *jvm, void **penv, jint version);
  ...
  // This routine is called by the interpreter and by compiled native
  // wrappers upon exit from a native method
  // JVMDI jframeIDs are thus invalidated on exit from native method.
  // JVMTI does not use jframeIDs, this whole mechanism must be removed when JVMDI is removed.
  static void thread_leaving_native_code();
  ...
  static void post_dynamic_code_generated(const char *name, const address code_begin, const address code_end);

  // used at a safepoint to post a CompiledMethodUnload event
  static void post_compiled_method_unload_at_safepoint(jmethodID mid, const address code_begin);
