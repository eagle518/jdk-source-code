...
#pragma ident "@(#)psVirtualspace.cpp	1.7 03/12/23 16:40:19 JVM"
#endif
/*
...
bool PSVirtualSpace::shrink_by(size_t bytes) {
  ...
  bool result = os::uncommit_memory(base_addr, bytes);
  if (result) {
    _committed_high_addr -= bytes;
  }
...
PSVirtualSpace::expand_into(PSVirtualSpace* other_space, size_t bytes) {
  ...
  if (tmp_bytes > 0) {
    ...
    if (os::commit_memory(commit_base, tmp_bytes, alignment())) {
