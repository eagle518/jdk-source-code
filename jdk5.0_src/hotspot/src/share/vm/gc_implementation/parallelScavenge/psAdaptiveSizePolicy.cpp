...
#pragma ident "@(#)psAdaptiveSizePolicy.cpp	1.62 04/04/12 13:37:56 JVM"
#endif
/*
...
elapsedTimer PSAdaptiveSizePolicy::_minor_timer;
elapsedTimer PSAdaptiveSizePolicy::_major_timer;
...
void PSAdaptiveSizePolicy::minor_collection_end(GCCause::Cause gc_cause) {
  ...
  if (gc_cause != GCCause::_java_lang_system_gc ||
    ...
    if (PrintAdaptiveSizePolicy && Verbose) {
      gclog_or_tty->print("psAdaptiveSizePolicy::minor_collection_end: "
        "minor gc cost: %f  average: %f", collection_cost * MILLIUNITS,
...
void PSAdaptiveSizePolicy::major_collection_end(size_t amount_live,
  ...
  if (gc_cause != GCCause::_java_lang_system_gc ||
	...
	major_pause_in_seconds / interval_in_seconds;
      _avg_major_gc_cost->sample(collection_cost);
...
double PSAdaptiveSizePolicy::decaying_gc_cost() const {
  double decayed_major_gc_cost = major_gc_cost();
  double avg_major_interval = _avg_major_interval->average();
  if (UseAdaptiveSizeDecayMajorGCCost &&
      (AdaptiveSizeMajorGCDecayTimeScale > 0) &&
      (avg_major_interval > 0.00)) {
    _major_timer.stop();
    double time_since_major_gc = _major_timer.seconds();
    _major_timer.start();
    ...
    if (time_since_major_gc >
      ...
      decayed_major_gc_cost = decayed_major_gc_cost *
        (((double) AdaptiveSizeMajorGCDecayTimeScale) * avg_major_interval)
        / time_since_major_gc;
      if (PrintGCDetails && Verbose) {
        gclog_or_tty->print_cr("\ndecaying_gc_cost: major interval average:"
	  " %f  time since last major gc: %f", 
	  avg_major_interval, time_since_major_gc);
...
void PSAdaptiveSizePolicy::compute_generation_free_space(size_t young_live,
  ...
  size_t promo_limit = max_old_gen_size - avg_old_live()->average();
  ...
  if ((_avg_minor_pause->padded_average() > _gc_pause_goal_sec) ||
      (_avg_major_pause->padded_average() > _gc_pause_goal_sec)) {
    //
    // Check pauses
    //
  ...
  } else if (_avg_minor_pause->padded_average() > _gc_minor_pause_goal_sec) {
    ...
    if (UseAdaptiveSizePolicyFootprintGoal && 
        ...
        _avg_major_gc_cost->average() >= 0.0 &&
        _avg_minor_gc_cost->average() >= 0.0) {
  ...
  if (PrintAdaptiveSizePolicy) {
    if (desired_promo_size > promo_limit)  {
      ...
      size_t free_in_old_gen = max_old_gen_size - avg_old_live()->average();
      gclog_or_tty->print_cr(
		    ...
		    max_old_gen_size, avg_old_live()->average());
    }
  ...
  const size_t free_in_old_gen = max_old_gen_size - avg_old_live()->average();
  ...
  if (PrintAdaptiveSizePolicy) {
    // Timing stats
    gclog_or_tty->print("AdaptiveSizePolicy::compute_generation_free_space: "
               " minor_time: %f"
               " major_cost: %f"
	       " mutator_cost: %f"
	       " throughput_goal: %f",
...
void PSAdaptiveSizePolicy::adjust_for_pause_time(bool is_full_gc,
  ...
  if (PrintAdaptiveSizePolicy && Verbose) {
    gclog_or_tty->print_cr(
      ...
      _avg_major_pause->average(), _gc_pause_goal_sec,
