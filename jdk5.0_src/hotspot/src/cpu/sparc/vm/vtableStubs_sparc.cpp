...
#pragma ident "@(#)vtableStubs_sparc.cpp	1.45 03/12/23 16:37:27 JVM"
#endif
/*
...
VtableStub* VtableStubs::create_vtable_stub(int vtable_index, int receiver_location) {
  ...
  VtableStub* s = new(sparc_code_length) VtableStub(true, vtable_index, receiver_location);
  ResourceMark rm;
  MacroAssembler* masm = new MacroAssembler(new CodeBuffer(s->entry_point(), sparc_code_length));

#ifndef PRODUCT
#ifdef COMPILER2
  if (CountCompiledCalls) {
    Address ctr(G5, OptoRuntime::nof_megamorphic_calls_addr());
    __ sethi(ctr);
  ...
  COMPILER2_ONLY(assert(receiver_location == R_O0_num, "receiver expected in O0");)
...
VtableStub* VtableStubs::create_itable_stub(int vtable_index, int receiver_location) {
  ...
  VtableStub* s = new(sparc_code_length) VtableStub(false, vtable_index, receiver_location);
  ResourceMark rm;
  MacroAssembler* masm = new MacroAssembler(new CodeBuffer(s->entry_point(), sparc_code_length));
...
#ifdef COMPILER2
  if (CountCompiledCalls) {
    Address ctr(L0, OptoRuntime::nof_megamorphic_calls_addr());
    __ sethi(ctr);
    __ ld(ctr, L1);
    __ inc(L1);
    __ st(L1, ctr);
