...
#pragma ident "@(#)assembler_sparc.cpp	1.175 04/03/31 18:13:17 JVM"
#endif
/*
...
void MacroAssembler::breakpoint_trap()    { trap( ST_RESERVED_FOR_USER_0 ); }
...
void MacroAssembler::write_ccr_trap(Register ccr_save, Register scratch1,
  Register scratch2) {
...
void MacroAssembler::set_last_Java_frame(Register last_java_sp, Register last_Java_pc) {
  ...
  Address flags(G2_thread,
	        0,
	        in_bytes(JavaThread::frame_anchor_flags_offset()));
  ...
  delayed() -> nop();
  set((intptr_t)"Stack Not Biased in set_last_Java_frame", O0);
  set((intptr_t)warning, O7);
  jmpl( O7, 0, O7 );
  delayed() -> nop();
  bind(StackOk);
#endif // ASSERT
...
void MacroAssembler::reset_last_Java_frame(void) {
  ...
  // check that it WAS previously set
    ...
    tst(L0);
#ifdef _LP64
    breakpoint_trap(Assembler::zero, Assembler::xcc);
#else
    breakpoint_trap(Assembler::zero);
#endif // _LP64
    restore();
#endif // ASSERT
...
# ifdef ASSERT
    ...
    restore();
#ifdef _LP64
    breakpoint_trap(notZero, Assembler::xcc);
#else
    breakpoint_trap(notZero);
#endif
    // }
# endif
...
void MacroAssembler::setx( jlong value, Register tmp, Register d, relocInfo::relocType rtype ) {
# ifdef CHECK_DELAY
  guarantee( delay_state == no_delay,  "cannot put two instructions in delay slot" );
# endif
// [RGV] I think I need to pass rtype on to sethi's etc..

//  assert(rspec.type() == relocInfo::none, "cannot relocate a 64-bit constant");
  v9_dep();
  int hi = (int)(value >> 32);
  int lo = (int)(value & ~0);
  if (hi == 0 && lo >= 0) {
...
class RegistersForDebugging : public StackObj {
 public:
  ...
  static int i_offset(int j) { return (intptr_t)&((RegistersForDebugging*)0)->i[j]; }
  static int l_offset(int j) { return (intptr_t)&((RegistersForDebugging*)0)->l[j]; }
  static int o_offset(int j) { return (intptr_t)&((RegistersForDebugging*)0)->o[j]; }
  static int g_offset(int j) { return (intptr_t)&((RegistersForDebugging*)0)->g[j]; }
  static int f_offset(int j) { return (intptr_t)&((RegistersForDebugging*)0)->f[j]; }
  static int d_offset(int j) { return (intptr_t)&((RegistersForDebugging*)0)->d[j / 2]; }
...
void MacroAssembler::verify_oop(Register reg, const char* msg) {
...
void MacroAssembler::verify_oop_subroutine() {
  ...
  if ((NULL & Universe::verify_oop_mask()) == Universe::verify_oop_bits()) {
  ...
  cmp(O4_temp, O3_bits);
  brx(notEqual, false, pn, fail);
  delayed()->wrccr( O1_save_flags ); // Restore CCR's
  ...
  wrccr( O1_save_flags ); // Restore CCR's
  ...
  // No specific message; use stack trace to find bad oop
  stop("verify_oop failed");
}
...
void MacroAssembler::test() {
  ...
  MacroAssembler* a = new MacroAssembler(new CodeBuffer(10000, 10000, 0, 0, 0, false));
...
void MacroAssembler::load_unaligned_double(Register r1, Register r2, FloatRegister d) {
  assert_not_delayed();

#ifdef _LP64
  ldf(FloatRegisterImpl::D, r1, r2, d);
#else
  ldf(FloatRegisterImpl::S, r1, r2, d);
  inc(r2, sizeof(jdouble)/2);
  ldf(FloatRegisterImpl::S, r1, r2, d->successor());
#endif
}

// Known good alignment in _LP64 but unknown otherwise
void MacroAssembler::store_unaligned_double(FloatRegister d, Register r1, Register r2) {
  assert_not_delayed();

#ifdef _LP64
  stf(FloatRegisterImpl::D, d, r1, r2);
#else
  stf(FloatRegisterImpl::S, d, r1, r2);
  inc(r2, sizeof(jdouble)/2);
  stf(FloatRegisterImpl::S, d->successor(), r1, r2);
#endif
}

// Known good alignment in _LP64 but unknown otherwise
void MacroAssembler::load_unaligned_double(Register r1, int offset, FloatRegister d) {
  assert_not_delayed();
  ...
  ldf(FloatRegisterImpl::S, r1, offset + sizeof(jdouble)/2, d->successor());
#endif
}

// Known good alignment in _LP64 but unknown otherwise
void MacroAssembler::store_unaligned_double(FloatRegister d, Register r1, int offset) {
  assert_not_delayed();
  ...
  stf(FloatRegisterImpl::S, d->successor(), r1, offset + sizeof(jdouble)/2);
#endif
}

// Known good alignment in _LP64 but unknown otherwise
void MacroAssembler::load_unaligned_long(Register r1, Register r2, Register rd) {
  assert_not_delayed();
#ifdef _LP64
  ldx(r1, r2, rd);
#else
  ld(r1, r2, rd);
  inc(r2, sizeof(jlong)/2);
  ld(r1, r2, rd->successor());
#endif
}

// Known good alignment in _LP64 but unknown otherwise
void MacroAssembler::store_unaligned_long(Register rd, Register r1, Register r2) {
  assert_not_delayed();
#ifdef _LP64
  stx(rd, r1, r2);
#else
  st(rd, r1, r2);
  inc(r2, sizeof(jlong)/2);
  st(rd->successor(), r1, r2);
#endif
}

// Known good alignment in _LP64 but unknown otherwise
void MacroAssembler::load_unaligned_long(Register r1, int offset, Register rd) {
  assert_not_delayed();
#ifdef _LP64
  ldx(r1, offset, rd);
#else
  ld(r1, offset, rd);
  ld(r1, offset + sizeof(jlong)/2, rd->successor());
#endif
}

// Known good alignment in _LP64 but unknown otherwise
void MacroAssembler::store_unaligned_long(Register rd, Register r1, int offset) {
  assert_not_delayed();
#ifdef _LP64
  stx(rd, r1, offset);
#else
  st(rd, r1, offset);
  st(rd->successor(), r1, offset + sizeof(jlong)/2);
#endif
}
...
void MacroAssembler::compiler_lock_object(Register Roop, Register Rmark, Register Rbox, Register Rscratch, Label& done) {
   Address mark_addr(Roop, 0, oopDesc::mark_offset_in_bytes());

   verify_oop(Roop);

   // Load markOop from object into Rmark
...
void MacroAssembler::compiler_unlock_object(Register Roop, Register Rmark, Register Rbox, Register Rscratch, Label& done) {
   Address mark_addr(Roop, 0, oopDesc::mark_offset_in_bytes());
