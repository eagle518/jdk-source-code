...
#pragma ident "@(#)nativeInst_i486.hpp	1.70 04/05/05 16:15:15 JVM"
#endif
/*
...
class NativeCall: public NativeInstruction {
 public:
  ...
  // Creation
  friend NativeCall* nativeCall_at(address address) {
...
class NativeMovConstReg: public NativeInstruction {
 public:
  ...
  friend NativeMovConstReg* nativeMovConstReg_at(address address) {
    NativeMovConstReg* test = (NativeMovConstReg*)(address - instruction_offset);
    #ifdef ASSERT
      test->verify();
    #endif
    return test;
  ...
  friend NativeMovConstReg* nativeMovConstReg_before(address address) {
    NativeMovConstReg* test = (NativeMovConstReg*)(address - instruction_size - instruction_offset);
    #ifdef ASSERT
      test->verify();
    #endif
    return test;
...
class NativeMovRegMem: public NativeInstruction {
 ...
 private:
  friend NativeMovRegMem* nativeMovRegMem_at (address address) {
    NativeMovRegMem* test = (NativeMovRegMem*)(address - instruction_offset);
    #ifdef ASSERT
      test->verify();
    #endif
    return test;
...
class NativeJump: public NativeInstruction {
 public:
  ...
  friend NativeJump* nativeJump_at(address address) {
    NativeJump* jump = (NativeJump*)(address - instruction_offset);
    #ifdef ASSERT
      jump->verify();
    #endif
    return jump;
