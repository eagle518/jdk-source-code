...
#pragma ident "@(#)templateTable_i486.cpp	1.282 04/03/08 13:59:01 JVM"
#endif
/*
...
// local variables
static inline Address iaddress(int n)            { return Address(edi, -n*wordSize); }
...
static inline Address iaddress(Register r)       { return Address(edi, r, Address::times_4); }
static inline Address laddress(Register r)       { return Address(edi, r, Address::times_4, -1 * wordSize); }
static inline Address haddress(Register r)       { return Address(edi, r, Address::times_4,  0 * wordSize); }
...
static inline Address at_esp   ()                { return Address(esp,  0 * wordSize); }
static inline Address at_esp_p1()                { return Address(esp,  1 * wordSize); }
static inline Address at_esp_p2()                { return Address(esp,  2 * wordSize); }
...
void TemplateTable::aastore() {
  ...
  __ movl(ecx, at_esp_p1());  // Index
  __ movl(edx, at_esp_p2());  // Array
  index_check(edx, ecx);  // kills ebx
  // do array store check - check for NULL value first
  __ testl(eax, eax);
  __ jcc(Assembler::zero, is_null);

  __ profile_checkcast(false, ebx); // Blows EBX
...
void TemplateTable::dup2() {
  transition(vtos, vtos);
  // stack: ..., a, b
  __ pushl(Address(esp, 1 * wordSize));
  __ pushl(Address(esp, 1 * wordSize));
  // stack: ..., a, b, a, b
}
...
void TemplateTable::iop2(Operation op) {
  transition(itos, itos);
  switch (op) {
    case add  :                    __ popl(edx); __ addl (eax, edx); break;
    case sub  : __ movl(edx, eax); __ popl(eax); __ subl (eax, edx); break;
    case mul  :                    __ popl(edx); __ imull(eax, edx); break;
    case _and :                    __ popl(edx); __ andl (eax, edx); break;
    case _or  :                    __ popl(edx); __ orl  (eax, edx); break;
    case _xor :                    __ popl(edx); __ xorl (eax, edx); break;
    case shl  : __ movl(ecx, eax); __ popl(eax); __ shll (eax);      break; // implicit masking of lower 5 bits by Intel shift instr.
    case shr  : __ movl(ecx, eax); __ popl(eax); __ sarl (eax);      break; // implicit masking of lower 5 bits by Intel shift instr.
    case ushr : __ movl(ecx, eax); __ popl(eax); __ shrl (eax);      break; // implicit masking of lower 5 bits by Intel shift instr.
...
void TemplateTable::dop2(Operation op) {
  ...
  switch (op) {
    ...
    case mul: {
      ...
      __ jcc(Assembler::notZero, L_strict);
      __ fmul_d (at_esp());
      __ jmp(L_join);
      __ bind(L_strict);
...
void TemplateTable::branch(bool is_jsr, bool is_wide) {
  ...
  if (UseLoopCounter) {
    ...
    if (UseOnStackReplacement) {
      ...
      assert(!Bytecodes::safepoint_safe(bytecode()), "can call OSR method which may not stop");
      ...
      // ebx: target bytecode
      // edx: osr adapter frame return address
      // edi: locals pointer
      // esi: bcp
      ...
      // and the return address in edx (the osr-adapter).  We need to
      // prepare for the call.

      // Reserve space for incoming arguments.  This is needed since an I2C
      // adapter could be introduced as part of deoptimization.  The I2C
      // adapter will think that it owns the incoming arguments area.  If we
      // do not reserve space for this explicitly, then the I2C adapter might
      // override the old-ebp/old-esp offsets in the OSR adapter frame, and
      // frame traversal will fail.  Figure out size of outgoing arguments.
      __ movl(ebx, Address(eax, nmethod::method_offset_in_bytes()));
      // Make sure not to use ecx in the src part.  Otherwise, wrong assembly
      // will be generated.
      __ load_unsigned_word(ecx, Address(ebx, methodOopDesc::size_of_parameters_offset()));      

      __ shll(ecx, 2); // Multiply by 4 to get size in bytes
      __ subl(esp, ecx);
      ...
      __ pushl(edx);
      // nmethod to EDX to free up EAX
      __ movl(edx,eax);
      // Load monitor address into inline-cache register
      const int entry_size =         (     frame::interpreter_frame_monitor_size()           * wordSize);
      const Address monitor_block_bot(ebp, frame::interpreter_frame_initial_sp_offset        * wordSize - entry_size);
      __ leal(eax, monitor_block_bot);            // points to bottom entry of monitor block
      ...
      // edi: locals pointer
      // esi: bcp
      __ jmp(Address(edx, nmethod::osr_entry_point_offset()));
    }
  }
#endif // not CORE
}
...
void TemplateTable::fast_binaryswitch() {
  ...
  { Label loop;
    ...
    if (VM_Version::supports_cmov()) {
      ...
      __ jcc(Assembler::greaterEqual, set_i);    // {
      __ movl(j, h);                             //   j = h;
      __ jmp(end_of_if);                         // }
      __ bind(set_i);                            // else {
      __ movl(i, h);                             //   i = h;
...
void TemplateTable::getfield_or_static(int byte_no, bool is_static) {
  ...
  if (JvmtiExport::can_post_field_access()) {
    ...
    resolve_cache_and_index(byte_no, ecx, edx);
    // cache entry pointer
    __ addl(ecx, in_bytes(constantPoolCacheOopDesc::base_offset()));
    __ shll(edx, LogBytesPerWord);
    __ addl(ecx, edx);
    if (is_static) {
      __ movl(eax, 0);      // NULL object reference
    } else {
      __ popl(eax);         // Get the object
      __ verify_oop(eax);
      __ pushl(eax);         // Restore stack state
    }
...
void TemplateTable::jvmti_post_field_mod(int byte_no, bool is_static) {
  ...
  if (JvmtiExport::can_post_field_modification()) {
    ...
    // We rely on the bytecode being resolved and the cpCache entry filled in.
    resolve_cache_and_index(byte_no, eax, edx);
    ...
    if (is_static) {
      ...
      __ cmpl(ecx, ltos);
      __ jcc(Assembler::equal, two_word);
      __ cmpl(ecx, dtos);
      __ jcc(Assembler::equal, two_word);
      __ addl(ebx, wordSize * 1);         // one word jvalue (not ltos, dtos)
      __ jmp(valsize_known);

      __ bind(two_word);
      __ addl(ebx, wordSize * 2);         // two words jvalue
    
      __ bind(valsize_known);
      // setup object pointer
      __ movl(ebx, Address(ebx, 0 * wordSize));
    }
    // cache entry pointer
...
void TemplateTable::jvmti_post_fast_field_mod() {
  if (JvmtiExport::can_post_field_modification()) {
    ...
    __ popl(ebx);                  // copy the object pointer from tos
    __ verify_oop(ebx);
    __ pushl(ebx);                 // put the object pointer back on tos
    __ subl(esp, sizeof(jvalue));  // add space for a jvalue object      
    __ movl(ecx, esp);
    __ pushl(ebx);                 // save object pointer so we can steal ebx
    __ movl(ebx, 0);
    ...
    __ popl(ebx);  // restore copy of object pointer
...
void TemplateTable::fast_accessfield(TosState state) {
  ...
  if (JvmtiExport::can_post_field_access()) {
    ...
    __ pushl(eax);  // save object pointer before call_VM() clobbers it
    __ verify_oop(eax);
    // eax: object pointer copied above
    // ecx: cache entry pointer
    __ call_VM(noreg, CAST_FROM_FN_PTR(address, InterpreterRuntime::post_field_access), eax, ecx);
    __ popl(eax);   // restore object pointer
    __ bind(L1);
  }
...
void TemplateTable::prepare_invoke(Register method, Register index, int byte_no, Bytecodes::Code code) {
  ...
  if (load_receiver) {
    __ movl(recv, flags);
    __ andl(recv, 0xFF);
    __ movl(recv, Address(esp, recv, Address::times_4, -wordSize));
    __ verify_oop(recv);
  }
...
void TemplateTable::invokevirtual_helper(Register index, Register recv,
  ...
  __ movl(edx, Address(method, methodOopDesc::interpreter_entry_offset()));  
  __ movl(eax, method); // the i2c_adapters need methodOop in eax  
  // do the call
#ifdef ASSERT
  { Label L;
    __ testl(edx, edx);
...
void TemplateTable::invokespecial(int byte_no) {
  ...
  __ verify_oop(ebx);
  __ profile_call(eax);
  __ jmp(Address(ebx, methodOopDesc::interpreter_entry_offset()));
}
...
void TemplateTable::invokeinterface(int byte_no) {
  ...
  __ movl(edx, Address(ebx, methodOopDesc::interpreter_entry_offset()));  

  COMPILER2_ONLY(__ movl(eax, ebx);) // the i2c_adapters need methodOop in eax      
  // do the call
  #ifdef ASSERT
  { Label L;
    __ testl(edx, edx);
...
void TemplateTable::_new() {
  ...
  const int tags_offset = typeArrayOopDesc::header_size(T_BYTE) * wordSize;
  __ cmpb(Address(eax, edx, Address::times_1, tags_offset), JVM_CONSTANT_UnresolvedClass);
  ...
  // has_finalizer
  __ movl(eax, Address(ecx, Klass::access_flags_offset_in_bytes() + sizeof(oopDesc)));
  __ testl(eax, JVM_ACC_CAN_BE_FASTPATH_ALLOCATED);
  __ jcc(Assembler::zero, slow_case);

  // get instance_size in instanceKlass (already aligned)
  __ movl(edx, Address(ecx, Klass::size_helper_offset_in_bytes() + sizeof(oopDesc)));
  ...
  if (UseTLAB) {
    ...
    __ leal(ebx, Address(eax, edx, Address::times_4));
  ...
  __ decrement(edx, sizeof(oopDesc) / oopSize);
  __ jcc(Assembler::zero, initialize_header);

  // Initialize topmost object field, divide edx by 2, check if odd and
  // test if zero.
  __ xorl(ecx, ecx);    // use zero reg to clear memory (shorter code)
  __ shrl(edx, 1);      // divide by 2 and set carry flag if odd

  // edx must have been multiple of 2
#ifdef ASSERT
  // make sure edx was multiple of 2
  Label L;
...
void TemplateTable::checkcast() {
  ...
  __ profile_checkcast(false, ecx); // Blows ECX
...
void TemplateTable::monitorenter() {
  ...
  { Label entry, loop, exit;
    ...
    __ bind(loop);
    __ cmpl(Address(ecx, BasicObjectLock::obj_offset_in_bytes()), (int)NULL);  // check if current entry is used
...
// TODO - need new func here - kbt
    ...
    __ jcc(Assembler::equal, exit);              // if same object then stop searching
...
void TemplateTable::monitorexit() {
  ...
  __ bind(found);
  __ pushl(eax);                                 // make sure object is on stack (contract with oopMaps)  
  __ unlock_object(edx);    
  __ popl(eax);                                  // discard object  
  __ bind(end);
}
