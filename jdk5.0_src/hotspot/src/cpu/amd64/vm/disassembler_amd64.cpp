...
#pragma ident "@(#)disassembler_amd64.cpp	1.2 03/12/23 16:35:45 JVM"
#endif
/*
...
# include "incls/_disassembler_amd64.cpp.incl"
...
bool Disassembler::load_library()
{
  if (_library == NULL) {
    ...
    if (_library != NULL) {
      ...
      _decode_instruction = 
        CAST_TO_FN_PTR(Disassembler::decode_func,
                       hpi::dll_lookup(_library, "decode_instruction"));
    }
  }
...
class amd64_env 
  : public DisassemblerEnv
{
 private:
  nmethod* code;
  outputStream* output;
 public:
  amd64_env(nmethod* rcode, outputStream* routput)
  {
    code = rcode;
    output = routput;
  }
...
void amd64_env::print_label(intptr_t value)
{
...
void amd64_env::print(char* format, ...)
{
  va_list ap;
  va_start(ap, format);
...
char* amd64_env::string_for_offset(intptr_t value)
{
  stringStream st;
...
char* amd64_env::string_for_constant(unsigned char* pc, 
                                      intptr_t value, 
                                      int is_decimal)
{
  stringStream st;
...
void Disassembler::decode(nmethod* nm, outputStream* st)
{
  ...
  st->print_cr("Decoding compiled method " PTR_FORMAT ":", nm);
  st->print("Code:");
  st->cr();
