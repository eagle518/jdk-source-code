...
int JavaVM5::StartJavaVM(const char *addClasspath)
{
	...
	for (i = 0; i < 3; i++) {
        ...
        plugin_error("Exec of \"%s\" failed: %s\n<", fullprogname, strerror(errno));
        exit(6);
    }
...
JavaVM5::ProcessSpontaneousQueue() {
    ...
    int wp = g_unixService->JDFileDesc_To_FD(state->spont_pipe);
    if (wp < 0) {
    ...
    for (;;) {
        ...
        tv.tv_sec =0;
        tv.tv_usec = 0;
        trace("JavaVM5:Doing spontainious work");
        FD_ZERO(&fs);
        FD_SET(wp, &fs);
        rc = select(wp + 1, &fs, NULL, NULL, &tv);
        if (rc < 1) {
...
char *JavaVM5::FindJRE(void) {
	...
	char JREOsName[200];
	char JREOsArch[200];
	sprintf(JREPath, "deployment.javapi.jre.%s.path", RELEASE);
        strcat(JREPath, "=%s");
	sprintf(JREOsName, "deployment.javapi.jre.%s.osname", RELEASE);
        strcat(JREOsName, "=%s");
	sprintf(JREOsArch, "deployment.javapi.jre.%s.osarch", RELEASE);
        strcat(JREOsArch, "=%s");
