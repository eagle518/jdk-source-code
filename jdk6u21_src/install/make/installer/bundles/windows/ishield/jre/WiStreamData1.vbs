...
Else 'If adding a stream, insert a row, else if removing a stream, delete the row
	...
	' Update Media Table	
	Dim mediaName     : mediaName = "Data1.cab"

'	Dim sqlQueryListMedia : sqlQueryListMedia = "SELECT * FROM `Media`"
'	Dim viewListMedia     : Set viewListMedia = database.OpenView(sqlQueryMediaList)
'	Dim recordListMedia	  : Set recordListMedia = installer.CreateRecord(6)
''	viewListMedia.Execute recordListMedia : CheckError
'	viewListMedia.Execute : CheckError
'	
'	Do
'		Set recordListMedia = viewListMedia.Fetch
'		If recordListMedia Is Nothing Then Exit Do
'		WScript.Echo "Media table row= {" & recordListMedia.StringData(1) & ", " & recordListMedia.StringData(2) & ", " & recordListMedia.StringData(3) & ", " & recordListMedia.StringData(4) & ", " & recordListMedia.StringData(5) & ", " & recordListMedia.StringData(6) & " } "
'	Loop


'	Dim sqlQueryMedia : sqlQueryMedia = "SELECT * FROM `Media` WHERE `Cabinet` = `Data1.cab`"
'	Dim sqlQueryMedia : sqlQueryMedia = "SELECT * FROM `Media` WHERE `Cabinet` = " & mediaName
	Dim sqlQueryMedia : sqlQueryMedia = "SELECT * FROM `Media`"
	Dim viewMedia     : Set viewMedia = database.OpenView(sqlQueryMedia)
	'Create and Insert the row.
	Dim recordMedia	  : Set recordMedia = installer.CreateRecord(6)
'	viewMedia.Execute recordMedia : CheckError

	viewMedia.Execute : CheckError
	Do
		Set recordMedia = viewMedia.Fetch
		If recordMedia Is Nothing Then Exit Do
		WScript.Echo "Media table row= {" & recordMedia.StringData(1) & ", " & recordMedia.StringData(2) & ", " & recordMedia.StringData(3) & ", " & recordMedia.StringData(4) & ", " & recordMedia.StringData(5) & ", " & recordMedia.StringData(6) & " } "
	Loop

	viewMedia.Execute : CheckError
	Set recordMedia = viewMedia.Fetch
	If recordMedia Is Nothing Then Wscript.Echo "Media not present:", mediaName : Wscript.Quit 2

	If recordMedia.StringData(4) <> mediaName Then Wscript.Echo "The Cabinet is ", recordMedia.StringData(4), ", not ", mediaName , "." : Wscript.Quit 2

'	WScript.Echo "Media table row= {" & recordMedia.StringData(1) & ", " & recordMedia.StringData(2) & ", " & recordMedia.StringData(3) & ", " & recordMedia.StringData(4) & ", " & recordMedia.StringData(5) & ", " & recordMedia.StringData(6) & " } "

	'Insert storage - copy data into stream
	recordMedia.StringData(4) = "#" & mediaName
	viewMedia.Modify updateMode, recordMedia : CheckError
	WScript.Echo "New table row  = {" & recordMedia.StringData(1) & ", " & recordMedia.StringData(2) & ", " & recordMedia.StringData(3) & ", " & recordMedia.StringData(4) & ", " & recordMedia.StringData(5) & ", " & recordMedia.StringData(6) & " } "

	database.Commit : CheckError
	Set viewMedia = Nothing
	Set view = Nothing
...
Sub CheckError
	...
	Set message = Err.Source & " " & Hex(Err) & ": " & Err.Description
