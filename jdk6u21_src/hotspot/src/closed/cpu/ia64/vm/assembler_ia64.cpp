...
address Assembler::start_a_stub(int required_space) {
  assert(!code()->needs_flush_bundle(), "stub in mid-bundle?");
  assert(code()->current_section() == code()->insts(), "sanity");
  code()->set_current_section(code()->stubs());
  return AbstractAssembler::start_a_stub(required_space);
}
void Assembler::end_a_stub() {
  assert(!code()->needs_flush_bundle(), "stub end in mid-bundle?");
  assert(code()->current_section() == code()->stubs(), "sanity");
  code()->set_current_section(code()->insts());
  AbstractAssembler::end_a_stub();
}
address Assembler::start_handler(int required_space) {
  assert(!code()->needs_flush_bundle(), "handler in mid-bundle?");
  assert(code()->current_section() == code()->insts(), "sanity");
  code()->set_current_section(code()->handler());
  return AbstractAssembler::start_handler(required_space);
}
void Assembler::end_handler() {
  assert(!code()->needs_flush_bundle(), "handler end in mid-bundle?");
  assert(code()->current_section() == code()->handler(), "sanity");
  code()->set_current_section(code()->insts());
  AbstractAssembler::end_handler();
}
address Assembler::start_a_const(int required_space, int required_align) {
  ShouldNotReachHere();
  //return AbstractAssembler::start_a_const(required_space, required_align);
  return NULL;
}
void Assembler::end_a_const() {
  ShouldNotReachHere();
  //AbstractAssembler::end_a_const(required_space);
}


// This is called after every call to code->emit() to resynch the pc.
void Assembler::set_pc_from_code() {
  assert(code()->current_section() == code_section(), "sanity");
  _code_pos = code_section()->end();
}
...
int CodeBuffer::encoded_target(Label *L) {
  // If L->unused(), L->loc_pos() is undefined.

  // If the label is bound, then just return the 25 bit displacement
  // (Compare with CodeSection::target.)
  if ( L->is_bound() ) {
    assert(L->loc_sect() == SECT_INSTS, "ia64 uses only inst. section");
    return (L->loc_pos() - code_size());
  } else {
    int current_pos = code_size() | code_slot();
    L->add_patch_at(this, locator(current_pos, SECT_INSTS));
    return current_pos;
  }
  return 0;
}
...
void MacroAssembler::serialize_memory(Register thread,
                                      ...
                                      Register tmp2)
{
  mov(tmp1, os::vm_page_size() - sizeof(int));
  shru(tmp2, thread, os::get_serialize_page_shift_count());
  and3(tmp2, tmp1, tmp2);
  mova(tmp1, os::get_memory_serialize_page());
  add(tmp1, tmp2, tmp1);
  st4(tmp1, GR0, Assembler::ordered_release);
}
...
void MacroAssembler::return_from_dummy_frame() {
  ...
  if (TraceJumps)
    adds(return_addr, sizeof(IPF_Bundle)*5, return_addr);
  else {
...
void CodeBuffer::BundleInst::set_label(Label *label) {
  if (label->is_bound()) {
    _bound_label.init();
    _bound_label.bind_loc(label->loc());
    label = &_bound_label;
  }
  _label = label;
  _has_label = true;
...
void CodeBuffer::set_current_section(CodeSection* cs) {
  assert(cs != NULL && cs->outer() == this, "sanity");
  assert(cs != current_section(), "don't bore me");
  _current_section = cs;
}
...
void CodeBuffer::flush_bundle(bool start_new_bundle) {
  ...
  IPF_Bundle* bundles = (IPF_Bundle*)current_section()->end();
  ...
  for ( uint x = 0; x < _inst_count; x++ ) {
    ...
    set_code_slot(slot);
    current_section()->set_end( (address)(&bundles[bundle]) );
    ...
    if( element.has_reloc() ) {
      if( element.reloc() == relocInfo::oop_type ) {
        ...
        current_section()->relocate(current_section()->end(), oop_Relocation::spec(oop_index), code_slot());
      } else {
         current_section()->relocate(current_section()->end(), element.reloc(), code_slot());
      }
    }
  ...
  current_section()->set_end( (address)(&bundles[solution.length()]) );
...
#ifdef ASSERT
bool CodeBuffer::needs_flush_bundle() const {
  if (_inst_count != 0)  return true;
  // We don't want a partial bundle either.
  return (_previous_bundle_format != IPF_Bundle::none);
}
#endif //ASSERT
