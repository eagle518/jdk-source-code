...
void InterpreterMacroAssembler::push_tag(Register tos, frame::Tag tag) {
  Register scratch = GR29_SCRATCH;  // seems good to me
  if (TaggedStackInterpreter)  {
     assert(scratch != tos, "scratch isn't");
     mov(scratch, (intptr_t) tag);
     st8(tos, scratch, -BytesPerWord);
  }
}
