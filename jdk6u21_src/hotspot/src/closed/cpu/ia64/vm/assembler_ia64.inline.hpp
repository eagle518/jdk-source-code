...
inline void MacroAssembler::pd_patch_instruction(address branch, address target) {
  intptr_t offset = (intptr_t) branch;
  intptr_t dest_offset = (intptr_t) target;
  intptr_t inst_offset = offset & ~3;
  IPF_Bundle* bundle = (IPF_Bundle*)inst_offset;
  uint slot = offset & 3;
  uint41_t inst = bundle->get_slot(slot);
  Inst::ImmedClass dclass =
    Assembler::get_immed_class(bundle->get_template(), slot, inst);

  uint41_t new_inst;
  Assembler::patched_branch(dest_offset, dclass, inst, inst_offset, new_inst);

  bundle->set_slot(new_inst, slot);
}
