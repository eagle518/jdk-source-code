...
void AwtFont::_Dispose(void *param) {
    JNIEnv *env = (JNIEnv *)JNU_GetEnv(jvm, JNI_VERSION_1_2);

    jobject peer = (jobject)param;
    if (peer != NULL) {
        PDATA pData = (PDATA)env->GetLongField(peer, AwtFont::pDataID);
        if (pData != NULL) {
            AwtFont *f = (AwtFont *)pData;
            if (f != NULL) {
                delete f;
            }
            env->SetLongField(peer, AwtFont::pDataID, ptr_to_jlong(NULL));
        }
...
Java_java_awt_Font_pDispose(JNIEnv *env, jobject self) 
{
    ...
    AwtToolkit::GetInstance().SyncCall(AwtFont::_Dispose, selfGlobalRef);
    // selfGlobalRef is deleted in AwtFont::_Dispose
