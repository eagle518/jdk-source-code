...
// param should be a global ref to peer object
void AwtObject::_Dispose(void *param) {
    jobject selfGlobalRef = (jobject)param;
    if (selfGlobalRef != NULL) {
        ...
        // selfGlobalRef is deleted in AwtToolkit::WndProc()
    }
}

void AwtObject::_Dispose() {
    JNIEnv *env = (JNIEnv *)JNU_GetEnv(jvm, JNI_VERSION_1_2);
...
Java_sun_awt_windows_WObjectPeer_initIDs(JNIEnv *env, jclass cls) {
    ...
    AwtObject::targetID = env->GetFieldID(cls, "target", "Ljava/lang/Object;");
