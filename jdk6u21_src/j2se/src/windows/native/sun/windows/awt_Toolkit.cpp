/*
 * @(#)awt_Toolkit.cpp	1.220 10/05/18
 *
  ...
  if( ERROR_SUCCESS == ::RegOpenKeyEx(
    ...
    if( ERROR_SUCCESS == ::RegQueryValueEx(
      ...
      if (_tcsstr(nBuffer, TEXT("%SYSTEMROOT%"))) {
        ...
        LPTSTR systemRoot = _tgetenv(TEXT("SYSTEMROOT"));
        if (systemRoot == NULL) {
            ::RegCloseKey(regKey);
            return;
        }
        _tcscpy(nBufferCorrect, systemRoot);
...
void AwtToolkit::SetEnv(JNIEnv *env) {
    if (m_env != NULL) { // If already cached (by means of embeddedInit() call).
        return;
    }
...
void AwtToolkit::GetWindowRect(HWND hWnd, LPRECT lpRect)
{
    ...
    const DWORD AWT_DWMWA_EXTENDED_FRAME_BOUNDS = 9;
...
/*
 * Class:     sun_awt_windows_WToolkit
 * Method:    isWindowOpacityControlSupported
 * Signature: ()Z
 */
extern "C" JNIEXPORT jboolean JNICALL
Java_sun_awt_windows_WToolkit_isWindowOpacityControlSupported(JNIEnv *env, 
        jobject self) 
{
    TRY;

    return IS_WIN2000 ? JNI_TRUE : JNI_FALSE;
 ...
 */
extern "C" JNIEXPORT jboolean JNICALL
Java_sun_awt_windows_WToolkit_isWindowTranslucencySupported(JNIEnv *env, 
        jobject self) 
{
    TRY;
