/*
 * @(#)HAE_API_WinOS_Capture.c	1.30 10/03/23
 *
...
INT32 HAE_MaxCaptureDevices(void) {
     ...
     */
    UINT32 encodings, sampleRates, channels, bits;
    int formatCount;
    UINT32 num = waveInGetNumDevs();
    if (num > 0) {
        num++;  // add WAVE_MAPPER
        /* $$am 2006-11-30: fix for 6495567: CR 4964288 (sound, Unexpected IAE
         * raised while getting TargetData) is not fixed in windows-amd64 JDK
         * with Remote Desktop input devices are not accessible,
         * but waveInGetNumDevs reports actual number of devices
         * Detect the case trying to get supported formats
         */
         formatCount = HAE_GetSupportedCaptureFormats(0,    // WAVE_MAPPER
                                &encodings, &sampleRates, &channels, &bits, 1);
         if (formatCount == 0) {
             num = 0;
         }
    }
