			...
			/* OA-relative, tileSize-scaled coordinates */
{
	...
	do {
	    ...
	    /* Make sure that tileloxi and tileloyi don't run out of the buffer 
	     * bounds. (Note: outWTi is a valid index, but not outHTi) 
	     * This is optimized form for the following check:
	     * 
	     * tileloxi >= 0 && tileloxi <= p->outWTi &&
	     * tileloyi >= 0 && tileloyi < p->outHTi
	     * 
	     * See bug 5089985 for more info. 
	     */
	    if (((u32) tileloxi) <= p->outWTi && ((u32) tileloyi) < p->outHTi) {
