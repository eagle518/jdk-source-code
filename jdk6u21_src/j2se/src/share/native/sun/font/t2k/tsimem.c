 ...
 * tt_uint32:	MAGIC3
 * tt_uint32:	n
 * [ On WIN64 the header has two additional unused tt_uint32's
 *   for padding to 16 byte alignment, as some structs require this. ]
 ...
 */
#ifdef _WIN64
#define headerSize (4*sizeof(tt_uint32))
#else
#define headerSize (sizeof(tt_uint32)+sizeof(tt_uint32))
#endif
#define tailSize (sizeof(char) + sizeof(char))

/* Handy macro to check for potential overflow in alloc array params.
   size_t is supposed to be unsigned but we do check value anyway.
   (to be safe if it is not true).
   IMPORTANT: zero size allocation requests are allowed. */
#define SAFE_TO_ALLOC(c, sz) \
    (((c) == 0) || ((sz) == 0) || \
      (((sz) > 0) && ((c) > 0) && (0xffffffffu / ((tt_uint32)(c))) > (sz)))
