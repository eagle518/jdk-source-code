...
static tt_uint8 *CreateBitMap( tsiMemObject *mem, InputStream *in, int width, int height, int bitDepth, char greyScaleLevel, int byteAligned, tt_int32 *rowBytesPtr )
{
    ...
    baseAddr = row = (unsigned char*) tsi_AllocArray(mem, rowBytes, height);
...
static void ScaleBits( tsiMemObject *mem, sbitGlypInfoData *bits, tt_uint8 greyScaleLevel )
{
	...
	for ( n = 0; n < 2; n++ ) {
		if ( yOrder == n ) {
			/* Y scale */
            baseAddr2 = (tt_uint8 *) tsi_AllocArray(mem, height2, bits->rowBytes);
            ...
            baseAddr2 = (tt_uint8 *) tsi_AllocArray(mem, rowBytes2, height2);
...
readNumComponents:
        ...
        baseAddr = (unsigned char*) tsi_AllocArray(t->mem, rowBytes, gInfo->bigM.height);
