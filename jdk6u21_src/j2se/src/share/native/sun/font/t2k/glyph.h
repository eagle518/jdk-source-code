...
typedef struct {
   short   *hstems;         /* unscaled stems (as we read from font) */
   short   maxOOY, minOOY;  /* max/min y (unscaled!) coordinates among points covered by this frame */
   F26Dot6 maxY, minY;      /* max/min y in scaled and hinted version of glyph */
   char    isFixedTop, isFixedBottom; /* if frame range overlaps with allignment zones? */
   F26Dot6 *scaledHstems;   /* stem representation in the scaled space */
   char    *fixedHstems;    /* flags to mark stems overlapping with alignment zones */
   short hstemsCount;
   short hstemsCountMax;

   short   *vstems;         /* unscaled stems (as we read from font) */
   F26Dot6 *scaledVstems;   /* stem representation in the scaled space */
   short vstemsCount;
   short vstemsCountMax;

   /* frame is associated with points from particular interval (of indices in the point array)
      - starting from given point and until start of interval for next frame.
      Note that value of firstPoint refer to index of the point in the 
      glyph definition in the type1 font. Type1 contours are flipped 
      and therefore they are renumbered in oox/ooy arrays.
      It is necessary to restore original index when frame is selected. */
...
typedef struct {
	...
	short 	pointCount;		/* number of points in the characters + 0 
	                                   for the sidebearing points.
	...
	short  stemFramesCount;
	short  stemFramesCountMax;
} GlyphClass;
