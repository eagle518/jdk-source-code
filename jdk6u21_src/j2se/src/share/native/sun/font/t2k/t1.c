...
/* Caclulates min/max y values for all stem frames.
   NB: This routine is supposed to be called before with flip contours
       (i.e. point indices in the array and stem frames need no conversion) */
static void PrepareForHinting(GlyphClass *glyph) {
    int i, pt, endPt;

    /* ensure we have at least one frame */
    glyph_AddDummyStemFrame(glyph);

    for (i=0; i<glyph->stemFramesCount; i++) {
        pt = glyph->stemFrames[i].firstPointIndex;
        if (pt < glyph->pointCount) {
            glyph->stemFrames[i].minOOY = glyph->ooy[pt];
            glyph->stemFrames[i].maxOOY = glyph->ooy[pt];

            if (i < glyph->stemFramesCount-1) { 
                endPt = glyph->stemFrames[i+1].firstPointIndex;
            } else {
                endPt = glyph->pointCount;
            }
            pt++; /* skip firstPoint */
            while (pt < endPt && pt < glyph->pointCount) {
                if (glyph->stemFrames[i].minOOY > glyph->ooy[pt]) {
                    glyph->stemFrames[i].minOOY = glyph->ooy[pt];
                } else if (glyph->stemFrames[i].maxOOY < glyph->ooy[pt]) {
                    glyph->stemFrames[i].maxOOY = glyph->ooy[pt];
                }
                pt++;
            }
...
/* This function extracts segment content from PFB representation of Type1 font
   and merges them into single array.
...
unsigned char *ExtractPureT1FromPCType1( unsigned char *src, tt_uint32 *length )
{
    ...
    while (((tt_uint32)(src-base) + 6) <= *length) {
        ...
        if (b1 != 0x80 || b2 < 0x01 || b2 > 0x03) { /* malformed pfb */
            return NULL;
        }
...
/* 
   ...
   See http://www.unicode.org/Public/MAPPINGS/VENDORS/ADOBE/stdenc.txt
         for canonical version of mapping */
...
static tt_uint16 adobeCharCodeMapping[128] = {
  ...
  /*0xa0*/  NOTDEF, 0x00a1, 0x00a2, 0x00a3, 0x2044, 0x00a5, 0x0192, 0x00a7,
  /*0xa8*/  0x00a8, 0x0027, 0x201c, 0x00ab, 0x2039, 0x203a, 0xfb01, 0xfb02,
  /*0xb0*/  NOTDEF, 0x2013, 0x2020, 0x2021, 0x00b7, NOTDEF, 0x00b6, 0x2022,
  /*0xb8*/  0x201a, 0x201e, 0x201d, 0x00bb, 0x2026, 0x2030, NOTDEF, 0x00bf,
  /*0xc0*/  NOTDEF, 0x0060, 0x00b4, 0x02c6, 0x02dc, 0x00af, 0x02d8, 0x02d9,
  /*0xc8*/  0x00a8, NOTDEF, 0x02da, 0x00b8, NOTDEF, 0x02dd, 0x02db, 0x02c7,
  /*0xd0*/  0x2014, NOTDEF, NOTDEF, NOTDEF, NOTDEF, NOTDEF, NOTDEF, NOTDEF,
  /*0xd8*/  NOTDEF, NOTDEF, NOTDEF, NOTDEF, NOTDEF, NOTDEF, NOTDEF, NOTDEF,
  /*0xe0*/  NOTDEF, 0x00c6, NOTDEF, 0x00aa, NOTDEF, NOTDEF, NOTDEF, NOTDEF,
  /*0xe8*/  0x0141, 0x00d8, 0x0152, 0x00ba, NOTDEF, NOTDEF, NOTDEF, NOTDEF,
  /*0xf0*/  NOTDEF, 0x00e6, NOTDEF, NOTDEF, NOTDEF, 0x0131, NOTDEF, NOTDEF,
  /*0xf8*/  0x0142, 0x00f8, 0x0153, 0x00df, NOTDEF, NOTDEF, NOTDEF, NOTDEF
};
...
static tt_uint16 tsi_T1GetGlyphIndexFromAdobeCode( T1Class *t,
    ...
    tt_uint16 code;
    if (adobeCharCode >= 0x20 && adobeCharCode <= 127) {
        /* from space to ~ codes are the same except few cases */
        if (adobeCharCode == 0x27)  {  /* quoteright */ 
            code = 0x2019;
        } if (adobeCharCode == 0x60){  /* quoteleft */
            code = 0x2018;
        } else { 
      code = adobeCharCode;
        }
        ...
        if ( v1 < 32 ) {
            ...
            switch( v1 ) {
              ...
              case  1:	/* hstem */
                ...
                glyph_AddHStem(t->glyph, 
                               t->gStackValues[STACK_BOTTOM+0]+t->lsby,
                               t->gStackValues[STACK_BOTTOM+1]);
              ...
              case  3:	/* vstem */
                ...
                if ( t->gNumStackValues < 2 ) break;
                glyph_AddVStem(t->glyph, 
                               t->gStackValues[STACK_BOTTOM+0]+t->lsbx, 
                               t->gStackValues[STACK_BOTTOM+1]);
                t->gNumStackValues = 0; /* clear the stack */
                break;
              case  4:	/* vmoveto */
 ...
 * returns numDimension
 */
static int tsi_T1GetShortArray(T1Class *t, const tt_uint8 *param, int maxDim, short *arr)
{
    register tt_uint8 *start, *p;
    tt_int32 count, limit, numDimension = 0;
    ...
    /* /BlueValues [ -25 0 563 588 430 455 606 631 ] |-
       /OtherBlues [ 234 243 330 330 128 153 ] |-         
       /BlueValues [-20 0 547 567 739 759] def
    */

    if ((p = start) != NULL ) {
        count = 0; limit = 32;
        for ( ; *p != '[' && count < limit; p++, count++) {} 
        p++;  /* skip the '[' */

        limit = 512;
        for (numDimension=0; numDimension < maxDim;) {
            for(;*p == ' ' && *p != ']' && count < limit; p++, count++) {} /* skip spaces */
            if( *p == ']') {
                break; /***** We are Done *****/
            }
            arr[numDimension++] = ATOI(p);
            for ( ; *p != ' ' && *p != ']' && count < limit; p++, count++) {} /* skip the number */
        }
    }
    return numDimension; 
}
...
static void BuildCMAP( T1Class *t, int *errCode)
{
...
static void ReadGlobalHints(T1Class *t) 
{
    t->blueValuesCnt = tsi_T1GetShortArray(t, (unsigned char*)"/BlueValues ", MAX_BLUE_VALUES_CNT, t->blueValues);
    t->otherBluesCnt = tsi_T1GetShortArray(t, (unsigned char*)"/OtherBlues ", MAX_BLUE_VALUES_CNT, t->otherBlues);   

    //copy baseline range to the end of other values
    if (t->blueValuesCnt > 0) {
      t->otherBlues[t->otherBluesCnt]=t->blueValues[0];
      t->otherBlues[t->otherBluesCnt+1]=t->blueValues[1];
      t->otherBluesCnt+=2;
    }

    /* TODO: We should look for value in in the font file */
    t->blueFuzz = 1; /* 1 is default value according to spec */

    /* these are not supported yet */
#if 0
    t->stdHW = 0;
    t->stdVW = 0;
    cnt = tsi_T1GetShortArray(t, (unsigned char*)"/StdHW ", 1, &t->stdHW);
    cnt = tsi_T1GetShortArray(t, (unsigned char*)"/StdVW ", 1, &t->stdVW);

    t->stemSnapHCnt = tsi_T1GetShortArray(t, (unsigned char*)"/StemSnapH ", MAX_STEM_SNAP_CNT, t->stemSnapH);
    t->stemSnapVCnt = tsi_T1GetShortArray(t, (unsigned char*)"/StemSnapV ", MAX_STEM_SNAP_CNT, t->stemSnapV);
#endif
    ...
    int i;
    printf("Blue values: ");
    for(i=0; i<t->blueValuesCnt; i++) {
        printf("%d(%d) ", t->blueValues[i], t->blueValues[i]>>6);
    }
    printf("\n");

    printf("Other blues: ");
    for(i=0; i<t->otherBluesCnt; i++) {
        printf("%d(%d) ", t->otherBlues[i], t->otherBlues[i]>>6);
    }
    printf("\n");
    }
#endif
...
void tsi_DeleteT1Class( T1Class *t )
{
    if ( t != NULL ) {
        ...
        if ( t->decryptedData != t->dataInPtr && t->decryptedData != NULL) {
        ...
        if (t->unicodeToGITable != NULL) {
          for (i = 0; i < MAPPING_HASHTABLE_SIZE; i++) {
            if (t->unicodeToGITable[i] != NULL) {
