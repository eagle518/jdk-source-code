...
/* get the individual records */
			...
			Index < ProfileSeqDesc->Count; Index++)
	{
		...
		if ((SpTypeMultiLanguage == Record->DeviceManufacturerDesc.TagType) ||
			...
			Record = (SpProfileSeqDescRecord2_t   KPHUGE *)
...
#define IS_VALID_SIGNATURE_CHAR(c) \
    ((0x30 <= ((unsigned char)(c)) && ((unsigned char)(c)) <= 0x39) ||    \
    (0x41 <= ((unsigned char)(c)) && ((unsigned char)(c)) <= 0x5a) ||     \
...
/* do the TagType specific conversions */
	...
	case Sp_AT_ColorantOrder:
		...
		Value->Data.ColorOrder.Count = SpGetUInt32 (&Buf);
                if (Value->Data.ColorOrder.Count > SpMaxColorCount) {
                    Value->Data.ColorOrder.Count = SpMaxColorCount;
                }
