...
KpUInt32_t size;
    ...
    if (theCallerId == NULL) {
        ...
        theStatus = SpProfileCheckSize(dataP, size);

        if (theStatus == SpStatSuccess) {
            theStatus = SpProfileLoadFromBuffer (theCallerId, dataP, 
                                                 &theSProfile.pf);
        } 
 ...
 * Signature: ([Lsun/awt/color/ICC_Transform;Lsun/awt/color/ICC_Transform;)I
 * static native int cmmCombineTransforms (ICC_Transform[] transforms, ICC_Transform result);
 */
JNIEXPORT jint JNICALL 
    Java_sun_awt_color_CMM_cmmCombineTransforms (JNIEnv *env, jobject obj, jobjectArray transforms, jobject result) 
{
SpStatus_t    theStatus = SpStatSuccess;
SpCallerId_t    theCallerId;
storeID_t    theXform;
...
jfieldID    fid;
    ...
    if (transforms == NULL) {
        JNU_ThrowNullPointerException(env, "transforms array is null");
    ...
    if (theCallerId == NULL) {
        ...
        for (i1 = 0; i1 < nXforms; i1++) {
            jobject trans = (jobject) (*env)->GetObjectArrayElement(
                env, transforms, i1);
            if (trans == NULL) {
                JNU_ThrowNullPointerException(env, "transforms array contains "
                                                   "null element");
            ...
            /* get tranform IDs */
            theStatus = getObjectID(env, trans, xformIDs + i1);
            if (theStatus != SpStatSuccess) {
                break;
            }
        ...
        if (theStatus == SpStatSuccess) {
            theStatus = SpConnectSequence (nXforms, xformIDs, &theXform.xf,
                                           &FailingXform, NULL, NULL);
        }
    ...
    if (theStatus == SpStatSuccess) {
        /* set ID in the transform object */
        setObjectID (env, result, theXform); 
    }
...
/* get value of the ID field from object */
static SpStatus_t getObjectID (JNIEnv *env, jobject theObject, SpXform_p theID)
{
jclass        cls;
jfieldID    fid;
storeID_t res;
    ...
    /* We are using res union for conversion from jlong to SpXform_t.
     * This is common for all CMM code, so replacing the code below with 
     * casting will cause problems on CPUs with big endian architecture. 
     */ 
    res.j = (*env)->GetLongField (env, theObject, fid);
    *theID = res.xf;
    return SpStatSuccess;
}
