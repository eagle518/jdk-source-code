/*
 * @(#)jpegimagedecoderimpl.c	1.22 10/03/23
 *
   ...
   jboolean	colCvt) 
{
  ...
  // get fields required for allocation of destination buffer.
  ...
  // check whether required destination buffer can be allocated.
  num_components = unpack ? 4 : cinfo.num_components;

  if (!SAFE_TO_ALLOC(cinfo.image_width, num_components)) {
    JNU_ThrowByName( env, IMAGE_FORMAT_EXCEPTION, "Scanline is too long.");
    error_exit((j_common_ptr)&cinfo);
  }
  lineStride = cinfo.image_width * num_components;

  if (!SAFE_TO_ALLOC(cinfo.image_height, lineStride)) {
    JNU_ThrowByName( env, IMAGE_FORMAT_EXCEPTION, "Invalid buffer size.");
    error_exit((j_common_ptr)&cinfo);
  }
  ...
  /* in case of the multi-scan JPEG image the SOS markers can appear
   * during the decompression, so the info from these merkers should
   * be copied to the java JPEGParam object.
   */
  if (jpeg_has_multiple_scans(&cinfo)) {
      CopyCompInfoToJava(&jpp_info);
  }

  scanLinePtr = (JSAMPROW)malloc(lineStride);
  if (scanLinePtr == NULL) {
   ...
   */ 
	...
	if (cinfo.num_components == 3)
	  ...
	  unpackBuffer4(ptr, scanLinePtr, cinfo.image_width, flip);
          ...
          memcpy(ptr, scanLinePtr, lineStride);
