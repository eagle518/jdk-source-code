...
CopyCompInfoFromJava(jpeg_param *jpp)
{
  ...
  if (numComp > MAX_COMPONENTS) {
    numComp = MAX_COMPONENTS;
  }
...
CopyQTablesFromJava( jpeg_param *jpp, int tableNum)
{
  ...
  // Copy over the
  arraylength = (*(jpp->env))->GetArrayLength(jpp->env, qTable);
  if (arraylength > DCTSIZE2) {
      arraylength = DCTSIZE2;
  }
  body = (*(jpp->env))->GetIntArrayElements(jpp->env, qTable, 0);
  if ( jpp->is_decompressor ) {
    UINT16     *ptr;
