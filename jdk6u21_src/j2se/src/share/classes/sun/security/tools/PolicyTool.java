...
public class PolicyTool {
    ...
    */
	...
	if (numAliases > 0) {
	    ...
	    while (enum_.hasMoreElements()) {
		aliases[numAliases] = new String((String)enum_.nextElement());
		numAliases++;
	    }
	}
	return aliases;
...
class ToolDialog extends Dialog {
	...
	for (int i = 0; i < prinList.getItemCount(); i++) {
	    prins.add(prinList.getObject(i));
	}
	ge.principals = prins;
	...
	for (int i = 0; i < permList.getItemCount(); i++) {
            perms.addElement(permList.getObject(i));
	}
