/*
 * @(#)SignatureFileVerifier.java	1.29 10/03/23
 *
...
public class SignatureFileVerifier {
     ...
     *
     */
    public void process(Hashtable signers, List manifestDigests)
    ...
    private void processImpl(Hashtable signers, List manifestDigests)
	throws IOException, SignatureException, NoSuchAlgorithmException,
	    JarException, CertificateException
    {
	...
	for (int i = 0; i < infos.length; i++) {
		...
		debug.println("Signature Block Certificate: " +
		    (X509Certificate)chain.get(0));
	    }
	}

	if (signers != null) {
	    return 
		(CodeSigner[])signers.toArray(new CodeSigner[signers.size()]);
	} else {
	    return null;
	}
    }
    ...
    void updateSigners(CodeSigner[] newSigners, Hashtable signers, String name)
    {
	CodeSigner[] oldSigners = (CodeSigner[])signers.get(name);
	...
	for (int i = signerCache.size() - 1; i != -1; i--) {
	    cachedSigners = (CodeSigner[]) signerCache.get(i);
