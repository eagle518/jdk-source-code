/*
 * @(#)FontDesignMetrics.java	1.67 10/03/23
 *
...
public final class FontDesignMetrics extends FontMetrics {
	 ...
	 * synchronize to make sure this no one comes in between the
	 * get and the remove which is unlikely anyway.
	 */
	public void dispose() {
	...
	/* Synchronize access to this on the metrics cache */
	static final MetricsKey key = new MetricsKey();
    }

    /* All accesses to this must be synchronized. It should also
     * be used to synchronize access to the shared MetricsKey.key
     * instance
     */
    private static final HashMap<Object, KeyReference>
	metricsCache = new HashMap<Object, KeyReference>();
	...
	if (m == null) {
	     ...
	     * But we do need to synchronize the put() on the metricscache.
	     */
