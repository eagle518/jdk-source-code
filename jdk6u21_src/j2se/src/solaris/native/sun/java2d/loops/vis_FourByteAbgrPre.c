/*
 * @(#)vis_FourByteAbgrPre.c	1.5 10/03/23
 *
					      ...
					      CompositeInfo * pCompInfo)
{
	...
	for (j = 0; j < height; j++) {
	    ...
	    mlib_u8  *dst8;
            ...
            /* Ensure the data is int aligned, and convert all the
             * components to non-premultiplied form. Note there is no
             * conversion from 4ByteABGR to IntARGB, since instead we have
             * swapped the components in the fg color (srcG_f) so they
             * can operate on data in the ABGR organisation.
             */
            if (((mlib_s32)dstBase | scan) & 3) {
