	...
	(aData->awt_depth >= 15)) 
    {
        ...
        if (!aData->isTranslucencySupported) {
            ...
            if (aData->renderPictFormat.direct.red == 16) {
                awt_colormodel = JNU_CallStaticMethodByName(
                    env, &hasException, 
                    "sun/awt/X11GraphicsConfig", "createDCM32",
                ...
                awt_colormodel = JNU_CallStaticMethodByName(
                    env, &hasException, 
                    "sun/awt/X11GraphicsConfig", "createABGRCCM", 
                    "()Ljava/awt/image/ComponentColorModel;").l;
        ...
        cspace = JNU_CallStaticMethodByName(
            env, &hasException, 
            "java/awt/color/ColorSpace", "getInstance",
            "(I)Ljava/awt/color/ColorSpace;", 
            java_awt_color_ColorSpace_CS_GRAY).l;

        if (hasException || cspace == NULL) {
            (*env)->PopLocalFrame(env, 0);
            return NULL;
        }
