...
#ifndef XSUN_KP_BEHAVIOR
#define  XSUN_KP_BEHAVIOR  1
#define  XORG_KP_BEHAVIOR  2
#endif
...
isXsunKPBehavior(XEvent *event) {
    ...
    int kc7;
    if( !awt_IsXsunKPBehavior ) {
        kc7 = XKeysymToKeycode(event->xkey.display, XK_KP_7);
        if( !kc7 ) {
            // keycode is not defined. Why, it's a reduced keyboard perhaps:
            // report arbitrarily false.
            awt_IsXsunKPBehavior = XORG_KP_BEHAVIOR;
        } else {
            long ks2 = XKeycodeToKeysym(event->xkey.display, kc7, 2);
            if( ks2 == XK_KP_7 ) {
    ...
    return awt_IsXsunKPBehavior == XORG_KP_BEHAVIOR ? False : True;
}    
static Boolean
isKPevent(XEvent *event)
{
    ...
    Boolean bsun = isXsunKPBehavior( event );
...
handleKeyEventWithNumLockMask_New(XEvent *event, KeySym *keysym) 
{
    ...
    if( isXsunKPBehavior( event ) && !awt_UseXKB ) {
  ...
  Time when, jint keycode, jchar keychar, jint modifiers, jint keyLocation, XEvent *anEvent)
{
    ...
    if(anEvent != NULL && (id == java_awt_event_KeyEvent_KEY_PRESSED || id == java_awt_event_KeyEvent_KEY_RELEASED) ){
        (*env)->SetLongField(env, hEvent, keyEventIDs.rawCode, (jlong)(anEvent->xkey.keycode));
    }
