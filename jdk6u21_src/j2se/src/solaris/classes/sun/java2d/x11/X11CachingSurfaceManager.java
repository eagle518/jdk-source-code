/*
 * @(#)X11CachingSurfaceManager.java	1.32 10/03/23
 *
    ...
    implements DisposerTarget
{
	...
	if (gc instanceof GLXGraphicsConfig) {
	    return createGLXSurface(gc, width, height);
	} else {
	    return createX11Surface(gc, width, height);
	}
    }
                                           ...
                                           int width, int height)
    {
        return GLXSurfaceData.createData((GLXGraphicsConfig)gc,
                                         width, height,
                                         gc.getColorModel(transparency), bImg,
