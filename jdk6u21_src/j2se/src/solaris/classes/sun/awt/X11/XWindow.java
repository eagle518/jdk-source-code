...
//       DO NOT INVOKE CLIENT CODE ON THIS THREAD!
	...
	/* 
        ...
        */
         ...
         */
        if ((dragging && clickCount == 0) || !dragging){ 
            MouseEvent mme = new MouseEvent(source, type, jWhen,
...
//  }
    ...
    */
    ...
    static private long xkeycodeToPrimaryKeysym(XKeyEvent ev) {
        return XlibWrapper.XKeycodeToKeysym(XToolkit.getDisplay(), ev.get_keycode(), 0);
    }
    ...
    void handleKeyPress(XKeyEvent ev) {
        ...
        if ( //TODO check if there's an active input method instance
            ...
            // Process additional modifiers here.
            // I know the only useful application of this: password fields with IM
            // disabled where some characters (like @ in German layouts) are available
            // with AltGr only. 
            if( keysym[0] < 256 ) {
                keysym[0] = XlibWrapper.XLookupStringShort(ev.pData);
            }
    ...
    final void handleKeyRelease(XKeyEvent ev) { 
        int keycode = java.awt.event.KeyEvent.VK_UNDEFINED;
        long keysym[] = new long[2];
        int unicodeKey = 0;
        keysym[0] = NoSymbol;
