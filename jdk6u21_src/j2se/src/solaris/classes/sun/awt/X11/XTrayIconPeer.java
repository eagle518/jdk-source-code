/*
 * @(#)XTrayIconPeer.java	1.28 10/03/23
...
public class XTrayIconPeer implements TrayIconPeer {
    ...
    static class TrayIconEventProxy implements MouseListener, MouseMotionListener {
        ...
        public void mousePressed(MouseEvent e) {
              // This is currently triggered from TrayIcon.procerssMouseEvent
              // for consistency with the behaviour on Windows.
//            if (e.isPopupTrigger()) {
...
//            }
    ...
    public static class Tooltip extends InfoWindow {
        ...
        public Tooltip(XTrayIconPeer xtiPeer, Frame parent) {
            this(parent);
            this.xtiPeer = xtiPeer;
        }

        public Tooltip(XWindow xw, Frame parent, String tooltipString) {
            this(parent);
            this.xw = xw;
        ...
        private Object getTargetObject() {
            if (xtiPeer != null) {
                return xtiPeer.target;
            } else {
                return xw.getTarget();
            }
        }

        private boolean isTargetDisposed() {
            if (xtiPeer != null) {
                return xtiPeer.isDisposed();
            } else {
                return xw.isDisposed();
            }
        }

        private Rectangle getTargetBounds() {
            if (xtiPeer != null) {
                return xtiPeer.getBounds();
            } else {
                return xw.getBounds();
            }
        }
        ...
        void display() {
            ...
            SunToolkit.executeOnEventHandlerThread(getTargetObject(), new Runnable() {
                    public void run() {
                        if (isTargetDisposed()) {
                            return;
                        }
                        Point pointer = (Point)AccessController.doPrivileged(new PrivilegedAction() {
                                public Object run() {
                                    if (!isPointerOverTrayIcon(getTargetBounds())) {
                                        return null;
                                    }
