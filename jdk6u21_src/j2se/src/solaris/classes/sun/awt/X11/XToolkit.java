 ...
 */
    ...
    void processGlobalMotionEvent(XEvent e) {
  ...
  /*
    ...
    */
    ...
    static Set<Long> getChildren(long window) {
        XBaseWindow bw = windowToXWindow(window);
        if (bw != null) {
    ...
    static boolean isXAWTToplevelWindow(long window) {
        return windowToXWindow(window) instanceof XWindowPeer;
    }

    // NOTICE:  Right now returns only decorated top-levels (not Window)
    static boolean isToplevelWindow(long window) {
        if (windowToXWindow(window) instanceof XDecoratedPeer) return true;
    ...
    // Treats XEmbeddedFramePeer as non-toplevel.
    // In accord with: XWindowPeer XBaseWindow.getToplevelXWindow().
    static boolean isTrueToplevelWindow(long window) {
        XBaseWindow bw = windowToXWindow(window);

        if (bw instanceof XWindowPeer && !(bw instanceof XEmbeddedFramePeer)) return true;
    ...
    static long getParentWindow(long window) {
        XBaseWindow bw = windowToXWindow(window);
        if (bw != null && bw.getParentWindow() != null) {
            return bw.getParentWindow().getWindow();
        }
    ...
    static boolean isMapped(long window) {
        awtLock();
        try {
            ...
            try {
                ...
                return !(wattr.get_map_state() == IsUnmapped);
            } finally {
                wattr.dispose();
            }        
        } finally {
    ...
    */
    static boolean isXKBenabled() {
        ...
        try {
            ...
            awt_UseXKB = XlibWrapper.XQueryExtension( display, name, XlibWrapper.larg1, XlibWrapper.larg2, XlibWrapper.larg3);
            return awt_UseXKB;
        } finally {
            awtUnlock();
        }
    ...
    public boolean isWindowTranslucencySupported() {
        //NOTE: it may be not supported. The actual check is being performed
    ...
    public boolean isTranslucencyCapable(GraphicsConfiguration gc) {
        ...
        AwtGraphicsConfigData graphicsConfigData =
            new AwtGraphicsConfigData(((X11GraphicsConfig)gc).getAData());
        return 0 != graphicsConfigData.get_isTranslucencySupported();
    }
