/*
 * @(#)XWindowPeer.java	1.138 10/03/23
 *
...
/* --- DisplayChangedListener Stuff --- */
    ...
    /**
     * Called to update our GraphicsConfig when dragged onto another screen
     * (in the Xinerama case), or after changing the display mode.
     */
    public void displayChanged(int screenNum) {
        // update our GC
        resetLocalGC(screenNum);         /* upcall to XCanvasPeer */
    ...
    public final XWindowPeer requestXFocusGently(long time) {
...
*/
    ...
    */
    ...
    public void handleMapNotifyEvent(XEvent xev) {
        ...
        if (focusAllowedFor() && firstMapped && !isModalBlocked()) {
            requestInitialFocus();
            firstMapped = false;
        }
        updateAlwaysOnTop();
                                                ...
                                                boolean updateChain, boolean allStates)
    {
        ...
        while (!XToolkit.isToplevelWindow(bpw) && !XToolkit.isXAWTToplevelWindow(bpw)) {
            bpw = XToolkit.getParentWindow(bpw);
        }
        long tpw = transientForWindow.getWindow();
        while (!XToolkit.isToplevelWindow(tpw) && !XToolkit.isXAWTToplevelWindow(tpw)) {
            tpw = XToolkit.getParentWindow(tpw);
        }
     ...
     * @see #setModalBlocked
     */
    protected synchronized void addToTransientFors(XDialogPeer blockerPeer) {
    ...
    public Insets getWMSetInsets(XAtom changedAtom) {
        ...
        wm_set_insets = net_protocol.getWMExtents(this, changedAtom);
        insLog.log(Level.FINER, "WM_EXTENTS: {0}", new Object[]{String.valueOf(wm_set_insets)});
        if (wm_set_insets != null) {
    ...
    public void requestWMSetInsets() {
        ...
        if (net_protocol != null) {
            net_protocol.requestWMExtents(this);
        }
    }
