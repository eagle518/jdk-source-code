/*
 * @(#)XComponentPeer.java	1.131 10/03/23
 *
...
public class XComponentPeer extends XWindow implements ComponentPeer, 
    DropTargetPeer, XConstants, BackBufferCapsProvider
{
    ...
    static void callRequestFocus(Component target, CausedFocusEvent.Cause cause) {
        if (requestFocusWithCause == null) {
            requestFocusWithCause = XToolkit.getMethod(Component.class, "requestFocus", new Class[] {CausedFocusEvent.Cause.class});
        }
        if (requestFocusWithCause != null) {
            try {
...
//                   }
    ...
    public void handleButtonPressRelease(XEvent xev) {
        /*
         * Fix for 6385277.
         * We request focus on override_redirect Window by click in order
    ...
    public void pSetCursor(Cursor cursor) {
        pSetCursor( cursor, null );
    }

    // This (another pSetCursor) method is added for fixing CR 6431076.
    // It's a hack, consider redesigning.
