/*
 * @(#)BuildMetaIndex.java	1.8 10/03/23
 *
...
public class BuildMetaIndex {
	...
	try {
		...
		/* 
		 * meta-index file plays different role in JVM and J2SE side.
...
class JarMetaIndex {
	...
	for (Iterator<String> keysIterator = knownPrefixMap.keySet().iterator();  
	    ...
	    if (setSize == 0) {
		continue;
	    } /* "sun" is the special case as we want to avoid reading 
                 of rt.jar completely on lookup for plugin class */
