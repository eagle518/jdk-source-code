...
typedef unsigned int (WINAPI *LPFNPostKernelComp)(LPVOID err);
HANDLE PostKIC(HMODULE hModule, DWORD err)
{
    LPFNPostKernelComp lpfnPostKernelComp;
    HANDLE hThread = NULL;

    lpfnPostKernelComp = (LPFNPostKernelComp)GetProcAddress(hModule, 
                                                            "PostKernelComp");
    if (lpfnPostKernelComp != NULL) {
        hThread = (HANDLE)_beginthreadex(NULL, 0, lpfnPostKernelComp, 
                                             (LPVOID)err, 0, NULL);
        if (hThread == NULL) 
            lpfnPostKernelComp((LPVOID)err);
    }
    return hThread;
}
                ...
                if (strlen(javaHome) + strlen(finishedPath) < MAX_PATH) {
                    ...
                    if (result == 0) { // JRE has been fully downloaded but not yet cleaned up
                        ...
                        if (highIntegrity()) {
                            ...
                            if (hModule != NULL)
                                hThread = PostKIC(hModule, ERROR_SUCCESS); 
                            if (KERNEL_DEBUG)
