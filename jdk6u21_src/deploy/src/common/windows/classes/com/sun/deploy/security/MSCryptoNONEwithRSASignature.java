/*
 * @(#)MSCryptoNONEwithRSASignature.java	1.7 10/03/24
 *
 ...
 */
public final class MSCryptoNONEwithRSASignature extends java.security.SignatureSpi 
{
    // buffer for the data
    private byte[] buffer;
    ...
    public MSCryptoNONEwithRSASignature() 
    {
    }
	...
	throws InvalidKeyException
    {
	// This signature could only accept MSCryptoRSAPrivateKey
	if ((key instanceof MSCryptoRSAPrivateKey) == false)
	    throw new InvalidKeyException("Key not supported");
	    	
	privateKey = (MSCryptoRSAPrivateKey) key;	
	...
	try 
	{
	    ...
	    byte[] result = nativeSignHash(hash, hash.length, privateKey.getHCryptProvider(), privateKey.getHCryptKey());
    ...
    /**
     * Sign hash using Microsoft Crypto API with HCRYPTKEY. The returned 
     * data is in little-endian.
     */
    private native static byte[] nativeSignHash(byte[] hash, int hashSize, int hCryptProv, int hCryptKey) throws SignatureException;
