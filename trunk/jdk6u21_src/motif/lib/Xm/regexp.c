...
#define CHARLEN(p)  (MB_CUR_MAX > 1 ? (abs(mblen(p, MB_CUR_MAX))): (*p ? 1 : 0))
#ifndef CHARBITS
	...
	while ((len = CHARLEN(p))) {
		...
		case 1:
			...
			if (*p == *s)
				return 1;
			break;
                /* Solaris 2.6 motif diff bug 1236359 3 lines */
		case -1: /* invalid character string */
			return 0;
			break;
...
regtail(char *p, char *val)
{
	register char	*scan;
	register char	*temp;
	register long offset; /* Wyoming 64-bit fix */ 
