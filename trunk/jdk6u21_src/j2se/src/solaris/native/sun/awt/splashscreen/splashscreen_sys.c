/*
 * @(#)splashscreen_sys.c	1.11 10/03/23
 *
...
SplashInitFrameShape(Splash * splash, int imageIndex) {
    ...
    frame = splash->frames + imageIndex;
...
SplashCreateWindow(Splash * splash) {
    ...
    wnd = XCreateWindow(splash->display, XRootWindowOfScreen(splash->screen),
        ...
        &attr);
    sizeHints.flags = USPosition | PPosition | USSize | PSize | PWinGravity;
    sizeHints.width = sizeHints.base_width = splash->width;
    sizeHints.height = sizeHints.base_height = splash->height;
    sizeHints.win_gravity = NorthWestGravity;
    XSetWMNormalHints(splash->display, wnd, &sizeHints);
    ...
    if (splash->wmHints) {
        ...
        XSetWMHints(splash->display, wnd, splash->wmHints);
    }
    return wnd;
}
...
SplashRemoveDecoration(Splash * splash) {
    ...
    if (atom_set != None) {
        ...
        XChangeProperty(splash->display, splash->window, atom_set, XA_ATOM, 32, 
                PropModeReplace, (unsigned char *) atom_list, 1);
    }

    /* mwm hints */
    atom_set = XInternAtom(splash->display, "_MOTIF_WM_HINTS", True);
    if (atom_set != None) {
        mwm_hints.flags = 1L << 1;      /* flags for decoration */
...
//    pthread_setspecific(key, splash);
    ...
    splash->window = SplashCreateWindow(splash);
    fflush(stdout);
    if (splash->window) {
