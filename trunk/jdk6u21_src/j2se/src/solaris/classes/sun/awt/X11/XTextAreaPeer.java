/*
 * @(#)XTextAreaPeer.java	1.57 10/03/23
 *
...
//          }
    ...
    // This method is a hack for fixing CR 6431076. Also, JavaMouseEventHandler
    // was not initially designed for being used this way. Consider redesigning.

    void pSetCursor(Cursor cursor, Component sourceOfCursor) {
        if( sourceOfCursor==null || sourceOfCursor!=target ) {
            super.pSetCursor( cursor, null );
            return;
        }

        Point onScreen = getLocationOnScreen();

        // This probably can never happen, but I'm not sure.
        if( onScreen==null ) {
            super.pSetCursor( cursor, null );
            return;
        }
        ...
        ((XGlobalCursorManager)XGlobalCursorManager.getCursorManager())
            .getCursorPos( cursorPos );
        Point inLocalSpace = new Point(
            cursorPos.x-onScreen.x, cursorPos.y-onScreen.y );

        JavaMouseEventHandler temp = new JavaMouseEventHandler( this );
        temp.setPointerToUnderEventPoint( inLocalSpace );
        temp.setCursor();
    }
    ...
    private static final class JavaMouseEventHandler {
        ...
        void handle( MouseEvent event ) {
            if ( ! grabbed ) {
                ...
                setPointerToUnderEventPoint( event.getPoint() );
            }
            dispatch( event );
        ...
        // Following was internally private, before fixing CR 6431076:
        ...
        void setPointerToUnderEventPoint( Point point ) {
