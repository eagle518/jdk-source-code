/*
 * @(#)XBaseWindow.java	1.64 10/03/23
 *
...
//              XlibWrapper.XGetWMNormalHints(XToolkit.getDisplay(), getWindow(), p_hints, XlibWrapper.larg1);
    ...
    static Point toOtherWindow(long src, long dst, int x, int y) {
        ...
        if (srcPeer != null && dstPeer != null) {
        ...
        } else if (dstPeer != null && isRoot(src, dstPeer.getScreenNumber())) {
            // from root into peer
            rpt.x = x - dstPeer.getAbsoluteX();
            rpt.y = y - dstPeer.getAbsoluteY();
        } else if (srcPeer != null && isRoot(dst, srcPeer.getScreenNumber())) {
            // from peer into root
            ...
            try {
                XTranslateCoordinates xtc = 
                    new XTranslateCoordinates(src, dst, x, y);
                try {
                    ...
                    if (status == 0 || 
                        ...
                        rpt.x = xtc.get_dest_x();
                        rpt.y = xtc.get_dest_y();
                    }
                } finally {
                    xtc.dispose();
                }
