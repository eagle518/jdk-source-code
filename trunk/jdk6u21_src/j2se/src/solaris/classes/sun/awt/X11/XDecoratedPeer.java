/*
 * @(#)XDecoratedPeer.java	1.104 10/03/23
 *
 ...
 **************************************************************************************/
                        ...
                        boolean userReshape)
    {
        ...
        if (userReshape) {
            ...
            if (!newBounds.equals(reqBounds)) {
                // This method may be invoked with either the full exterior dimensions or
                // the "client area" dimensions only. Hence the slightly different processing.
                if (newDimensions.isClientSizeSet()) {
                    Insets insets = newDimensions.getInsets();

                    newDimensions = new WindowDimensions(new Rectangle(
                                newBounds.x + insets.left,
                                newBounds.y + insets.top,
                                newBounds.width - (insets.left + insets.right),
                                newBounds.height - (insets.top + insets.bottom)),
                                newDimensions.getInsets(), true);
                } else {
                    newDimensions = new WindowDimensions(newBounds, newDimensions.getInsets(), false);
                }
            }
        }
        XToolkit.awtLock();
    ...
    public void handleConfigureNotifyEvent(XEvent xev) {
        ...
        if (!isMaximized() && (xe.get_serial() == reparent_serial || xe.get_window() != getShell()) && !no_reparent_artifacts) {
 ...
 **************************************************************************************/
    ...
    private void handleWmTakeFocus(XClientMessageEvent cl) {
        ...
        XWindowPeer toFocus = requestXFocusGently(cl.get_data(1));
        if (toFocus != null) {
            focusLog.log(Level.FINE, "WM_TAKE_FOCUS accepted focus on {0}", new Object[]{String.valueOf(toFocus)});
        }
    }
    ...
    private Point queryXLocation() {
        XToolkit.awtLock();
        try {
            XTranslateCoordinates trans = 
                new XTranslateCoordinates(getContentWindow(), 
                                          XlibWrapper.RootWindow(XToolkit.getDisplay(), getScreenNumber()),
                                          0, 0);
            try {
                if (trans.execute() != 0) {
                    return new Point(trans.get_dest_x(), trans.get_dest_y());
                } else {
                    return null;
                }
            } finally {
