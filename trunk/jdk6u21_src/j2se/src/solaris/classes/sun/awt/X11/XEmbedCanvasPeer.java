/*
 * @(#)XEmbedCanvasPeer.java	1.21 10/03/23
 *
...
public class XEmbedCanvasPeer extends XCanvasPeer implements WindowFocusListener, KeyEventPostProcessor, ModalityListener, WindowIDProvider {
    ...
    public void dispatchEvent(XEvent ev) {
        ...
        switch (ev.get_type()) {
	      ...
	      if (xembedLog.isLoggable(Level.FINEST)) xembedLog.finest("Message on embedder: " + cr);
              if (xembedLog.isLoggable(Level.FINER)) xembedLog.finer("Create notify for parent " + Long.toHexString(cr.get_parent()) + 
                                                                       ", window " + Long.toHexString(cr.get_window()));
              embedChild(cr.get_window());
              break;
          case DestroyNotify:
	      XDestroyWindowEvent dn = ev.get_xdestroywindow();
	      if (xembedLog.isLoggable(Level.FINEST)) xembedLog.finest("Message on embedder: " + dn);
              if (xembedLog.isLoggable(Level.FINER)) xembedLog.finer("Destroy notify for parent: " + dn);
              childDestroyed();
              break;
          case ReparentNotify:
	      XReparentEvent rep = ev.get_xreparent();
	      if (xembedLog.isLoggable(Level.FINEST)) xembedLog.finest("Message on embedder: " + rep);
              if (xembedLog.isLoggable(Level.FINER)) xembedLog.finer("Reparent notify for parent " + Long.toHexString(rep.get_parent()) + 
...
//             setBounds(my_bounds.x, my_bounds.y, bounds.width, bounds.height, SET_BOUNDS);
    ...
    static byte[] getBData(KeyEvent e) {
        try {
            if (bdataField == null) {
                bdataField = XToolkit.getField(java.awt.AWTEvent.class, "bdata");
            }
    ...
    class XEmbedServer extends XEmbedHelper implements XEventDispatcher {
        ...
        boolean processXEmbedInfo() {
            ...
            try {
                ...
                boolean currently_mapped = XToolkit.isMapped(handle);
