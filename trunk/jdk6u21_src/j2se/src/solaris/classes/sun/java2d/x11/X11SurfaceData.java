/*
 * @(#)X11SurfaceData.java	1.68 10/03/23
 *
...
import static sun.awt.X11GraphicsEnvironment.*;
...
public abstract class X11SurfaceData extends SurfaceData {
	...
	if (accelerationEnabled == null) {
		...
		if (prop != null) {
		    ...
		    accelerationEnabled = Boolean.valueOf(prop);
                    ...
                    // EXA based drivers tend to place pixmaps in VRAM, slowing down
                    // readbacks.
	...
	return new X11PixmapSurfaceData(gc, width, height, image,
					getSurfaceType(gc, bitmask != 0, true),
					cm, drawable, bitmask);
    }
    ...
    public static SurfaceType getSurfaceType(X11GraphicsConfig gc,
                                             boolean transparent)
    {
        return getSurfaceType(gc, transparent, false);
