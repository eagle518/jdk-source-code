...
class GenSrc extends BackEnd {
	...
	try {
		...
		for (int i = 0; i < size; i++) {
		    out.write(" * " + rule.get(i).getLine() + m_return);
		}
		...
		out.write(m_zone);
		for (int i = 0; i < size; i++) {
		    out.write(" * " + zone.get(i).getLine() + m_return);
		}
	    }
		...
		for (int i = 0; i < size; i++) {
		    ...
		    if ((dstoffset = dstOffsets.get(i)) == -1) {
			dstoffset = 0;
		    }

		    out.write(tra + "L<<12|" + dstoffset + "<<4|" +
			      offsets.get(i) + ", " +
			      " // " + Time.gcal.getCalendarDate(tra) + "\n\t");
		}
	...
	try {
		...
		Set<String> perRO = roit.get(i);
		for (String key : perRO) {
		    pad_num = key.length();
