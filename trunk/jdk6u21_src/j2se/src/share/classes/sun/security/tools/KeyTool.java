...
public final class KeyTool {
	...
	for (i=0; (i < args.length) && args[i].startsWith("-"); i++) {
		...
		if (args.length > (i+1)) {
			...
			i += 2;
                ...
                providers.add(new Pair(providerClass, providerArg));
	    }
	...
	chain[0] = keypair.getSelfCertificate
		(x500Name, getStartDate(startDate), (long)validity*24*60*60);
	...
	for (Enumeration e = srckeystore.aliases(); e.hasMoreElements(); ) {
	...
	lastDate.setTime(firstDate.getTime() + (long)validity*1000*24*60*60);
