/*
 * @(#)DownloadManager.java	1.74 10/03/23
 *
...
public class DownloadManager {
    ...
    private static List<String> pendingLog = null; 

    /**
     * Outputs to the error stream.
     * Delays first output till the end of initialization of system classes.
     */
    static synchronized void println(String msg) {
        if (sun.misc.VM.isBooted()) {
            PrintStream err = System.err;
            if (err != null) { //should be initialized before isBooted()
                               //but System.setErr(null) is possible
                if (pendingLog != null) {
                    err.println("Download Manager: flush pending messages");
                    for(String s: pendingLog) {
                        err.println(s);
                    }
                    pendingLog = null;
                }
            ...
            if (pendingLog == null) 
                pendingLog = new ArrayList<String>();
            pendingLog.add(msg);
        }
    }
