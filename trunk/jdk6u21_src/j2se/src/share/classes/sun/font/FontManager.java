/*
 * @(#)FontManager.java	1.48 04/06/10
 *
...
public final class FontManager {
	...
	String[] ffiles = jreFontMap.values().toArray(STR_ARRAY);
	for (int i=0; i<ffiles.length; i++) {
	    jreLucidaFontFiles.add(ffiles[i]);
	}
	...
	java.security.AccessController.doPrivileged(
  	   ...
  	   public Object run() {
	       ...
	       System.loadLibrary("fontmanager");
               ...
               String disableHints = System.getProperty("sun.java2d.font.experimental.nogridfitting");

	       // JNI throws an exception if a class/method/field is not found,
	       // so there's no need to do anything explicit here.
	       initIDs("true".equals(disableHints));
    ...
    static Font2D findFontFromPlatformMap(String lcName, int style) {
	if (platformFontMap == null) {
        ...
        if (failure) {
            if (logging) {
                logger.info("Hardcoded file missing looking for " + lcName);
            }
    ...
    private static native void initIDs(boolean disableHints);
...
//     }
	...
	FontRegistrationInfo[] infos = (FontRegistrationInfo[])
	    deferredFontFiles.values().toArray(new FontRegistrationInfo[0]);
	for (int i=0; i< infos.length; i++) {
	    if (infos[i].fontFilePath != null &&
		infos[i].fontFilePath.startsWith(jreDir)) {
		initialiseDeferredFont(infos[i].fontFilePath);
	    }
	}
    }
	...
	synchronized (jreLucidaFontFiles) {
		...
		for (int i=0; i<fileNames.length; i++) {
		    ...
		    otherFontFiles.add(fileNames[i]);
		}
	...
	for (int i=0; i<fileNames.length; i++) {
	    ...
	    PhysicalFont physicalFont = initialiseDeferredFont(fileNames[i]);
	    if (physicalFont != null &&
		(physicalFont.getFontName(null).equalsIgnoreCase(name) ||
		physicalFont.getFamilyName(null).equalsIgnoreCase(name)) &&
		physicalFont.style == style) {
    ...
    private static void getPathDirs() {
        if (pathDirs == null) {
            StringTokenizer parser = new StringTokenizer
                (getFontPath(SunGraphicsEnvironment.noType1Font), 
                 File.pathSeparator);
	...
	String[] files = fontToFileMap.values().toArray(STR_ARRAY);
	for (int i=0;i<files.length;i++) {
	    if (new File(files[i]).isAbsolute() &&
		!registeredFontFiles.contains(files[i])) {
		int fontFormat = FONTFORMAT_NONE;
		int fontRank = Font2D.UNKNOWN_RANK;
		if (SunGraphicsEnvironment.ttFilter.accept(null, files[i])) {
		    fontFormat = FONTFORMAT_TRUETYPE;
		    fontRank = Font2D.TTF_RANK;
		} else if
		    (SunGraphicsEnvironment.t1Filter.accept(null, files[i])) {
		...
		registerFontFile(files[i], null, fontFormat, false, fontRank);
	    }
	}
    }
	...
	String[] names = fontToFamilyNameMap.values().toArray(STR_ARRAY);
	for (int i=0;i<names.length;i++) {
	    familyNames.put(names[i].toLowerCase(requestedLocale), names[i]);
	}
	return true;
    }
	...
	if (sgEnv == null) {
        ...
        if (isWindows) {
            ...
            if (logging) {
                logger.info("findFontFromPlatformMap returned " + font);
            }
            if (font != null) {
                fontNameCache.put(mapName, font);
                return font;
