...
//     protected long[] canDisplayBitmaskMap;
	...
	if (strike != null && desc.equals(strike.desc)) {
	    ...
	    Reference strikeRef;
	    synchronized (strikeCache) {
		strikeRef = (Reference)strikeCache.get(desc);
	    }
	    if (strikeRef != null) {
	    ...
	    /* Updates to the strikeCache need to be synchronized,
	     * as the Disposer thread may be removing items.
	     */
	    synchronized (strikeCache) {
	...
	synchronized (strikeCache) {
	    Reference ref = (Reference)strikeCache.get(desc);
	    if (ref != null) {
		Object o = ref.get();
		if (o == null) {
