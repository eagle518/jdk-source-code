/*
 * @(#)AWTUtilities.java	1.9 10/03/23
 *
...
public final class AWTUtilities {
    ...
    private static boolean isWindowOpacityControlSupported() {
        ...
        return ((SunToolkit)curToolkit).isWindowOpacityControlSupported();
    }
    ...
    public static void setWindowOpacity(Window window, float opacity) {
        ...
        if (!isTranslucencySupported(Translucency.TRANSLUCENT)) {
            throw new UnsupportedOperationException(
                    "The TRANSLUCENT translucency kind is not supported.");
        }
    ...
    public static void setWindowShape(Window window, Shape shape) {
        ...
        if (!isTranslucencySupported(Translucency.PERPIXEL_TRANSPARENT)) {
            throw new UnsupportedOperationException(
                    "The PERPIXEL_TRANSPARENT translucency kind is not supported");
        }
        if (window.getGraphicsConfiguration().getDevice().getFullScreenWindow()
                == window && shape != null) {
            throw new IllegalArgumentException(
        ...
        {
            // If the window is decorated, reset the shape
            shape = null;
        }
