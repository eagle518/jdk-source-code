...
tsiScanConv *tsi_NewScanConv(tsiMemObject *mem, short numberOfContours, short *startPtr, 
                             short *endPtr, tt_int32 *xPtr, tt_int32 *yPtr, char *onCurvePtr, 
                             char greyScaleLevel, char curveType );

/* NB: there is no real dropout control for AA text yet */
void MakeGreyscaleBits(tsiScanConv *t, char greyScaleLevel, char xWeightIsOne, 
                       char omitBitMap, char computeBBox, short dropoutMode);
void MakeBWBits(tsiScanConv *t, char omitBitMap, char computeBBox, short dropoutMode);
