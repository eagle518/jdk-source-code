...
local void sc_mark( tt_int32 x0, tt_int32 y0, tt_int32 x1, tt_int32 y1, tt_int32 bx, tt_int32 by,
				tt_int16 **xBase, tt_int16 **yBase, sc_BitMapData *bbox, tt_int16 isSmartDropoutAllowed );
...
local void sc_lineGen(sc_BitMapData *bbox, tt_int32 *px, tt_int32 *py, tt_int32 *pend,
						tt_int16 **xBase, tt_int16 **yBase, tt_int16 isSmartDropoutAllowed );
...
local void sc_wnNrowFill(tt_int32 rowM, tt_int16 nRows, sc_BitMapData *bbox, tt_int16 isSmartDropoutAllowed);

static void sc_orSomeBits( sc_BitMapData *bbox, tt_int32 dropoutMode, tt_int16 isSmartDropoutAllowed);
...
static tt_int32 nUpperXings(tt_int16** lineBase, tt_int16** valBase, tt_int16 gxLine, tt_int16 val,
                            tt_int16 lineChanges, tt_int16 valChanges, tt_int16 valMin, tt_int16 valMax,
                            tt_int16 lineMax, tt_int16 isSmartDropoutAllowed); /*<14>*/
static tt_int32 nLowerXings(tt_int16** lineBase, tt_int16** valBase, tt_int16 gxLine, tt_int16 val,
                            tt_int16 lineChanges, tt_int16 valChanges, tt_int16 valMin, tt_int16 valMax, 
                            tt_int16 lineMin, tt_int16 isSmartDropoutAllowed); /*<14>*/
static void invpixSegY(tt_int16 llx, tt_uint16 k, tt_uint32* bitmapP, int preferLeft );

static void invpixSegX(tt_int16 llx, tt_uint16 k, tt_uint32* bitmapP, int preferBottom );

static void invpixOn(tt_int16 llx, tt_uint16 k, tt_uint32* bitmapP );

/* 
   If we are doing smart dropout control then we need to reserve last
   PIXSHIFT bits in the elements of xLines and yLines arrays to store
   fractional part. Elements are of tt_int16 type - we need to ensure we 
   will not truncate main value.
*/
#define MAX_SIZE_FOR_SMART_DROPOUT  (1<<(15-PIXSHIFT))

/*
 * Returns the gxBitmap
 * This is the top level call to the scan converter.
 *
 * Assumes that (*bbox).xmin,...xmax,...ymin,...ymax
 ...
 * dropoutMode specifies what kind of dropout control should be performed
 *              (see dtypes.h for details)
*/
int sc_ScanChar2(tsiScanConv *t, sc_BitMapData *bbox,
		tt_int16 lowBand, tt_int16 highBand, tt_int32 dropoutMode) 
{
	register F26Dot6 *x = t->x;
	register F26Dot6 *y = t->y;
    register ArrayIndex i, endPt, nextPt;
	register tt_int8 *onCurve = (tt_int8*) t->onCurve;
	ArrayIndex startPt, j;
	LoopCount ctr;
	sc_GlobalData sc_GD, *p;
    F26Dot6 *xp, *yp, *x0p, *y0p;
    register F26Dot6 xx, yy, xx0, yy0;
	int quit;
	tt_int16 **lowRowP, **highRowP;
	tt_int32 vecCount;
	tt_int16 isSmartDropoutAllowed = false;
	...
	if (dropoutMode != DOCONTROL_DISABLED)
	{
		...
		/* Disable smart dropout for large sizes - we have no space to store offsets and 
		   impact of dropout control at large sizes is hardly noticeable.
		   Note that if smart dropout is disabled due to this reason offset will not be calculated 
		   and treated as 0. Therefore we will behave same as simple dropout for such sizes. */
		if (dropoutMode == DOCONTROL_SMART_NOSTUBS || dropoutMode == DOCONTROL_SMART) {
			if (bbox->xMax < MAX_SIZE_FOR_SMART_DROPOUT && bbox->xMin > -MAX_SIZE_FOR_SMART_DROPOUT
			    && bbox->yMax < MAX_SIZE_FOR_SMART_DROPOUT && bbox->yMin > -MAX_SIZE_FOR_SMART_DROPOUT) {
			    isSmartDropoutAllowed = true;
			}
		}					   					   
	}
	else
	...
	for (ctr = 0; ctr < t->numberOfContours; ctr++) {
		x0p = xp = p->xPoints;
        y0p = yp = p->yPoints;
		startPt = i = t->startPoint[ctr];
		endPt = t->endPoint[ctr];
	    ...
	    while ( true ) {
		    while ( onCurve[i] & ONCURVE )
			{
				...
				{ /*Ran out of local memory. Consume data and continue. */
					if (dropoutMode != DOCONTROL_DISABLED)
							sc_lineGen(bbox, x0p, y0p, yp-1, bbox->xBase, bbox->yBase, isSmartDropoutAllowed);
...
sc_exit:
		if (dropoutMode != DOCONTROL_DISABLED)
		{
			...
			sc_mark( *(p->xPoints), *(p->yPoints), *(p->xPoints+1), *(p->yPoints+1),
				*(xp-2), *(yp-2), bbox->xBase, bbox->yBase, bbox, isSmartDropoutAllowed);
		}
		else
		{
	...
	if (dropoutMode != DOCONTROL_DISABLED) sortCols(bbox);
...
/* Take care of problem of very small thin glyphs - always fill at least one pixel
   ...
   NB: in case of smart dropout control we need to make sure we 
       use appropriate data format. See comment for sc_mark() 
 */
	if( highRowP < lowRowP )
	{
        ...
        if (isSmartDropoutAllowed) {
            /* we may safely assume fractional part is equal to 0 
               but we still need to reserve space for it */ 
            *(p+*p) = *(p+*p) << PIXSHIFT;
            *(s-*s) = *(s-*s) << PIXSHIFT;
        }
		highBand = lowBand+1;
		...
		for( p = *lowRowP; p <= *highRowP; p += inc+1 )
		{
			*p=1;
            if (isSmartDropoutAllowed) {
                /* we may safely assume fractional part is equal to 0 
                   but we still need to reserve space for it */ 
                *(p+inc) = (bbox->xMin+1) << PIXSHIFT; 
                *(++p) = bbox->xMin << PIXSHIFT;
            } else {
                *(p+inc) = bbox->xMin+1; 
                *(++p) = bbox->xMin;
            }
	...
	sc_wnNrowFill(lowBand, (tt_int16)(highBand - lowBand), bbox, isSmartDropoutAllowed);
	
 	if (dropoutMode != DOCONTROL_DISABLED) 
 		sc_orSomeBits(bbox, dropoutMode, isSmartDropoutAllowed);
 	return scalerError_NoError; /* NO_ERR; */
}
...
/* These two macro used to extract scanline number and offset from given tt_int16 value.
   (offsets are used by smart dropout)
   Storing offsets in the lower bits makes them transparent 
   in most of cases (e.g. sorting of scan line arrays works without any changes)
*/

#define GET_SCANLINE(v)   (isSmartDropoutAllowed ? (tt_int16)((v)>>PIXSHIFT) : (v))
#define GET_OFFSET(v)     (isSmartDropoutAllowed ? ((v)& FRACPART) : 0)
...
{																	\
	if( moreOns )													\
	{																\
		on = GET_SCANLINE(*onTp);									\
		if( moreOffs )												\
		{															\
			off = GET_SCANLINE(*offTp);								\
			if( on < off )											\
			{														\
				--moreOns;											\
				++onTp;												\
...
while (py <= pend)
{
	...
	x1 = *px++;
	y1 = *py++;
	sc_mark(x0, y0, x1, y1, xb, yb, xBase, yBase, bbox, isSmartDropoutAllowed);
}
}
...
*
*	To support smart dropout control this procedure calculates distances between 
* vector crossing scan line and left/bottom pixel center. These calculation only performed if
* requested and results are stored in the lower bits of same xBase and yBase arrays.
* 
...
#define SET(p, val, offset) {                          \
    register tt_int16 *row = *p;                      \
    ++*row;                                           \
    if (isSmartDropoutAllowed) {                      \
        *(row+*row)=LSH(val)| ((offset) & FRACPART);  \
    } else {                                          \
        *(row+*row)=val;                              \
    };                                                \
}

#define SETX(p, val) SET(p, val, DX(px-xBase))
#define SETY(p, val) SET(p, val, DY(py-yBase))

#define OFF(start, val, offset) {                     \
    register tt_int16 *s = (start);                   \
    ++*s;                                             \
    if (isSmartDropoutAllowed) {                      \
        *(s-*s) = LSH(val) | ((offset) & FRACPART);   \
    } else {                                          \
        *(s-*s) = val;                                \
    }                                                 \
}

#define OFFX(val) OFF(*px+wideX, val, DX(px-xBase)) 
#define OFFY(val) OFF(*py+wideY, val, DY(py-yBase))

#define BETWEEN(a,b,c) (a < b && b < c )
#define INTERIOR  ((x0-xb)*dy < (y0-yb)*dx)
#define ZERO( prev, mid, next, more, less ) (prev == mid && next == mid && more > less )

/* converts scanline number to fractional coordinate in Dot6 format
    0 -> 32, 1 -> 96, etc. */
#define SCANLINE_POS(k) (LSH(k) + HALF)

/* calculate distance between given point p and nearest bottom/left scanline */
#define OFFSET_FROM_SCANLINE(p) (((p) - HALF) & FRACPART)

/* returns distance between line (x0, y0)->(x1, y1) and nearest left pixel
   center along jyp scanline */
/* The only trick here is compensation for rounding error. According to C
   specification "when integers are divided, the result of the / operator is
   the algebraic quotinent with any fractional part discarded" 
   (i.e. they are truncated towards zero - 
    e.g. both 7.3 and 7.8 to 7 and both -7.3 and -7.8 to -7).
   This may cause glitches in rasterization and to compensate for this we need 
   to add or substract 0.5 before rounding happens, so we adjust numerator.
   Whether it should be addition or substraction depends on sign of result 
   that is in turn defined by dx (because dy=y1-y0 and scanline jyp is between y1 and y0).
*/   
#define DY(jyp) ((tt_int16) (dy == 0 ? OFFSET_FROM_SCANLINE(x0) : \
    OFFSET_FROM_SCANLINE(x0 + (((((tt_int32) dx)*(SCANLINE_POS(jyp) - y0)) + (dx > 0 ? dy >> 1 : -dy >> 1))/(dy)))))

/* similar to DY but returns distance between line and nearest bottom pixel center along jxp scanline */
#define DX(jxp) ((tt_int16) (dx == 0 ? OFFSET_FROM_SCANLINE(y0) : \
    OFFSET_FROM_SCANLINE(y0 + (((((tt_int32) dy)*(SCANLINE_POS(jxp) - x0)) + (dy > 0 ? dx >> 1 : -dx >> 1))/(dx)))))
...
if( dy >= 0 && dx > 0 )					/* FIRST QUADRANT CASE include ---> */
{
	...
	{				
		...
		if LINE(x0)							/* can also start on column scan gxLine */
		{
			...
			else
			{
				...
				if BETWEEN(yb, y0, y1) SETY( py, jx )
			}
		}
		else								/* starts on row scan but not col */
		{
			...
			else if BETWEEN( yb, y0, y1 ) SETY( py, jx )
		}
	...
	else if LINE(x0)						/* starts on col scan gxLine but not row */
	{
		...
		{
			OFFX( jy )
			if( xb > x0 ) SETX( px, jy )		/* cross extremum vertex twice */
		}
...
else if( dy > 0 && dx <= 0 )					/* SECOND QUADRANT CASE include vertical up*/
	{
		...
		{				
			...
			if LINE(x0)							/* can also start on column scan gxLine */
			{
				...
				{
					if BETWEEN(yb, y0, y1) SETY( py, jx );
					if BETWEEN(x1, x0, xb) SETX( px, jy );
				}
			}
			else								/* starts on row scan but not col */
			{
		...
		else if LINE(x0)						/* starts on col scan gxLine but not row */
		{
			...
			else if BETWEEN( x1, x0, xb ) SETX( px, jy )
		}
...
/* Current algorithm supports smart dropout control introduced by Microsoft.
   Here is excerpt from Microsoft truetype specification 
   (see definition of SCANTYPE command there)
   
   1. simple dropout control scan conversion including stubs
       rules 1, 2, and 3 are invoked 
   2. simple dropout control scan conversion excluding stubs
       rules 1, 2, and 4 are invoked
   3. no dropout 
       rules 1 and 2 only are invoked
   4. smart dropout control scan conversion including stubs
       rules 1, 2, and 5 are invoked
   5. smart dropout control scan conversion excluding stubs
       rules 1, 2, and 6 are invoked
   ...
   The scan conversion rules are shown here:
      ...
      Rule 3 
         ...
         turned on by rules 1 and 2, turn on the left-most pixel (horizontal
         scan line) or the bottom-most pixel (vertical scan line). This is
      ...
      Rule 4 
         ...
         intersected scan line segment are examined to verify that they are
         intersected by two contours.  It is possible that these could be
         different contours than the ones intersecting the dropout scan line
      ...
      Rule 5 
         ...
         or horizontal) is intersected by both an on-Transition contour and
         an off-Transition contour and neither of the pixels was already
         turned on by rules 1 and 2, turn on the pixel which is closer to
         the midpoint between the on-Transition contour and off-Transition
         contour. This is .Smart. dropout control.
      Rule 6 
         Apply Rule 5 only if the two contours continue to intersect other
         scan lines in both directions. That is, do not turn on pixels for
         "stubs".

   Note that this procedure only deals with rules 3-6. Rules 1 and 2 are handled 
   by regular scan conversion code.
*/   
...
#define MIDDLE(on, off)   ((on + off + 1) >> 1)  
...
for (scanline = ymin; scanline <= ymax; ++scanline) {
	...
	while (nIntOn--) {
		onOffset = GET_OFFSET(*pOn);
		coordOn = GET_SCANLINE(*pOn++);
		index = nIntOff;
		pOff2 = pOff;
		offOffset = GET_OFFSET(*pOff2);
		while (index-- && ((coordOff = GET_SCANLINE(*pOff2++)) < coordOn)) {
		    offOffset = GET_OFFSET(*pOff2);
		}
		
		if (coordOn == coordOff) {  /* This segment was crossed twice  */
		    if (dropoutMode == DOCONTROL_NOSTUBS || dropoutMode == DOCONTROL_SMART_NOSTUBS) {
		        upper = nUpperXings(yBase, xBase, scanline, coordOn,
		                            (tt_int16)(incY-2), (tt_int16)(incX-2),
		                            xmin,(tt_int16)(xmax+1), ymax,
		                            isSmartDropoutAllowed);               /*<14>*/ 
		        lower = nLowerXings(yBase, xBase, scanline, coordOn,
		                            (tt_int16)(incY-2), (tt_int16)(incX-2),
		                            xmin, (tt_int16)(xmax+1), ymin,
		                            isSmartDropoutAllowed);              /*<14>*/
		        if (upper < 2 || lower < 2) {
		            continue;
			}
		    ...
		    if (coordOn > xmax) {
		       invpixOn((tt_int16)(xmax-xmin), longsWide, scanP);
		    } else if (coordOn == xmin) {
		        invpixOn( 0, longsWide, scanP);
		    } else {
		        tt_int16 preferLeft = true; /* if we are doing simple dropout control 
		                                       we need to turn on leftmost pixel */
		        if (dropoutMode == DOCONTROL_SMART_NOSTUBS || dropoutMode == DOCONTROL_SMART) {
		            if (offOffset == 0) {
		                 /* offOffset >= onOffset but due to compensation of rounding error 
		                    it might be stored as 0 when we mean 64.
		                    In case offOffset was supposed to be 0 onOffset should be 0 too and 
		                    preferLeft will be set to true anyway (HALF rounded towards left). */
		                    offOffset = ONE;
		            }
		            preferLeft = MIDDLE(onOffset, offOffset) <= HALF ? true : false;  
		        } 
		        invpixSegY( (tt_int16)(coordOn-xmin-1), longsWide, scanP, preferLeft);
		    }
		}
	}
	rowPt += incY;
		...
		if (coordOn == coordOff) {
		    if (dropoutMode  == DOCONTROL_NOSTUBS || dropoutMode == DOCONTROL_SMART_NOSTUBS) {
		        upper = nUpperXings(xBase, yBase, scanline, coordOn,
		                            ...
		                            ymin, (tt_int16)(ymax+1), xmax,
		                            isSmartDropoutAllowed);               /*<14>*/
		        lower = nLowerXings(xBase, yBase, scanline, coordOn,
		                            (tt_int16)(incX-2), (tt_int16)(incY-2),
		                            ymin, (tt_int16)(ymax+1), xmin,
			                    isSmartDropoutAllowed);              /*<14>*/
		        if (upper < 2 || lower < 2) {
		           continue;
		        }
		    ...
		    if (coordOn > ymax) {
		        invpixOn((tt_int16)(scanline-xmin), longsWide,
		                 bitmapP - longsWide*(ymax-ymin));
		    } else if (coordOn == ymin) {
		        invpixOn((tt_int16)(scanline-xmin), longsWide, bitmapP);
		    } else {
		        tt_int16 preferBottom = true; /* if we are doing simple dropout control 
		                                         we need to turn on bottommost pixel */
		        if (dropoutMode == DOCONTROL_SMART_NOSTUBS || dropoutMode == DOCONTROL_SMART) {
		            if (offOffset == 0) {
		                /* offOffset >= onOffset but due to compensation of rounding error 
		                   it might be stored as 0 when we mean 64.
		                   In case offOffset was supposed to be 0 onOffset should be 0 too and 
		                   preferBottom will be set to true anyway (HALF rounded towards bottom). */
		                offOffset = ONE;
		            }
		            preferBottom = MIDDLE(onOffset, offOffset)  <= HALF ? true : false;  
		        } 
		        invpixSegX( (tt_int16)(scanline-xmin), longsWide, bitmapP - longsWide*(coordOn-ymin-1), preferBottom);
		    }
		}
	}
	rowPt += incX;
...
register tt_uint32 maskL, maskR;								/*<9>*/
	...
	else
	{
		...
		if (preferLeft) {
		  ...
		  if (*bitmapP & maskL) return;
		  /* now check right pixel.
		     Note if current bit is last in the word we need to move to next word */
		  if (nBits < 31) {
		    maskR = maskL >> 1;
		  } else {
		    maskR = 0x80000000;
 ...
 */ 				
static tt_int32 nOnOff( tt_int16** base, tt_int32 k, tt_int16 val, tt_int32 nChanges, tt_int16 isSmartDropoutAllowed )
{
...
while(--rowP > endP)
{
	if ((v = GET_SCANLINE(*rowP)) == val) return ++count;
	if( v < val ) break;
}
return count;
}
                            ...
                            tt_int16 lineMax, tt_int16 isSmartDropoutAllowed) /*<14>*/
{
    ...
    if (gxLine < lineMax) 
        count += nOnOff(lineBase, gxLine+1, val, lineChanges, isSmartDropoutAllowed); 	/*<14>*/
    if (count > 1) 
        return count;
    else if(val > valMin) 
        count += nOnOff(valBase, val-1, (tt_int16)(gxLine+1), valChanges, isSmartDropoutAllowed);
    if (count > 1) 
        return count;
    else if(val < valMax) 
        count += nOnOff(valBase, val, (tt_int16)(gxLine+1), valChanges, isSmartDropoutAllowed);
                            ...
                            tt_int16 lineMin, tt_int16 isSmartDropoutAllowed) /*<14>*/
{
    ...
    if (count > 1) 
        return count;
    if (val > valMin) 
        count += nOnOff(valBase, val-1, gxLine, valChanges, isSmartDropoutAllowed);
    if (count > 1) 
        return count;
    if (val < valMax) 
        count += nOnOff(valBase, val, gxLine, valChanges, isSmartDropoutAllowed);
...
*/
	ny = ymax - ymin + 1;
	n = ny + ((dropoutMode != DOCONTROL_DISABLED) ? xmax - xmin + 1 : 0);

	ptrY = (short*) tsi_AllocMem(t->mem, 2*n*sizeof(short));
	ptrX = ptrY + ny;
	p = ptrY;
	if( n ) do{ *p++ = 0; } while (--n);
	...
	for(ctr = 0; ctr < t->numberOfContours; ctr++) 
	{
		...
		if (t->x[startPoint] == t->x[endPoint] && t->y[startPoint] == t->y[endPoint])
		{
			--endPoint;
			t->onCurve[startPoint] |= ONCURVE;
		}
...
*/
			...
			{													
				...
				register tt_int8 *newC = (tt_int8*)&(t->onCurve[aPoint-2]);  
		...
		t->startPoint[ctr] = startPoint < endPoint ? startPoint : endPoint;
		t->endPoint[ctr] = endPoint;			/* 7/93 change */
	}
