/*
 * @(#)InterpreterGlue.c	1.14 10/04/02 
 */
...
void SetGlobalGSDefaults( fnt_GlobalGraphicStateType *globalGS )
{
    ...
    globalGS->defaultParBlock.scanControl = DEFAULT_DROPOUT_MODE;
...
void RunFontProgram(fsg_SplineKey* key, voidFunc traceFunc, jmp_buf* env) {
    ...
    elements[GLYPHELEMENT] = NULL; /* recovery code checks this */
	SetGlobalGSStackData(  key,globalGS) ;
    fnt_Execute(elements, globalGS, globalGS->pgmList[fontProgramIndex],
                globalGS->pgmList[fontProgramIndex] + tableLength, traceFunc,
                key->memContext, false, false, false, env);
...
void RunPreProgram(register fsg_SplineKey *key, const gxMapping* stretch, 
    ...
    SetGlobalGSMapping(globalGS, stretch, TheFont(key)->emResolution);
    ...
    fnt_Execute(elements, globalGS, globalGS->pgmList[preProgramIndex],
                ...
                traceFunc, key->memContext, false, false, false, env);
...
void RunGlyphProgram(fsg_SplineKey* key, fnt_ElementType* outline, const gxMapping* stretch,
    ...
    fnt_Execute(elements, globalGS, instructionData, instructionData + instructionSize,
                (voidFunc)nil, key->memContext, TheVari(key)->hasStyleCoord,
                TheFont(key)->expectCVAR, hintingAComposit, env);
#ifdef UIDebug
	{
		int glyphCode;
		glyphCode= outline->glyphIndex;
