...
typedef struct fnt_LocalGraphicStateType {
    ...
    tt_uint8 *endPtr;     /* pointer to last instruction */
    tt_uint8 *basePtr;    /* pointer to first instruction */
...
#endif
    jmp_buf*  env; /* state for exception handling, allocated somethere else */
} fnt_LocalGraphicStateType;

void CorrectUnscaledOutline(fnt_LocalGraphicStateType* gs);
void fnt_Execute(fnt_ElementType* elements[], fnt_GlobalGraphicStateType *globalGS, 
                 tt_uint8 *ptr, tt_uint8 *eptr, voidFunc TraceFunc, memoryContext* context, 
                 boolean hasStyleCoord, boolean hasVariantCVT, 
                 boolean hintingACompositGlyph, jmp_buf* env);
/*
