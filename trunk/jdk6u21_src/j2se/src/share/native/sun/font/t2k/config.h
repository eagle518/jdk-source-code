...
/*** #1 ***/
/* do now try to alloc anything bigger than 32Mb. 
   In fact no known fonts need memory chunks bigger than 1Mb. */
#define CLIENT_MALLOC(size)         ((size) < 0x2000000 ? malloc(size) : NULL)
...
/* 
  Enables usage of global dropout preferences.
  
  NB: These preferences were used before Truetype hinting was integrated.
      It might be useful to evaluate their impact for cases where 
      dropout control is not controled explicitly
      (e.g. TrueType fonts without hinting instructions or Type1 fonts)
      
  Note: if UIDebug is turned on, then the hinted dropout control can be forced on. 
*/

#define xEnableGlobalDropoutPreferences
#ifdef  EnableGlobalDropoutPreferences
 
/* These parameters are used for non-greyscale processing 
   (i.e. not just for B&W but also for subpixel rendering) */
#define MinT2KDropoutControl         4  /* no dropout if size is below this limit */
#define MaxCriticalT2KDropoutControl 9  /* at this size (down to MinT2KDropoutControl),
                                           the dropout is forced on. */
#define MaxT2KDropoutControl       200  /* Above this value, dropout control 
                                           is always turned off. */

/* AntiAlias dropout applies to greyscale antialiasing */
