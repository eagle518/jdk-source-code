...
typedef struct {
    ...
    /* Requested glyph transform is stored in decomposed form.
       Full transform is 
         compensation * (t00 t01) * (xPixelsPerEm       0      )
                        (t10 t11)   (      0       yPixelsPerEm)
    ...
    /* quadrant rotation and flip */
    T2K_TRANS_MATRIX compensation; 

    /* Is glyph transform quadrant rotation and/or flip only? 
       (i.e. if transform with extracted compensation part is identity transform) */
    int is_Identity;

	/* dropout control mode specific for this particular glyph. 
	   In particular it could be set by truetype hinting instructions.
	   16 most significant bits specify dropout mode (see SCANTYPE in truetype spec for details).
	   Other 16 bits indicate when to turn dropout on (see SCANCTRL instruction).
	*/
	tt_int32 glyphDropOutControlFlags;
...
/* For lcd displays where the subpixels stripes increase the horizontal
 * resolution of the display. 
 */
#define T2K_LCD_MODE            0x100
/* For lcd displays where the subpixels stripes increase the vertical
 * resolution of the display
 */
#define T2K_LCD_MODE_V          0x200
