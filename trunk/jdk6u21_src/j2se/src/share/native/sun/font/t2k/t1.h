...
/* max numbers of global type1 hints */
#define MAX_BLUE_VALUES_CNT  10
#define MAX_STEM_SNAP_CNT    10
...
typedef struct {
    ...
    /* global type1 hints (see type1 spec for details) */
        tt_int32 blueValuesCnt;
        short blueValues[MAX_BLUE_VALUES_CNT];
        tt_int32 otherBluesCnt;
        short otherBlues[MAX_BLUE_VALUES_CNT+2]; /* 2 extra for replictated 0 zone 
                                                    from blueValues */
        /* number of character space units to extend (in both directions) 
           the effect of an alignment zone on a horizontal stem. */
        short blueFuzz; 

#if 0 /* support for these hinting parameters is pending */
        short stdHW;
        short stdVW;
        short stemSnapHCnt;
        short stemSnapH[MAX_STEM_SNAP_CNT];
        short stemSnapVCnt;
        short stemSnapV[MAX_STEM_SNAP_CNT];
#endif
} T1Class;
