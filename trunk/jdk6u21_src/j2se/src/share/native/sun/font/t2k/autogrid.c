...
static void ag_AnalyzeChar( register ag_DataType *hData )
{
	...
	for ( ctr = 0; ctr < hData->numberOfContours; ctr++ ) {
		...
		/* For glyph with empty contours lastPoint might be -1.
		   We still want to continue topology analysis for other contours though. */ 
...
static int ag_GetStems( register ag_DataType *hData, ag_ElementType *e, short **xDistPtr, tt_int32 *xCountPtr, short **yDistPtr, tt_int32 *yCountPtr )
{
    ...
    /* We do + 1 just to ensure that we do not get a non null pointer. */
    /* This is important in case we have a zero count in one direction but not the other. */
    xDist = (short *) tsi_AllocArray(hData->mem, xDistCount + 1, sizeof(short)); 
    yDist = (short *) tsi_AllocArray(hData->mem, yDistCount + 1, sizeof(short)); 
...
/*#define JAM_WEIGHTS */
    ...
    hData->ox = (F26Dot6 *) tsi_AllocArray(mem, maxPointCount, 2*sizeof(F26Dot6));
	hData->oy							= (F26Dot6 *)&hData->ox[maxPointCount];

    hData->nextPt = (tt_int16 *) tsi_AllocArray(mem, maxPointCount, 3*sizeof(tt_int16));
	hData->prevPt						= &hData->nextPt[maxPointCount];
	hData->searchPoints					= &hData->nextPt[2 * maxPointCount];
	
    hData->flags = (tt_uint16 *) tsi_AllocArray(mem, maxPointCount, sizeof(tt_uint16));

    hData->realX = (tt_int16 *) tsi_AllocArray(mem, maxPointCount, 2*sizeof(tt_int16));
	hData->realY						= &hData->realX[maxPointCount];
	
    hData->forwardAngleOrthogonalPair = (tt_int16 *) tsi_AllocArray(mem, maxPointCount, 3*sizeof(tt_int16));
    ...
    hData->cos_f = (SMALLFRAC *) tsi_AllocArray(mem, maxPointCount, 4*sizeof(SMALLFRAC));
	hData->sin_f						= &hData->cos_f[ maxPointCount ];
	hData->cos_b						= &hData->cos_f[ 2*maxPointCount ];
	hData->sin_b						= &hData->cos_f[ 3*maxPointCount ];
	
	hData->maxLinks						= 2*maxPointCount; /* Use 2.0, the CheckerBox in Arial has a ratio of 1.88 */
									/* Some barcode fonts have even higher ration but these are rare */
    hData->links = (tp_LinkType *)tsi_AllocArray(mem, hData->maxLinks, sizeof(tp_LinkType));
