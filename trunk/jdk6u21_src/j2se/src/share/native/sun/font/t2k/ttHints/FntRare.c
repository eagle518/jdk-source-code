...
void fnt_SDPVTL(register fnt_LocalGraphicStateType *gs)
{
    ...
    if (IS_BAD_POINT(gs, gs->CE1, arg1) || IS_BAD_POINT(gs, gs->CE2, arg2)) {
...
void fnt_ISECT(fnt_LocalGraphicStateType *gs)
{
    ...
    if (IS_BAD_POINT(gs, element, arg1) || IS_BAD_POINT(gs, element, arg2)) {
        FatalInterpreterError(gs, interp_range_error);
    }
...
void fnt_UTP(register fnt_LocalGraphicStateType *gs)
{
    ...
    if (IS_BAD_POINT(gs, gs->CE0, gxPoint)) {
        FatalInterpreterError(gs, interp_range_error);
    }
	if ( gs->free.x ) {
		f[gxPoint] &= ~XMOVED;
...
{
    ArrayIndex j1, j2, jMove, cvtIndex;
	pixel target;
    ...
    target = gs->GetCVTEntry(gs, cvtIndex);
    if (target < 0 || target > 32767) {
        FatalInterpreterError(gs, interp_range_error);
    }
	target += HALFPIXEL;
	target >>= LG2PIXEL;
	...
	for (; gs->loop >= 0; --gs->loop) {
        ...
        if (IS_BAD_POINT(gs, ce0, j1) || IS_BAD_POINT(gs, ce0, j2)) {
            FatalInterpreterError(gs, interp_range_error);
        }
...
void fnt_FLIPRGON(register fnt_LocalGraphicStateType *gs)
{
    ...
    if (IS_BAD_POINT(gs, gs->CE0, hi) || IS_BAD_POINT(gs, gs->CE0, lo)) {
        FatalInterpreterError(gs, interp_range_error);
    }
...
void fnt_IDefPatch( register fnt_LocalGraphicStateType* gs )
{
    ...
    if (IS_BAD_PGM_INDEX(gs, instrDef->pgmIndex)) {
        FatalInterpreterError(gs, interp_font_program_error);
    }
...
void fnt_IDEF( register fnt_LocalGraphicStateType* gs )
{
    ...
    tt_uint8 *program, *instrStart = gs->insPtr;

    if (IS_BAD_PGM_INDEX(gs, pgmIndex)) {
        FatalInterpreterError(gs, interp_font_program_error);
    }
    ...
    while (!IS_BAD_INSTR_POINTER(gs, gs->insPtr) && (gs->opCode = *gs->insPtr++) != ENDF_CODE) {
		fnt_SkipPushCrap( gs );
    }
