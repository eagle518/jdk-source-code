...
static void ReAllocEdges( register tsiScanConv *t )
{
    ...
    xEdge = (tt_int32*) tsi_AllocArray(t->mem, k, sizeof(char) + sizeof(tt_int32)*2);
	assert( xEdge != NULL );
	yEdge   	= &xEdge[k];
...
static void Make3rdDegreeBezierEdgeList( register tsiScanConv *t, char greyScaleLevel )
{
	...
	for ( ctr = 0; ctr < t->numberOfContours; ctr++ ) {
		ptA = t->startPoint[ctr];
		lastPoint = t->endPoint[ctr];

                /* In theory valid contour may contain only 2 types of point sequences
                        (see Type1BuildChar and Type1/Postscript specs for details): 
                    1) A straight line (product of *lineto instructions)
                        (onCurve onCurve)
                    2) A 3rd degree bezier (product of *curveto instructions) 
                        (onCurve offcurve offCurve onCurve)

                   One special case is result of "closepath" instruction that 
                     need to connect last point with first onCurve point 
                     in this contour.

                   Anything else is generally unexpected here but 
                     we probably still need to protect from weird input.
		*/

                
                /* Theoretically first point must be onCurve but just in case 
                     we skip leading offCurve points
		     (contour with zero onCurve points is special case) */
		while ( !t->onCurve[ ptA ] && ptA <= lastPoint) {
		        ptA = (short)(ptA + 1);
		}

		if (ptA > lastPoint) continue; 
...
SC_startContour:
	...
	while ( ctr < t->numberOfContours ) {
		...
		lastPoint = t->endPoint[ctr];
                ...
                /* skip empty contour */
                if (lastPoint < startPoint) {ctr++; goto SC_startContour;}
...
/* NB: dropout for antialised text is not supported */
void MakeGreyscaleBits(register tsiScanConv *t, char greyScaleLevel, char xWeightIsOne, 
	       char omitBitMap, char computeBBox, short dropoutMode)
{
	...
	N = rowBytes * h;
    t->baseAddr = baseAddr = (unsigned char*) tsi_AllocArray(t->mem, h, rowBytes);
...
void MakeBWBits(register tsiScanConv *t, char omitBitMap, 
                char computeBBox, short dropoutMode) 
{
    int error=0;
    ...
    memset(glyphImage, 0, sizeof(scalerBitmap));

    if (t->numberOfContours <= 0) {
         return; 
    }
      
    error = fs_CalculateBounds( &bbox, t, glyphImage );    
    if (error) {
        /* reset results to be safe */
        t->rowBytes = NULL;
        t->right = 0;
        t->left = 0;
        t->top = 0;
        ...
        t->fLeft26Dot6 = 0;
        t->fTop26Dot6 = 0;
        return; /* we have not allocated anything yet */
    }

    t->right = glyphImage->bounds.xMax;
    t->left = glyphImage->bounds.xMin;
    t->bottom  = glyphImage->bounds.yMax;
    t->top = glyphImage->bounds.yMin;
    t->fLeft26Dot6 = glyphImage->topLeft.x>>10; /* fixed to 26.6 */
    t->fTop26Dot6 = glyphImage->topLeft.y>>10; /* fixed to 26.6 */
    t->rowBytes = glyphImage->rowBytes;
    t->baseAddr = NULL;
    ...
    fs_FindBitMapSize4(&bbox, t, glyphImage, dropoutMode);

    if (glyphImage->scanConvertScratchSize) {
         bbox.yLines = tsi_AllocMem(t->mem, glyphImage->scanConvertScratchSize);
    }
      
    if (glyphImage->actualBitmapSize) {
        bbox.bitMap = tsi_AllocMem(t->mem, glyphImage->actualBitmapSize);
    }
    
    if (glyphImage->dropoutScratchSize) {
         bbox.xLines = tsi_AllocMem(t->mem, glyphImage->dropoutScratchSize);
    }
      
    error = fs_ContourScan3(t, &bbox, glyphImage, dropoutMode);
    ...
    if (error == scalerError_NoError) {
                ...
                t->baseAddr = (char *) bbox.bitMap; //FIXME: can we make it char* internally???
                bbox.bitMap = NULL; /* scratch no longer owns it */
    } else {
         /* In this case t->baseAddr = NULL; */
         /* no error, let t2k handle the result. */
         /* note: this pathleads to a release of the memory, immediately following.
               however, the routine itself does not return an error. */
    }
                          
    if (bbox.bitMap != NULL) {
        tsi_DeAllocMem(t->mem, bbox.bitMap);
        bbox.bitMap = NULL;
    }

    if (bbox.xLines != NULL) {
        tsi_DeAllocMem(t->mem, bbox.xLines);
        bbox.xLines = NULL;
    }

    if (bbox.yLines != NULL) {
        tsi_DeAllocMem(t->mem, bbox.yLines);
        bbox.yLines = NULL;
    }
}
