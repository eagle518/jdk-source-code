...
Fixed_p			lMatrixP;
	...
	/* get the clut data */
        if (mft2CLutSize < 0 || srcCLutSize < 0 || mft2CLutSize < srcCLutSize) {
            goto ErrOut0;
        }
...
KpUInt16_t		gData;
	...
	if (MabOffsets.clutOffset) {
		...
		/* get the clut data */
                if (mabCLutSize < 0 || srcCLutSize < 0 || mabCLutSize < srcCLutSize) {
                    goto ErrOut0;
                }
	...
	for (i1 = 0; i1 < nChan; i1++)
	{
		...
		if (CURVE_TYPE_SIG == nSig)
		{
...
#endif
			nTblSize = nTblEntries * sizeof (mab_tbldat_t);	/* size in bytes of each table */
                        if (nTblSize < 0 || nTblSize > sizeof(tmpTbl)) {
                            return (-1);
                        }
			...
			nTblSize = getNumParaParams(PTParaCurve[i1].nFunction);
                        if (nTblSize < 0 || nTblSize >  NUM_PARA_PARAMS) {
                            return (-1);
                        }
