/*
 * @(#)jpegimageencoderimpl.c	1.17 10/03/23
 *
		       ...
		       jint start, jint lineStride)
{
  ...
  if (pack && comp_info.num_components != 3 && comp_info.num_components !=4) {
    JNU_ThrowByName(env, IMAGE_FORMAT_EXCEPTION, "Number of components doesn't match the encoder state");
    jpeg_destroy_compress(&comp_info);
    return;
  }
