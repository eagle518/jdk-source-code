...
{
    SurfaceDataOps *ops = SurfaceData_GetOpsNoSetup(env, sdObject);
    if (ops != NULL) {
        ops->dirty = dirty;
    }
}
...
void
SurfaceData_SetValid(JNIEnv *env, SurfaceDataOps *ops, jboolean valid)
{
    jobject sdObject;
    ...
    (*env)->SetBooleanField(env, sdObject, validID, valid);
    (*env)->DeleteLocalRef(env, sdObject);
}
