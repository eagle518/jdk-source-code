...
ZFILE_Open(const char *fname, int flags) {
    ...
    return (jlong) CreateFile(
        ...
        NULL);
#else
    return ZIP_AdjustFD(JVM_Open(fname, flags, 0));
#endif
}
...
{
    jint i;
    const jint new_metacount = 2 * zip->metacount;
    zip->metanames =
...
/*
 * Returns a file descriptor to be used in place of that given.  The returned
 * fd is marked for close-on-exec.  On solaris systems which do not support
 * enable_extended_FILE_stdio (those prior to s10u4), the returned file
 * descriptor value  is > 255.
 *
 * In case of any errors, returns the given fd.
 */
int
ZIP_AdjustFD(int fd) {
    if (fd < 0) return fd;
    ...
    /* On releases of Solaris prior to s10u4, we want to re-map file
     * descriptors to be greater than 256, so that those less than 256 are
     * available for fopen().
     */
    if (fd < 256) {
...
#ifdef FD_CLOEXEC
    /* Sub-processes shouldn't need jar/zip files from this process. */
    {
