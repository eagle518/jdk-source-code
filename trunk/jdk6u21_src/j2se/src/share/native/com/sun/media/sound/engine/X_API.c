...
XPTR XNewPtr(INT32 size)
{
    ...
    if (size < 0) // ensure we have no INT32 overflow
    {
        return NULL;
    }
#if USE_HAE_EXTERNAL_API == TRUE
...
#if USE_HAE_EXTERNAL_API == TRUE
        {
            // ensure XFILENAME::theFile has enough space for the filename
            char *str = (char *)file;
            short int size = FILE_NAME_LENGTH;
            while (*str && size > 0) {
                str++;
                size--;
            }
            if (size <= 0) {
                // no enough space for the string or final '0'
                return;
            }
        }
	    {
