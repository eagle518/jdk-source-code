...
SetLayeredWindowAttributesType fn_set_layered_window_attributes;
UpdateLayeredWindowType fn_update_layered_window;
...
load_user_procs()
{
    ...
    fn_set_layered_window_attributes = 
        (SetLayeredWindowAttributesType)GetProcAddress(lib, 
                "SetLayeredWindowAttributes");
    fn_update_layered_window = (UpdateLayeredWindowType)GetProcAddress(lib, 
            "UpdateLayeredWindow");
    initialized = 1;
}
...
DwmIsCompositionEnabledType fn_dwm_is_composition_enabled = NULL;
DwmGetWindowAttributeType fn_dwm_get_window_attribute = NULL;

void load_dwmapi_procs()
{
    static int initialized = 0;
    if (initialized) {
        return;
    ...
    // dwmapi.dll is known to appear in Vista.
    if (!IS_WINVISTA) {
        initialized = -1;
        return;
    }

    HMODULE lib = ::LoadLibrary(TEXT("DWMAPI.DLL"));
    if (lib == NULL) {
        initialized = -2;
        return;
    }

    fn_dwm_is_composition_enabled = (DwmIsCompositionEnabledType)
        GetProcAddress(lib, "DwmIsCompositionEnabled");
    fn_dwm_get_window_attribute = (DwmGetWindowAttributeType)
