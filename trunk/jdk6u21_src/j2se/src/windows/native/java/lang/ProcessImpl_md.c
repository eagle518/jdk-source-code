/*
 * @(#)ProcessImpl_md.c	1.24 10/03/23
 *
...
static void
throwIOException(JNIEnv *env, const char *defaultDetail)
{
    static const char * const format = "CreateProcess error=%d, %s";
    char buf[256];
    char errmsg[sizeof(buf) + 100];
    int errnum = GetLastError();
    int n = JVM_GetLastErrorString(buf, sizeof(buf));
    sprintf(errmsg, format, errnum, (n > 0) ? buf : defaultDetail);
				  ...
				  jobject err_fd)
{
    ...
    if (!ret) {
	throwIOException(env, "CreateProcess failed");
	goto Catch;
    }
