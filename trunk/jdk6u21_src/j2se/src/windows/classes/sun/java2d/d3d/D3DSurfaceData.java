/*
 * @(#)D3DSurfaceData.java	1.20 10/03/23
 *
...
import static java.awt.BufferCapabilities.*;
...
public class D3DSurfaceData extends SurfaceData implements AccelSurface {
    ...
    /**
     * Permanently disable acceleration for this surface.
     * The default implementation flags to the manager that it should no
     * longer attempt to re-create a D3DSurfaceData.
    ...
    public static class D3DWindowSurfaceData extends D3DSurfaceData {
        ...
        @Override
        void disableAccelerationForSurface() {
            /* For on-screen surfaces we need to make sure a backup GDI
             * surface is used until a new one is set (which may happen during
             * a resize). We don't want the screen update manager to replace
             * the surface right away because it causes repainting issues in
             * Swing, so we invalidate it, this will prevent SUM from issuing
             * a replaceSurfaceData call.
             */
            setSurfaceLost(true);
            invalidate();
            flush();
