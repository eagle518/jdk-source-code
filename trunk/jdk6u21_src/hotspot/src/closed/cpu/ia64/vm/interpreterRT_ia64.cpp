...
void InterpreterRuntime::SignatureHandlerGenerator::pass_prev(int slot_offset) {
  ...
  // Adjust for local value (for TaggedStackInterpreter)
  __ add(GR3_SCRATCH, GR_I0, Interpreter::value_offset_in_bytes());
  // Point to next GR_Olocal to next local
  if (slot_offset != 0) {
  ...
  if (Argument::is_integral(sig)) {
    ...
    if (jni_arg.is_register()) {
      if (Argument::is_4byte(sig)) {
        ...
        __ ldfs(tmp, GR3_SCRATCH);
      } else {
        // Double precision float
        __ ldfd(tmp, GR3_SCRATCH);
      }
    } else {
      __ ld8(GR2_SCRATCH, GR3_SCRATCH);
      __ add(GR3_SCRATCH, SP, jni_arg.jni_offset_in_frame());
...
void InterpreterRuntime::SignatureHandlerGenerator::pass_curr(Argument::Sig sig) {
  if ((sig == Argument::long_sig || sig == Argument::double_sig) &&
    ...
    __ sub(GR_I0, GR_I0, Interpreter::stackElementSize());
  }
  _prev_sig        = sig;
...
void InterpreterRuntime::SignatureHandlerGenerator::pass_long() {
  pass_prev(Interpreter::stackElementSize());
...
IRT_ENTRY(address, InterpreterRuntime::slow_signature_handler(
  ...
  assert(!TaggedStackInterpreter, "this won't work");
  ...
  SlowSignatureHandler(m, from, m->is_static() ? to+2 : to+1, to).iterate(UCONST64(-1));
  // return result handler
