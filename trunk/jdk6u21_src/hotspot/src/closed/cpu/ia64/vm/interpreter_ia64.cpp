...
address AbstractInterpreterGenerator::generate_tosca_to_stack_converter(BasicType type) {
  ...
  case T_BOOLEAN:
    ...
    __ push_tag(tos, frame::TagValue);
    __ st4(tos, res);
    break;
  case T_BYTE:
  case T_CHAR:
  ...
  case T_OBJECT:
//  __ verify_oop(GR_RET);
    __ push_tag(tos, frame::TagReference);
    __ st8(tos, GR_RET);
    break;
  case T_FLOAT:
    __ push_tag(tos, frame::TagValue);
  ...
  __ sub(GR_RET, tos, type == T_VOID ? 0 : Interpreter::stackElementSize());
...
//  __ verify_oop(res);
    ...
    break;
  case T_VOID:
    __ push_tag(tos, frame::TagValue);  // add space for the tag to align stack
    break;
  default:
    ShouldNotReachHere();
  }
...
void InterpreterGenerator::generate_counter_incr(Label& overflow) {
  ...
  // Must preserve these predicates
...
//
void InterpreterGenerator::generate_counter_overflow(Label& continue_entry) {
  ...
  __ mov_to_pr(GR_Lscratch0);

  // Returns verified_entry_point or NULL, we don't care which as we ignore it and run interpreted

  __ br(continue_entry);
}
...
void InterpreterGenerator::generate_compute_interpreter_state(Label& stack_overflow_return) {
  ...
  //   GR_Lsave_SP    - intptr_t*     previous SP (does not include any extension by possible c2i)
  //
  // On exit: same plus
  ...
  const Register state_offset      = GR25_SCRATCH;
  ...
  __ mov(state_offset, round_to(2*Interpreter::stackElementSize() + sizeof(cInterpreter), BytesPerWord));
  ...
  __ shladd(GR_Ilocals, parameter_count, Interpreter::logStackElementSize(), GR_Itos);

  // Because Itos points two past value on tos, need to add 1 word for tag
  if (TaggedStackInterpreter) __ add(GR_Ilocals, GR_Ilocals, BytesPerWord);
  ...
  __ add(is_native, max_stack, parameter_count, 2*Interpreter::stackElementWords());
  ...
  __ shl(max_stack, max_stack, Interpreter::logStackElementSize());  // max_stack is now in bytes
  ...
  __ shladd(state_offset, local_count, Interpreter::logStackElementSize(), state_offset);
  ...
  __ sub(GR_Lstate, SP, state_offset);               // Set interpreter state address, though it isn't yet allocated
...
//  const Register sender_sp_addr        = GR29_SCRATCH;
  ...
  const Register locals_addr           = GR24_SCRATCH;
  ...
  const Register callee_method_addr    = GR24_SCRATCH;
  ...
  //
  const Register last_Java_pc_addr     = GR24_SCRATCH;
  ...
  __ add(native_mirror_addr, state_(_oop_temp));
  ...
  __ st8(native_mirror_addr, GR0);                  // state->_oop_temp = NULL;
  ...
  __ sub(tos, stack_base, Interpreter::stackElementSize()); // tos = stack_base - 1;
  ...
  const Register lockee_addr      = GR31_SCRATCH;
  ...
  __ add(is_dynamic, lockee_addr, GR_Ilocals, Interpreter::value_offset_in_bytes());
  __ ld8(is_dynamic, lockee, lockee_addr);          // else
                                                    //   lockee = *(oop*)locals;
  ...
  if (true /* zerolocals */ || ClearInterpreterLocals) {
    ...
    __ shl(parameter_count, parameter_count, Interpreter::logStackElementSize());
    __ sub(local_addr, GR_Ilocals, parameter_count);

    __ mov(AR_LC, local_count);  // local count is in words
    __ br(no_locals_to_zero, locals_zeroed, Assembler::dpnt);

    Label zero_slot;
    __ bind(zero_slot);

    if (TaggedStackInterpreter) __ st8(local_addr, GR0, -BytesPerWord);
...
address InterpreterGenerator::generate_interpreter_frame_manager(void) {
  ...
  // Now override the saved SP with the senderSP so we can
  // pop c2i arguments (if any) off when we return

  __ mov(GR_Lsave_SP, GR28_sender_SP);
  ...
  const PredicateRegister do_OSR                = PR12_SCRATCH;
  ...
  __ cmp4(do_OSR,                PR0, cInterpreter::do_osr,             msg, Assembler::equal);
  __ br(do_return_from_method, return_from_interpreted_method);

  __ cmp4(do_more_monitors,      PR0, cInterpreter::more_monitors,      msg, Assembler::equal);
...
// that there is an exception to process.
  ...
  __ shladd(tos, parameter_count, Interpreter::logStackElementSize(), tos);
                                              // New tos = old tos + size of callee parameters
  // Because Itos points two past value on tos, need to add 1 word for tag
  if (TaggedStackInterpreter) __ add(tos, tos, BytesPerWord);
  ...
  __ ld8(stub_addr, stub_addr);               // Stub entry address
  ...
  __ add(GR_O0, GR_O0, Interpreter::stackElementSize()); // Bias callee's tos for stub
  // Address of stub descriptor address
  __ shladd(stub_addr, result_index, LogBytesPerOop, stub_addr);
  ...
  __ add(GR_O0, GR_O0, Interpreter::stackElementSize());
  ...
  __ sub(new_sp, GR_Otos, Interpreter::stackElementSize());
  ...
  // ??? Point to tag or value???
  if (TaggedStackInterpreter) __ sub(GR_RET, GR_RET, BytesPerWord);
...
//__ verify_oop(GR27_method);
  ...
  const Register result_handler = GR_Lscratch0;

  __ mov(result_handler, GR_RET);             // Save across call to native method


  // Set up fixed parameters and call the native method.
  // If the method is static, get mirror into GR_O1.

  const PredicateRegister not_static = PR14_SCRATCH;
  ...
  __ tbit(is_static, not_static, access_flags, JVM_ACC_STATIC_BIT, Assembler::notEqual);
  __ add(pool_holder_addr, constants, constantPoolOopDesc::pool_holder_offset_in_bytes());

  __ add(is_static, GR_O1, state_(_oop_temp));      // if (method->is_static()) {
                                                    //   GR_O1 = &state->_oop_temp;
  ...
  __ st8(is_static, GR_O1, mirror);                 //   state->_oop_temp = mirror;
  __ st8(not_static, GR0, mirror);                  //   state->_oop_temp = NULL;
                                                    // }
  ...
  if (UseMembar) {
    ...
    __ serialize_memory(GR4_thread, GR2_SCRATCH, GR3_SCRATCH);
  }
  ...
  address target = CAST_FROM_FN_PTR(FuncDesc*, JavaThread::check_special_condition_for_native_trans)->entry();
  address target_gp = CAST_FROM_FN_PTR(FuncDesc*, JavaThread::check_special_condition_for_native_trans)->gp();
  ...
  __ mov(GR_I0, result_handler);             // pass result handler across frame pop
  __ pop_dummy_full_frame();
  __ mov(GP, GR_Lsave_GP);

  __ mov(result_handler, GR_O0);             // put result handler in expected local
  ...
  __ mov(result_handler_br, result_handler);
...
int AbstractInterpreter::size_top_interpreter_activation(methodOop method) {
  ...
  const int locals_size = method->max_locals() * Interpreter::stackElementSize();
  ...
  const int full_frame_size = round_to(short_frame_size + method->max_stack() * Interpreter::stackElementSize(), 16);
                                           ...
                                           )
{
  ...
  *current->register_addr(GR_Iprev_state) = (intptr_t) 0;
  ...
  *current->register_addr(GR_Itos) = (intptr_t) (locals - method->size_of_parameters());
  ...
  //
  // verify that the stack limit (which is first word not part of the stack)
  // is at worst 1 word short of the frame bottom. This will ensure that there
  // are 2 words available for  the abi scratch area and if they don't exist
  // then InterpretMethod can overwrite the expression stack.
  assert(to_fill->_stack_limit >= frame_bottom + 1, "bad frame setup");
  assert(!is_top_frame || current->sp() == frame_bottom, "bad frame bottom");
...
int AbstractInterpreter::layout_activation(methodOop method,
  ...
  int extra_locals_size = (method->max_locals() - method->size_of_parameters()) * Interpreter::stackElementSize();
  int monitor_size = sizeof(BasicObjectLock) * moncount;

  int short_frame_size = size_activation_helper(extra_locals_size,
                                                monitor_size);

  int full_frame_size = round_to(short_frame_size +
            method->max_stack() * Interpreter::stackElementSize(), 16);

  // Adjust temp to add in tag slots, popframe args already has tags
  int temp_slots = tempcount*Interpreter::stackElementWords() + popframe_extra_args;
  short_frame_size = round_to(short_frame_size + temp_slots * BytesPerWord, 16);
  ...
  */
  if (interpreter_frame != NULL) {
    ...
    } else if (caller->is_entry_frame()) {
      // Entry frame has interpreted layout so args are already in place and local begin
      // in its frame
      locals = (intptr_t*) ((intptr_t)caller->sp() + ((method->size_of_parameters() - 1) * BytesPerWord));
    } else {
      locals = (intptr_t*) ((intptr_t)caller->sp() - (1 * BytesPerWord));
    }

    interpreterState cur_state = (interpreterState) ((intptr_t)(locals - (method->max_locals() - 1)) - sizeof(cInterpreter));
    // END MUCHO HACK

    intptr_t* monitor_base = (intptr_t*) cur_state;
    intptr_t* stack_base = (intptr_t*) ((intptr_t) monitor_base - monitor_size);
    /* +1 because stack is always prepushed */
    intptr_t* stack = (intptr_t*) ((intptr_t) stack_base - (temp_slots + 1) * BytesPerWord);
...
//    assert(*interpreter_frame->register_addr(GR_Lsave_caller_BSP) == (intptr_t) interpreter_frame->fp(), "Ack");
    ...
    */
#ifdef ASSERT
    /* would be nice to do some double checking of the layout here */
#endif // ASSERT
...
static char* C_msg(cInterpreter::messages msg) {
  switch (msg) {
     ...
     case cInterpreter::do_osr:  return("do_osr");
     // deopt
...
cInterpreter::print() {
  ...
  tty->print_cr("oop temp (native mirror): " INTPTR_FORMAT, (uintptr_t) this->_oop_temp);
