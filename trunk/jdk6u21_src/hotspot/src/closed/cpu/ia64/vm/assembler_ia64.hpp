...
//
const Register GR28_sender_SP      = GR28;
...
public:
  ...
  static uint41_t u_field(uint41_t x, size_t pos, size_t count) {
    assert((x & ~right_n_bits((int)count)) == 0, "value out of range");
    return x << pos;
  }
#else
  ...
  static int64_t inv_s_field_at(uint64_t x, size_t pos, size_t count, size_t dst_pos) {
    return (((int64_t)x << (64 - count - pos)) >> (64 - count - dst_pos)) & ~right_n_bits((int)dst_pos);
  }

  static uint64_t fmask(size_t pos, size_t count) {
    return right_n_bits((int)count) << pos;
  }
...
public:
  ...
  /// These shadow corresponding methods in AbstractAssembler.
  /// They manage the platform-specific CodeBuffer::_current_section field.
  address    start_a_stub(int required_space);
  void       end_a_stub();
  address    start_handler(int required_space);
  void       end_handler();
  address    start_a_const(int required_space, int required_align);
  void       end_a_const();
...
#define Cmp_Relation_Reverse \
  ...
  static void verify_signed_range(intptr_t imm, size_t size) {
    assert(imm >= -nth_bit((int)size-1) && imm < nth_bit((int)size-1), "signed immediate out of range");
  }

  static void verify_unsigned_range(uintptr_t imm, size_t size) {
    assert((imm & right_n_bits((int)size)) == imm, "unsigned immediate out of range");
  }
  ...
  static bool in_signed_range(intptr_t imm, size_t size) {
    return imm >= -right_n_bits((int)size-1)-1 && imm <= right_n_bits((int)size-1);
  }
...
class MacroAssembler: public Assembler {
 ...
 public:
  ...
  // Debugging
  // unimplemented since c++ interpreter does a decent job
  void verify_oop(Register reg, const char* s = "broken oop");             // only if +VerifyOops
  void verify_oop_addr(Address addr, const char* s = "broken oop");        // only if +VerifyOops
