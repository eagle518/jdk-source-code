/*
 * @(#)awt_InputMethod.c	1.70 04/07/13
 *
...
static void onoffStatusWindow(StatusWindow *statusWindow, 
#ifdef XAWT
				Window parent, 
#else
				Widget parent, 
...
static void CommitStringCallback(XIC ic, XPointer client_data, XPointer call_data) {
    ...
    X11InputMethodData *pX11IMData = getX11InputMethodData(env, (jobject)client_data);
    jstring javastr;
					      ...
					      jboolean active)
{
	...
	if (active && pX11IMData->statusWindow && pX11IMData->statusWindow->on)
  	    onoffStatusWindow(pX11IMData->statusWindow, w, True);
#endif
    } else {
 ...
 * Signature: (ZJ)V
 */
JNIEXPORT void JNICALL Java_sun_awt_X11InputMethod_setCompositionEnabledNative
