...
// Returns value of focusableWindowState field of this Java Window
// winObject - Java Window object, shouldn't be NULL
Boolean isFocusableWindowByObject(JNIEnv * env, jobject winObject) {
    return (*env)->GetBooleanField(env, winObject, windowIDs.focusableWindowState);    
}
...
Boolean isFocusableWindowByShell(JNIEnv* env, Widget shell) {
    ...
    target = (*env)->GetObjectField(env, peer,
                                    mComponentPeerIDs.target);
    DASSERT(target != NULL);
    
    focusable = isFocusableWindowByObject(env, target);
...
shellEH(Widget w, XtPointer data, XEvent *event, Boolean *continueToDispatch)
{
    ...
    case FocusOut: {
        ...
        XEvent inEvent;
#ifdef DEBUG_FOCUS
        printf("Focusout on shell; window = %d\n", event->xfocus.window);
#endif
        // Fix for 4514547 - check for grab on top-level
        if (event->xfocus.mode == NotifyGrab) {
            // Generate COMPONENT_MOVED event to indicate user has grabbed the window
            // This will cause Swing to close its popups
            JNU_CallMethodByName(env, NULL,
                                 this, "handleMoved", "(II)V",
                                 0, 0); // Coordinates are not used inside of handleMoved
            if ((*env)->ExceptionOccurred(env)) {
                (*env)->ExceptionDescribe(env);
                (*env)->ExceptionClear(env);
            }            
        ...
        if (oppositeShell != NULL 
            ...
            && isFocusableWindowByShell(env, w) && w != oppositeShell)
        {
        ...
        if (!awt_isModal() || awt_isWidgetModal(w)) {
            ...
            if ( oppositeShell != NULL 
                 && isFocusableWindowByShell(env, oppositeShell) 
                 && isFocusableWindowByShell(env, w) 
                 || (oppositeShell == NULL)) 
            {
    ...
    case FocusIn: {
#ifdef DEBUG_FOCUS
        printf("FocusIn on shell; window = %d\n", event->xfocus.window);
#endif
        if (/*  event->xfocus.mode == NotifyNormal */ 1) {
            ...
            if (!awt_isModal() || awt_isWidgetModal(w)) {
#endif //NOMODALFIX
                if (isFocusableWindowByShell(env, w)) {                    
...
#define MAX_ARGC 50
    ...
    if (wdata->initialFocus == False || !isFocusableWindowByObject(env, target)) {
        ...
        */
        ...
        if ( (cname != NULL && strcmp(cname, "###overrideRedirect###") == 0) 
            || (!isFrameOrDialog(target, env) 
                && !isFocusableWindowByObject(env, target)
                ) 
            )
        {    /* mbron */
            XtSetArg(args[argc], XmNoverrideRedirect, True);
    ...
    if (!JNU_IsNull(env, warningString) ) {
        ...
        XtVaSetValues(wdata->warningWindow,
                      ...
                      NULL);
#ifdef _pauly_debug
        fprintf(stdout,"    create. warning window\n");
        fflush(stdout);
#endif /* _pauly_debug */
...
#ifdef _pauly_debug
        fprintf(stdout,"    create. top offset: %d, bottom offset: %d\n",
                wdata->top, wdata->bottom);
        fflush(stdout);
#endif /* _pauly_debug */
    } else {
...
#ifdef _pauly_debug    
    fprintf(stdout,"    create. shell: %x, form: %x, inner canvas: %x, outer canvas: %x\n",
            wdata->winData.shell, wdata->mainWindow, innerCanvasW, wdata->winData.comp.widget);
    ...
    jboolean isModal)
{
    ...
    if (isFocusable) {
        XSetInputFocus(awt_display, XtWindow(wdata->winData.shell), RevertToParent, CurrentTime);
    }
...
/*
 * Class:     sun_awt_motif_MWindowPeer
 * Method:    doUpdateAlwaysOnTop
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_sun_awt_motif_MWindowPeer_doUpdateAlwaysOnTop
