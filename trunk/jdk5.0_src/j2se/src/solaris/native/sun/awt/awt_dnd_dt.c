...
dt_postDropTargetEvent(JNIEnv* env, jobject component, int x, int y, 
    ...
    {
	XClientMessageEvent* copy = NULL;

	if (event != NULL) {
	    copy = (XClientMessageEvent*)malloc(sizeof(XClientMessageEvent));
...
struct EmbeddedDropSiteListEntryRec {
    ...
    Window root;
    Window motif_proxy;
    /* True if the toplevel was already registered as a Motif drag receiver and
       we just changed the proxy. False, otherwise */
    Boolean motif_override;
    Window xdnd_proxy;
    /* 
     ...
     */
    unsigned int xdnd_protocol_version;
    /* True if the toplevel was already registered as an XDnD drag receiver and
       we just changed the proxy. False, otherwise */
    Boolean xdnd_override;
    /*
...
awt_dnd_dt_init_proxy(Display* dpy, Window root, Window toplevel, Window window) {
	...
	ProxyRegistrationStatus xdnd_status = RegFailure;
	...
	if (register_xdnd) {
	    xdnd_status = set_xdnd_proxy(dpy, toplevel, awt_root_window, &xdnd_proxy,
	...
	entry->root = root;
	entry->motif_proxy = motif_proxy;
	entry->motif_override = motif_override;
	entry->xdnd_proxy = xdnd_proxy;
	entry->xdnd_protocol_version = xdnd_protocol_version;
	entry->xdnd_override = xdnd_override;
 ...
 * Otherwise, it checks if the 'toplevel' is a registered drop site for adds (window, component) pair to the list and returns True 
...
remove_from_embedded_drop_site_list(Display* dpy, Window toplevel, Window window) {
	...
	if (entry->toplevel == toplevel) {
		...
		if (entry->embedded_sites[idx] == window) {
			...
			if (entry->xdnd_override == True) {
			    XChangeProperty(dpy, toplevel, XA_XdndAware, XA_ATOM,
					    32, PropModeReplace, 
					    (unsigned char*)&entry->xdnd_protocol_version, 
					    1);   

			    XChangeProperty(dpy, toplevel, XA_XdndProxy, XA_WINDOW,
					    32, PropModeReplace, 
					    (unsigned char*)&entry->xdnd_proxy, 1);   
			} else {
			    XDeleteProperty(dpy, toplevel, XA_XdndAware);   
			    XDeleteProperty(dpy, toplevel, XA_XdndProxy);
			}

			if (entry->motif_override == True) {
			    /* Request status */
			    int status;
				...
				if (MOTIF_BYTE_ORDER == byte_order) {
				    ...
				    write_card32(&p, entry->motif_proxy);
...
set_xdnd_proxy_for_toplevel(Window toplevel, Window proxy_window, 
    ...
    entry->xdnd_proxy = proxy_window;
    entry->xdnd_protocol_version = version;
    entry->xdnd_override = override;
...
set_motif_proxy_for_toplevel(Window toplevel, Window proxy_window, Boolean override) {
    ...
    entry->motif_proxy = proxy_window;
    entry->motif_override = override;
...
forward_client_message_to_toplevel(Window toplevel, XClientMessageEvent* event) {
	...
	if (!entry->motif_override) {
	    return False;
	}
	proxy = entry->motif_proxy;
    } else {
	/* Assume XDnD */
	if (!entry->xdnd_override) {
	    return False;
	}
	proxy = entry->xdnd_proxy;
	{
	    /* Adjust the event to match the XDnD protocol version. */
	    unsigned int version = entry->xdnd_protocol_version;
