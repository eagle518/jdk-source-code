/*
 * @(#)X11SurfaceData.c	1.57 04/05/11
 *
...
static GetGCFunc X11SD_GetGC;
static ReleaseGCFunc X11SD_ReleaseGC;
...
static void X11SD_InitClip(JNIEnv *env, X11SDOps *xsdo, GC xgc, jobject clip);
static void X11SD_InitCompAndPixel(JNIEnv *env, X11SDOps *xsdo, GC xgc,
                                   jobject comp, jint pixel);
 ...
 */
JNIEXPORT jboolean JNICALL
Java_sun_awt_X11SurfaceData_isDgaAvailable(JNIEnv *env, jobject this) 
{
 ...
 */
JNIEXPORT void JNICALL
Java_sun_awt_X11SurfaceData_initOps(JNIEnv *env, jobject xsd,
				    jobject peer, 
				    jobject graphicsConfig, jint depth)
{
    ...
    xsdo->sdOps.dirty = JNI_FALSE;
    xsdo->GetGC = X11SD_GetGC;
    xsdo->ReleaseGC = X11SD_ReleaseGC;
 ...
 */
JNIEXPORT void JNICALL
Java_sun_awt_X11SurfaceData_initOps2(JNIEnv *env, jobject xsd,
				    jlong drawable, 
				    jobject graphicsConfig, jint depth)
{
#ifndef HEADLESS
...
X11SD_Dispose(JNIEnv *env, SurfaceDataOps *ops)
{
    ...
    if (xsdo->lastclip != NULL) {
	(*env)->DeleteWeakGlobalRef(env, xsdo->lastclip);
    }
    if (xsdo->lastcomp != NULL) {
        (*env)->DeleteWeakGlobalRef(env, xsdo->lastcomp);
    }
    if (xsdo->gc != NULL) {
	XFreeGC(awt_display, xsdo->gc);
    }
    AWT_UNLOCK();
#endif /* !HEADLESS */
}
 ...
 */
JNIEXPORT void JNICALL
Java_sun_awt_X11SurfaceData_setInvalid(JNIEnv *env, jobject xsd)
{
#ifndef HEADLESS
 ...
 */
JNIEXPORT void JNICALL
Java_sun_awt_X11SurfaceData_initSurface(JNIEnv *env, jclass xsd,
					jint depth, 
					jint width, jint height, 
					jlong drawable, jint bitmap)
{
#ifndef HEADLESS
	...
	if (bitmap != 0) {
	    xsdo->bitmask = (Pixmap)(unsigned int)bitmap;
 	}
...
shminfo->shmid = shmget(IPC_PRIVATE, height * img->bytes_per_line, IPC_CREAT|0777);
    if (shminfo->shmid < 0) {
#ifdef DEBUG
	perror("X11SD_SetupSharedSegment shmget has failed");
#endif
	return NULL;
    }
...
#ifdef DEBUG
	perror("X11SD_SetupSharedSegment shmat has failed");
#endif
	return NULL;
    }
...
#ifdef DEBUG
        perror("X11SD_SetupSharedSegment XShmAttach has failed");
#endif
        return NULL;
    }
...
#ifdef DEBUG
	fprintf(stderr,"X11SD_InitWindow: widget is unrealized\n");
#endif
		       ...
		       jint lockflags)
{
	...
	xsdo->sdOps.dirty != JNI_TRUE) {
	SurfaceData_MarkDirty(env, &xsdo->sdOps);
    }
    return ret;
    /* AWT_UNLOCK() called in ReleaseRasData */
}
			 ...
			 SurfaceDataRasInfo *pRasInfo)
{
	...
	if (xpriv->lockFlags & SD_LOCK_WRITE) {
	    ...
	    GC xgc = xsdo->gc;
	    if (xgc == NULL) {
		xsdo->gc = xgc = XCreateGC(awt_display, drawable, 0L, NULL);
	    }
	    X11SD_InitClip(env, xsdo, xgc, NULL);
		...
		if (xsdo->depth > 16) {
            ...
            if (xsdo->lastcomp != NULL) {
                /* temporarily use copy mode for the XPutImage */
...
static GC
X11SD_GetGC(JNIEnv *env, X11SDOps *xsdo,
            jobject clip, jobject comp, jint pixel)
{
    GC xgc;
    ...
    xgc = xsdo->gc;
    if (xgc == NULL) {
	xsdo->gc = xgc = XCreateGC(awt_display, xsdo->drawable, 0L, NULL);
	xsdo->lastpixel = ~pixel;
    }

    X11SD_InitClip(env, xsdo, xgc, clip);
    X11SD_InitCompAndPixel(env, xsdo, xgc, comp, pixel);

    if (xsdo->sdOps.dirty != JNI_TRUE) {
	SurfaceData_MarkDirty(env, &xsdo->sdOps);
    }
    return xgc;
}
...
static void
X11SD_InitClip(JNIEnv *env, X11SDOps *xsdo, GC xgc, jobject clip)
{
    if (clip == NULL) {
	if (xsdo->lastclip != NULL) {
    ...
    } else if (!(*env)->IsSameObject(env, clip, xsdo->lastclip)) {
	RegionData clipInfo;
	...
	/* REMIND: Optimize case where clip is larger than window? */
	XSetClipRectangles(awt_display, xgc, 0, 0, pRect, numrects, YXBanded);
	if (xsdo->lastclip != NULL) {
	    (*env)->DeleteWeakGlobalRef(env, xsdo->lastclip);
	}
	xsdo->lastclip = (*env)->NewWeakGlobalRef(env, clip);
	if (pRect != rects) {
	    free(pRect);
	}
    }
...
static void
X11SD_InitCompAndPixel(JNIEnv *env, X11SDOps *xsdo, GC xgc,
                       jobject comp, jint pixel)
{
    if ((comp == NULL) || !(*env)->IsInstanceOf(env, comp, xorCompClass)) {
        if (xsdo->lastcomp != NULL) {
            XSetFunction(awt_display, xgc, GXcopy);
            (*env)->DeleteWeakGlobalRef(env, xsdo->lastcomp);
            xsdo->lastcomp = NULL;
        ...
        if (!(*env)->IsSameObject(env, comp, xsdo->lastcomp)) {
            CompositeInfo compInfo;
            GrPrim_CompGetXorInfo(env, &compInfo, comp);
            xsdo->lastxorpixel = compInfo.details.xorPixel;
            XSetFunction(awt_display, xgc, GXxor);
            if (xsdo->lastcomp != NULL) {
                (*env)->DeleteWeakGlobalRef(env, xsdo->lastcomp);
            }
            xsdo->lastcomp = (*env)->NewWeakGlobalRef(env, comp);
        }
        pixel ^= xsdo->lastxorpixel;
    }

    if (pixel != xsdo->lastpixel) {
	XSetForeground(awt_display, xgc, pixel & xsdo->pixelmask);
	xsdo->lastpixel = pixel;
    }
}

static void
X11SD_ReleaseGC(JNIEnv *env, X11SDOps *xsdo, GC xgc)
{
    /* the background pixel is not valid anymore */
    xsdo->bgPixel = 0;
#ifdef MITSHM
