/*
 * @(#)XComponentPeer.java	1.105 04/01/21
 *
...
public class XComponentPeer extends XWindow implements ComponentPeer, DropTargetPeer, XConstants {
    ...
    public void focusGained(FocusEvent e) {
        ...
        XKeyboardFocusManagerPeer.setCurrentNativeFocusOwner(target);
        bHasFocus = true;
    }
                                      ...
                                      boolean focusedWindowChangeAllowed, long time)
    {
        ...
        switch (result) {
          ...
          case SNFH_SUCCESS_PROCEED:
              ...
              // by listeners.
              return XKeyboardFocusManagerPeer.simulateMotifRequestFocus(lightweightChild, target, temporary, focusedWindowChangeAllowed, time);
    ...
    XWindowPeer getParentWindow() {
        ...
        if (parent != null) {                
            return (XWindowPeer)((Window)parent).getPeer();
        } else {
            return null;
        }        
    }
    ...
    void handleJavaMouseEvent(MouseEvent e) {
        switch (e.getID()) {
          case MouseEvent.MOUSE_PRESSED:              
              ...
              {
                  XWindowPeer parentXWindow = getParentWindow();
                  ...
                  // by ourselfves.
//                   if (parentXWindow.isFocusableWindow() /*&& parentXWindow.isSimpleWindow()*/ && !(getCurrentNativeFocusedWindow() == parentWindow)) {
...
//                   }
    ...
    // Copied from Component, this needs to be duplicated here because we dont want to call client code.
    static java.util.Hashtable metrics = ComponentAccessor.getMetrics();
    ...
    public void cancelPendingPaint(int x, int y, int w, int h) {
        if (target instanceof Container) {
            Insets insets = ((Container)target).getInsets();
            paintArea.constrain(insets.left, insets.top,
                    ...
                    if (qt.execute() != 0) {
                        if (qt.get_nchildren() != 0) {
                            ...
                            for (int i = 0; i < qt.get_nchildren(); i++) {
                                Long w = new Long(Native.getLong(pchildren, i));
                                if (!set.contains(w)) {
                                    set.add(w);
                                    order.add(j++, w);
                                }
    ...
    private void addTree(Collection order, Set set, Container cont) {
        for (int i = 0; i < cont.getComponentCount(); i++) {
            ...
            if (peer instanceof XComponentPeer) {
                Long window = new Long(((XComponentPeer)peer).getWindow());
