...
Java_com_sun_security_auth_module_NTSystem_getCurrent
    ...
    // primary group must be part of supplementary groups
    // -  if supplementary groups contains more than just
    //    the primary group, then populate NTSystem's groupIDs array
    if (numGroups > 1) {
