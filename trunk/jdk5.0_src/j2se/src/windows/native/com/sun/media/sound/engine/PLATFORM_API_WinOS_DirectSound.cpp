/*
 * @(#)PLATFORM_API_WinOS_DirectSound.cpp	1.11 04/07/08
 *
...
/*
**	Overview:
**	This file provides platform specific DirectAudio implementation for
**      Windows using DirectSound 5 or later.
...
int DAUDIO_Start(void* id, int isSource) {
    ...
    if (info->isSource)  {
	    ...
	    if (info->writePos >= 0) {
		res = info->playBuffer->Play(0, 0, DSBPLAY_LOOPING);
	...
	res = info->captureBuffer->Start(DSCBSTART_LOOPING);
    }
    if (FAILED(res)) {
...
/* First, it calculates the number of bytes in the buffer that are currently
 * invalid data. If that equals info->dsBufferSizeInBytes,
 * then a buffer underrun occured.
 * This method uses the same calculation as in DS_clearBuffer.
...
void DS_CheckUnderrun(DS_Info* info, DWORD playCursor, DWORD writeCursor) {
    ...
    TRACE3("DS_CheckUnderrun: start=%d, end=%d, count=%d\n",
           (int) start, (int) end, (int) count);
    if (count <= 0) {
	/* underrun */
	ERROR0("DS_CheckUnderrun: ERROR: detected underrun!\n");
	info->underrun = TRUE;
    }
}


/* available returns bufferSize - (writePos - writeCursor)
 * where bufferSize is not the DirectSound buffer, but the
 * buffersize as specified by Java
 * if fromPlayCursor==TRUE, the available bytes may be used
 * for estimating the position. Otherwise it returns
 * how many bytes can be written to the buffer
 *
 * For capture, playCursor is "CapturePosition", and writeCursor is "readPosition"
 *
 * returns in bufferSize the relevant buffersize (i.e. if playing, and
 * fromPlayCursor=TRUE, it is bufferSizeInBytes+(writeCursor-playCursor)
 */
int DS_GetAvailable(DS_Info* info,
	...
	*bufferSize = info->bufferSizeInBytes;
	if (fromPlayCursor) {
		...
		*bufferSize += (int) ((*playCursor + info->dsBufferSizeInBytes) - *writeCursor);
	    } else {
		*bufferSize += (int) (*writeCursor - *playCursor);
	    }
	}
	...
	if (info->writePos == -1 || (info->underrun && !fromPlayCursor)) {
	    ...
	    int currWriteAhead;
	    if (fromPlayCursor) {
		currWriteAhead = (info->writePos - (int) (*playCursor));
	    } else {
		currWriteAhead = (info->writePos - (int) (*writeCursor));
	    }
	    if (currWriteAhead < 0) {
	    	currWriteAhead += info->dsBufferSizeInBytes;
	    }
	    if (currWriteAhead > *bufferSize) {
		if (!fromPlayCursor) {
		    /* underrun */
		    ERROR2("  DS_GetAvailable: ERROR: underrun detected: "
		           "DirectSoundBufferSize=%d, bufferSize=%d, ",
		           info->dsBufferSizeInBytes, info->bufferSizeInBytes);
		    ERROR3("playCursor=%d, writeCursor=%d, info->writePos=%d\n",
		           *playCursor, *writeCursor, info->writePos);
		    info->underrun = TRUE;
		}
		available = *bufferSize;
...
int DAUDIO_Write(void* id, char* data, int byteSize) {
	...
	info->playBuffer->Play(0, 0, DSBPLAY_LOOPING);
    }
...
int DAUDIO_Read(void* id, char* data, int byteSize) {
	...
	if (res != DS_OK) {
	    ...
	    thisReadPos += byteSize;
	    while (thisReadPos > info->dsBufferSizeInBytes) {
		thisReadPos -= info->dsBufferSizeInBytes;
	    }
	    /* commit data to directsound */
...
int DAUDIO_StillDraining(void* id, int isSource) {
    ...
    draining = (!info->underrun) && (available < bufferSize);
