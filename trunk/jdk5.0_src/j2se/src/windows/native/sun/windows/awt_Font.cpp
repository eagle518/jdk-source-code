...
jfieldID AwtFont::fontCharsetID;
...
AwtFont::AwtFont(int num, JNIEnv *env, jobject javaFont)
{
    ...
    m_javaFont = env->NewWeakGlobalRef(javaFont);
}
...
AwtFont::~AwtFont()
{
    ...
    if (localObj != NULL) {
        env->SetLongField(localObj, AwtFont::pDataID, NULL);
	env->DeleteLocalRef(localObj);
    }
    env->DeleteWeakGlobalRef(m_javaFont);
}
...
JNIEXPORT void JNICALL
Java_java_awt_Font_pDispose(JNIEnv *env, jobject self) 
{
    TRY_NO_VERIFY;

    jlong pData = env->GetLongField(self, AwtFont::pDataID);
    AwtFont* awtFont = (AwtFont*)jlong_to_ptr(pData);

    if (awtFont != NULL) {
        env->SetLongField(self, AwtFont::pDataID, (long)NULL);
	awtFont->scheduleDelete();
    }
...
Java_sun_awt_FontDescriptor_initIDs(JNIEnv *env, jclass cls) 
{
    ...
    AwtFont::fontCharsetID =
    	env->GetFieldID(cls, "fontCharset", "Lsun/io/CharToByteConverter;");
    AwtFont::nativeNameID = env->GetFieldID(cls, "nativeName",
					    "Ljava/lang/String;");
    AwtFont::useUnicodeID = env->GetFieldID(cls, "useUnicode", "Z");

    DASSERT(AwtFont::fontCharsetID != NULL);
