/*
 * @(#)awt_InputMethod.cpp	1.50 03/12/19
 *
 ...
 * Signature: (Lsun/awt/windows/WComponentPeer;)V
 */
JNIEXPORT void JNICALL Java_sun_awt_windows_WInputMethod_hideWindowsNative
  (JNIEnv *env, jobject self, jobject peer)
{
    //get C++ Class of Focused Component
    if (peer == 0)	return;
    AwtComponent* p = (AwtComponent*)JNI_GET_PDATA(peer);
    ...
    HWND hwndIME = ImmGetDefaultIMEWnd(p->GetHWnd());
  ...
  (JNIEnv *env, jobject self, jobject peer, jint x, jint y)
{
    ...
    AwtToolkit::GetInstance().SendMessage(WM_AWT_OPENCANDIDATEWINDOW, 
					  reinterpret_cast<WPARAM>(p), MAKELONG(x, y));
