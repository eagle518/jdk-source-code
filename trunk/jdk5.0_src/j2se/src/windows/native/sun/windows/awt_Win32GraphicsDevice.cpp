/*
 * @(#)awt_Win32GraphicsDevice.cpp	1.47 03/12/19
 *
...
int AwtWin32GraphicsDevice::DeviceIndexForWindow(HWND hWnd) 
{
    ...
    int screen = getScreenFromMHND(mon);
    return screen;
}
					      ...
					      int deviceIndex)
{
    ...
    jobject cm = devArray[deviceIndex]->GetColorModel(env, dynamic);
    devices->Unlock();
    return cm;
}
...
MONITOR_INFO *AwtWin32GraphicsDevice::GetMonitorInfo(int deviceIndex)
{
    AwtWin32GraphicsDevice **devArray = 
	(AwtWin32GraphicsDevice**)devices->Lock();
    MONITOR_INFO *mi = devArray[deviceIndex]->GetMonitorInfo();
    devices->Unlock();
    return mi;
}
...
void AwtWin32GraphicsDevice::ResetAllMonitorInfo()
{
    ...
    for (int deviceIndex = 0; deviceIndex < awt_numScreens; deviceIndex++) {
        MHND monitor = getMHNDFromScreen(deviceIndex);
        ::GetMonitorInfo(monitor, devArray[deviceIndex]->pMonitorInfo);
    }
    ...
    MHND hMonitor)
{
	...
	(AwtWin32GraphicsDevice**)devices->Lock();
    if (hMonitor == NULL) {
	devArray[0]->DisableOffscreenAcceleration();
    } else {
	for (int i = 0; i < awt_numScreens; ++i) {
	    if (devArray[i]->GetMonitor() == hMonitor) {
		devArray[i]->DisableOffscreenAcceleration();
	    }
	}
    }
    devices->Unlock();
...
{
    DxCapabilities *retVal = NULL;
    AwtWin32GraphicsDevice **devArray = 
	(AwtWin32GraphicsDevice**)devices->Lock();
    if (hMonitor == NULL) {
	retVal = devArray[0]->GetDxCaps();
    } else {
	for (int i = 0; i < awt_numScreens; ++i) {
	    if (devArray[i]->GetMonitor() == hMonitor) {
		retVal = devArray[i]->GetDxCaps();
		break;
	    }
	}
    }
...
MHND AwtWin32GraphicsDevice::GetMonitor(int deviceIndex)
{
    AwtWin32GraphicsDevice **devArray = 
	(AwtWin32GraphicsDevice**)devices->Lock();
    MHND mon = devArray[deviceIndex]->GetMonitor();
    devices->Unlock();
    return mon;
}
...
HPALETTE AwtWin32GraphicsDevice::GetPalette(int deviceIndex) 
{
    AwtWin32GraphicsDevice **devArray = 
	(AwtWin32GraphicsDevice**)devices->Lock();
    HPALETTE pal = devArray[deviceIndex]->GetPalette();
    devices->Unlock();
    return pal;
}
...
void AwtWin32GraphicsDevice::UpdateDynamicColorModel(int deviceIndex) 
{
    AwtWin32GraphicsDevice **devArray = 
	(AwtWin32GraphicsDevice**)devices->Lock();
    devArray[deviceIndex]->UpdateDynamicColorModel();
    devices->Unlock();
}
...
BOOL AwtWin32GraphicsDevice::UpdateSystemPalette(int deviceIndex) 
{
    AwtWin32GraphicsDevice **devArray = 
	(AwtWin32GraphicsDevice**)devices->Lock();
    BOOL updated = devArray[deviceIndex]->UpdateSystemPalette();
    devices->Unlock();
    return updated;
}
...
HPALETTE AwtWin32GraphicsDevice::SelectPalette(HDC hDC, int deviceIndex) 
{
    AwtWin32GraphicsDevice **devArray = 
	(AwtWin32GraphicsDevice**)devices->Lock();
    HPALETTE retPal = devArray[deviceIndex]->SelectPalette(hDC);
    devices->Unlock();
    return retPal;
}
...
void AwtWin32GraphicsDevice::RealizePalette(HDC hDC, int deviceIndex) 
{
    AwtWin32GraphicsDevice **devArray = 
	(AwtWin32GraphicsDevice**)devices->Lock();
    devArray[deviceIndex]->RealizePalette(hDC);
    devices->Unlock();
}
...
ColorData *AwtWin32GraphicsDevice::GetColorData(int deviceIndex) 
{
    AwtWin32GraphicsDevice **devArray = 
	(AwtWin32GraphicsDevice**)devices->Lock();
    ColorData *data = devArray[deviceIndex]->GetColorData();
    devices->Unlock();
    return data;
}
...
int AwtWin32GraphicsDevice::GetGrayness(int deviceIndex) 
{
    AwtWin32GraphicsDevice **devArray = 
	(AwtWin32GraphicsDevice**)devices->Lock();
    int grayness = devArray[deviceIndex]->GetGrayness();
    devices->Unlock();
    return grayness;
}
...
extern HDC getDCFromScreen(int screen);
...
Java_sun_awt_Win32GraphicsDevice_enterFullScreenExclusive(
    ...
    HMONITOR monitor = (HMONITOR)getMHNDFromScreen(screen);
 ...
 */
JNIEXPORT jboolean JNICALL
Java_sun_awt_Win32GraphicsDevice_isD3DEnabledOnDeviceNative
    (JNIEnv* env, jobject graphicsDevice, jint screen) 
{
    TRY;
    
    HMONITOR monitor = (HMONITOR)getMHNDFromScreen(screen);
    return DeviceUseD3D(monitor);
    ...
    (JNIEnv* env, jobject graphicsDevice, jint screen)
{
    ...
    if (DeviceUseDDraw(monitor)) {
        ...
        DEVMODE dm;
        LPTSTR pName = NULL; // REMIND : Extend for multi-monitor support
     ...
     jint width, jint height, jint bitDepth, jint refreshRate)
{
	...
	if (sendResult == 0) {
        ...
        // REMIND : Call fn_change_display_settings_ex() for multi-monitor
        // support
    ...
    (JNIEnv *env, jobject thisPtr, jint screen, jboolean dynamic)
{
    AwtWin32GraphicsDevice **devArray = 
	(AwtWin32GraphicsDevice**)devices->Lock();
    jobject cm = devArray[screen]->GetColorModel(env, dynamic);
    devices->Unlock();
    return cm;
}
    ...
    (JNIEnv *env, jobject thisPtr, jint screen)
{
    AwtWin32GraphicsDevice **devArray = 
	(AwtWin32GraphicsDevice**)devices->Lock();
    devArray[screen]->SetJavaDevice(env, thisPtr);
    devices->Unlock();
}
    ...
    (JNIEnv *env, jobject thisPtr, jint screen)
{
    AwtWin32GraphicsDevice **devArray = 
	(AwtWin32GraphicsDevice**)devices->Lock();
    jint mem = DDGetAvailableMemory((HMONITOR)devArray[screen]->GetMonitor());
    devices->Unlock();
    return mem;
}
