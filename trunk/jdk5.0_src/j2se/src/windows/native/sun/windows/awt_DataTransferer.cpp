/*
 * @(#)awt_DataTransferer.cpp	1.27 03/12/19
 *
...
const char * const OleStrongLock::LOCK_FAILED_MESSAGE = 
   "Failed to place a strong lock on an object.";

OleStrongLock::OleStrongLock(JNIEnv* env, IUnknown* pUnk, HRESULT& ret) {
    DASSERT(!::IsBadReadPtr(pUnk, sizeof(IUnknown)));
    m_bLockSucceeded = FALSE;
    m_pUnk = NULL;

    if (ret == S_OK) {
        ret = ::CoLockObjectExternal(pUnk, TRUE, FALSE);

        switch (ret) {
        case S_OK:
            m_bLockSucceeded = TRUE;
            m_pUnk = pUnk;
            break;
        case E_OUTOFMEMORY:
            JNU_ThrowOutOfMemoryError(env, LOCK_FAILED_MESSAGE);
	    break;
        case E_INVALIDARG:
        case E_UNEXPECTED:
...
OleStrongLock::~OleStrongLock() {
    if (m_bLockSucceeded) {
        DASSERT(!::IsBadReadPtr(m_pUnk, sizeof(IUnknown)));
        ::CoLockObjectExternal(m_pUnk, FALSE, FALSE);
    }
}
...
Java_sun_awt_windows_WDataTransferer_imageDataToPlatformImageBytes(JNIEnv *env, 
    ...
    case CF_METAFILEPICT: {
        ...
        UINT uMfpSize = sizeof(METAFILEPICT) + uMfSize;
        
        LPBYTE lpbMfpBuffer = NULL;
        try {
            lpbMfpBuffer = (LPBYTE)safe_Malloc(uMfpSize);
            VERIFY(::GetMetaFileBitsEx(hmf, uMfSize,
                     lpbMfpBuffer + sizeof(METAFILEPICT)) == uMfSize);
        } catch (...) {
            VERIFY(::DeleteMetaFile(hmf));
            throw;
        }
        ...
        LPMETAFILEPICT lpMfp = (LPMETAFILEPICT)lpbMfpBuffer;
        lpMfp->mm = MM_ANISOTROPIC; // should use MM_ANISOTROPIC exactly (MSDN)
        lpMfp->xExt = abs(p.x);
        lpMfp->yExt = abs(p.y);

        bytes = env->NewByteArray(uMfpSize);
        ...
        env->SetByteArrayRegion(bytes, 0, uMfpSize, (jbyte*)lpbMfpBuffer);
        free(lpbMfpBuffer);
        break;
    }
    default:
