...
BOOL AwtDragSource:: m_isOldCursorCustom = FALSE;
...
void AwtDragSource::_DoDragDrop(void* param) {
    ...
    ::CoLockObjectExternal((IDropSource*)dragSource, FALSE, FALSE);
...
HCURSOR AwtDragSource::SetCursor(jobject cursor) {
    JNIEnv* env = (JNIEnv *)JNU_GetEnv(jvm, JNI_VERSION_1_2);

    m_isOldCursorCustom = FALSE;
    if( NULL != m_cursor ) {
      m_isOldCursorCustom = m_cursor->IsCustomCursor();
    }
    ...
    if( TRUE == m_cursor->IsCustomCursor() ) {
      AwtToolkit::GetInstance().NotifyCustomCursor();
    } else {
      AwtToolkit::GetInstance().NotifySystemCursor();
    }

    return ::SetCursor(m_cursor->GetHCursor());
}
...
HRESULT __stdcall  AwtDragSource::QueryContinueDrag(BOOL fEscapeKeyPressed, DWORD grfKeyState) {
    ...
    if (m_cursor != NULL && TRUE == m_cursor->IsCustomCursor()) {
        AwtToolkit::GetInstance().NotifyCustomCursor();
    } else {
        AwtToolkit::GetInstance().NotifySystemCursor();
    }
    ...
    jlongArray formats, jobject formatMap)
{
    ...
    ::CoLockObjectExternal((IDropSource*)ds, TRUE, FALSE);
