...
AwtCursor::~AwtCursor() {
    ...
    if (localObj != NULL) {
        env->SetLongField(localObj, AwtCursor::pDataID, ptr_to_jlong(NULL));
...
AwtCursor * AwtCursor::CreateSystemCursor(jobject jCursor)
{
    ...
    env->SetLongField(jCursor, AwtCursor::pDataID, ptr_to_jlong(awtCursor));
...
void AwtCursor::UpdateCursor(AwtComponent *comp) {
    ...
    HCURSOR cur = GetCursor(env , comp);
    if (cur != NULL) {
        ::SetCursor(cur);
    }
 ...
 */
JNIEXPORT void JNICALL 
Java_java_awt_Cursor_finalizeImpl(JNIEnv *env, jobject self)
{
    TRY_NO_VERIFY;

    jlong pData = env->GetLongField(self, AwtCursor::pDataID);

    if (0 != pData) {
    	AwtCursor *awtCursor = (AwtCursor *)jlong_to_ptr(pData);
        env->SetLongField(self, AwtCursor::pDataID, ptr_to_jlong(NULL));
	awtCursor->scheduleDelete();
    }
    ...
    jint nSS, jint nW, jint nH, jint xHotSpot, jint yHotSpot)
{
    ...
    env->SetLongField(self, AwtCursor::pDataID,
		      ptr_to_jlong(new AwtCursor(env, hCursor, self, xHotSpot,
						    ...
						    jobject, jobject cursor, jboolean u)
{
    ...
    if (cursor != NULL) {  // fix for 4430302 - getCursor() returns NULL
        ...
        AwtToolkit::GetInstance().SendMessage(WM_AWT_SETCURSOR, (WPARAM)
            (awtCursor->GetHCursor()));
