 ...
 *********************************************************************/
     ...
     */
    Attribute.Compound enterAnnotation(Annotation a,
                                       Type expected,
				       Env<AttrContext> env) {
	a.type = chk.checkType(a.annotationType.pos,
                               attr.attribType(a.annotationType, env),
                               expected);
	if (a.type.isErroneous())
	    return new Attribute.Compound(a.type, new List<Pair<MethodSymbol,Attribute>>());
	...
	if (args.length() == 1 && args.head.tag != Tree.ASSIGN) {
	...
	for (List<Tree> tl = args; tl.nonEmpty(); tl = tl.tail) {
	    Tree t = tl.head;
	    if (t.tag != Tree.ASSIGN) {
		log.error(t.pos, "annotation.value.must.be.name.value");
                continue;
	    }
	    Assign assign = (Assign)t;
	    if (assign.lhs.tag != Tree.IDENT) {
		log.error(t.pos, "annotation.value.must.be.name.value");
                continue;
	    }
	    Ident left = (Ident)assign.lhs;
	...
	if (expected.tsym == syms.classType.tsym) {
	    ...
	    return new Attribute.Class(types,
				       (((Select)tree).selected).type);
	}
	if ((expected.tsym.flags() & Flags.ANNOTATION) != 0) {
	    if (tree.tag != Tree.ANNOTATION) {
		log.error(tree.pos, "annotation.value.must.be.annotation");
                expected = syms.errorType;
	    }
	    return enterAnnotation((Annotation)tree, expected, env);
	}
	if (expected.tag == TypeTags.ARRAY) { // should really be isArray()
	    if (tree.tag != Tree.NEWARRAY) {
		tree = make.at(tree.pos).
		    NewArray(null, Tree.emptyList,
			     Tree.emptyList.prepend(tree));
	    }
	    NewArray na = (NewArray)tree;
	    if (na.elemtype != null) {
		log.error(na.elemtype.pos, "new.not.allowed.in.annotation");
		return new Attribute.Error(expected);
	    }
	    ListBuffer<Attribute> buf = new ListBuffer<Attribute>();
	    for (List<Tree> l = na.elems; l.nonEmpty(); l=l.tail) {
