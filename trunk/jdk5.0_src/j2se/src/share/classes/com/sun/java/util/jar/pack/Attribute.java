...
class Attribute implements Comparable, Constants {
	...
	define(sd, ATTR_CONTEXT_CODE, "StackMap",
	       ("[NH[PHNH[(1)]NH[(1)]]]"
		+"[TB(7)[RCH](8,9)[PH]()[]]"));
	...
	boolean prevBCI = false;
		...
		for (;;) {
			...
			for (int cp = 0, endp;; cp = endp+1) {
			    ...
			    Layout.Element ce = self.new Element();
			    ce.body = cbody;  // all cases share the one body
			    ce.kind = EK_CASE;
			    ce.removeBand();
			    if (cp > 0)
				// "backward case" is one which repeats a body
				ce.flags |= EF_BACK;
			    String cstr1 = cstr.substring(cp, endp);
			    if (cstr1.length() == 0)
				cstr1 = "empty";  // will fail parse
			    ce.value = Integer.parseInt(cstr1);
			    cases.add(ce);
			    if (endp == cstrlen) {
