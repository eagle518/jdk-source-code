...
public final class LdapClient implements PooledConnection {
	...
	int connectTimeout, OutputStream trace, PoolCallback pcb) 
	throws NamingException {

	if (debug > 0)
	    System.err.println("LdapClient: constructor called " + host + ":" + port );
	conn = new Connection(this, host, port, socketFactory, connectTimeout,
	    trace);
	...
	String factory, int timeout, OutputStream trace,
	...
	String user, Object passwd, Hashtable env) throws NamingException {
        ...
        if (usePool
	    && LdapPoolManager.isPoolingAllowed(factory, trace,
		authMechanism, protocol, env)) {
	    LdapClient answer = LdapPoolManager.getLdapClient(hostname, port, 
		factory, timeout, trace, version, authMechanism, ctls, 
		protocol, user, passwd, env);
	    answer.referenceCount = 1;   // always one when starting out
	    return answer;
	} else {
	    return new LdapClient(hostname, port, factory, timeout, trace, null);
	}
    }
}
