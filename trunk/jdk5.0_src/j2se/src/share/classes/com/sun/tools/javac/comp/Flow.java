...
public class Flow extends TreeScanner {
	...
	lintSwitchFallThrough = options.lint("fallthrough");
	lintFinally = options.lint("finally");
    }
    ...
    boolean resolveBreaks(Tree tree,
 ...
 *************************************************************************/
	...
	if (!alive && tree != null) {
	    ...
	    if (tree.tag != Tree.SKIP) alive = true;
	}
	scan(tree);
    }
	...
	try {
		...
		for (List<Tree> l = tree.defs; l.nonEmpty(); l = l.tail) {
			...
			List<Type> mthrown =
			    ((MethodDef)l.head).sym.type.thrown();
			if (firstConstructor) {
			    caught = mthrown;
	...
	for (List<Case> l = tree.cases; l.nonEmpty(); l = l.tail) {
	    ...
	    // Warn about fall-through if lintSwitchFallThrough enabled.
	    if (!loopPassTwo &&
		lintSwitchFallThrough &&
		alive &&
	...
	private static void addVars(List<Tree> stats, Bits inits,
				    Bits uninits) {
	...
	scanExpr(tree.meth);
	scanExprs(tree.args);
	for (List<Type> l = tree.meth.type.thrown(); l.nonEmpty(); l = l.tail)
	...
	scanExpr(tree.encl);
	scanExprs(tree.args);
	for (List<Type> l = tree.constructor.type.thrown();
	     l.nonEmpty();
	     l = l.tail)
	    markThrown(tree, l.head);
	scan(tree.def);
	...
	if (!(lhs instanceof Ident)) scanExpr(lhs);
	scanExpr(tree.rhs);
	letInit(lhs);
    }
	...
	case Tree.NOT:
	    ...
	    uninitsWhenTrue = t;
	    break;
	case Tree.PREINC: case Tree.POSTINC:
	case Tree.PREDEC: case Tree.POSTDEC:
	    scanExpr(tree.arg);
	    letInit(tree.arg);
	    break;
	default:
