/*
 * @(#)PackageWriter.java	1.24 04/01/06
 *
...
class PackageWriter extends BandStructure {
	...
	if (verbose > 0)
	   Utils.log.info("Consensus version number is "+bestVerStr);
...
// with a zipped unbanded file.
	...
	for (int i = 0; i < ATTR_CONTEXT_LIMIT; i++) {
		...
		if (predefIndex == null) {
		    ...
		    Entry le = ConstantPool.getUtf8Entry(def.layout());
	...
	for (int i = 0; i < defs.length; i++) {
	    ...
	    attr_definition_layout.putRef(ConstantPool.getUtf8Entry(def.layout()));
