/**
 * @(#)ClassDocImpl.java	1.79 04/05/18
 *
...
import com.sun.tools.javac.tree.Tree.Select;
import com.sun.tools.javac.tree.Tree.Import;
...
public class ClassDocImpl extends ProgramElementDocImpl implements ClassDoc {
    ...
    public ClassDocImpl(DocEnv env, ClassSymbol sym, String documentation,
			ClassDef tree) {
        super(env, sym, documentation, tree);
	...
	TypeVariable res[] = new TypeVariable[type.typarams().length()];
	TypeMaker.getTypes(env, type.typarams(), res);
	return res;
    }
    ...
    private boolean hasParameterTypes(MethodSymbol method, String[] argTypes) {
        ...
        int i = 0;
        List<Type> types = method.type.argtypes();
	...
	for (Tree t : compenv.toplevel.defs) {
