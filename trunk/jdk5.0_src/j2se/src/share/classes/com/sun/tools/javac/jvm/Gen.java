 ...
 */
public class Gen extends Tree.Visitor {
    ...
    /** Set when CLDC-style verifier tables are to be generated. */
	...
	stackMap =
	    target.generateStackmap() ||
	    options.get("-cldc") != null;
	...
	case METHOD:
	    ...
	    for (List<Type> args = t.argtypes(); args.nonEmpty(); args = args.tail)
 ...
 *************************************************************************/
	...
	while (env.tree != target) {
	    if (env.tree.tag == Tree.TRY && env.info.finalize.hasFinalizer())
		return true;
	    env = env.next;
	}
	return false;
 ...
 *************************************************************************/
     ...
     */
    List<Tree> normalizeDefs(List<Tree> defs, ClassSymbol c) {
	...
	for (List<Tree> l = defs; l.nonEmpty(); l = l.tail) {
		...
		if ((block.flags & STATIC) != 0) clinitCode.append(def);
		else initCode.append(def);
	        break;
	    case Tree.METHODDEF:
	...
	if (initCode.length() != 0) {
	    ...
	    for (Tree t : methodDefs) {
                normalizeMethod((MethodDef)t, inits);
	    }
	}
	...
	if (clinitCode.length() != 0) {
		...
		STATIC, names.clinit,
		new MethodType(
		    Type.emptyList, syms.voidType,
		    Type.emptyList, syms.methodClass),
		c);
	    c.members().enter(clinit);
	    List<Tree> clinitStats = clinitCode.toList();
	    Block block = make.at(clinitStats.head.pos).Block(0, clinitStats);
     ...
     */
    void normalizeMethod(MethodDef md, List<Tree> initCode) {
	if (md.name == names.init && TreeInfo.isInitialConstructor(md)) {
	    // We are seeing a constructor that does not call another
	    // constructor of the same class.
	    List<Tree> stats = md.body.stats;
	    ListBuffer<Tree> newstats = new ListBuffer<Tree>();
 ...
 *********************************************************************/
	...
	pmt.thrown = chk.intersect(pmt.thrown(), imt.thrown());
    }
 ...
 *************************************************************************/
	...
	genStat(tree, env);
	if (tree.tag == Tree.BLOCK) crtFlags |= CRT_BLOCK;
	...
	if (code.isAlive()) {
	    ...
	    genDef(tree, env);
	} else if (env.info.isSwitch && tree.tag == Tree.VARDEF) {
 ...
 *************************************************************************/
	...
	this.pt = tree.sym.erasure(types).restype();
...
//where
	 ...
	 */
	void genMethod(MethodDef tree, Env<GenContext> env, boolean fatcode) {
	    MethodSymbol meth = tree.sym;
    //	System.err.println("generating " + f + " in " + f.owner); //DEBUG
	    if (Code.width(types.erasure(env.enclMethod.sym.type).argtypes())  +
		...
		meth.code = code = new Code(
		    meth,
		    fatcode, lineDebugInfo, varDebugInfo,
		    stackMap, debugCode,
	...
	if (env.tree.tag != Tree.METHODDEF) code.endScopes(limit);
    }
	 ...
	 */
        private void genLoop(Tree loop,
			     Tree body,
			     Tree cond,
			     List<Tree> step,
			     boolean testFirst) {
...
//where
	 ...
	 */
	void genTry(Tree body, List<Catch> catchers, Env<GenContext> env) {
	    int limit = code.nextreg;
	    int startpc = code.curPc();
	    ...
	    endFinalizerGap(env);
	    if (startpc != endpc) for (List<Catch> l = catchers; l.nonEmpty(); l = l.tail) {
	...
	if (tree.expr.tag == Tree.POSTINC) tree.expr.tag = Tree.PREINC;
	else if (tree.expr.tag == Tree.POSTDEC) tree.expr.tag = Tree.PREDEC;
 ...
 *************************************************************************/
	...
	genArgs(tree.args,
		TreeInfo.symbol(tree.meth).externalType(types).argtypes());
	result = m.invoke();
    }
	...
	genArgs(tree.args, tree.constructor.externalType(types).argtypes());
	...
	if (tree.elems != null) {
	    ...
	    int i = 0;
	    for (List<Tree> l = tree.elems; l.nonEmpty(); l = l.tail) {
		arr.duplicate();
		loadIntConst(i);
		i++;
...
//where
	...
	if (operator.opcode == string_add) {
		...
		if (tree.tag == Tree.MINUS_ASG) ival = -ival;
	    ...
	    l.duplicate();
	    l.coerce(operator.type.argtypes().head).load();
	...
	if (tree.tag == Tree.NOT) {
	    ...
	    Item od = genExpr(tree.arg, operator.type.argtypes().head);
	    switch (tree.tag) {
	    case Tree.POS:
		result = od.load();
		break;
	...
	if (operator.opcode == string_add) {
	    ...
	    Item od = genExpr(tree.lhs, operator.type.argtypes().head);
	    od.load();
...
//where
	 ...
	 */
	    ...
	    code.emitop0(dup);
	    callMethod(
		pos, stringBufferType, names.init, Type.emptyList, false);
	}
	 ...
	 */
	    ...
	    callMethod(
		tree.pos, stringBufferType, names.append,
	...
	Item completeBinop(Tree lhs, Tree rhs, OperatorSymbol operator) {
		...
		Type rtype = operator.erasure(types).argtypes().tail.head;
	...
	if (sym.kind == VAR && ((VarSymbol) sym).constValue != null) {
	    ...
	    result = items.
		makeImmediateItem(sym.type, ((VarSymbol) sym).constValue);
	} else {
	    if (!accessSuper)
 ...
 *************************************************************************/
     ...
     */
    public boolean genClass(Env<AttrContext> env, ClassDef cdef) {
	try {
	    attrEnv = env;
	    ...
	    for (List<Tree> l = cdef.defs; l.nonEmpty(); l = l.tail) {
		...
		for (List<Tree> l = cdef.defs; l.nonEmpty(); l = l.tail) {
		    if (l.head.tag == Tree.METHODDEF)
			((MethodDef)l.head).sym.code = null;
		}
	    }
            cdef.defs = Tree.emptyList; // discard trees
	    return nerrs == 0;
	} finally {
