/*
 *  @(#)BandStructure.java	1.26 04/06/12
 *
...
class BandStructure implements Constants {
	...
	return getCodingChooser().choose(values, start, end, regular, sizes);
    }
...
/*  // Comment out predefined StackMap bands, for now.
    // bands for predefined StackMap attribute
    IntBand   code_StackMap_N = code_attr_bands.newIntBand("code_StackMap_N");
    IntBand   code_StackMap_P = code_attr_bands.newIntBand("code_StackMap_P", BCI5);
    IntBand   code_StackMap_local_N = code_attr_bands.newIntBand("code_StackMap_local_N");
    IntBand   code_StackMap_stack_N = code_attr_bands.newIntBand("code_StackMap_stack_N");
    IntBand   code_StackMap_T = code_attr_bands.newIntBand("code_StackMap_T", BYTE1);
    CPRefBand code_StackMap_RC = code_attr_bands.newCPRefBand("code_StackMap_RC", CONSTANT_Class);
    IntBand   code_StackMap_uninit_P = code_attr_bands.newIntBand("code_StackMap_uninit_P", BCI5);
*/  // End out-comment of StackMap bands.
...
/*      // Comment out predefined StackMap bands, for now.
	predefineAttribute(CODE_ATTR_StackMap, ATTR_CONTEXT_CODE,
			   new Band[] {
			       code_StackMap_N,
			       code_StackMap_P,
			       code_StackMap_local_N,
			       code_StackMap_stack_N,
			       code_StackMap_T,
			       code_StackMap_RC,
			       code_StackMap_uninit_P,
			   },
			   "StackMap",
			   "[NH[PHNH[(1)]NH[(1)]]]"
			   +"[TB(7)[RCH](8,9)[PH]()[]]");
