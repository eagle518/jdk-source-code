/*
 * @(#)LdapPoolManager.java	1.4 03/12/19
 *
...
public final class LdapPoolManager {
	...
	// Cannot use pooling if using customized socketfactory or tracing
	...
	int timeout, OutputStream trace, int version, String authMech,
	Control[] ctls, String protocol, String user,
	...
	switch (p) {
	case NONE:
	    id = new ClientId(version, host, port, protocol, ctls, trace);
	    break;

	case SIMPLE:
	    // Add identity information used in simple authentication
	    id = new SimpleClientId(version, host, port, protocol, ctls, trace, 
		user, passwd);
	    break;

	case DIGEST:
	    // Add user/passwd/realm/authzid/qop/strength/maxbuf/mutual/policy*
	    id = new DigestClientId(version, host, port, protocol, ctls, trace, 
		user, passwd, env);
	    break;
	}

	return (LdapClient) pool.getPooledConnection(id, timeout, 
	    new LdapClientFactory(host, port, socketFactory, timeout, trace));
    }
