...
public class Main {
	...
	new XOption("-Xlint:{all,deprecation,unchecked,switchcheck,path,serial,finally,-deprecation,-unchecked,-switchcheck,-path,-serial,-finally}",
	...
	new AptOption("-s",		"opt.arg.path",	"opt.s"),
	...
	new AptOption("-A", 		"opt.proc.flag",	"opt.A") {
		String helpSynopsis() {
		    hasSuffix = true;
		    return super.helpSynopsis();
		}
	...
	try {
		...
		if (paths.sourcePath() != null) {
		    for(String s: paths.sourcePath())
			augmentedSourcePath += (s + File.pathSeparator);
		...
		if (paths.userClassPath() != null) {
		    for(String s: paths.userClassPath())
			baseClassPath += (s + File.pathSeparator);
		...
		if (options.get("-factorypath") != null) // same CL even if new class files written
	...
	if (options.get("-nocompile") == null &&
		...
		if (args[i] != null && args[i].startsWith("-A")) {
			newArgsLength--;
			args[i] = null;
			continue;
		} else {
	    ...
	    // need call to create new argument string to javac
	    // strip out apt specific args (e.g. -print), (Can we
	    // use a different java.Main.compile entry point? -- no, only two)
