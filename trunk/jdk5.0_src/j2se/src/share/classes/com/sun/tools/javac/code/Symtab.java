...
public class Symtab {
    ...
    private void enterBinop(String name,
        ...
        predefClass.members().enter(
            new OperatorSymbol(
		names.fromString(name),
		new MethodType(List.make(left, right), res,
			       Type.emptyList, methodClass),
		opcode,
		predefClass));
    }
	...
	unknownType = new Type(TypeTags.UNKNOWN, null) {
		...
		public boolean isSubType(Type that, Warner warn) { return false; }
	...
	arrayCloneMethod = new MethodSymbol(
	    PUBLIC,
	    names.clone,
	    new MethodType(Type.emptyList, objectType,
			   Type.emptyList, methodClass),
	    arrayClass);
	arrayClass.members().enter(arrayCloneMethod);

	// Enter predefined constants.
        nullConst = enterConstant("null", botType);
        trueConst = enterConstant("true", booleanType.constType(1));
        falseConst = enterConstant("false", booleanType.constType(0));
