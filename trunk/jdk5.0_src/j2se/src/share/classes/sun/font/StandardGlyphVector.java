/*
 * @(#)StandardGlyphVector.java	1.63 04/01/13
 *
...
public class StandardGlyphVector extends GlyphVector {
    ...
    public Rectangle2D getVisualBounds() {
        return getGlyphsVisualBounds(0, glyphs.length);
    }
	...
	if (result == null) {
            result = new DelegatingShape(getGlyphsVisualBounds(ix, 1));
      	    vbcache[ix] = result;
	}
    ...
    public float[] getGlyphInfo() {
        ...
        for (int i = 0, n = 0; i < glyphs.length; ++i, n += 8) {
            ...
            Rectangle2D vb = getGlyphsVisualBounds(i, 1).getBounds2D();
     ...
     */
    private StandardGlyphVector(GlyphVector gv) {
    ...
    private Rectangle2D getGlyphsVisualBounds(int start, int count) {
        setFRCTX();
        initPositions();
        ...
        for (int i = start, n = start * 2, e = start + count; i < e; ++i, n += 2) {
            getGlyphStrike(i).getGlyphOutlineBounds(glyphs[i], r, invdtx); // invdtx needed?
            if (!r.isEmpty()) {
                float px = positions[n];
                float py = positions[n+1];
        ...
        Rectangle2D result;
        if (minx >= maxx || miny >= maxy) { // watch out for overflow
            result = new Rectangle2D.Float(positions[start * 2],
                                           positions[start * 2 + 1], 0, 0);
        } else {
            result = new Rectangle2D.Float(minx, miny, maxx - minx, maxy - miny);
        }
        return result;
    }
    ...
    public static final class GlyphStrike {
        ...
        static GlyphStrike create(StandardGlyphVector sgv, AffineTransform dtx, AffineTransform gtx) {
            ...
            FontStrikeDesc desc = new FontStrikeDesc(dtx, 
                                                     tx, 
                                                     sgv.font.getStyle(), 
                                                     sgv.frc.isAntiAliased(), 
                                                     sgv.frc.usesFractionalMetrics());
        ...
        void getGlyphOutlineBounds(int glyphID, Rectangle2D.Float result, AffineTransform invdtx) {
            // !!! more garbage.  
            // Not that it matters, but I wish you could just translate a rect, sigh.

            Rectangle2D.Float r = strike.getGlyphOutlineBounds(glyphID);
            if (r.isEmpty()) {
                result.x = result.y = result.width = result.height = 0; // mark empty
            } else if (invdtx == null) {
                result.x = r.x + dx;
                result.y = r.y + dy;
                ...
                // System.out.print("gid: " + glyphID + " ");
                // for (int i = 0; i < work.length; ++i) {
                //     System.out.print((float)work[i] + " ");
                // }
                // System.out.println();
                invdtx.transform(work, 0, work, 0, 4);
                // for (int i = 0; i < work.length; ++i) {
                //     System.out.print((float) work[i] + " ");
                // }
                // System.out.println();
                ...
                result.x = (float)(minx);
                result.y = (float)(miny);
                result.width = (float)(maxx - minx);
                result.height = (float)(maxy - miny);
            }
        }
