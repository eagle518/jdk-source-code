/*
 * @(#)CalendarSystem.java	1.5 03/12/19
 *
...
public abstract class CalendarSystem {
    ...
    // calendar name : calendar class name
    static Map names;

    // calendar name : instance
    // (pseudo) calendar class name : (pseudo) calendar date class name
    // locale : instance
    ...
    synchronized private static void initNames() {
	if (names != null) {
	    return;
	}
	...
	for (int i = 0; i < namePairs.length; i += 2) {
	    clName.setLength(0);
	    String cl = clName.append(pkg).append(namePairs[i+1]).toString();
    ...
    /**
     * Returns the <code>Gregorian</code> instance.
     *
     ...
     * <code>CalendarSystem</code> associated with the specified name.
     */
	...
	synchronized (calendars) {
	    ...
	    className = (String) names.get(calendarName);
	    if (className == null) {
