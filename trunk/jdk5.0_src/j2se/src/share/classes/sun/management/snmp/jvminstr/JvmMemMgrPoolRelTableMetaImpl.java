/*
 * @(#)file      JvmMemMgrPoolRelTableMetaImpl.java
...
public class JvmMemMgrPoolRelTableMetaImpl extends JvmMemMgrPoolRelTableMeta 
	 ...
	 **/
	private static Map buildPoolIndexMap(SnmpTableHandler handler) {
	    // optimization...
	 ...
	 * Optimized algorithm.
	 **/
	private static Map buildPoolIndexMap(SnmpCachedData cached) {
	    if (cached == null) return Collections.EMPTY_MAP; 
	    final SnmpOid[] indexes = cached.indexes;
	    final Object[]  datas   = cached.datas;
	    final int len = indexes.length;
	    final Map m = new HashMap(len); 
	    for (int i=0; i<len; i++) {
	...
	protected SnmpCachedData updateCachedDatas(Object userData) {
	    ...
	    final TreeMap table = new TreeMap(SnmpCachedData.oidComparator);
	...
	protected String[] getMemoryPools(Object userData,
		...
		((Map)userData).put(listTag,result);

	    return result;
	}

	protected void updateTreeMap(TreeMap table, Object userData,
	...
	final Map m;
	if (userData instanceof Map) m=(Map) userData;
	else m=null;
