...
public class PolicyFile extends java.security.Policy {
	...
	try {
	    ...
	    pp.read(isr);
	    isr.close();
	    KeyStore keyStore = initKeyStore(policy,
					pp.getKeyStoreUrl(),
					pp.getKeyStoreType(),
...
*/
	...
	if (!imp.booleanValue()) {
		...
		for (int j = 0; j < pdp.length; j++) {
			...
			(pdp[j].getClass().getName(), pdp[j].getName());
		    domainPs.add(pe);
		}
	    }
	    if (debug != null) {
		...
		while (li.hasNext()) {
			...
			Set pSet = new HashSet(pdp.length);
			for (int j = 0; j < pdp.length; j++) {
			    pSet.add(pdp[j]);
			}
			final Subject subject = new Subject(true, pSet,
	...
	if (path != null) {
		...
		if (path.endsWith("*")) {
		    ...
		    boolean appendFileSep = path.endsWith(File.separator);
		    if (path.equals("")) {
