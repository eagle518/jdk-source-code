/*
 * @(#)TokenTracker.java	1.6 03/12/19
 *
...
public class TokenTracker {
	...
	if (prevEntryPos != -1) {
	    entryBefore = (Entry) list.get(prevEntryPos);
	...
	if ((nextEntryPos) < list.size()) {
	    entryAfter = (Entry) list.get(nextEntryPos);
	...
	if (list.size() < MAX_INTERVALS) {
	    ...
	    entry = (Entry) list.remove(oldWindowStartIndex);
	...
	TokenTracker tt = new TokenTracker(readVal);
	System.out.println("Now enter actual received sequence numbers and watch the list and the MessageProp:");

	while (true) {
	    readVal = Integer.parseInt(br.readLine());
	    try {
		tt.getProps(readVal, prop);
	    } catch (Exception e) {
