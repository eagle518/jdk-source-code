/*
 * @(#)ProviderList.java	1.9 04/05/18
 *
...
public final class ProviderList {
     ...
     * of all Providers already loaded plus the specified providers (if
     * not already present in list). Return null if no JAR list is needed
     * and this ProviderList object should be used.
     */
    ProviderList getJarList(String[] jarClassNames) {
	// all Providers already loaded, no special handling needed
	if (allLoaded) {
	    return null;
	}
	List<ProviderConfig> newConfigs = new ArrayList<ProviderConfig>();
	// add all providers that are already loaded
	for (ProviderConfig config : configs) {
	    if (config.isLoaded()) {
		newConfigs.add(config);
	    }
	}
	if (newConfigs.size() == configs.length) {
	    // all providers were loaded already, no special handling needed
	    allLoaded = true;
	    return null;
	}
	// add specified providers
	for (String className : jarClassNames) {
	    ProviderConfig newConfig = new ProviderConfig(className);
	    // already present skip
	    if (newConfigs.contains(newConfig)) {
		continue;
	    }
	    for (ProviderConfig config : configs) {
