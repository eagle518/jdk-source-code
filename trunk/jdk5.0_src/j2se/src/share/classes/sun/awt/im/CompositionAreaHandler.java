/*
 * @(#)CompositionAreaHandler.java	1.12 03/12/19
 *
...
class CompositionAreaHandler implements InputMethodListener,
    ...
    boolean isCompositionAreaVisible() {
        // use isVisible() for the input method window in the containment hierarchy
	// since that's the one that we toggle when showing or hiding the input window
        if (compositionArea != null) {
	    Container parent = compositionArea.getParent();
	    while (!(parent instanceof InputMethodWindow)) {
