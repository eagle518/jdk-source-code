...
public class Main {
    ...
    private URL parseURL(String url) throws ParseException {
        ...
        try {
            if (url.indexOf(':') <= 1) {
                // this is a file path, not a URL
                u = ParseUtil.fileToEncodedURL(new File(url));
            } else if (url.startsWith("file:")) {
                if (url.length() == "file:".length()) {
                    // the url is exactly "file:" -- no filename after the colon
                    u = new URL(url);  // this will fail later on since there is no file
                } else { 
                    // ensure that this file URL is absolute
                    u = ParseUtil.fileToEncodedURL(new File(new URL(url).getPath()));
                } 
            } else {
                u = new URL(url);
            }
