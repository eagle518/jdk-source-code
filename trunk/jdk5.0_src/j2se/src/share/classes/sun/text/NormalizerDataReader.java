/*
 * @(#)NormalizerDataReader.java	1.1 03/08/12
 *
...
final class NormalizerDataReader implements ICUBinary.Authenticate 
{
    ...
    * @draft 2.1
	...
	//normTrieStream= new ByteArrayInputStream(normBytes);
        ...
        byte[] bb = new byte[(extraData.length + combiningTable.length) * 2];
        dataInputStream.read(bb);
        int  j = 0;
	for (int i = 0; i < extraData.length; i ++) {
	    extraData[i] = (char)((bb[j++] << 8) | (bb[j++] & 0xff));
	}
	 	
	//Read the combining class table
	for (int i = 0; i < combiningTable.length; i ++) {
	    combiningTable[i] = (char)((bb[j++] << 8) | (bb[j++] & 0xff));
	}
	...
	//sherman/Note: we need neither AuxTrie nor canonStartSets in 1.5
        //cuting them out saves nnK memory and 30% more init time
	//Read the AuxTrie	 	
