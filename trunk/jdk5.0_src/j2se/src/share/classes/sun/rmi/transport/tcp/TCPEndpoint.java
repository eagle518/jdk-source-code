/*
 * @(#)TCPEndpoint.java	1.36 03/12/19
 *
...
public class TCPEndpoint implements Endpoint {
	...
	return ((Integer)java.security.AccessController.doPrivileged(
                new sun.security.action.GetIntegerAction(name,
							 def))).intValue();
    }
	...
	return ((Boolean)java.security.AccessController.doPrivileged(
            new sun.security.action.GetBooleanAction(name))).booleanValue();
    }
    ...
    private static final Map localEndpoints = new HashMap(3);
	...
	synchronized (localEndpoints) {
	    ...
	    LinkedList epList = (LinkedList) localEndpoints.get(endpointKey);
	...
	synchronized (localEndpoints) {
		...
		Collection lists = localEndpoints.values();
		Iterator listIter = lists.iterator();
		while (listIter.hasNext()) {
		    LinkedList epList = (LinkedList) listIter.next();
		    synchronized (epList) {
			Iterator iter = epList.iterator();
			while (iter.hasNext()) {
			    TCPEndpoint ep = (TCPEndpoint) iter.next();
			    ep.host = host;
			}
		    }
		}
	...
	synchronized (localEndpoints) {
		...
		TCPEndpoint lastEp = (TCPEndpoint) epList.getLast();
		if (lastEp.port != 0) {
		    return;
		} else {
		    Iterator iter = epList.iterator();
	    ...
	    // TBD: what if there already is an entry here??
	...
	TCPEndpoint localEndpoint = getLocalEndpoint(0, csf, ssf);
	...
	synchronized (localEndpoints) {
	    ...
	    s = new HashSet(localEndpoints.size());
	    Collection lists = localEndpoints.values();
	    Iterator listIter = lists.iterator();
	    while (listIter.hasNext()) {
		/*
		 ...
		 */
		LinkedList epList = (LinkedList) listIter.next();
		TCPEndpoint ep = (TCPEndpoint) epList.getFirst();
		s.add(ep.transport);
	    }
	}
	return s;
	...
	Iterator transportList = allKnownTransports().iterator();
	while (transportList.hasNext()) {
	    ((TCPTransport)transportList.next()).shedConnectionCaches();
	}
    }
    ...
    /**
     * Returns the port for this endpoint.  Note that the port may be
     * zero which indicates the default port for the VM for which this
     * is an endpoint.
     */
    public int getPort() {
	return port;
    ...
    /**
     * Returns the transport for incoming connections this endpoint.  
     **/
	...
	try {
	    ...
	    throw new UnknownHostException("Unknown host: " + host, e);
	} catch (java.net.ConnectException e) {
	    throw new ConnectException("Connection refused to host: " +
		host, e);
	} catch (IOException e) {
