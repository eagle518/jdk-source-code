...
/*
 * @(#)LayoutEngine.cpp	1.11 03/10/24
 *
...
const le_int32 DefaultCharMapper::controlCharsZWJCount = ARRAY_SIZE(controlCharsZWJ);
				     ...
				     LEErrorCode &success)
{
    ...
    mapCharsToGlyphs(chars, offset, count, rightToLeft, rightToLeft, glyphs, 
		     charIndices, success);
...
// Input: glyphs
// Output: positions
void LayoutEngine::positionGlyphs(const LEGlyphID glyphs[], le_int32 glyphCount, 
    float x, float y, float *&positions, LEErrorCode &success)
{
    if (LE_FAILURE(success)) {
        return;
    }
...
void LayoutEngine::adjustGlyphPositions(const LEUnicode chars[], le_int32 offset, 
    le_int32 count, le_bool /*reverse*/, LEGlyphID glyphs[], le_int32 glyphCount, 
    float positions[], LEErrorCode &success)
{
    ...
    if (chars == NULL || glyphs == NULL || positions == NULL || offset < 0 || 
        count < 0 || glyphCount < 0) {
...
void LayoutEngine::adjustMarkGlyphs(const LEGlyphID glyphs[], le_int32 glyphCount,
    le_bool reverse, LEGlyphFilter *markFilter, float positions[], LEErrorCode &success)
{
    float xAdjust = 0;
...
void LayoutEngine::mapCharsToGlyphs(const LEUnicode chars[], le_int32 offset, 
    le_int32 count, le_bool reverse, le_bool mirror, LEGlyphID *&glyphs, 
    le_int32 *&charIndices, LEErrorCode &success)
{
    if (LE_FAILURE(success)) {
        return;
    }
    ...
    float x, float y, LEErrorCode &success)
{
    ...
    fGlyphCount = computeGlyphs(chars, offset, count, max, rightToLeft, fGlyphs, 
				fCharIndices, success);
    positionGlyphs(fGlyphs, fGlyphCount, x, y, fPositions, success);
    adjustGlyphPositions(chars, offset, count, rightToLeft, fGlyphs, fGlyphCount, 
			 fPositions, success);
    ...
    le_int32 scriptCode, le_int32 languageCode, LEErrorCode &success)
{
    ...
    if (gsubTable != NULL && 
	gsubTable->coversScript(scriptTag = OpenTypeLayoutEngine::getScriptTag(scriptCode))) {
	  ...
	  (MorphTableHeader *) fontInstance->getFontTable(mortTableTag);
        ...
        if (morphTable != NULL) {
            ...
            case arabScriptCode:
            case hebrScriptCode:
                result = new UnicodeArabicOpenTypeLayoutEngine(fontInstance, 
