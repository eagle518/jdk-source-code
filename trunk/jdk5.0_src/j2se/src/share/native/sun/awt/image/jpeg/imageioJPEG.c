...
static int setQTables(JNIEnv *env, 
    ...
    for (i = 0; i < qlen; i++) {
        ...
        for (j = 0; j < 64; j++) {
            quant_ptr->quantval[j] = qdataBody[j];
        }
...
static void setHuffTable(JNIEnv *env, 
    ...
    for (i = 0; i < hvalsLen; i++) {
        huff_ptr->huffval[i] = hvalsBody[i];
    }
    (*env)->ReleaseShortArrayElements(env, 
                                      huffValues, 
                                      hvalsBody,
...
Java_com_sun_imageio_plugins_jpeg_JPEGImageReader_readImageHeader
    ...
    if (ret == JPEG_HEADER_TABLES_ONLY) {
         ...
         * default_decompress_parms, to reflect our differences from IJG+NIFTY
         */
	...
	default :
	  break;
        case JCS_YCbCr:
            ...
            } else if (!cinfo->saw_JFIF_marker) {  // We saw neither marker
                /*
            ...
            break;
        case JCS_YCC:
            /* NIFTY left this converted to RGB, unnecessarily */
