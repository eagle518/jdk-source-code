...
#define DeclareByteBinaryInitialLoadVars(TYPE, pRas, PREFIX, x) \
    int PREFIX ## index = (x) / TYPE ## PixelsPerByte; \
    int PREFIX ## bits = TYPE ## MaxBitOffset - \
                             (((x) % TYPE ## PixelsPerByte) * \
                              TYPE ## BitsPerPixel); \
...
#define BBBlitLoopWidthHeight(SRCTYPE, SRCPTR, SRCBASE, SRCINFO, SRCPREFIX, \
	...
	do { \
            Declare ## SRCTYPE ## InitialLoadVars(SRCPTR, SRCPREFIX, srcx1) \
            Declare ## DSTTYPE ## InitialLoadVars(DSTPTR, DSTPREFIX, dstx1) \
	    juint w = WIDTH; \
...
{ \
 ...
 \
    ...
    do { \
        Declare ## DST ## InitialLoadVars(pPix, DstPix, lox) \
	jint w = width; \
	do { \
...
{ \
 ...
 \
	...
	DST ## DataType *pPix = PtrCoord(pBase, \
					 ...
					 y, scan); \
	do { \
            Declare ## DST ## InitialLoadVars(pPix, DstPix, x) \
	    jint relx = w; \
	    do { \
                InitialLoad ## DST(pPix, DstPix); \
	        Store ## DST ## PixelData(pPix, 0, pixel, DstPix); \
...
{ \
 ...
 \
    ...
    if (errmajor == 0) { \
	do { \
            Declare ## DST ## InitialLoadVars(pPix, DstPix, x1) \
	    Store ## DST ## PixelData(pPix, 0, pixel, DstPix); \
            FinalStore ## DST(pPix, DstPix); \
            x1 += bumpmajor; \
	} while (--steps > 0); \
...
\
        do { \
            Declare ## DST ## InitialLoadVars(pPix, DstPix, left) \
            jint x = 0; \
            do { \
                InitialLoad ## DST(pPix, DstPix); \
                if (pixels[x]) { \
...
\
        ...
        do { \
            Declare ## DST ## InitialLoadVars(pPix, pix, left) \
            int x = 0; \
...
{ \
 ...
 \
    ...
    do { \
        Declare ## SRC ## InitialLoadVars(pSrc, SrcRead, srcx1) \
        Declare ## DST ## InitialLoadVars(pDst, DstWrite, dstx1) \
	jint w = width; \
...
{ \
 ...
 \
    ...
    srcA = MultiplyAlphaFor ## STRATEGY(srcA, extraA); \
 ...
 \
    ...
    do { \
        Declare ## TYPE ## InitialLoadVars(pRas, DstWrite, x1) \
	jint w = width; \
