 ...
 *	(REMIND: Should this be distance in bits instead?)
 *
 ...
 *
 *	unsigned char *redErrTable;
 *	unsigned char *grnErrTable;
 *	unsigned char *bluErrTable;
 * [Requires SD_LOCK_INVCOLOR]
...
typedef struct {
    ...
    unsigned int	pixelStride;		/* bytes to next X pixel */
    unsigned int	scanStride;		/* bytes to next Y pixel */
    ...
    char		priv[SD_RASINFO_PRIVATE_SIZE];
} SurfaceDataRasInfo;
 ...
 * native code to track the state of the surface.  The dirty flag 
 * is used to determine whether, upon a lock for WRITE, we need to
 * notify the Java code that this object is being written to (used
 * for volatile images).  The sdObject is a pointer to the Java 
 * SurfaceData object; this is set in SurfaceData_InitOps() and used by
 * any object using the ops structure to refer to elements in the
 * Java object (such as fields that we need to set from native code).
 */
...
/*
 * This function sets a dirty and needsBackup fields of the SurfaceData java
 * object and resets a number of times the surface data was copied from w/o.
 * modification.
 */
void SurfaceData_MarkDirty(JNIEnv *env, SurfaceDataOps *ops);
