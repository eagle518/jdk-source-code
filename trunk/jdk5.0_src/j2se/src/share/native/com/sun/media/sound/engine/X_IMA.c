		   ...
		   short* predictorCache, short index)
{
    ...
    while (sampleCount > 0)
	{
	    ...
	    //src += sizeof(XDWORD);
	    // $$fb the above causes unaligned data access on Itanium...
	    nybbles = *src; ++src; nybbles |= (*src<<8); ++src; nybbles |= (*src<<16); ++src; nybbles |= (*src<<24); ++src;
		     ...
		     XDWORD srcBytes, XDWORD channelCount)
{
	...
	if (extraBytes >= WAV_IMA_HEADER_BYTES)
		...
		if (channelCount == 2)
		    {
			extraBytes -= extraBytes % (XDWORD)sizeof(XDWORD);
		    }
...
///////////////////////////////////////////////// AIFF-IMA DECOMPRESSION FUNCTIONS:
			...
			XDWORD frameCount, XDWORD channelCount)
	{
	    ...
	    char			*pDest8;
	    INT32			stepL, indexL;
	    INT32			diffL, predsampleL;
	    char			codebufL, codeL;
	    INT32			stepR, indexR;
	    INT32			diffR, predsampleR;
	    char			codebufR, codeR;
	    XDWORD			count;
	    char			macBlockOffset;
...
//--------------------------------------------------------------------------;
	 ...
	 )
	{
	    ...
	    //  don't have to clamp.
	    //
	    if( (INT32)(short)lNewSample == lNewSample )
		{
