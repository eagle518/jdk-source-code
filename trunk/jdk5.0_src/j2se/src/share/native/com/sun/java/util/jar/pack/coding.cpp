...
maybe_inline
int coding::reduceToUnsignedRange(int x) {
  assert(isSubrange);
...
int getDeltaValue(value_stream* self, uint uval, bool isSubrange) {
  ...
  if (isSubrange)
    sum = self->c.reduceToUnsignedRange(sum);
  return (self->sum = sum);
}
...
int value_stream::getInt() {
  ...
  case cmk_pop:
    ...
    if (D != 0) {
      ...
      if (c.isSubrange)
	sum = c.reduceToUnsignedRange(sum);
      uval = (uint) sum;
    }
...
void coding_method::init(byte* &band_rp, byte* band_limit,
  ...
  if (op == _meta_default) {
    ...
    next = U_NEW(coding_method, 1);
    CHECK_NULL(next);
    next->u = u;
    if (ABDef == 2) {
      next->init(band_rp, band_limit, NO_META, mode, defc, N2, valueSink);
    } else {
      next->init(band_rp, band_limit, meta_rp, mode, defc, N2, valueSink);
    }
    ...
    // Last of all, adjust this->vs0.cmk to the 'pop' flavor
