...
**	History	-
**	12/1/96		Created
**	2/13/97		Added code to UNI_PTEFFECTB that was different that previous
**				versions
**				Added INLINE's to reduce code size and help performance
...
static UWORD
    getlinearperiod(UBYTE note,UWORD fine)
{
...
static UWORD
    getlogperiod(UBYTE note,UWORD fine)
{
    UBYTE n,o;
    UWORD p1,p2,i;
    ...
    return Interpolate((SWORD)(fine / 16), 0, 15, p1, p2) >> o;
}


static UWORD
    getoldperiod(UBYTE note,UWORD c2spd)
{
    UBYTE n,o;
    ULONG period;
...
static UWORD
    GetPeriod(UBYTE note,UWORD c2spd)
{
...
static void INLINE DoS3MSlideUp(UBYTE inf)
{
    UBYTE hi,lo;
...
static void INLINE DoS3MSpeed(UBYTE speed)
{
    ...
    DoPTEffect0(UBYTE dat)
{
	...
	a->period=GetPeriod((UBYTE)(note + a->transpose), a->c2spd);
	a->ownper=1;
    }
}
    ...
    PlayNote(void)
{
	...
	case UNI_NOTE:
		...
		period = GetPeriod((UBYTE)(note + a->transpose), a->c2spd);
	...
	case UNI_INSTRUMENT:
	    ...
	    period=GetPeriod((UBYTE)(a->note + a->transpose), a->c2spd);
...
static void
    StartEnvelope(ENVPR *t,UBYTE flg,UBYTE pts,UBYTE sus,UBYTE beg,UBYTE end,ENVPT *p)
{
    t->flg=flg;
    t->pts=pts;
    t->sus=sus;
...
static SWORD
    ProcessEnvelope(ENVPR *t,SWORD v,UBYTE keyon)
{
    if (t->flg & EF_ON) {
    ...
    MP_HandleTick(void)
{
	...
	*/
	...
	/* Do we have to get a new patternpointer ?
	   ...
	   a patternbreak is active). Also check for 255 - if mod 
	...
	if (a->kick) {
	    MD_VoicePlay((UBYTE)t,a->handle,a->start,s->length,s->loopstart,s->loopend,s->flags);
	    a->kick=0;
	    a->keyon=1;
	...
	MD_VoiceSetVolume((UBYTE)t, (UBYTE)tmpvol);

	if (s->flags& SF_OWNPAN) {
	    MD_VoiceSetPanning((UBYTE)t, DoPan(envpan, a->panning));
	}
	else {
	    MD_VoiceSetPanning((UBYTE)t, a->panning);
	}

	if (pf->flags&UF_LINEAR)
	    MD_VoiceSetFrequency((UBYTE)t, GetFreq2(a->period));
	else
	    MD_VoiceSetFrequency((UBYTE)t, (3579546UL<<2) / a->period);
