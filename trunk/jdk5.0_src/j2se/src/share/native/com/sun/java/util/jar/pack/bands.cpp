...
void band::readData(int expectedLength) {
  ...
  printcr(3,"readFrom %s at %p [%d values, %d bytes, cp=%d/%d]",
	  name, minRP(), length, size(), cp1, cp2);
...
//BAND_INIT(band_headers, BYTE1_spec, 0),
  ...
  BAND_INIT(code_StackMap_N, UNSIGNED5_spec, 0),
  BAND_INIT(code_StackMap_P, BCI5_spec, 0),
  BAND_INIT(code_StackMap_local_N, UNSIGNED5_spec, 0),
  BAND_INIT(code_StackMap_stack_N, UNSIGNED5_spec, 0),
  BAND_INIT(code_StackMap_T, BYTE1_spec, 0),
  BAND_INIT(code_StackMap_RC, UNSIGNED5_spec, INDEX(CONSTANT_Class)),
  BAND_INIT(code_StackMap_uninit_P, BCI5_spec, 0),
#endif
