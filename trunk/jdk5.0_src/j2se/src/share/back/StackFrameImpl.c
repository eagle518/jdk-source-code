/*
 * @(#)StackFrameImpl.c	1.31 03/12/19
 *
...
validateThreadFrame(jthread thread, FrameID frame)
{
	...
	if ( count > 0 ) {
	    error = validateFrameID(thread, frame);
	} else {
                   ...
                   FrameNumber fnum, jint slot, jbyte typeKey)
{
	...
	WITH_LOCAL_REFS(env, 1) {
		...
		(void)outStream_writeByte(out, specificTypeKey(value.l));
		(void)outStream_writeObjectRef(out, value.l);
	    }
...
thisObject(PacketInputStream *in, PacketOutputStream *out)
{
	...
	if (error == JVMTI_ERROR_NONE) {
		...
		if (modifiers & (MOD_STATIC | MOD_NATIVE)) {
		    this_object = NULL;
		    (void)outStream_writeByte(out, specificTypeKey(this_object));
		    (void)outStream_writeObjectRef(out, this_object);
		} else {
		    error = JVMTI_FUNC_PTR(gdata->jvmti,GetLocalObject)
