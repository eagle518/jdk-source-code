...
eventFilterRestricted_alloc(jint filterCount)
{
    ...
    HandlerNode *node = jvmtiAllocate(size);
...
***/
	...
	default: 
	    EXIT_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT,"Invalid filter modifier");
	    return JNI_FALSE;
        }
    }
    return JNI_TRUE;
                          ...
                          jthread thread, jint size, jint depth)
{
    ...
    /* the end of this call */
    saveGlobalRef(env, thread, &(filter->thread));
    error = stepControl_beginStep(filter->thread, size, depth, node);
    if (error != JVMTI_ERROR_NONE) {
