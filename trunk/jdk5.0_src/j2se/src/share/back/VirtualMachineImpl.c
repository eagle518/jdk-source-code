/*
 * @(#)VirtualMachineImpl.c	1.96 03/12/19
 *
...
static int minorVersion = 5;  /* JDWP minor version */
...
redefineClasses(PacketInputStream *in, PacketOutputStream *out) 
{
    ...
    /*LINTED*/
    classDefs = jvmtiAllocate(classCount*sizeof(jvmtiClassDefinition));
    if (classDefs == NULL) {
        outStream_setError(out, JDWP_ERROR(OUT_OF_MEMORY));
        return JNI_TRUE;
    }
...
topLevelThreadGroups(PacketInputStream *in, PacketOutputStream *out)
{
	...
	if (error != JVMTI_ERROR_NONE) {
		...
		(void)outStream_writeObjectRef(out, groups[i]);
	    }
...
createString(PacketInputStream *in, PacketOutputStream *out)
{
	...
	if (JNI_FUNC_PTR(env,ExceptionOccurred)(env)) {
	    ...
	    (void)outStream_writeObjectRef(out, string);
	} 
...
capabilitiesNew(PacketInputStream *in, PacketOutputStream *out)
{
    ...
    /* remaining reserved */
    (void)outStream_writeBoolean(out, (jboolean)JNI_FALSE); /* 16 */
    (void)outStream_writeBoolean(out, (jboolean)JNI_FALSE); /* 17 */
    (void)outStream_writeBoolean(out, (jboolean)JNI_FALSE); /* 18 */
    (void)outStream_writeBoolean(out, (jboolean)JNI_FALSE); /* 19 */
    (void)outStream_writeBoolean(out, (jboolean)JNI_FALSE); /* 20 */
    (void)outStream_writeBoolean(out, (jboolean)JNI_FALSE); /* 21 */
...
writePaths(PacketOutputStream *out, char *string) {
    ...
    (void)outStream_writeInt(out, countPaths(string));

    ps = gdata->property_path_separator;
    if ( ps == NULL ) {
	ps = ";";
    }
    while ((newPos = strchr(pos, ps[0])) != NULL) {
	/*LINTED*/
        (void)outStream_writeByteArray(out, (int)(newPos-pos), (jbyte *)pos);
        pos = newPos + 1;
    }
    (void)outStream_writeByteArray(out, strlen(pos), (jbyte *)pos);
}
...
void *VirtualMachine_Cmds[] = { (void *)20
    ,(void *)version
