...
getHandlerChain(EventIndex i)
{
	...
	EXIT_ERROR(JVMTI_ERROR_INVALID_EVENT_TYPE,"bad index for handler");
    }
                              ...
                              IteratorFunction func, void *arg)
{
    ...
    if ( func == NULL ) {
	EXIT_ERROR(JVMTI_ERROR_INTERNAL,"iterator function NULL");
    }
...
synthesizeUnloadEvent(void *signatureVoid, void *envVoid)
{
     ...
     * classname
     */
    classname = jvmtiAllocate(strlen(signature)+1);
    ...
    while (node != NULL) {
        ...
        if (eventFilterRestricted_passesUnloadFilter(env, classname, 
             ...
             */
            char *durableSignature = jvmtiAllocate(strlen(signature)+1);
...
event_callback(JNIEnv *env, EventInfo *evinfo)
{
	...
	classname = getClassname(evinfo->clazz);
		...
		if ( func == NULL ) {
	            EXIT_ERROR(JVMTI_ERROR_INTERNAL,"handler function NULL");
		}
