...
removeResumed(JNIEnv *env, ThreadList *list)
{
    ...
    while (node != NULL) {
        ...
        if (node->suspendCount == 0) {
            (void)removeThread(env, list, node->thread);
        }
        node = temp;
    }
}
...
threadControl_initialize(void)
{
    ...
    if (gdata->threadClass==NULL) {
        EXIT_ERROR(JVMTI_ERROR_NULL_POINTER, "no java.lang.thread class");
    }
    if (gdata->threadResume==0) {
        EXIT_ERROR(JVMTI_ERROR_NULL_POINTER, "cannot resume thread");
    }
...
static void
trackAppResume(JNIEnv *env, jthread thread)
{
    jvmtiError error;
    FrameNumber fnum = 0;
...
threadControl_onHook(void)
{
	...
	if (threads == NULL) {
	    EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"thread table");
	} else {
		...
		if (node == NULL) {
		    EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"thread table entry");
		} 
...
commonResumeList(void)
{
    ...
    if (reqList == NULL) {
	EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"resume request list");
    }
    /*LINTED*/
    results = newArray(reqCnt, sizeof(jvmtiError));
    if (results == NULL) {
	EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"resume list");
    }
    ...
    for (i = 0; i < reqCnt; i++) {
        ThreadNode *node = findThread(env, &runningThreads, reqList[i]);
        if (node == NULL) {
	    EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"entry in thread table");
        }
 ...
 */
static jvmtiError 
commonSuspendList(jint initCount, jthread *initList) 
{
    JNIEnv *env = getEnv();
    jvmtiError    error = JVMTI_ERROR_NONE;
    jint    i;
    ...
    if (reqList == NULL) {
	EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"request list");
    }
    ...
    for (i = 0; i < initCount; i++) {
         ...
         * to a separate list of threads so that we'll resume it later. 
         */
        ThreadNode *node = findThread(env, &runningThreads, initList[i]);
        if (node == NULL) {
	...
	jvmtiError *results = newArray(reqCnt, sizeof(jvmtiError));
        ...
        if (results == NULL) {
	    EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"suspend list results");
        }
        ...
        error = JVMTI_FUNC_PTR(gdata->jvmti,SuspendThreadList)
			(gdata->jvmti, reqCnt, reqList, results);
        for (i = 0; i < reqCnt; i++) {
            ThreadNode *node = findAnyThread(env, reqList[i]);
            if (node == NULL) {
		EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"entry in thread table");
            }
...
threadControl_suspendAll(void)
{
	...
	if (canSuspendResumeThreadLists()) {
	    error = commonSuspendList(count, threads);
	    if (error != JVMTI_ERROR_NONE) {
		goto err;
	    }
	} else {
...
{
    jvmtiError error = JVMTI_ERROR_INVALID_THREAD;
    JNIEnv *env = getEnv();
    int i;
    int j;
...
setPopFrameThread(jthread thread, jboolean value)
{
	...
	if (node == NULL) {
	    EXIT_ERROR(JVMTI_ERROR_NULL_POINTER,"entry in thread table");
	} else {
...
checkForPopFrameEvents(JNIEnv *env, EventIndex ei, jthread thread)
{
	...
	switch (ei) {
		...
		/* Excuse me? */
		EXIT_ERROR(JVMTI_ERROR_INTERNAL, "thread start during pop frame");
		break;
            case EI_THREAD_END:
...
threadControl_onEventHandlerEntry(jbyte sessionID, EventIndex ei, jthread thread, jobject currentException) 
{
    ...
    node = findThread(env, &otherThreads, thread);
    if (node != NULL) {
        (void)moveThread(env, &otherThreads, &runningThreads, node->thread);
    } else {
        /*
                                 ...
                                 struct bag *eventBag) 
{
    ...
    node = findThread(env, &runningThreads, thread);
    if (node == NULL) {
	EXIT_ERROR(JVMTI_ERROR_NULL_POINTER,"thread list corrupted");
    } else {
        if (ei == EI_THREAD_END) {
            jboolean inResume = (node->resumeFrameDepth > 0);
            (void)removeThread(env, &runningThreads, thread);
...
threadControl_getInstructionStepMode(jthread thread) 
{
    JNIEnv *env = getEnv();
    ThreadNode *node;
    jvmtiEventMode mode = JVMTI_DISABLE;
