...
/* Debug version defaults */
    ...
    #define DEFAULT_LOGPRECRASH 	NULL
#endif
...
static jboolean doexitpause = JNI_FALSE;    /* pause at exit */
static jboolean doprecrash = JNI_FALSE;     /* run precrash -p PID at exit */
...
cbEarlyVMInit(jvmtiEnv *jvmti_env, JNIEnv *env, jthread thread) 
{
    ...
    if ( gdata->vmDead ) {
	EXIT_ERROR(JVMTI_ERROR_INTERNAL,"VM dead at VM_INIT time");
    }
    if (initOnStartup)
...
cbEarlyVMDeath(jvmtiEnv *jvmti_env, JNIEnv *env) 
{
    ...
    if ( gdata->vmDead ) {
	EXIT_ERROR(JVMTI_ERROR_INTERNAL,"VM died more than once");
    }
	...
	jmethodID catch_method, jlocation catch_location)
{
    ...
    if ( gdata->vmDead ) {
	EXIT_ERROR(JVMTI_ERROR_INTERNAL,"VM dead at initial Exception event");
    }
    if (!vmInitialized)  {
...
jniFatalError(JNIEnv *env, const char *msg, jvmtiError error, int exit_code)
{
	...
	(void)fprintf(stderr, "ERROR: JDWP: Can't call JNI FatalError(NULL, \"%s\")\n", buf);
    }
    forceExit(exit_code);
}
...
initialize(JNIEnv *env, jthread thread, EventIndex triggering_ei) 
{
    ...
    if ( gdata->vmDead ) {
	EXIT_ERROR(JVMTI_ERROR_INTERNAL,"VM dead at initialize() time");
    }
    ...
    if ( gdata->vmDead ) {
	EXIT_ERROR(JVMTI_ERROR_INTERNAL,"VM dead before initialize() completes");
    }
...
printUsage(void)
{
 ...
 "exitpause=y|n                pause at exit to debug PID        n\n"
 "coredump=y|n                 coredump at exit                  n\n"
 "errorexit=y|n                exit on any error                 n\n"
 "precrash=y|n                 run precrash -p PID at exit       n\n"
...
add_to_options(char *options, char *new_options)
{
    ...
    combinedOptions = jvmtiAllocate(originalLength + 1 +
				strlen(new_options) + 1);
...
parseOptions(char *options)
{
    ...
    /* These buffers are never freed */
    {
	    ...
	    if ( options==NULL ) {
		EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"options");
	    }
        }
	...
	(void)strcpy(gdata->options, options);
        ...
        if (transports == NULL) {
	    EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"transports");
        }
	...
	/*LINTED*/
	    ...
	    if ( !get_boolean(&str, &doexitpause) ) {
		goto syntax_error;
	    }
        } else if (strcmp(buf, "precrash") == 0) {
	    if ( !get_boolean(&str, &doprecrash) ) {
		goto syntax_error;
	    }
    ...
    /* Setup logging now */
    if ( logfile!=NULL ) {
	setup_logging(logfile, logflags, (doprecrash?"precrash":NULL), 
			SIGABRT, 0);
