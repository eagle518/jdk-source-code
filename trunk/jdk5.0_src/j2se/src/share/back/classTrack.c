...
deleteTable(JNIEnv *env, KlassNode *oldTable[])
{
    ...
    if (signatures == NULL) {
	EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"signatures");
    }
    ...
    for (slot = 0; slot < CT_HASH_SLOT_COUNT; slot++) {
        ...
        while (node != NULL) {
            ...
            if (sigSpot == NULL) {
		EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"signature bag");
            }
...
classTrack_processUnloads(JNIEnv *env)
{
    ...
    if (newTable == NULL) {
	EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY, "classTrack table");
    } else {
...
classTrack_addPreparedClass(JNIEnv *env, jclass klass)
{
    ...
    if (node == NULL) {
	EXIT_ERROR(JVMTI_ERROR_OUT_OF_MEMORY,"KlassNode");
    }
    ...
    if ((node->klass = JNI_FUNC_PTR(env,NewWeakGlobalRef)(env, klass)) == NULL) {
        ...
        jvmtiDeallocate(node);
	EXIT_ERROR(JVMTI_ERROR_NULL_POINTER,"NewWeakGlobalRef");
    }
