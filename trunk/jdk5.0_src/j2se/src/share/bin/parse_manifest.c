/*
 * @(#)parse_manifest.c	1.13 04/06/15
 *
...
/*
 * This function is not only platform independent, but also independent of
 * the program logic of the Java launcher.  The following include is to
 * handle the anomaly that Windows doesn't put the function prototype
 * for lseek where POSIX says it should.  "java_md.h" handles this mapping.
 * Using this function elsewhere will require a different method of handling
 * that mapping. One possible solution would be to introduce headers of the
 * expected POSIX name which simply include the appropriate Windows headers.
 *
 * This also gets you the definitions for str[n]casecmp which go by the
 * name str[n]icmp on Windows.
 */
#include "java_md.h"
...
static const int	manifest_name_len = 20;
 ...
 */
static char*
inflate_manifest(int fd, zentry* entry)
{
    char*	in;
    char*	out;
    z_stream	zs;
...
static int
find_manifest(int fd, zentry* entry)
{
    int	    bytes;
    int	    res;
    int	    entry_size;
	 ...
	 */
	if (CENNAM(p) == manifest_name_len &&
	  memcmp((p + CENHDR), manifest_name, manifest_name_len) == 0) {
 ...
 */
int
parse_manifest(char *jarfile, manifest_info *info)
{
    int     fd;
    zentry  entry;
    char    *lp;
    ...
    if (rc = find_manifest(fd, &entry) != 0) {
	close(fd);
	return (-2);
    }
    manifest = inflate_manifest(fd, &entry);
    if (manifest == NULL) {
	close(fd);
	return (-2);
    }
 ...
 */
int
manifest_iterate(const char* jarfile, attribute_closure ac, void* user_data)
{
    int     fd;
    zentry  entry;
