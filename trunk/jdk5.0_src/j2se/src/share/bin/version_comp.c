 ...
 */
int
prefix_version_id(char *id1, char *id2)
{
    char	*s1 = strdup(id1);
    char	*s2 = strdup(id2);
    char	*m1 = s1;
 ...
 */
int
exact_version_id(char *id1, char *id2)
{
    char	*s1 = strdup(id1);
    char	*s2 = strdup(id2);
    char	*m1 = s1;
...
acceptable_simple_element(char *release, char *simple_element)
{
	...
	*modifier = '\0';
	...
	return ((prefix_version_id(release, simple_element) == 0)?1:0);
    } else if (*modifier == '+') {
	*modifier = '\0';
	if (strchr(release, '-'))
	    return ((strcmp(release, simple_element) == 0)?1:0);
	return ((exact_version_id(release, simple_element) >= 0)?1:0);
    } else {
	return ((exact_version_id(release, simple_element) == 0)?1:0);
    }
}
 ...
 */
int
acceptable_release(char *release, char *version_string)
{
    char	*vs;
    char	*m1;
    char	*end;
    m1 = vs = strdup(version_string);
    do {
 ...
 */
int
valid_version_string(char *version_string)
{
    char	*vs;
    char	*m1;
    char	*end;
