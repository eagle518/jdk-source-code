...
%install
    ...
    cp -dpr $RPM_BUILD_DIR/%{jre_name}%{jre_version}/* \
            $RPM_BUILD_ROOT/%{jre_prefix}/%{jre_name}%{jre_version}/
...
#
if [ "${RPM_INSTALL_PREFIX}" != "%{jre_prefix}" ]; then
    if [ -d $RPM_INSTALL_PREFIX/%{jre_name}%{jre_version} ]; then
        ln -s $RPM_INSTALL_PREFIX/%{jre_name}%{jre_version} \
              %{jre_prefix}/%{jre_name}%{jre_version}
    fi
fi

UNPACK_EXE=%{jre_prefix}/%{jre_name}%{jre_version}/bin/unpack200
...
%else
    ...
    %{jre_prefix}/%{jre_name}%{jre_version}/bin/java -client -Xshare:dump > /dev/null 2>&1
...
%preun
if [ ! -d %{jre_prefix}/%{jre_name}%{jre_version}/man/man1 ]; then
    mkdir -p %{jre_prefix}/%{jre_name}%{jre_version}/man/man1;
fi
if [ -f %{jre_manpath}/java.1 ]; then
    mv -f %{jre_manpath}/java.1 %{jre_prefix}/%{jre_name}%{jre_version}/man/man1;
fi
if [ -f %{jre_manpath}/keytool.1 ]; then
    mv -f %{jre_manpath}/keytool.1 %{jre_prefix}/%{jre_name}%{jre_version}/man/man1;
fi
if [ -f %{jre_manpath}/rmid.1 ]; then
    mv -f %{jre_manpath}/rmid.1 %{jre_prefix}/%{jre_name}%{jre_version}/man/man1;
fi
if [ -f %{jre_manpath}/rmiregistry.1 ]; then
    mv -f %{jre_manpath}/rmiregistry.1 %{jre_prefix}/%{jre_name}%{jre_version}/man/man1;
fi
if [ -f %{jre_manpath}/tnameserv.1 ]; then
    mv -f %{jre_manpath}/tnameserv.1 %{jre_prefix}/%{jre_name}%{jre_version}/man/man1;
fi
...
%else
    ...
    SHARED_CLASS_PATH=%{jre_prefix}/%{jre_name}%{jre_version}/lib/${ARCH}/client
...
if [ "${RPM_INSTALL_PREFIX}" != "%{jre_prefix}" ]; then
    ...
    if [ -h %{jre_prefix}/%{jre_name}%{jre_version} ]; then
        rm -f %{jre_prefix}/%{jre_name}%{jre_version}
    fi
    if [ -d $RPM_INSTALL_PREFIX/%{jre_name}%{jre_version} ]; then
        rm -rf $RPM_INSTALL_PREFIX/%{jre_name}%{jre_version}
    fi
else
