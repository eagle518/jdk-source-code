...
#pragma ident "@(#)cInterpreter_ia64.inline.hpp	1.5 03/12/23 16:36:37 JVM"
#endif
/*
...
// LOCALS
// ia64 implementation - locals is array on the stack with indices going from 0..-(locals-1)
...
// Also longs and doubles are kept in a single slot rather than broken up across two
// slots. The assembly code for the interpreter (in debug mode) will fill the unused
// slot with a known value during result handling to help during debugging. 
// We do similar here.
//
...
#ifdef ASSERT
  ((VMJavaVal64*) &_base[-(slot)])->d = 1.234; 	/* known value in the unused slot */
#endif
}
inline void JavaLocals::set_Long(jlong value, int slot) { 
  ((VMJavaVal64*) &_base[-(slot+1)])->l = value; 
#ifdef ASSERT
  ((VMJavaVal64*) &_base[-(slot)])->l = 0xDEAD3852; 	/* known value in the unused slot */
#endif
}
...
#ifdef ASSERT
  ((VMJavaVal64 *)&_base[-(slot)])->d = 929.52;
#endif
}
...
#ifdef ASSERT
  ((VMJavaVal64 *)&_base[-(slot)])->l = 0xDEAF3852;
#endif
}
