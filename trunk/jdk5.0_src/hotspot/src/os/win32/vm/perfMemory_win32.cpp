...
#pragma ident "@(#)perfMemory_win32.cpp	1.21 04/02/05 12:03:06 JVM"
#endif
/*
...
# include "incls/_perfMemory_win32.cpp.incl"
...
#define PERFDATA_NAME "hsperfdata"
#define UINT_CHARS    11            // 2^32 - 1 can be represented as a string
                                    // using 11 characters, not including a
...
static void save_memory_to_file(char* addr, size_t size) {
  ...
  // sizeof(char array) should include the terminating '\0', add 1 for "_"
  char destfile[sizeof(PERFDATA_NAME) + UINT_CHARS + 1];

  // create the name of the file for retaining the instrumentation memory
  jio_snprintf(destfile, sizeof(destfile), 
               "%s_%d", PERFDATA_NAME, os::current_process_id());

  int fd = ::_open(destfile, _O_BINARY | _O_CREAT | _O_WRONLY | _O_TRUNC, _S_IREAD|_S_IWRITE);
  if (fd == OS_ERR) {
    if (PrintMiscellaneous && Verbose) {
      ...
      if (filename_to_pid(udentry->d_name) == vmid) {
        ...
        if (statbuf.st_size > 0 && statbuf.st_ctime > oldest_ctime) {
          ...
          if (statbuf.st_ctime > oldest_ctime) {
            char* user = strrchr(dentry->d_name, '_') + 1;
...
static char *get_sharedmem_objectname(const char* user, int vmid) {
  ...
  // construct file mapping object's name
