...
#pragma ident "@(#)objectMonitor_linux.cpp	1.60 04/04/19 18:35:02 JVM"
#endif
...
# include "incls/_objectMonitor_linux.cpp.incl"

typedef os::Linux::Event* event_t;
...
void ObjectMonitor::wait(jlong millis, bool interruptible, TRAPS) {
  ...
  if (node._notified == 1 && _succ == &node) {
   ...
   if (jvmpi::is_event_enabled(JVMPI_EVENT_MONITOR_WAITED)) {
      ...
      long millis_waited = (end_wait.tv_sec - start_wait.tv_sec) * 1000;
      millis_waited += (end_wait.tv_usec - start_wait.tv_usec) / 1000;
      jvmpi::post_monitor_waited_event((oop)object(), millis_waited);
   }
   ...
   // NOTES: Spurious wake up will be consider as timeout.
