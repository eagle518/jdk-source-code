...
#pragma ident "@(#)osThread_solaris.cpp	1.50 03/12/23 16:37:46 JVM"
#endif
/*
...
// 1. SetThreadPC_Callback - used by VM thread to change a JavaThread's PC
// 2. GetThreadPC_Callback - used by WatcherThread to profile VM thread
...
// execute "this" callback from the signal handler. If target thread is
// suspended, we will first resume the thread to deliver the signal.
int OSThread::Sync_Interrupt_Callback::interrupt(Thread * target,
                        bool target_is_suspended, int timeout) {
  // Let signals to the vm_thread go even if the Threads_lock is not aquired
  ...
  if (target_is_suspended) {
    // if target thread is suspended, we need to resume it so the signal 
    // can be delivered - only used by set_thread_pc_and_resume()
    assert(target->is_vm_suspended(), "target thread not suspended");
    target->vm_resume();
  }
