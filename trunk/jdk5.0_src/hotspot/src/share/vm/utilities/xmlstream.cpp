...
#pragma ident "@(#)xmlstream.cpp	1.6 04/03/03 17:04:49 JVM"
#endif
/*
...
void xmlStream::method(methodHandle method) {
  ...
  int      nmid   = (nm == NULL) ? 0 : nm->compile_id();
  if (nmid != 0)    print(" compile_id='%d'", nmid);
  ...
  if (method->tier1_compile_done())  print(" tier1='1'");
