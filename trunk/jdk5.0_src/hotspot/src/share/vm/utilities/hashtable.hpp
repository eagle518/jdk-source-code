...
#pragma ident "@(#)hashtable.hpp	1.6 04/02/23 13:55:10 JVM"
#endif
/*
...
public:
  ...
  inline BasicHashtableEntry* get_entry() const;
  inline void set_entry(BasicHashtableEntry* l);
...
public:
  inline BasicHashtable(int table_size, int entry_size);
  inline BasicHashtable(int table_size, int entry_size,
...
protected:
  ...
  inline void initialize(int table_size, int entry_size, int number_of_entries);
...
public:
  inline void set_entry(int index, BasicHashtableEntry* entry);

  inline void add_entry(int index, BasicHashtableEntry* entry);

  inline void free_entry(BasicHashtableEntry* entry);
...
public:
  inline Hashtable(int table_size, int entry_size)
...
protected:
  ...
  static inline unsigned int hash_symbol(const char* s, int len);

  inline unsigned int compute_hash(symbolHandle name) {
    return (unsigned int) name->identity_hash();
  }

  inline int index_for(symbolHandle name) {
...
public:
  inline unsigned int compute_hash(symbolHandle name, Handle loader) {
    return (unsigned int) name->identity_hash()
         ^ (unsigned int) (loader.is_null() ? 0 : loader->identity_hash());
  }

  inline int index_for(symbolHandle name, Handle loader) {
