...
#pragma ident "@(#)templateTable.cpp	1.99 03/12/23 16:40:44 JVM"
#endif
/*
...
void TemplateTable::jsr_w() {
  transition(vtos, itos);       // result is not an oop, so do not transition to atos
  branch(true, true);
}
...
#if 0
// [RGV] Getting error that this template is already defined.
// [RGV] bool is unique type in CC 5.0, I'm not sure what this problem is
// [RGV] will check this out later.
...
void TemplateTable::initialize() {
  ...
  def(Bytecodes::_jsr	              , ubcp|disp|____|____, vtos, itos, jsr                 ,  _           ); // result is not an oop, so do not transition to atos
  ...
  def(Bytecodes::_jsr_w	              , ubcp|____|____|____, vtos, itos, jsr_w               ,  _           );
