...
#pragma ident "@(#)interpreter.cpp	1.222 04/04/05 14:16:25 JVM"
#endif
/*
...
void InterpreterCodelet::initialize(const char* description, bool is_safepoint_safe, Bytecodes::Code bytecode) {
  _description       = description;
  _is_safepoint_safe = is_safepoint_safe;
  _bytecode          = bytecode;
}
...
void InterpreterCodelet::verify() {
  if (bytecode() >= 0) {
    assert(is_safepoint_safe() == Bytecodes::safepoint_safe(bytecode()), "is_safepoint_safe flag inconsistent");
  }
}
...
void InterpreterCodelet::print() {
  ...
  const char* _not = is_safepoint_safe() ? "" : "not ";
  tty->print_cr("[" INTPTR_FORMAT ", " INTPTR_FORMAT "[  %d bytes  %ssafepoint safe",
		code_begin(), code_end(), code_size(), _not);
...
void AbstractInterpreter::initialize() {
  ...
  // assertions
#ifndef CC_INTERP
  assert(Bytecodes::number_of_codes <= DispatchTable::length, "dispatch table too small");
#endif /* !CC_INTERP */
  ...
  { ResourceMark rm;
    ...
    int code_size = Interpreter_Code_Size;
    NOT_PRODUCT(code_size *= 4);     // extra space for StopInterpreterAt, etc.
...
void AbstractInterpreter::print() {
  ...
  tty->cr();
  tty->print_cr("code size        = %6d bytes", _code->used_space());
  tty->print_cr("total space      = %6d bytes", _code->total_space());
  tty->print_cr("wasted space     = %6d bytes", _code->available_space());
  tty->cr();
...
class CodeletMark: ResourceMark {
 ...
 public:
  CodeletMark(
    ...
    bool is_safepoint_safe = true,
    Bytecodes::Code bytecode = Bytecodes::_illegal)
  { // request all space (add some slack for Codelet data)
#ifdef PRODUCT
  #ifdef _LP64
    int codelet_size = 40*K;
  #else
    int codelet_size = 21*K;
  #endif
#else
  #ifdef _LP64
    int codelet_size = 80*K;  // extra space for VerifyThread, VerifyFPU, etc..
  #else
    int codelet_size = 60*K;
  #endif
#endif // PRODUCT
    _clet = (InterpreterCodelet*)AbstractInterpreter::code()->request(codelet_size);
    guarantee(_clet != NULL, "not enough space for interpreter generation");
    // initialize Codelet attributes
    _clet->initialize(description, is_safepoint_safe, bytecode);
    // create assembler for code generation
    masm  = new InterpreterMacroAssembler(new CodeBuffer(_clet->code_begin(), _clet->code_size()));
    _masm = &masm;
  }  
  ...
  ~CodeletMark() {
    ...
    // commit Codelet
    AbstractInterpreter::code()->commit((*_masm)->code()->code_size());
...
void AbstractInterpreterGenerator::generate_all() {
  ...
  { CodeletMark cm(_masm, "continuation entry points", false);
  ...
  { CodeletMark cm(_masm, "throw exception entrypoints");
    ...
    Interpreter::_throw_ClassCastException_entry             = generate_klass_exception_handler("java/lang/ClassCastException"                  );
...
#define method_entry(kind) Interpreter::_entry_table[Interpreter::kind] = generate_method_entry(Interpreter::kind)
  ...
  { CodeletMark cm(_masm, "(kind = frame_manager)", false);
...
void AbstractInterpreterGenerator::set_entry_points(Bytecodes::Code code) {
  CodeletMark cm(_masm, Bytecodes::name(code), Bytecodes::safepoint_safe(code), code);
...
AbstractInterpreter::MethodKind AbstractInterpreter::method_kind(methodHandle m) {
  ...
  switch (m->intrinsic_id()) {
    case methodOopDesc::_dsin : return java_lang_math_sin ;
    case methodOopDesc::_dcos : return java_lang_math_cos ;
    case methodOopDesc::_dsqrt: return java_lang_math_sqrt;
  }
...
address AbstractInterpreter::continuation_for(methodOop method, address bcp, int callee_parameters, bool is_top_frame, bool& use_next_mdp) {
  ...
  // (an invoke is continued in a compiler's illegal instruction handler)
  use_next_mdp = true;
  switch (code) {
...
int AbstractInterpreter::TosState_as_index(TosState state) {
  ...
  assert(0 <= state && state < AbstractInterpreter::number_of_return_addrs, "index out of bounds");
  return state;
}
...
#ifndef CORE
// This method lays out the basic structure of an interpreted frame:
void AbstractInterpreter::layout_activation( methodOop method,
					     int tempcount,
					     int moncount,
					     int callee_params,
					     int callee_locals,
					     ...
					     bool is_top_frame) {
  layout_activation_impl(method, tempcount, moncount, callee_params, callee_locals, caller, interpreter_frame, is_top_frame);
}
#endif // !CORE
//------------------------------------------------------------------------------------------------------------------------
// Safepoint suppport
