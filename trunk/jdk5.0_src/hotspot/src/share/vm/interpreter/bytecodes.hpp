...
#pragma ident "@(#)bytecodes.hpp	1.65 03/12/23 16:40:35 JVM"
#endif
/*
...
class Bytecodes: AllStatic {
 ...
 private:
  ...
  static u_char      _can_trap      [number_of_codes];
  static u_char      _safepoint_safe[number_of_codes];
  static Code        _java_code     [number_of_codes];
  static u_char      _can_rewrite   [number_of_codes];

  static int         special_length_at(address bcp);
  static void        def(Code code, const char* name, const char* format, const char* wide_format, BasicType result_type, int depth, bool can_trap, bool safepoint_safe = true);
  static void        def(Code code, const char* name, const char* format, const char* wide_format, BasicType result_type, int depth, bool can_trap, Code java_code, bool safepoint_safe = true);
 ...
 public:
  ...
  static Code        cast           (int  code)    { check((Code)code); return (Code)code; }
  ...
  static const char* wide_format    (Code code)    { wide_check(code); return _wide_format   [code]; }
  ...
  static int         length_for     (Code code)    { check(code);      return _length        [code]; }
  static bool        can_trap       (Code code)    { check(code);      return _can_trap      [code]; }
  static bool        safepoint_safe (Code code)    { check(code);      return _safepoint_safe[code]; }
  ...
  static int         wide_length_for(Code code)    { wide_check(code); return (int)strlen(wide_format(code)); }
