...
#pragma ident "@(#)genCollectedHeap.cpp	1.130 04/05/11 12:44:23 JVM"
#endif
/*
...
jint GenCollectedHeap::initialize() {
  ...
  assert(!UseSharedSpaces || !(UseISM || UsePermISM),
         "Either sharing or ISM should have been turned off before now.");
...
char* GenCollectedHeap::allocate(size_t alignment,
  ...
  if (UseISM || UsePermISM) { 
    ...
    if (total_reserved < LargePageSizeInBytes) {
      vm_exit_during_initialization(overflow_msg);
    }
  } 
  ...
  *heap_rs = ReservedSpace(total_reserved, alignment,
                           UseISM || UsePermISM, heap_address); 
  return heap_address;
}
...
void GenCollectedHeap::do_collection(bool  full,
  ...
  *notify_ref_lock = false;
  ...
  assert(my_thread->is_VM_thread() ||
         my_thread->is_ConcurrentMarkSweep_thread(),
  ...
  { // Call to jvmpi::post_class_unload_events must occur outside of active GC
    ...
    TraceMemoryManagerStats tmms(full);
...
void GenCollectedHeap::process_strong_roots(int level,
  ...
  if (!_gen_process_strong_tasks->is_task_claimed(CH_PS_Universe_oops_do))
    Universe::oops_do(not_older_gens, false);
  ...
  if (!_gen_process_strong_tasks->is_task_claimed(CH_PS_Management_oops_do))
    Management::oops_do(not_older_gens);
  if (!_gen_process_strong_tasks->is_task_claimed(CH_PS_jvmti_oops_do))
    JvmtiExport::oops_do(not_older_gens); 
...
size_t GenCollectedHeap::unsafe_max_tlab_alloc() const {
  size_t result = 0;
  for (int i = 0; i < _n_gens; i += 1) {
    if (_gens[i]->supports_tlab_allocation()) {
...
void GenCollectedHeap::print_gc_threads() const {
  if (UseParNewGC) {
    workers()->print_worker_threads();
  }
  if (UseConcMarkSweepGC) {
    ConcurrentMarkSweepThread::print_all();
  }
}
