...
#pragma ident "@(#)compactibleFreeListSpace.cpp	1.105 04/01/11 09:11:04 JVM"
#endif
/*
  ...
  _collector(NULL)
{
  ...
  if (!use_adaptive_freelists) {
    ...
    // Is this necessary?  Has the BOT already been initialized for the
    // entire committed part of the heap?
    _bt.alloc_block(addr, addr + fc->size());
  } else {
...
void CompactibleFreeListSpace::set_end(HeapWord* value) {
  ...
  assert(value > prevEnd, "can't yet deal with shrinking "
  ...
  if (prevEnd != NULL) {
    ...
    if (!_adaptive_freelists && _smallLinearAllocBlock._ptr == NULL) {
      ...
      if (ParallelGCThreads == 0) {
        addChunkToFreeLists(prevEnd, newFcSize);
    	splitBirth(newFcSize);
      } else {  // ParallelGCThreads > 0
        // One of the parallel gc task threads may be here
        // whilst others are allocating.
        Mutex* lock = &_parDictionaryAllocLock;
        if (newFcSize < SmallForDictionary) {
          lock = _indexedFreeListParLocks[newFcSize];
        }
        // lock the appropriate list; since this is a rare event,
        // the resulting overhead is tolerable.
...
HeapWord*
CompactibleFreeListSpace::object_iterate_mem_careful(MemRegion mr,
  MemObjectClosureCareful* cl) {
  assert_lock_strong(freelistLock());
  ...
  HeapWord *addr, *end;
  size_t size;
  for (addr = (prev == mr.start() ? prev : block_start_careful(mr.start())),
       end  = mr.end();
       addr < end; addr += size) {
...
size_t CompactibleFreeListSpace::block_size_nopar(const HeapWord* p) const {
  ...
  if (fc->isFree()) {
    ...
    assert(oop(p)->is_oop() || oop(p)->mark() == NULL, "Should be an oop");
...
bool CompactibleFreeListSpace::block_is_obj(const HeapWord* p) const {
  FreeChunk* fc = (FreeChunk*)p;
  assert(contains(p), "Should be in space");
...
HeapWord* CompactibleFreeListSpace::allocate(size_t size, 
					     bool large_noref) {
  // large_noref is not used.  Selection of source of the allocation is
  // based on the different free lists.
  assert_lock_strong(freelistLock());
  HeapWord* res = NULL;
  ...
  if (_adaptive_freelists) {
    res = allocate_adaptive_freelists(size, large_noref);
  } else {  // non-adaptive free lists
    res = allocate_non_adaptive_freelists(size, large_noref);
  }
  
  if (res != NULL) {
    // check that res does lie in this space!
    assert(contains(res), "Not in this space!");
...
oop CompactibleFreeListSpace::promote(oop obj, size_t obj_size, oop* ref) {
  ...
  // is a virtual method.
  HeapWord* res = allocate(adjustObjectSize(obj_size), false);
  if (res != NULL) {
...
CompactibleFreeListSpace::addChunkToFreeLists(HeapWord* chunk,
  ...
  assert(chunk != NULL && contains(chunk), "Not in this space!");
  assert_locked();
...
void PromotionInfo::promoted_oops_iterate##nv_suffix(OopClosureType* cl) {  \
  ...
  for (curObj = _promoHead; curObj != NULL; curObj = nextObj) {             \
    ...
    if (curObj->hasDisplacedMark()) {                                       \
      ...
      oop(curObj)->set_mark(markOopDesc::prototype());                      \
    }                                                                       \
...
SPECIALIZED_SINCE_SAVE_MARKS_CLOSURES_TRAIN(PROMOTED_OOPS_ITERATE_DEFN)
...
class VerifyAllBlksClosure: public BlkClosure {
  ...
  const bool                      _is_in_sweep;

 public:
  VerifyAllBlksClosure(const CompactibleFreeListSpace* sp,
    MemRegion span, bool is_in_sweep) :
    _sp(sp), _span(span), _is_in_sweep(is_in_sweep) { }
  ...
  size_t do_blk(HeapWord* addr) {
    ...
    assert(res != 0, "Livelock: no rank reduction!");
    return res;
  }
};
    ...
    if (p != NULL) {
      if (_span.contains(p)) { // the interior oop points into CMS heap
        if (!_span.contains(ptr)) { // reference from outside CMS heap
          ...
          assert(!_sp->contains(p) || _sp->block_is_obj((HeapWord*)p),
                 "Should be an object");
          assert(p->is_oop(), "Should be an oop");
          p->verify();
          if (_past_remark) {
          ...
          if (_past_remark) {
            ...
            if (_bit_map->isMarked(_collector->block_start(ptr))) {
              assert(_bit_map->isMarked((HeapWord*)p), "Marking error?");
            }
          }
        }
      } else if (_sp->contains(ptr)) {
...
void CompactibleFreeListSpace::verify(bool ignored) const {
  ...
  {
    VerifyAllBlksClosure cl(this, span, past_remark);
...
void PromotionInfo::verify() const {
  ...
  assert(_promoHead == NULL || _promoTail != NULL, "list consistency");
  ...
  for (PromotedObject* curObj = _promoHead; curObj != NULL; curObj = curObj->next()) {
    assert(space()->contains((HeapWord*)curObj), "Containment");
    // the last promoted object may fail the mark() != NULL test of is_oop().
    assert(curObj->next() == NULL || oop(curObj)->is_oop(), "must be an oop");
    if (curObj->hasDisplacedMark()) {
  ...
  for (SpoolBlock* curSpool = _spoolHead;
    ...
    assert((void*)curSpool->displacedHdr == (void*)&curSpool->displacedHdr,
           "first entry of displacedHdr should be self-referential");
    numDisplacedHdrs += curSpool->bufferSize - 1;
  }
  assert((_spoolHead == _spoolTail) == (numDisplacedHdrs == 0),
  ...
  assert(numDisplacedHdrs == numObjsWithDisplacedHdrs, "Displaced hdr count");
}
#endif  // PRODUCT
...
CFLS_LAB::CFLS_LAB(CompactibleFreeListSpace* cfls) :
  _cfls(cfls)
{
  _blocks_to_claim = ParCMSPromoteBlocksToClaim;
...
par_get_chunk_of_blocks(size_t word_sz, size_t n, FreeList* fl) {
  ...
  while (cur_sz < CompactibleFreeListSpace::IndexSetSize && k == 1) {
	...
	while ((fc = fl_for_cur_sz.getChunkAtHead()) != NULL) {
	  ...
	  for (int i = k-1; i >= 0; i--) {
	    ...
	    _bt.split_block((HeapWord*)fc, fc_size, fc_size - word_sz);
	    // Push this on "fl".
  ...
  }
  {
    MutexLockerEx x(_indexedFreeListParLocks[word_sz],
                    Mutex::_no_safepoint_check_flag);
    int new_births = (int)_indexedFreeList[word_sz].splitBirths() + (int)n;
    _indexedFreeList[word_sz].set_splitBirths(new_births);
    int new_surplus = (int)_indexedFreeList[word_sz].surplus() + (int)n;
...
// for parallel rescan. See ParRescanTask where this is currently used.
// XXX This method may need to be suitably generalized if/when we have
// other such sequentially claimed parallel tasks in CMS.
void
CompactibleFreeListSpace::
initialize_sequential_subtasks_for_rescan(int n_threads) {
  ...
  SequentialSubTasksDone* pst = par_seq_tasks();
