...
#pragma ident "@(#)allocation.hpp	1.62 04/01/05 16:38:42 JVM"
#endif
/*
...
// Optionally, objects may be allocated on the C heap with new(ResourceObj::C_HEAP) Foo(...)
class ResourceObj ALLOCATION_SUPER_CLASS_SPEC {
 public:
  enum allocation_type { C_HEAP, RESOURCE_AREA };
  void* operator new(size_t size, allocation_type type);
  void* operator new(size_t size, Arena *arena) { return arena->Amalloc(size);}
  void* operator new(size_t size) { return resource_allocate_bytes(size); }
  void  operator delete(void* p) {} // nothing to do
};

//----------------------------------------------------------------------
// Base class for objects which don't get printing after all!
// (On some compilation systems, templates with virtuals are expensive.)
class NonPrintingResourceObj {
 public:
  void* operator new(size_t size, ResourceObj::allocation_type type) { return ResourceObj::operator new(size, type); }
  void* operator new(size_t size, Arena *arena)                      { return ResourceObj::operator new(size, arena); }
