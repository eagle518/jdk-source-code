...
#pragma ident "@(#)concurrentMarkSweepGeneration.inline.hpp	1.25 03/12/23 16:41:04 JVM"
#endif
/*
...
inline size_t CMSCollector::get_eden_used() const {
  ...
  return ((DefNewGeneration*)_young_gen)->eden()->used();
}
...
inline size_t CMSCollector::get_eden_capacity() const {
  assert(_young_gen->kind() == Generation::DefNew ||
         _young_gen->kind() == Generation::ParNew,
         "incorrect type for cast");
  return ((DefNewGeneration*)_young_gen)->eden()->capacity();
}
...
inline void CMSStats::record_gc0_end(size_t cms_gen_bytes_used) {
  ...
  gc_stats().avg_promoted()->sample(promoted_bytes);
  _gc0_promoted = (size_t) gc_stats().avg_promoted()->average();
