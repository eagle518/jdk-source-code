...
#pragma ident "@(#)archDesc.hpp	1.119 03/12/23 16:38:46 JVM"
#endif
/*
...
private:
  ...
  // Itemizing these here would not have been necessary if the operations
  // like ModF were subclassed objects.  We only special case
  // the operations which are treated differently on different architectures -
  // ie. the parser generates a call on one architecture but generates an
  // operation node on another.
  bool		_foundModFMatchRule;    // found ModF AD rule in <arch>.ad
  bool		_foundModDMatchRule;    // found ModD AD rule in <arch>.ad
  bool		_foundSinDMatchRule;    // found SinD AD rule in <arch>.ad
  bool		_foundCosDMatchRule;    // found CosD AD rule in <arch>.ad
  bool		_foundTanDMatchRule;    // found TanD AD rule in <arch>.ad
  bool		_foundAtanDMatchRule;   // found AtanD AD rule in <arch>.ad
  bool		_foundSqrtDMatchRule;   // found SqrtD AD rule in <arch>.ad
  bool		_foundPowDMatchRule;    // found PowD AD rule in <arch>.ad
...
public:
  ...
  const bool foundModFMatchRule(void) const {return _foundModFMatchRule;}
  void foundModFMatchRule(const bool b) {_foundModFMatchRule = b;}

  const bool foundModDMatchRule(void) const {return _foundModDMatchRule;}
  void foundModDMatchRule(const bool b) {_foundModDMatchRule = b;}

  const bool foundSinDMatchRule(void) const {return _foundSinDMatchRule;}
  void foundSinDMatchRule(const bool b) {_foundSinDMatchRule = b;}

  const bool foundCosDMatchRule(void) const {return _foundCosDMatchRule;}
  void foundCosDMatchRule(const bool b) {_foundCosDMatchRule = b;}

  const bool foundTanDMatchRule(void) const {return _foundTanDMatchRule;}
  void foundTanDMatchRule(const bool b) {_foundTanDMatchRule = b;}

  const bool foundAtanDMatchRule(void) const {return _foundAtanDMatchRule;}
  void foundAtanDMatchRule(const bool b) {_foundAtanDMatchRule = b;}

  const bool foundSqrtDMatchRule(void) const {return _foundSqrtDMatchRule;}
  void foundSqrtDMatchRule(const bool b) {_foundSqrtDMatchRule = b;}

  const bool foundPowDMatchRule(void) const {return _foundPowDMatchRule;}
  void foundPowDMatchRule(const bool b) {_foundPowDMatchRule = b;}
  ...
  // build flags indicating that our machine can rematerialize this instruction
  void build_rematerialize_map(FILE *fp_hpp, FILE *fp_cpp); 
  ...
  void syntax_err  ( const char *fmt, ...);
  int  emit_msg(int quiet, int flag, int linenum, const char *fmt, 
       va_list args);

  // Generator for "xxxSupported" Methods
  void buildInstructMatchChecks(FILE *fp_cpp) const;
