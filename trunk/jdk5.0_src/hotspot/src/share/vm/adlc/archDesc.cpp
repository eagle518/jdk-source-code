...
#pragma ident "@(#)archDesc.cpp	1.257 03/12/23 16:38:45 JVM"
#endif
/*
...
ArchDesc::ArchDesc()
  : _globalNames(cmpstr,hashstr, Form::arena), 
    ...
    _cisc_spill_operand(NULL) {
      ...
      _foundModFMatchRule    = false;
      _foundModDMatchRule    = false;
      _foundSinDMatchRule    = false;
      _foundCosDMatchRule    = false;
      _foundTanDMatchRule    = false;
      _foundAtanDMatchRule   = false;
      _foundSqrtDMatchRule   = false;
      _foundPowDMatchRule    = false;
...
//------------------------------syntax_err----------------------------------
// Issue a parser error message, and skip to the end of the current line
void ArchDesc::syntax_err(const char *fmt, ...) {
  va_list args;

  va_start(args, fmt);
  _internal_errs += emit_msg(0, SYNERR, 0, fmt, args);  
  va_end(args);
...
void ArchDesc::addSunCopyright(FILE *fp) {
  ...
  fprintf(fp,"\n");
  fprintf(fp,"// Copyright 1997-1998 Sun Microsystems, Inc. 901 San Antonio Road,\n");
  fprintf(fp,"// Palo Alto, California, 94303, U.S.A.  All Rights Reserved.\n");
  fprintf(fp,"//\n");
  ...
  fprintf(fp,"// with Sun.\n");
  fprintf(fp,"//\n");
  fprintf(fp,"// CopyrightVersion 1.2\n");
}
