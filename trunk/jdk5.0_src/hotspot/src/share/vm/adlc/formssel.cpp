...
#pragma ident "@(#)formssel.cpp	1.162 04/05/06 13:43:11 JVM"
#endif
/*
...
//==============================Instructions===================================
//------------------------------InstructForm-----------------------------------
InstructForm::InstructForm(char *id, bool ideal_only)
...
bool InstructForm::needs_projections() {
  bool projections = (num_defs_or_kills() > 1 && (sets_result() || !_matrule))
                  || (num_defs_or_kills() > 0 && !sets_result() && _matrule);
...
int InstructForm::is_expensive() const {
  return _matrule ? _matrule->is_expensive() : 0;
}
...
Form::CallType InstructForm::is_ideal_call() const {
  ...
  idx = 0;
  if(_matrule->find_type("CallCompiledJava",idx)) return Form::JAVA_COMPILED;
  idx = 0;
  if(_matrule->find_type("CallInterpreter",idx))  return Form::JAVA_INTERP;
  idx = 0;
  ...
  idx = 0;
  if(_matrule->find_type("CallClearArray",idx))   return Form::JAVA_CLEARARR;
  idx = 0;
	  ...
	  op->interface_type(globals) == Form::register_interface ) {
        ...
        do {
          if( comp->isa(Component::USE) && strcmp(comp->_name,def_name)==0 ) {
            return _components.operand_position_format(def_name);
          }
...
bool InstructForm::rematerialize(FormDict &globals, RegisterForm *registers ) {
  ...
  if ( is_ideal_load() != Form::none || // Ideal load?  Do not rematerialize
       ...
       !strcmp(_ident,"tlsLoadP") ) {   // Thread-local pointer?
    rematerialize = false;
  }
...
bool InstructForm::check_for_anti_dependence(FormDict &globals) const {
...
bool InstructForm::captures_bottom_type() const {
  if( _matrule && _matrule->_rChild && 
       (!strcmp(_matrule->_rChild->_opType,"CastPP")     ||  // new result type
        !strcmp(_matrule->_rChild->_opType,"CastL2P")    ||  // new result type
...
void InstructForm::build_components() {
  ...
  for (_parameters.reset(); (name = _parameters.iter()) != NULL;) {
    ...
    if ( component  == NULL ) {
      if(e) _components.insert(name, opForm->_ident, e->_use_def, false);
      else  _components.insert(name, opForm->_ident, Component::INVALID, false);
    }
    else if (e) {
      // Component was found in the list
      Component  *comp = (Component*)component;
      // Check if there is a new effect that requires an extra component.
      // This happens when adding 'USE' to a component that is not yet a USE.
      if( ! (comp->isa( Component::USE ))
          && ((e->_use_def & Component::USE) != 0) ) {
      ...
      assert( ! (comp->isa(Component::DEF) && (pos >= 1)), 
...
int   InstructForm::label_position() {
  return _components.label_position();
}

int   InstructForm::method_position() {
  return _components.method_position();
}
    ...
    if (comp->isa( Component::DEF )) {
      if( _matrule && _matrule->_rChild && 
          (is_store_to_memory(_matrule->_rChild->_opType) ||
           !strcmp(_matrule->_rChild->_opType,"ClearArray")) ) {
        result = "Universe";
      } else {
        result = comp->_type;
      }
    }
    else result = "Universe";
  }
  else result = "Universe";
  return result;
}
...
const char *InstructForm::mach_base_class()  const {
  ...
  else if( is_ideal_call() == Form::JAVA_INTERP ) {
    return "MachCallInterpreterNode";
  }
  else if( is_ideal_call() == Form::JAVA_COMPILED ) {
    return "MachCallCompiledJavaNode";
  }
  ...
  else if( is_ideal_call() == Form::JAVA_CLEARARR ) {
    return "MachCallClearArrayNode";
  }
...
bool InstructForm::cisc_spills_to(ArchDesc &AD, InstructForm *instr) {
  ...
  if( (cisc_spill_operand != Not_cisc_spillable) && (op_name != NULL) && equivalent_predicates(this, instr) ) {
    ...
    if( def_oper == NameList::Not_in_list ) {
      // Do not support cisc-spilling for destination operands
      ...
      if( AD._cisc_spill_debug ) {
        ...
        fprintf(stderr, "   using operand %s at index %d\n", op_name, cisc_spill_operand);
      }
...
void InstructForm::rep_var_format(FILE *fp, const char *rep_var) {
  ...
  int          idx   = _components.operand_position_format(rep_var);
  if ( idx == -1 ) {
  ...
  if (idx == 0) 
    fprintf(fp,"int_format(ra, this);\n");
  else
    fprintf(fp,"ext_format(ra, this,idx%d);\n", idx );
}
...
void InstructForm::index_temps(FILE *fp, FormDict &globals, const char *prefix, const char *receiver) {
  ...
  // Generate starting points for other operands if they exist
  for ( idx = 2; idx < num_opnds(); ++idx ) {
    if( *receiver == 0 ) {
  ...
  if( *receiver == 0 )
    fprintf(fp,"      unsigned %sidx%d = req(); \n", prefix, idx);
  else
...
void InstructForm::output(FILE *fp) {
  ...
  if (_effects.Size()) fprintf(fp,"Effects\n");
...
void EncClass::add_parameter(char *parameter_type, char *parameter_name) {
...
void EncClass::add_code(char *code) {
  _code.addName(code);
}
...
void  OperandForm::int_format(FILE *fp, FormDict &globals, uint index) {
  ...
  if (_matrule && _matrule->is_base_register(globals)) {
    ...
    fflush(fp);
    fprintf(stderr,"tty->print(\"No format defined for %s\n\");\n", _ident);
...
bool Component::is(int use_def_kill_enum) {
  return (_usedef == use_def_kill_enum ? true : false);
}
// True if this component is used/def'd/kill'd as the parameter suggests.
bool Component::isa(int use_def_kill_enum) {
  // '&' will not produce non-zero result unless usedef info is defined.
  return ((_usedef==use_def_kill_enum) || (_usedef & use_def_kill_enum)
          ? true : false);
}
...
void Component::output(FILE *fp) {
  ...
  fprintf(fp, ", use/def = %s", 
          _usedef == USE ? "USE" : _usedef == DEF ? "DEF" 
          : _usedef == USE_DEF ? "USE_DEF" : _usedef == KILL ? "KILL"
          : _usedef == USE_KILL ? "USE_KILL" : "Undefined Use/Def info");
  fprintf(fp, "\n");
}
...
int Attribute::int_val(ArchDesc &ad) {
  ...
  if (!_val || !ADLParser::is_int_token(_val, result)) {
    ad.syntax_err("Attribute %s must have an integer value: %s",
		  _ident, _val ? _val : "");
  }
  return result;
}
