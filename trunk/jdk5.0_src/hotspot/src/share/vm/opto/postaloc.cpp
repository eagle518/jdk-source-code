...
#pragma ident "@(#)postaloc.cpp	1.70 04/05/13 09:57:25 JVM"
#endif
/*
...
int PhaseChaitin::use_prior_register( Node *n, uint idx, Node *def, Block *current_block, Node_List &value, Node_List &regnd ) {
  ...
  if( _matcher.is_save_on_entry(def_reg) &&
      may_be_copy_of_callee(def) ) {
...
int PhaseChaitin::elide_copy( Node *n, int k, Block *current_block, Node_List &value, Node_List &regnd, bool can_change_regs ) {
  ...
  for( uint reg = 0; reg < (uint)_max_reg; reg++ ) {
    ...
    if( vv == val ||            // Got a direct hit?
        (t && vv && vv->bottom_type() == t) ) { // Or same constant?
      assert( !n->is_Phi(), "cannot change registers at a Phi so easily" );
      if( nk_reg >= (int)SharedInfo::stack0 || // CISC-loading from stack OR
          reg < (uint)SharedInfo::stack0 || // turning into a register use OR
...
void PhaseChaitin::post_allocate_copy_removal() {
  Compile::TracePhase t3("postAllocCopyRemoval", &_t_postAllocCopyRemoval, TimeCompiler);
  ResourceMark rm;
  ...
  for( uint i = 0; i < _cfg._num_blocks; i++ ) {
    ...
    for( j = phi_dex; j < b->_nodes.size(); j++ ) {
      ...
      const uint two_adr = mach ? mach->two_adr() : 0;
      // Don't allow operands of box nodes to be elided
      const bool box_handle = mach ? mach->ideal_Opcode() == Op_Box : false;
      
      // Remove copies along input edges
      for( k = 1; k < n->req(); k++ ) 
        j -= elide_copy( n, k, b, value, regnd, two_adr!=k && !box_handle );
      ...
      if( n_ideal_reg == MachProjNode::fat_proj ) {
        ...
        while( (nreg = rm.find_first_elem()) != OptoReg::Bad ) {
          rm.Remove(nreg);
          value.map(nreg,n);
          regnd.map(nreg,n);
        }
