...
#pragma ident "@(#)coalesce.cpp	1.185 03/12/23 16:42:21 JVM"
#endif
/*
      ...
      if( phi ) {
        ...
        for( uint j = 1; j<cnt; j++ ) {
          ...
          if( src_name != phi_name ) {
            ...
            Node *copy;
            MachNode *machdef = m->is_Mach();
            assert(!m->is_Con() || machdef != NULL, "all Con must be Mach");
            // Rematerialize constants instead of copying them
            if( machdef && machdef->is_Con() && machdef->rematerialize() ) {
              copy = m->clone();
              ...
              copy = new MachSpillCopyNode(m,*rm,*rm);
        ...
        if( mach && (idx=mach->two_adr()) ) {
          ...
          if( _phc.Find(m) != name ) {
            ...
            MachNode *machdef = m->is_Mach(); 
            assert(!m->is_Con() || machdef != NULL, "all Con must be Mach");
            if( machdef && machdef->rematerialize() ) {
              copy = m->clone();
        ...
        if((b->_freq < OPTO_DEBUG_SPLIT_FREQ) && mach && (mach->is_MachSafePoint() != NULL)) {
          ...
          for(uint inpidx = debug_start; inpidx < debug_end; inpidx++) {
            ...
            if( lrg._maxfreq >= OPTO_LRG_HIGH_FREQ ) {
              ...
              Node *copy = new MachSpillCopyNode( inp, *rm, *rm );
              // Insert the copy in the use-def chain
