...
#pragma ident "@(#)parse2.cpp	1.339 04/05/04 13:26:18 JVM"
#endif
/*
...
bool Parse::create_jump_tables(Node* key_val, SwitchRange* lo, SwitchRange* hi) {
  ...
  if (!Matcher::jumpTableSupported())  return false;
  ...
  int lowval = lo->lo();
  key_val = _gvn.transform( new (3) SubINode(key_val, _gvn.intcon(lowval)) );
   
  // Generate a guard to protect against input keyvals that aren't
  // in the switch domain.
  if (needs_guard) {
    Node*   size = _gvn.intcon(num_cases);
    Node*   cmp = _gvn.transform( new (3) CmpUNode(key_val, size) );
    Node*   tst = _gvn.transform( new (2) BoolNode(cmp, BoolTest::ge) );
  ...
  for (SwitchRange* r = lo; r <= hi; r++) {
    for (int j = r->lo(); j <= r->hi(); j++, i++) {
      Node* input = _gvn.transform(new (1) JumpProjNode(jtn, i, r->dest(), j - lowval));
      {
        PreserveJVMState pjvms(this);
        set_control(input);
...
void Parse::l2f() {
  Node* f2 = pop();
  Node* f1 = pop();
  CallRuntimeNode *call = new CallLeafNode(OptoRuntime::l2f_Type(), CAST_FROM_FN_PTR(address, SharedRuntime::l2f), "l2f");
...
void Parse::do_ifnull(BoolTest::mask btest) {
  ...
  if( prob == PROB_UNKNOWN )
    prob =  (btest == BoolTest::eq) ? PROB_STATIC_INFREQUENT : PROB_STATIC_FREQUENT;
