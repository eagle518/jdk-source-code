...
#pragma ident "@(#)chaitin.cpp	1.92 04/03/26 14:51:48 JVM"
#endif
/*
...
void LRG::dump( ) const {
  ...
  if( _def == NodeSentinel ) tty->print("MultiDef ");
...
PhaseChaitin::PhaseChaitin(uint unique, PhaseCFG &cfg, Matcher &matcher)
  : PhaseRegAlloc(unique, cfg, matcher,
    ...
    _oldphi(unique)
{
  Compile::TracePhase t3("ctorChaitin", &_t_ctorChaitin, TimeCompiler);
  uint i,j;
  // Build a list of basic blocks, sorted by frequency
  _blks = NEW_RESOURCE_ARRAY( Block *, _cfg._num_blocks );
  // Experiment with sorting strategies to speed compilation
  double  cutoff = 1.0;            // Cutoff for high frequency bucket
...
void PhaseChaitin::Register_Allocate() {
  ...
  {
    Compile::TracePhase t3("computeLive", &_t_computeLive, TimeCompiler);
    _live = NULL;                 // Mark live as being not available
    rm.reset_to_mark();           // Reclaim working storage
    IndexSet::reset_memory(&live_arena);
  ...
  if( stretch_base_pointer_live_ranges(&live_arena) ) {
    Compile::TracePhase t3("computeLive (sbplr)", &_t_computeLive, TimeCompiler);
  ...
  _allocator_attempts += _trip_cnt;
  ++_allocator_successes;
  ...
  const int stack_alignment_in_words = Matcher::stack_alignment() - LogBitsPerInt; 
  if (stack_alignment_in_words > 0)
    _framesize = round_to(_framesize, 1 << stack_alignment_in_words);
...
void PhaseChaitin::gather_lrg_masks( bool after_aggressive ) {
  ...
  // Reset all live-ranges-per-inner-loops
  ...
  for( uint i = 0; i < _cfg._num_blocks; i++ ) {
    ...
    for( uint j = 1; j < b->_nodes.size(); j++ ) {
      ...
      if( mach ) input_edge_start = mach->oper_input_base();
      uint idx = n->is_Copy();
      ...
      if( vreg ) {              // No vreg means un-allocable (e.g. memory)
        ...
        // Add def'd live range to inner loop collection
        if( b->_inner_loop ) {
          // Don't count useless Phis as def's
          if( !(b->_inner_loop->_head == b && n->is_Phi() && n->in(2) == n) )
            b->_inner_loop->_ud_lrgs.set(vreg);
        }
      ...
      for( uint k = input_edge_start; k < cnt; k++ ) {
        ...
        // Add use'd live range to inner loop collection
        if( b->_inner_loop ) {
          if( !(b->_inner_loop->_head == b && n->is_Phi() && n->in(2) == n) )
            b->_inner_loop->_ud_lrgs.set(vreg);
        }
...
void PhaseChaitin::cache_lrg_info( ) {
  ...
  for( uint i = 1; i < _maxlrg; i++ ) {
    ...
    if( lrg.lo_degree() ||
      ...
      if( hi_reg >= SharedInfo::stack0 ) { // Can go to stack?
...
void PhaseChaitin::Simplify( ) {
  ...
  while( 1 ) {                  // Repeat till simplified it all
    ...
    uint lo_cost = _hi_degree;
    double cost = lrgs(lo_cost).score();
    ...
    for( uint i = _hi_degree; i; i = lrgs(i)._next ) {
      ...
      double icost = lrgs(i).score();

      // Compare cost/area of i vs cost/area of lo_cost.  Smaller cost/area
...
uint PhaseChaitin::Select( ) {
  ...
  while( _simplified ) {
    ...
    if(OptoReg::add(reg,-chunk) == OptoReg::Bad && is_allstack) {
...
void PhaseChaitin::fixup_spills() {
  ...
  Compile::TracePhase t3("fixupSpills", &_t_fixupSpills, TimeCompiler);
  ...
  for( uint i = 0; i < _cfg._num_blocks; i++ ) {
    ...
    for( uint j = 1; j <= last_inst; j++ ) {
      ...
      if( inp != AdlcVMDeps::Not_cisc_spillable ) {
        ...
        if( src_reg >= SharedInfo::stack0 ) { // If input is on stack
...
Node *PhaseChaitin::find_base_for_derived( Node **derived_base_map, Node *derived, uint &maxlrg ) {
  ...
  if( derived->is_Con() ) {
    Node *base = new (1) ConPNode( TypePtr::NULL_PTR );
  ...
  base = new PhiNode( derived->in(0), base->bottom_type() );
  for( i = 1; i < derived->req(); i++ )
    base->set_req(i, find_base_for_derived(derived_base_map,derived->in(i),maxlrg));
...
bool PhaseChaitin::stretch_base_pointer_live_ranges( ResourceArea *a ) {
  ...
  for( uint i=0; i<_cfg._num_blocks; i++ ) {
    ...
    for( uint j = b->end_idx() + 1; j > 1; j-- ) {
      ...
      // phi.
      MachNode *mach = n->is_Mach();
      if( mach && mach->ideal_Opcode() == Op_CmpI ) {
        PhiNode *phi = n->in(1)->is_Phi();
        if( phi && phi->region()->is_Loop() ) {
          Block *phi_block = _cfg._bbs[phi->_idx];
          if( _cfg._bbs[phi_block->pred(2)->_idx] == b ) {
            const RegMask *mask = C->matcher()->idealreg2spillmask[Op_RegI];
            Node *spill = new MachSpillCopyNode( phi, *mask, *mask );
...
void PhaseChaitin::dump( const Node *n ) const {
  ...
  if( r && n->Opcode() != Op_Phi ) {
    if( _node_regs ) {          // Got a post-allocation copy of allocation?
      tty->print("[");
      OptoReg::Name hi = get_reg_hi(n);
      if( hi != OptoReg::Bad ) {
        if( hi < SharedInfo::stack0 )
          tty->print("%s:",Matcher::regName[hi]);
        else
          tty->print("%s+%d:",SharedInfo::regName[SharedInfo::c_frame_pointer], reg2offset_unchecked(hi));
      }
      OptoReg::Name lo = get_reg_lo(n);
      if( lo < SharedInfo::stack0 )
        tty->print("%s]",Matcher::regName[lo]);
      else
         tty->print("%s+%d]",SharedInfo::regName[SharedInfo::c_frame_pointer], reg2offset_unchecked(lo));
    } else
    n->out_RegMask().dump();
  }
  tty->print("/N%d\t",n->_idx);
  const MachNode *mach = ((Node*)n)->is_Mach();
  tty->print("%s === ", n->Name());
  uint k;
  for( k = 0; k < n->req(); k++) {
    Node *m = n->in(k);
    if( !m ) tty->print("_ ");
      ...
      if( r && n->Opcode() != Op_Phi && n->Opcode() != Op_Proj && n->Opcode() != Op_SCMemProj) {
        if( _node_regs ) {
          tty->print("[");
          OptoReg::Name hi = get_reg_hi(n->in(k));
          if( hi != OptoReg::Bad ) {
            if( hi < SharedInfo::stack0 )
              tty->print("%s:",Matcher::regName[hi]);
            else
              tty->print("%s+%d:",SharedInfo::regName[SharedInfo::c_frame_pointer],
                         reg2offset_unchecked(hi));
          }
          OptoReg::Name lo = get_reg_lo(n->in(k));
          if( lo < SharedInfo::stack0 )
            tty->print("%s]",Matcher::regName[lo]);
          else
            tty->print("%s+%d]",SharedInfo::regName[SharedInfo::c_frame_pointer],
                       reg2offset_unchecked(lo));
        } else
          n->in_RegMask(k).dump();
      }
...
static char *print_reg( OptoReg::Name reg, const PhaseChaitin *pc, char *buf ) {
  ...
  else if (reg < SharedInfo::stack0)
    strcpy(buf, Matcher::regName[reg]);
  else
    sprintf(buf,"%s + #%d",SharedInfo::regName[SharedInfo::c_frame_pointer],
            pc->reg2offset(reg));
...
char *PhaseChaitin::dump_register( const Node *n, char *buf  ) const {
  if( !this ) {                 // Not got anything?
    ...
    print_reg( get_reg_lo(n), this, buf );
  } else {
...
void PhaseChaitin::dump_frame() const {
  const char *fp = SharedInfo::regName[SharedInfo::c_frame_pointer];
  const TypeTuple *domain = C->tf()->domain();
  const int        argcnt = domain->cnt() - TypeFunc::Parms;

  // Incoming arguments in registers dump
  for( int k = 0; k < argcnt; k++ ) {
    int parmreg = _matcher._parm_regs[k].lo();
    if( parmreg < SharedInfo::stack0 && parmreg != OptoReg::Bad )  {
      const char *reg_name = SharedInfo::regName[parmreg];
      tty->print("#r%3.3d %s", parmreg, reg_name);
      parmreg = _matcher._parm_regs[k].hi();
      if( parmreg < SharedInfo::stack0 && parmreg != OptoReg::Bad )
        tty->print(":%s",SharedInfo::regName[parmreg]);
      tty->print("   : parm %d: ", k);
  ...
  while( reg > begin_in_arg ) {
    ...
    int j;
    for( j = 0; j < argcnt; j++) {
      if( _matcher._parm_regs[j].lo() == reg ||
          _matcher._parm_regs[j].hi() == reg ) {
        tty->print("parm %d: ",j);
  ...
  while( reg > SharedInfo::stack0 ) {
    ...
    else if ((int)SharedInfo::reg2stack(reg) < C->sync()*C->sync_stack_slots())
      tty->print_cr("Lock %d", SharedInfo::reg2stack(reg)/C->sync_stack_slots());
    else
