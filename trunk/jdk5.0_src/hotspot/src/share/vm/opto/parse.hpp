...
#pragma ident "@(#)parse.hpp	1.250 04/05/04 13:26:14 JVM"
#endif
/*
...
class Parse : public GraphKit {
 public:
  ...
  class Block {
   private:
    ...
    BitMap             _live_locals;    // lazily initialized liveness bitmap
 ...
 private:
  ...
  Node *fetch_monitor(int index, Node *box, Node *locks_addrs);
  Node* check_interpreter_type(Node* l, const Type* type, SafePointNode* &bad_type_exit);
  void  load_interpreter_state(Node* locks_addrs, Node* local_addrs);
  ...
  void optimize_inlining( /*  inputs */ciMethod* caller, int bci, ciInstanceKlass* klass, ciMethod *dest_method,
  ...
  float   branch_prediction(float &cnt);

  void    do_ifnull(BoolTest::mask btest);
  void    do_if(Node* a, Node* b, BoolTest::mask btest, Node* c);
  ...
  void profile_virtual_call(Node* receiver);
 ...
 public:
  ...
  void show_parse_info() PRODUCT_RETURN;
#ifndef PRODUCT
...
extern FastLockNode *native_lock( PhaseGVN &gvn, Node *obj, SafePointNode *map, Node *top, Node *null );
extern void native_unlock( PhaseGVN &gvn, FastLockNode *lock, Node *map, Node *top, Node *null );
