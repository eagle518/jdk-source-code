...
#pragma ident "@(#)methodDataOop.hpp	1.39 04/03/02 02:08:45 JVM"
#endif
/*
...
public:
  ...
  // on that BCI.
  int trap_state() {
    return ((_struct._flags >> trap_shift) & trap_mask);
  }

  void set_trap_state(int new_state) {
    assert(ProfileTraps, "used only under +ProfileTraps");
    uint old_flags = (_struct._flags & flag_mask);
    _struct._flags = (new_state << trap_shift) | old_flags;
  ...
  void release_set_header(intptr_t value) {
    OrderAccess::release_store_ptr(&_header, value);
  }
  intptr_t header() {
    return _header;
  }
  ...
  static intptr_t flag_number_to_constant(int bitpos) {
    assert(0 <= bitpos && bitpos < flag_limit, "oob");
    DataLayout temp;
    temp.set_header(0);
    temp._struct._flags = 0x1 << bitpos;
    return temp.header();
  }
};
...
// BitData
//
// A BitData corresponds to a one-bit flag.
// This is used by the interpreter when it encounters a suprise null.
// (Currently, this is only done for checkcast bytecodes.)
class BitData : public ProfileData {
protected:
  enum {
    null_seen = DataLayout::first_flag
  };
...
public:
  ...
  // This flag bit is specially known to the interpreter.
...
public:
  ...
  static int receiver_cell_index(uint row) {
    return receiver0_offset + row * virtual_call_row_cell_count;
  }
  static int receiver_count_cell_index(uint row) {
    return count0_offset + row * virtual_call_row_cell_count;
  }
