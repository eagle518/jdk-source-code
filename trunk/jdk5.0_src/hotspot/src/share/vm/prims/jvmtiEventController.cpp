...
#pragma ident "@(#)jvmtiEventController.cpp	1.39 04/05/18 15:25:07 JVM"
#endif
/*
...
public:
  ...
  static jlong recompute_env_enabled(int env_index);
  static jlong recompute_env_thread_enabled(int env_index, JvmtiThreadState *state);
...
jlong 
JvmtiEventControllerPrivate::recompute_env_enabled(int env_index) {
  JvmtiEnv *env = JvmtiEnv::env_at(env_index);
...
jlong 
JvmtiEventControllerPrivate::recompute_env_thread_enabled(int env_index, JvmtiThreadState *state) {
  JvmtiEnvThreadState *ets = state->env_thread_state(env_index);
...
JvmtiEventControllerPrivate::recompute_thread_enabled(JvmtiThreadState *state) {
  ...
  int env_count = JvmtiEnvBase::env_count();
  for (int env_index = 0; env_index < env_count ; ++env_index) {
    any_env_enabled |= recompute_env_thread_enabled(env_index, state);
  } /* end env loop */
...
JvmtiEventControllerPrivate::recompute_enabled() {
  ...
  int env_count = JvmtiEnvBase::env_count();
  for (int env_index = 0; env_index < env_count ; ++env_index) {
    any_env_thread_enabled |= recompute_env_enabled(env_index);
  }
  ...
  if (delta != 0) {
    ...
    JvmtiExport::set_should_post_thread_life(any_env_thread_enabled & NEED_THREAD_LIFE_EVENTS); 
    ...
    if (delta & SINGLE_STEP_BIT) {
      ...
      case JVMTI_PHASE_DEAD:
        ...
        break;
      case JVMTI_PHASE_LIVE: {
        VM_ChangeSingleStep op(any_env_thread_enabled & SINGLE_STEP_BIT);
        VMThread::execute(&op);
        break;
      }
      default:
...
JvmtiEventControllerPrivate::thread_started(JavaThread *thread) {
  ...
  assert(JvmtiEnvBase::env_count() > 0, "to enter event controller, JVMTI environments must exist");
...
JvmtiEventControllerPrivate::env_initialize(JvmtiEnvBase *env) {
  ...
  for (JvmtiThreadState *state = JvmtiThreadState::first(); state != NULL; state = state->next()) {
    state->add_env(env);
    assert(state->env_thread_state(env->index())->get_env() == env, 
           "JvmtiEnvThreadState array out of sequence");
  }
  JvmtiEventControllerPrivate::recompute_enabled();
}
...
JvmtiEventControllerPrivate::env_dispose(JvmtiEnvBase *env) {
  ...
  env->dispose();
  // remove the JvmtiEnvThreadState from each JvmtiThreadState
  for (JvmtiThreadState *state = JvmtiThreadState::first(); state != NULL; state = state->next()) {
    state->remove_env(env);
    assert(state->env_count() == JvmtiEnv::env_count(), "sanity check");
  }
  JvmtiEventControllerPrivate::recompute_enabled();
}
