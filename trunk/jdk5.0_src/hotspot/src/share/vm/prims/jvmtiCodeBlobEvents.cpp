...
#pragma ident "@(#)jvmtiCodeBlobEvents.cpp	1.7 03/12/23 16:43:14 JVM"
#endif
/*
...
void CodeBlobCollector::do_blob(CodeBlob* cb) {
  static const char* OSRAdapter_name = "OSRAdapter";
#ifdef COMPILER2
  static const char* I2CAdapter_name = "I2CAdapter";
  static const char* C2IAdapter_name = "C2IAdapter";
#endif /* COMPILER2 */
  ...
  if (cb->is_i2c_adapter()) {
    name = I2CAdapter_name;
  }
  if (cb->is_c2i_adapter()) {
    name = C2IAdapter_name;
  }
#endif /* COMPILER2 */
 ...
 public:     
  ...
  ~nmethodCollector() {
	...
	if (blob->map()!= NULL) {
      ...
      _nmethods->clear_and_deallocate();
      FreeHeap(_nmethods);
    }
  }
							...
							jint *map_length_ptr)
{
  ...
  if (!mh->is_native()) {
	...
	assert(map_length < pcds_in_method, "checking");
        map[map_length].start_address = (const void*)pcd->pc_offset();
