...
<xsl:template match="function" mode="transition">
  ...
  <xsl:choose> 
    <xsl:when test="$interface='jvmdi' or count(@callbacksafe)=0 or not(contains(@callbacksafe,'safe'))">
  ...
  <xsl:if test="count(@impl)=0 or not(contains(@impl,'innative'))">
    ...
    <xsl:value-of select="$space"/>
    <xsl:text>debug_only(VMNativeEntryWrapper __vew;) 
</xsl:text>
  </xsl:if>
</xsl:template>
...
static jvmtiError JNICALL
  ...
  if (Threads::number_of_threads() == 0) {
    ...
    transition = ((this_thread != NULL) &amp;&amp; !this_thread->is_VM_thread());
  }
...
<xsl:template match="jclass" mode="dochecks">
 ...
 <xsl:if test="$interface='jvmdi' or count(@method)=0">
...
<xsl:template match="jmethodID" mode="dochecks">
  <xsl:param name="name"/>
  <xsl:text>  if (!jniIdSupport::is_method(</xsl:text>
  <xsl:value-of select="$name"/>
  <xsl:text>)) {&#xA;</xsl:text>
  <xsl:apply-templates select=".." mode="traceError">     
  ...
  <xsl:text>&#xA;</xsl:text>
  <xsl:text>  }&#xA;</xsl:text>
  <xsl:text>  methodOop method_oop = jniIdSupport::to_method_oop(</xsl:text>
  <xsl:value-of select="$name"/>
  <xsl:text>);&#xA;</xsl:text>
...
<xsl:template match="jobject|jvalue|jthreadGroup|enum|jlong|jfloat|jdouble|jlocation|jboolean|char|uchar|void|struct" mode="dochecks">
</xsl:template>
...
<xsl:template match="uchar" mode="traceInFormat">
  <xsl:param name="name"/>
  <xsl:text> </xsl:text>
  <xsl:value-of select="$name"/>
  <xsl:text>=0x%x</xsl:text>
...
<xsl:template match="jint|jlocation|jlong|jfloat|jdouble|char|uchar" mode="traceInValue">
  <xsl:param name="name"/>
  <xsl:text>, </xsl:text>
  <xsl:value-of select="$name"/>
</xsl:template>
