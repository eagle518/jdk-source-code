...
#pragma ident "@(#)psMarkSweepDecorator.cpp	1.13 03/12/23 16:40:14 JVM"
#endif
/*
...
void PSMarkSweepDecorator::precompact() {
  ...
  while (q < t) {                                                            
    ...
    if (oop(q)->is_gc_marked()) {  
      ...
      size_t compaction_max_size = pointer_delta(compact_end, compact_top, sizeof(HeapWord*));
	...
	compaction_max_size = 
	  pointer_delta(compact_end, compact_top, sizeof(HeapWord*));
      }
...
bool PSMarkSweepDecorator::insert_deadspace(ssize_t& allowed_deadspace_words, 
  ...
  if (allowed_deadspace_words >= 0) {
    ...
    if (deadlength >= aligned_min_int_array_size) {
      ...
      assert(((deadlength - aligned_min_int_array_size) * (HeapWordSize/sizeof(jint))) < max_jint,
