...
#pragma ident "@(#)javaCalls.cpp	1.204 03/12/23 16:43:52 JVM"
#endif
/*
...
void JavaCalls::call_virtual(JavaValue* result, KlassHandle spec_klass, symbolHandle name, symbolHandle signature, JavaCallArguments* args, TRAPS) {  
  ...
  KlassHandle recvrKlass(THREAD, receiver.is_null() ? NULL : receiver->klass());
...
void JavaCalls::call(JavaValue* result, methodHandle method, JavaCallArguments* args, TRAPS) {
  ...
  if (UseNewOSExceptions) {
    // Need to wrap each and everytime, since there might be native code down the
    // stack that has installed its own exception handlers
    os::os_exception_wrapper(call_helper, result, &method, args, THREAD);
  } else {
    // Old-style global exception handler
    call_helper(result, &method, args, CHECK);
  } 
}
...
void JavaCalls::call_helper(JavaValue* result, methodHandle* m, JavaCallArguments* args, TRAPS) {
  ...
  // We always call the interpreter. The interpreter entry will check for
  // the existence of compiled code and transfer to compiled code if so.
  address entry_point = AbstractInterpreter::entry_for_method(method);      
  ...
  // Find receiver
  Handle receiver = (!method->is_static()) ? args->receiver() : Handle(THREAD, (oop)NULL);
...
intptr_t* JavaCallArguments::parameters() {
  // Fist convert all handles to oops
