...
#pragma ident "@(#)reflection.cpp	1.160 04/03/30 15:35:02 JVM"
#endif
/*
...
oop Reflection::box(jvalue* value, BasicType type, TRAPS) {
  ...
  oop result = java_lang_boxing_object::create(type, value, CHECK_0);
  if (result == NULL) {
...
BasicType Reflection::array_get(jvalue* value, arrayOop a, int index, TRAPS) {
  ...
  if (a->is_objArray()) {
    ...
    BasicType type = typeArrayKlass::cast(a->klass())->type();
    switch (type) {
      case T_BOOLEAN:
...
void Reflection::array_set(jvalue* value, arrayOop a, int index, BasicType value_type, TRAPS) {
  ...
  if (a->is_objArray()) {
    ...
    assert(a->is_typeArray(), "just checking");
    BasicType array_type = typeArrayKlass::cast(a->klass())->type();
...
oop Reflection:: basic_type_arrayklass_to_mirror(klassOop basic_type_arrayklass, TRAPS) {
  BasicType type = typeArrayKlass::cast(basic_type_arrayklass)->type();
...
arrayOop Reflection::reflect_new_array(oop element_mirror, jint length, TRAPS) {
  ...
  if (java_lang_Class::is_primitive(element_mirror)) {
    klassOop tak = basic_type_mirror_to_arrayklass(element_mirror, CHECK_0);
...
arrayOop Reflection::reflect_new_multi_array(oop element_mirror, typeArrayOop dim_array, TRAPS) {
  ...
  assert(typeArrayKlass::cast(dim_array->klass())->type() == T_INT, "just checking");
  ...
  int dim = len;
  if (java_lang_Class::is_primitive(element_mirror)) {
    klass = basic_type_mirror_to_arrayklass(element_mirror, CHECK_0);
  } else {
  ...
  klass = Klass::cast(klass)->array_klass(dim, CHECK_0);
  oop obj = arrayKlass::cast(klass)->multi_allocate(len, dimensions, 1, THREAD);
...
oop Reflection::array_component_type(oop mirror, TRAPS) {
  ...
  if (arrayKlass::cast(klass)->dimension() == 1) {
    if (Klass::cast(klass)->oop_is_typeArray()) {
      return basic_type_arrayklass_to_mirror(klass, CHECK_0);
    } else {
      return Klass::cast(objArrayKlass::cast(klass)->element_klass())->java_mirror();
    }
  } else {
    klassOop lower_dim = arrayKlass::cast(klass)->lower_dimension();
    assert(Klass::cast(lower_dim)->oop_is_array(), "just checking");
    return Klass::cast(lower_dim)->java_mirror();
  }
}
...
bool Reflection::verify_class_access(klassOop current_class, klassOop new_class, bool classloader_only) {    
  ...
  return classloader_only &&
    Verifier::relax_verify_for(instanceKlass::cast(current_class)->class_loader()) &&
    instanceKlass::cast(current_class)->protection_domain() == instanceKlass::cast(new_class)->protection_domain() &&
    instanceKlass::cast(current_class)->class_loader() == instanceKlass::cast(new_class)->class_loader();
}    
...
bool Reflection::verify_field_access(klassOop current_class, 
  ...
  // The override_relax flag can be used to prevent relax checking. This is e.g. needed 
  // for the JVM_AllocateNewObject method
  return classloader_only &&
    Verifier::relax_verify_for(instanceKlass::cast(current_class)->class_loader()) &&
    instanceKlass::cast(current_class)->protection_domain() == instanceKlass::cast(field_class)->protection_domain() &&
    instanceKlass::cast(current_class)->class_loader() == instanceKlass::cast(field_class)->class_loader();
}
...
oop get_mirror_from_signature(methodHandle method, SignatureStream* ss, TRAPS) {
  switch (ss->type()) {
    ...
    case T_OBJECT:
    case T_ARRAY:
      symbolOop name        = ss->as_symbol(CHECK_0);
...
oop Reflection::new_method(methodHandle method, bool intern_name, bool for_constant_pool_access, TRAPS) {
  ...
  assert(!method()->is_initializer() ||
    ...
    && Klass::cast(method()->method_holder())->is_interface() && Universe::is_jdk12x_version()), "should call new_constructor instead");
  instanceKlassHandle holder (THREAD, method->method_holder());
  int                 slot   = holder->method_index_for(method(), CHECK_0);

  symbolHandle signature (THREAD, method->signature());
  int parameter_count = ArgumentCount(signature).size();
  oop return_type_oop = NULL;
  objArrayHandle parameter_types = get_parameter_types(method, parameter_count, &return_type_oop, CHECK_0);
  if (parameter_types.is_null() || return_type_oop == NULL) return NULL;
  
  Handle return_type(THREAD, return_type_oop);

  objArrayHandle exception_types = get_exception_types(method, CHECK_0);
  ...
  if (intern_name) {
    ...
    oop name_oop = StringTable::intern(method_name(), CHECK_0);
    name = Handle(THREAD, name_oop);
  } else {
    name = java_lang_String::create_from_symbol(method_name, CHECK_0);
  }
  if (name.is_null()) return NULL;

  int modifiers = method->access_flags().as_int() & JVM_RECOGNIZED_METHOD_MODIFIERS;

  Handle mh = java_lang_reflect_Method::create(CHECK_0);
  ...
  if (java_lang_reflect_Method::has_signature_field() &&
    ...
    Handle sig = java_lang_String::create_from_symbol(method->generic_signature(), CHECK_0);
...
oop Reflection::new_constructor(methodHandle method, TRAPS) {
  ...
  symbolHandle signature (THREAD, method->signature());
  int parameter_count = ArgumentCount(signature).size();
  objArrayHandle parameter_types = get_parameter_types(method, parameter_count, NULL, CHECK_0);
  ...
  Handle ch = java_lang_reflect_Constructor::create(CHECK_0);
...
oop Reflection::new_field(fieldDescriptor* fd, bool intern_name, TRAPS) {
  ...
  if (intern_name) {
    // intern_name is only true with UseNewReflection
    oop name_oop = StringTable::intern(field_name(), CHECK_0);
    name = Handle(THREAD, name_oop);
  } else {
    name = java_lang_String::create_from_symbol(field_name, CHECK_0);  
  }
  symbolHandle signature (THREAD, fd->signature());
  KlassHandle  holder    (THREAD, fd->field_holder());
  Handle type = new_type(signature, holder, CHECK_0);
  Handle rh  = java_lang_reflect_Field::create(CHECK_0);  
  ...
  if (java_lang_reflect_Field::has_signature_field() &&
    ...
    Handle sig = java_lang_String::create_from_symbol(fd->generic_signature(), CHECK_0);
...
oop Reflection::invoke(instanceKlassHandle klass, methodHandle reflected_method, 
  ...
  if (is_static) {
	...
	method = resolve_interface_call(klass, reflected_method, target_klass, receiver, CHECK_(NULL));
        ...
        // vtable index equals -1 => the method cannot be overridden.
  ...
  if (!override) {
    if (!(klass->is_public() && reflected_method->is_public())) {
      bool access = Reflection::reflect_check_access(klass(), reflected_method->access_flags(), target_klass(), is_method_invoke, CHECK_0);
      if (!access) {
  ...
  for (int i = 0; i < args_len; i++) {
    ...
    if (java_lang_Class::is_primitive(type_mirror)) {
      jvalue value;
      BasicType ptype = basic_type_mirror_to_basic_type(type_mirror, CHECK_0);
      BasicType atype = unbox_for_primitive(arg, &value, CHECK_0);
      if (ptype != atype) {
        widen(&value, atype, ptype, CHECK_0);
      }
      switch (ptype) {
  ...
  if (HAS_PENDING_EXCEPTION) {
    ...
    if (rtype == T_BOOLEAN || rtype == T_BYTE || rtype == T_CHAR || rtype == T_SHORT)
      narrow((jvalue*) result.get_value_addr(), rtype, CHECK_0);
    return box((jvalue*) result.get_value_addr(), rtype, CHECK_0);
  }
}
...
bool Reflection::match_parameter_types(methodHandle method, objArrayHandle types, int parameter_count, TRAPS) {
  ...
  if (parameter_count > 0) {
    objArrayHandle method_types = get_parameter_types(method, parameter_count, NULL, CHECK_0);
...
oop Reflection::new_field(FieldStream* st, TRAPS) {
  ...
  Handle name = java_lang_String::create_from_symbol(field_name, CHECK_0);  
  symbolHandle signature(THREAD, st->signature());
  Handle type = new_type(signature, st->klass(), CHECK_0);
  Handle rh  = java_lang_reflect_Field::create(CHECK_0);
  oop result = rh();
...
bool Reflection::resolve_field(Handle field_mirror, Handle& receiver, fieldDescriptor* fd, bool check_final, TRAPS) {
  ...
  if (!java_lang_reflect_Field::override(field_mirror())) {
    if (!(klass->is_public() && fd->is_public())) {
      bool access_check = reflect_check_access(klass(), fd->access_flags(), receiver_klass(), false, CHECK_0);
      if (!access_check) {
  ...
  if (check_final && fd->is_final()) {
    ...
    bool strict_final_check = !Universe::is_jdk12x_version();
...
objArrayOop Reflection::reflect_fields(oop mirror, jint which, TRAPS) {  
  ...
  if (java_lang_Class::is_primitive(mirror)
    ...
    klassOop klass = SystemDictionary::resolve_or_fail(name, true, CHECK_0);
    return oopFactory::new_objArray(klass, 0, CHECK_0);  // Return empty array
  }
  ...
  symbolHandle name = vmSymbolHandles::java_lang_reflect_Field();
  klassOop klass = SystemDictionary::resolve_or_fail(name, true, CHECK_0);
  objArrayOop r = oopFactory::new_objArray(klass, count, CHECK_0);
  ...
  { 
    for (FieldStream st(k, local_fields_only, false); !st.eos(); st.next()) {
      if (local_fields_only || st.access_flags().is_public()) {
        oop field = new_field(&st, CHECK_0);
...
oop Reflection::reflect_method(oop mirror, symbolHandle method_name, objArrayHandle types, jint which, TRAPS) {
  ...
  h_k->link_class(CHECK_0);

  // For interfaces include static initializers under jdk1.2.x (since classic does that)
  bool include_clinit = Universe::is_jdk12x_version() && h_k->is_interface();
      ...
      // Note that we can ignore checks for overridden methods, since we go up the hierarchy.
      {
        for (MethodStream st(h_k, false, false); !st.eos(); st.next()) {
          ...
          if (method_name() == m->name() && (include_clinit || (m->is_public() && !m->is_static() && !m->is_initializer()))) {
            ...
            bool parameter_match = match_parameter_types(m, types, ArgumentCount(signature).size(), CHECK_0);
            if (parameter_match) {
...
objArrayOop Reflection::reflect_methods(oop mirror, jint which, TRAPS) {
  ...
  // For interfaces include static initializers under jdk1.2.x (since classic does that)
  bool include_clinit = Universe::is_jdk12x_version() && is_interface;
        ...
        // Fill in results backwards
        {
          ...
          for (MethodStream st(h_k, false, false); !st.eos(); st.next()) {
            ...
            if (!m->is_static() && !m->is_initializer() && m->is_public() && !m->is_overridden_in(h_k())) {
              oop method = new_method(m, false, false, CHECK_0);
              if (method == NULL) {
                return NULL;
              } else {                
...
objArrayOop Reflection::reflect_constructors(oop mirror, jint which, TRAPS) {
  ...
  if (prim || k->is_interface() || k->oop_is_array()) {        
    return oopFactory::new_objArray(SystemDictionary::reflect_constructor_klass(), 0, CHECK_0);  // Return empty array
  }
  ...
  // Fill in results backwards
  {
    for (MethodStream st(h_k, true, true); !st.eos(); st.next()) {
      methodHandle m (THREAD, st.method());
      if (m->name() == vmSymbols::object_initializer_name() && (local_only || m->is_public())) {
        oop constr = new_constructor(m, CHECK_0);
        if (constr == NULL) {
          return NULL;
        } else {
...
oop Reflection::invoke_method(oop method_mirror, Handle receiver, objArrayHandle args, TRAPS) {
  ...
  bool override          = java_lang_reflect_Method::override(method_mirror);
  objArrayHandle ptypes(THREAD, objArrayOop(java_lang_reflect_Method::parameter_types(method_mirror)));

  oop return_type_mirror = java_lang_reflect_Method::return_type(method_mirror);
  BasicType rtype;
  if (java_lang_Class::is_primitive(return_type_mirror)) { 
    rtype = basic_type_mirror_to_basic_type(return_type_mirror, CHECK_0);
  } else {
    rtype = T_OBJECT;
  }
...
oop Reflection::invoke_constructor(oop constructor_mirror, objArrayHandle args, TRAPS) {
  ...
  klass->check_valid_for_instantiation(false, CHECK_0);
  Handle receiver = klass->allocate_instance_handle(CHECK_0);

  // Ignore result from call and return receiver
  invoke(klass, method, receiver, override, ptypes, T_VOID, args, false, CHECK_0);
  return receiver();
}
