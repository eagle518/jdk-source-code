...
#pragma ident "@(#)signature.hpp	1.38 03/12/23 16:44:10 JVM"
#endif
/*
 ...
 public:
  ...
  uint64_t fingerprint() {
    ...
    if (mh->size_of_parameters() > max_size_of_parameters ) {
      _fingerprint = CONST64(-1);
      mh->constMethod()->set_fingerprint(_fingerprint);
      return _fingerprint;
    }

    assert( mh->result_type() <= result_feature_mask, "bad result type");
...
class SignatureStream : public StackObj {
 ...
 public:
  ...
  SignatureStream(symbolHandle signature, bool is_method = true); 
