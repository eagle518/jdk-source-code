...
#pragma ident "@(#)fprofiler.cpp	1.108 02/10/24 13:38:58 JVM"
#endif
/*
...
// For runtime stubs, record as native rather than as compiled
   if (cb->is_runtime_stub()) {
        ...
        fr = fr.sender_for_compiled_frame(&map, cb, false);
        cb = CodeCache::find_blob(fr.pc());
        localwhere = tp_native;
  }
  methodOop method = (cb->is_nmethod()) ? ((nmethod *)cb)->method() : NULL;  

  if (method == NULL) {
    if (cb->is_i2c_adapter() || cb->is_c2i_adapter() || cb->is_osr_adapter())
      // These nodes do not have names
      adapter_update(localwhere);
...
void ThreadProfiler::record_tick(JavaThread* thread) {
  ...
  if (!thread->wait_for_ext_suspend_completion(SUSPENDRETRYCOUNT, &debug_bits )) {
...
void FlatProfiler::allocate_table() {
  ...
  NOT_CORE(if (ProfilerRecordPC) PCRecorder::init(); )
