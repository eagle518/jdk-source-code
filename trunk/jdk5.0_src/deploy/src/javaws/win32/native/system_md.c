 ...
 * provided that ../lib contains deploy.jar and javaws.jar
...
char* sysGetApplicationHome(void) {
  ...
  if (!initialized) {  
    ...
    *strrchr(home, FILE_SEPARATOR) = '\0'; /* remove .exe file name */
    ...
    sprintf(javawsJar, "%s%c%s%c%s", libPath, FILE_SEPARATOR, "lib", FILE_SEPARATOR, "javaws.jar");
    sprintf(deployJar, "%s%c%s%c%s", libPath, FILE_SEPARATOR, "lib", FILE_SEPARATOR, "deploy.jar");
    if (stat(javawsJar, &statBuf) == 0 && stat(deployJar, &statBuf) == 0) {
      initialized = TRUE;
    } 
...
char* sysGetDebugJavaCmd(char* javaCmd) {
  char *temp;
  char *debug_exe = "_g.exe";
  int index = 0;

  if (javaCmd == NULL) return NULL;

  temp = (char*)malloc(sizeof(char) * (strlen(javaCmd) + strlen(debug_exe)));
  
  index = strlen(javaCmd);

  strcpy(temp, javaCmd);
 
  temp[index-4] = '\0';
  
  return strcat(temp, debug_exe);;
}
