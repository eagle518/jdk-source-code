/*
 * @(#)FileOpenServiceImpl.java	1.25 04/06/13
 * 
...
public final class FileOpenServiceImpl implements FileOpenService {
    ...
    public FileContents openFileDialog(final String pathHint, final String[] extentions) throws IOException {
	
	try {
	    // Ask user permission first
	    if (!_fileSaveServiceImpl.askUser()) return null;
	    FileContents fc = (FileContents)AccessController.doPrivileged(
    ...
    public FileContents[] openMultiFileDialog(final String pathHint, final String[] extentions) throws IOException {

	try {
	    // Check permission with user first
	    if (!_fileSaveServiceImpl.askUser()) return null;
	
	    FileContents[] fcs = (FileContents[])AccessController.doPrivileged(
		new PrivilegedAction() {
		    public Object run() {
			JFileChooser jfc = null;
			FileSystemView fsv = getFileSystemView();
