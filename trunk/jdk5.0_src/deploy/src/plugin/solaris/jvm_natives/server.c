...
static jmethodID g_ojiplugin_releaseThreadPipe;
...
initialize_globals(JNIEnv* m_env) {
  ...
  if(getenv("JAVA_PLUGIN_TRACE")) {
    ...
    g_ojiplugin_releaseThreadPipe = 
      wrapGetStaticMethodID(m_env, g_ojiplugin_class, 
		      "releasePipeForCurrentThread", "()V");
...
void
ReleaseThreadPipe(JNIEnv* env) {
    jclass pluginclz;
    
    (*env)->CallStaticVoidMethod(env, g_ojiplugin_class, 
			   g_ojiplugin_releaseThreadPipe);

}

void
handleJNIJSResponse(JNIEnv* env, int pipe, 
    ...
    native_trace("Entered handleJNIJSResponse\n");
    while(cont) {
		      ...
		      jobject* ret)
{
    ...
    /* Acquire the appropriate pipe */
    pipe = AcquireThreadPipe(env);

    native_trace("Pipe acquired %d\n", pipe);
    ...
    handleJNIJSResponse(env, pipe, returnType, &returnObject);
    ...
    /* Release the pipe. Preserve any exception for after the java call */
    {
      jobject exc  = (*env)->ExceptionOccurred(env);
 ...
 */
JNIEXPORT void JNICALL
Java_sun_plugin_navig_motif_OJIPlugin_enterMonitor
(JNIEnv *env, jclass jClazz, jobject monitorObj) 
{
    
    native_trace("OJIPlugin_enterMonitor(): Trying to enter the spontaneous monitor: count= %d\n", monitor_count);
    (*env)->MonitorEnter(env, monitorObj);
    monitor_count++;
    native_trace("OJIPlugin_enterMonitor(): Entered the spontaneous monitor: count= %d\n", monitor_count);
}
 ...
 */
JNIEXPORT void JNICALL
Java_sun_plugin_navig_motif_OJIPlugin_exitMonitor
(JNIEnv *env, jclass jClazz, jobject monitorObj) 
{
    native_trace("OJIPlugin_enterMonitor(): Leaving the spontaneous monitor: count=%d\n", monitor_count);
    (*env)->MonitorExit(env, monitorObj);
    monitor_count--;
}
