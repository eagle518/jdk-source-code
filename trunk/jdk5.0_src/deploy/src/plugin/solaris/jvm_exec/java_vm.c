...
void AddPackage()
{
    ...
    char *argstr = NULL;
    char tok[] = "=";
    char *option = "-Djava.protocol.handler.pkgs";
    char *tmp = NULL;
    propstr = find_options();
    
    if (propstr != NULL)
        arg = strtok( propstr, seps );
    
    while (arg != NULL) {
        if (strncmp(arg, option, strlen(option)) == 0)
	{
	    /* 
	     * Argument -Djava.protocol.handler.pkgs is specified in 
	     * user's props - concat */
	    set = 1;
	    argstr = strtok( arg, tok);
	    argstr = strtok( NULL, tok);
	    tmp = (char *) MemAlloc(strlen(option) + strlen(argstr) + strlen("|sun.plugin.net.protocol")+2); 
	    /* Why 2?  one for null terminator, one for '=' sign */

	    strcpy(tmp, option);
	    strcat(tmp, "=");
	    strcat(tmp, argstr);
	    strcat(tmp, "|sun.plugin.net.protocol");
	    AddOption(tmp, NULL);
    ...
    {
        /*
         * There is no -Djava.protocol.handler.pkgs specified by user
         * just add our package.
         */
        AddOption("-Djava.protocol.handler.pkgs=sun.plugin.net.protocol", NULL);
    }
}
...
static jboolean getJVMEntryPoints(char *java_home, InvocationFunctions *ifn) {
    ...
    char *use_java_vm_debug = getenv("USE_JAVA_VM_DEBUG");
    if (use_java_vm_debug != NULL && strcasecmp(use_java_vm_debug, "TRUE") == 0)
        libname = "libjvm_g.so";
    else
        libname = "libjvm.so";
...
int main(int argc, char **argv) {
    ...
    */
    ...
    /* Set default value for "java.class.path" */
    SetClassPath(NULL);
    ...
    /* Set java.protocol.handler.pkgs property */
    AddPackage();        

    snprintf(classpath, sizeof classpath,
#if defined(DEBUG_VERSION)
	    "%s/lib/plugin_g.jar:"		/* from plugin_home */
	    "%s/lib/deploy_g.jar:"	/* from plugin_home */
#else
...
#if defined(DEBUG_VERSION)
	snprintf(buf, buflen, "-Djavaplugin.lib=%s/lib/" LIBARCH "/libjavaplugin_jni_g.so",
		plugin_home);
#else
...
find_options()
{
    ...
    sprintf(key, "deployment.javapi.jre.%s.args=", RELEASE);
