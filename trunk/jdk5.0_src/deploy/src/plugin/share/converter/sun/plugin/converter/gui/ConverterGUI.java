			  ...
			  implements ActionListener, ItemListener
{
	...
	{
	    // Load background gif
	    InputStream is = ConverterGUI.class.getClassLoader().getResourceAsStream("images/background.gif");
	    ...
	    bgImage = new ImageIcon(bos.toByteArray()).getImage();

	    bos.close();
	    is.close();

	    // Load icon gif
	    is = ConverterGUI.class.getClassLoader().getResourceAsStream("images/duke.gif");
	    ...
	    dukeImage = new ImageIcon(bos.toByteArray()).getImage();
...
converter = new PluginConverter();
	...
	JPanel mainPanel = new JPanel() {
				public void paint(Graphics g)
				{
				    ...
				    g.drawImage(bgImage, 0, 0, size.width, size.height, 
							 0, 0, bgImage.getWidth(this), bgImage.getHeight(this), this);
				    super.paint(g);
				}
			    };
    ...
    {
	AdvancedDialog dialog = new AdvancedDialog(this, false, converter);
	dialog.setVisible(true);
    }   
