  ...
  (JNIEnv *env, jclass clazz, jbyteArray jHash, jint jHashSize, jint hCryptProv, jint hCryptKey)
{
	...
	// public key algorithm, so AT_KEYEXCHANGE is used.
	
	// Determine size of buffer
	DWORD dwBufLen = 0;
	if (::CryptSignHash(hHash, AT_KEYEXCHANGE, NULL, NULL, NULL, &dwBufLen) == FALSE)
	{
    	    ThrowSignatureException(env, GetLastError());
	    __leave;
	 }  
	...
	if (::CryptSignHash(hHash, AT_KEYEXCHANGE, NULL, NULL, (BYTE*)pSignedHashBuffer, &dwBufLen) == FALSE)
	{
    	    ThrowSignatureException(env, GetLastError());
	    __leave;
	}   
